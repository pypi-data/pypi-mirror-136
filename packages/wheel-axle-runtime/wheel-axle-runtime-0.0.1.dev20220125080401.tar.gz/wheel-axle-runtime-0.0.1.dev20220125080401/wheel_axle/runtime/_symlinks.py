...
class SymlinksInstaller(Installer):
    ...
    def _install_symlinks(self, symlinks: Symlinks):
        ...
        symlinking_paths = list(chain(self.lib_dir, scheme_paths.values()))
        ...
        def assert_no_path_symlinking(symlink_path: str, symlink_target: str) -> None:
            for symlinking_path in symlinking_paths:
                if is_within_directory(symlinking_path, symlink_target):
                    return
            ...
            raise InstallationError(
                message.format(self.dist_meta.project_name, symlink_path, symlink_target, ",".join(symlinking_paths))
            )
        ...
        for symlink in symlinks:
            ...
            norm_symlink_target = os.path.normpath(os.path.join(
                os.path.normpath(os.path.dirname(norm_symlink_path)),
                symlink_target))

            assert_no_path_symlinking(norm_symlink_path, norm_symlink_target)

            if os.path.exists(norm_symlink_path):
                os.unlink(norm_symlink_path)

            # We preserve symlink target as de-normalized, while checking traversal as normalized
