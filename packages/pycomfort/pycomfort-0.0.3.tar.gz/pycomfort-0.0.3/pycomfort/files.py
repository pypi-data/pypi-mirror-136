...
def rename_files(files_or_path: Union[seq, Path], dictionary: dict, test: bool = False):

    if isinstance(files_or_path, Path):
        if files_or_path.is_dir():
            return rename_files(files(files_or_path), dictionary=dictionary)
        else:
            return rename_files(seq(files_or_path),  dictionary=dictionary)
    else:
        results = []
