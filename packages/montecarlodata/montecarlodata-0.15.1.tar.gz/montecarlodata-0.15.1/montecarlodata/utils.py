...
class AwsClientWrapper(Wrapper):
    ...
    @boxify(use_snakes=True, default_box_attr=None)
    def describe_redshift_cluster(self, cluster_id: str) -> Box:
        return self._session.client('redshift').describe_clusters(ClusterIdentifier=cluster_id)['Clusters'][0]

    @boxify(use_snakes=True)
    def describe_cluster_subnet_groups(self, subnet_group: str) -> Box:
        return self._session.client('redshift').describe_cluster_subnet_groups(ClusterSubnetGroupName=subnet_group)['ClusterSubnetGroups'][0]
    ...
    @boxify(use_snakes=True)
    def describe_routes(self, filter_vals: List[str], filter_key: Optional[str] = 'association.subnet-id') -> Box:
        return self._session.client('ec2').describe_route_tables(Filters=[{'Name': filter_key, 'Values': filter_vals}])
