...
class ResourceStage:
    ...
    def run(self, output_path: str, start_block: int):
        filtrate_output_path = ResourceStage.get_action_output_path(self.workspace_dir,
                                                                    self.resource_name,
                                                                    start_block,
                                                                    'filtrate')
        ETLCommandWrapper.remove_if_exists(filtrate_output_path)
        ResourceStage.do_filtrate(output_path,
                                  filtrate_output_path,
    ...
    @staticmethod
    def get_action_output_path(workspace_dir: str,
                               resource_name: str,
                               started_at: int,
                               action: str) -> str:
    ...
    @staticmethod
    def do_filtrate(output_path: str,
        ...
        with open(output_path, "r") as rf:
            with open(filtrated_output_path, "w") as wf:
                while True:
                    try:
                        line = ResourceStage.split_and_standardize(next(rf))
                        selected_items = []
    ...
    @staticmethod
    def split_and_standardize(line_str: str) -> [str]:
