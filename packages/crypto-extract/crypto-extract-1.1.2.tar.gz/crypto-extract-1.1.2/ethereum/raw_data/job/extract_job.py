...
class ExtractJob(BaseJob):
    ...
    def run(self):
        ...
        while start < self.end_block:
            ...
            progress = format((start - self.custom_start_block) / (self.end_block - self.custom_start_block) * 100,
                              '.2f')

            self.logger.info(f'>>>>>>>>>> Start to get {start} to {next_start} blocks, progress: {progress} <<<<<<<<<<')
