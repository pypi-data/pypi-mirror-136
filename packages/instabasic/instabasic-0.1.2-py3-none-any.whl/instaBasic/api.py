...
Functions:
    get_auth_url() -> string
    get_short_token(auth_code) -> string
    get_long_token(auth_code) -> string
    refresh_long_token(token) -> string

    get_media(token, media) -> object
    get_album_images(token, media_id) -> object

    get_user(token) -> object
    get_user_media(token) -> object
"""
...
class API:
    """API Class stores app_id, app_secret, ad redirect_url."""

    # __init__
    def __init__(self, app_id, app_secret, redirect_url) -> None:
        self.app_id = app_id
    ...
    def get_short_token(self, auth_code):
    ...
    def get_long_token(self, auth_code):
        """Get a long-lived access token."""
        url = f"{GRAPH_URL}/access_token"
        query = {
            "client_secret": self.app_secret,
            "grant_type": "ig_exchange_token",
            "access_token": self.get_short_token(auth_code),
        }
        resp = requests.get(url, params=query).json()
        return resp["access_token"]

    def refresh_long_token(self, long_token):
    ...
    #  Media
    # --------------------
    def get_media(self, token, media_id, fields=MEDIA_FIELDS):
        ...
        if req["media_type"] == "CAROUSEL_ALBUM":
            del req["media_url"]
            album_images = self.get_album_images(token, req["id"])["data"]
            req["album_images"] = album_images
        return req

    def get_album_images(self, token, media_id, fields=ALBUM_IMAGE_FIELDS):
    ...
    #  User
    # --------------------
    def get_user(self, token, api_version=API_VERSION, fields=USER_FIELDS):
    ...
    def get_user_media(self, token, api_version=API_VERSION, fields=MEDIA_FIELDS):
        """Get user's media."""
        ...
        for idx, media in enumerate(req["data"]):
            if media["media_type"] == "CAROUSEL_ALBUM":
                req["data"][idx] = self.get_media(token, media["id"])
        return req
