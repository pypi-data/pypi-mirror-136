...
from gooey import Gooey, GooeyParser, PrefixTokenizers, options, Events
...
with zipfile.ZipFile(r'C:\Users\Chris\Downloads\simplemaps_worldcities_basicv1.71\worldcities.zip') as zf:
    with zf.open('worldcities.csv', 'r') as f:
        ...
        data = ['{} - {}'.format(x[4], x[1]) for x in reader][1:][:1000]


@Gooey(program_name='FilterableDropdown Demo', use_events=[Events.VALIDATE_FORM])
def main():
