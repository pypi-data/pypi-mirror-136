...
class GUI():
    ...
    class WidgetButton(ttk.Button):

        def preprocess(self, kw):
            self.command = kw.pop('command', kw.pop('onclick', None)) 
            self.style = kw.pop('command', kw.pop('onclick', self.style)) 
            self.anchor = kw.pop('anchor', kw.pop('origin', self.anchor))
        ...
        def __init__(self, parent, x, y, w, h, **kw):
            ...
            self.style = kw.pop('style', 'TButton')
            self.anchor = 'nw'

            self.preprocess(kw)
            super().__init__(self.parent.canvas, **kw)
            self.place(x=x, y=y, width=w, height=h, anchor=self.anchor)
        def remove(self):
            self.place_forget()
            self.destroy()
        def config(self, *args, **kw):
            ...
            self.place(anchor=self.anchor)
            
    def add_button(self, x, y, w, h, **kw):
        return GUI.WidgetButton(self, x, y, w, h, **kw)
