...
def genmixdb(config: dict, show_progress: bool = False) -> dict:
    ...
    total_mixtures = len(targets) * len(noises) * len(target_augmentations) * len(noise_augmentations)
    logger.info('')
    logger.info('Found {} mixtures to process'.format(total_mixtures))

    total_duration = len(noises) * len(noise_augmentations) * \
                     sum([sub['duration'] for sub in targets]) * len(target_augmentations)
    ...
    logger.info('')
    logger.info('Estimated duration:   {} (HH:MM:SS)'.format(strftime('%H:%M:%S', gmtime(total_duration))))
    ...
    with tqdm(desc='Processing', total=total_mixtures, disable=(not show_progress)) as progress_bar:
    ...
    logger.info('')
    logger.info('Actual duration: {} (HH:MM:SS)'.format(strftime('%H:%M:%S', gmtime(total_duration))))
