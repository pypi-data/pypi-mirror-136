...
def read_audio(name: str, dither: bool = True) -> np.ndarray:
    ...
    try:
        ...
        return inp.build_array(input_filepath=expanded_name, sample_rate_in=int(sox.file_info.sample_rate(name)))
...
def apply_augmentation(audio_in: np.ndarray,
    ...
    try:
        ...
        if 'tempo' in augmentation:
            ...
            # TODO
            #  Why are we using stretch in this case? This seems wrong; we should at the very least invert
            #  factor before calling stretch.
            #  From sox documentation:
            #   stretch factor: >1 lengthen, <1 shorten duration
            #   tempo factor: >1 shorten, <1 lengthen duration
