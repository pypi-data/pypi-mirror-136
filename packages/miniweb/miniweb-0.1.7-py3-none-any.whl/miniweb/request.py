...
class HttpRequest(object):
    ...
    @classmethod
    def set_temporary_file_path(cls, path):
        cls.temporary_file_path = path
        ...
        for line in self.header_lines:
            ...
            if result:
                ...
                if info["filename"]:
                    ...
                    self.file = HttpRequestFile(original_filename=self.filename, workspace=self.http_request.temporary_file_path)
                    self.file.open("wb")
    ...
    def update_post_data(self, line):
        if self.block_name in self.http_request._post_data:
            if isinstance(self.http_request._post_data[self.block_name], str):
                self.http_request._post_data[self.block_name] = [self.http_request] + [line.decode(self.block_encoding)]
            else:
                self.http_request._post_data[self.block_name].append(line.decode(self.block_encoding))
        else:
