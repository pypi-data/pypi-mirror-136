...
def hist_plot(
        values,
        bin_num=100, bin_width=None,
        norm=False,
        kde=False, kde_points=1000,
        ax=None
):
    """
    ...
    for idx, frag_pos in enumerate([np.sort(df[df['IsUniqueAligned']]['Pos'].values),
        ...
        cumu_hist_num = np.where((np.repeat(frag_pos.reshape((1, -1)), bin_num, axis=0) - bin_edges.reshape((bin_num, -1))) < 0, 1, 0).sum(axis=1)
