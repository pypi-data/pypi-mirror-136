...
class PLinkResult(object):
    ...
    def add_plink_result_path(self, path, link_type, name=None) -> None:
        """Add one at one time"""
        link_type = {'cross': 'CrossLink', 'loop': 'LoopLink', 'mono': 'MonoLink'}.get(link_type.lower(), default=link_type)
        if name is not None:
    ...
    def stack_protein_groups(self) -> None:
        ...
        for link_type in list(self._plink_results.keys()):
            for name in list(self._plink_results[link_type].keys()):
                ...
                df = df.join(df[pg_col].str.strip('/').str.split('/', expand=True).stack().reset_index(level=1, drop=True).rename(new_col))
    ...
    def filter_merged_result(
            self,
            min_evalue: float = None, max_evalue: float = None,
            min_score: float = None, max_score: float = None,
            remove_gi: bool = True
    ) -> None:
        if self.logger is not None:
            self.logger.info(f'PLinkResult filtering plink results {min_evalue=}, {max_evalue=}, {min_score=}, {max_score=}, {remove_gi=}')
        ...
        if remove_gi:
            self._merged_result = self._merged_result[~self._merged_result['SingleProteinLink'].str.contains('gi|', regex=False)].copy()
    ...
    def extract_link_pair_in_merged_result(self) -> None:
        ...
        self._merged_result[new_prot_info_cols] = pd.DataFrame(self._merged_result['SingleProteinLink'].apply(extract_prot_link_info).tolist())
        self._merged_result['SortedProteinLink'] = self._merged_result[new_prot_info_cols].apply(lambda x: combine_id_link_info(*x.values), axis=1)
        ...
        self._merged_result[new_pep_info_cols] = self._merged_result.apply(extract_pep_link_info, axis=1, result_type='expand')
        self._merged_result['SortedPeptideLink'] = self._merged_result[
            [*new_pep_info_cols, 'RawProteinSiteNeedReverse']].apply(lambda x: combine_id_link_info(*x.values), axis=1)

    def get_link_pairs_from_merged_result(self) -> list:
        if self.logger is not None:
            self.logger.info(f'PLinkResult getting link pairs from merged result')

        prot_links = self._merged_result[
            ...
            ]['SortedProteinLink'].dropna().drop_duplicates().tolist()
        return prot_links
    ...
    def map_xwalk_dist_to_merged_result(
        ...
        if prot_to_standard_map is not None:
            if isinstance(prot_to_standard_map, dict):
                ...
                self._merged_result['StandardProtein_1'] = self._merged_result['RawProtein_1'].apply(prot_to_standard_map)
                self._merged_result['StandardProtein_2'] = self._merged_result['RawProtein_2'].apply(prot_to_standard_map)
    ...
    def write_merged_result(self, to_path: str) -> None:
        ...
        if self._merged_result is not None:
            rk.df_to_file(self._merged_result, to_path, index=False)
        else:
    ...
    def load_merged_result(self, path: str) -> None:
        self._merged_result = rk.load_file_to_df(path)
    ...
    def write_results_from_merged_result(self, path: str) -> None:
        ...
        df['I'] = df.groupby('SortedProteinLink')['Evalue'].transform(
            lambda x: x.astype(float).idxmin()).astype(int)
        ...
        with pd.ExcelWriter(path) as e:
            ...
            site_level_result = (site_level_result
                                 .drop_duplicates(['SortedProteinLink', 'Source'])
                                 .sort_values(['LinkType', 'SortedProteinLink', 'Source'])
                                 )
            ...
            unique_site_level_cols = [_ for _ in unique_site_level_cols if _ not in ['Source', 'RawIndex', 'DefinedIdentifier']]
            unique_site_result = unique_site_result[unique_site_level_cols]
            unique_site_result = (unique_site_result
                                  .drop_duplicates(['SortedProteinLink'])
                                  .sort_values(['LinkType', 'SortedProteinLink'])
                                  )
            ...
            pep_level_cols = [
                ...
                'RawPeptideSite_1', 'RawPeptideSite_2',
                'Best_Evalue', 'Score_In_Best_Evalue', 'Best_Score',
                'SAS', 'Euclidean',
            ]
            ...
            (self._merged_result
             .sort_values(['LinkType', 'Source', 'SortedProteinLink', 'SortedPeptideLink'])
