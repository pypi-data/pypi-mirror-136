...
const { RULEACTIONS } = require("../common");

class BaseFilter extends TargetProcessor {
    constructor(filterDef, type) {
        super("filter_" + String(hashStr(JSON.stringify(filterDef)))); // use hash of the whole thing as context id so we cache based on that
        ...
        this.rule_name = filterDef.rule_name;
        this.rule_internalName = filterDef.rule_internalName;
        this.status = filterDef.rule_status;
        this.confidence = filterDef.confidence;
        if (!this.cache) this.shouldCache = false;
    }

    getRuleDetails() {
        return { rulename: this.rule_name, rule_internalName: this.rule_internalName, status: RULEACTIONS[this.status], confidence: this.confidence }
    }
    getTargets() {
        return this.targets;
    }

    checkTargetCB(valueList, findingCb, doneCb) {
        let waitingCount = 0;
