...
def sampling(models, X, y, args):
  ...
  for model in models:
    ...
    if mode_eval_meth == "grid":
      ...
      ###MODEL FIT###
      model_objt.fit(X, y.values.ravel())
      model_rlts = pd.DataFrame(model_objt.cv_results_)
      model_indx = model_objt.best_index_
      ###MODEL SCR###
    ...
    if intr_mode_area in ["classification", "regression"]:
      ...
      model_spac = model_objt.best_params_
    ...
    print("Process_Parameters: {}".format(model_objt.best_params_))
...
def best(df_models):
  ...
  print("Model Process Parameters: {0}".format(best_model_objt.best_params_))
  return best_model_objt
