...
class Widget(widgets.DOMWidget):
    ...
    def __init__(self, renderer=None, data_definition=None, **kwargs):
        ...
        Args:
            renderer: optional renderer to use, if not specified the widget is creating a renderer
            data_definition: optional dataset definition
        """
        ...
        if (len(arrays)):
            size = list(arrays[0].array.shape)
            size.reverse()
            element_size = arrays[0].element_size

        # update the dataset info synced with the widget
        self.__update_dataset_info(size, element_size)
    ...
    def select_data_definition(self, data_definition: clara.viz.core.DataDefinition):
        ...
        # configure the data
        arrays = []
        for index, array in enumerate(data_definition.arrays):
            arrays.append(clara.viz.core.Array(array=array.array, dimension_order=array.order, permute_axes=array.permute_axes,
                          flip_axes=array.flip_axes, element_size=array.element_size))
            if index == 0:
                size = list(array.array.shape)
                size.reverse()
                self.__update_dataset_info(size, array.element_size)

        self._renderer.set_arrays(arrays)
    ...
    def __update_dataset_info(self, size, element_size):
        ...
        if (len(size) > 1):
            dataset_info['size']['x'] = size[1]
        if (len(size) > 2):
            dataset_info['size']['y'] = size[2]
        if (len(size) > 3):
            dataset_info['size']['z'] = size[3]

        if (len(element_size) > 1):
            dataset_info['elementSize']['x'] = element_size[1]
        if (len(element_size) > 2):
            dataset_info['elementSize']['y'] = element_size[2]
        if (len(element_size) > 3):
            dataset_info['elementSize']['z'] = element_size[3]
