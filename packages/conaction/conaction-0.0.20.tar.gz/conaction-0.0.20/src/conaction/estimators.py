...
def kendall_tau(X: np.ndarray, method='A', n_jobs=multiprocessing.cpu_count()) -> np.float64:
    '''
...
def grade_entropy(X: np.ndarray, n_jobs=multiprocessing.cpu_count()) -> np.float64:
    '''
    ...
    pool = ProcessPool(nodes=n_jobs)
    for i, v in enumerate(pool.imap(grade, range(X.shape[0]))):
        P[:, i] = v
    pool.close()
