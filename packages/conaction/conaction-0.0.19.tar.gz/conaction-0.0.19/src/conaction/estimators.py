...
def kendall_tau_old(X: np.ndarray, method='A') -> np.float64:
    '''
    ...
    for i, xi in enumerate(X):
        for j, xj in enumerate(X):
            if i < j:
                if (np.all(xi < xj) or np.all(xi > xj)):
                    concordant += 1
                else:
                    discordant += 1
            else:
                continue
    if method == 'A':
        return (concordant - discordant) / ((X.shape[0] **2 - X.shape[0]) / 2)
    elif method == 'B':
...
def kendall_tau(X: np.ndarray, method='A') -> np.float64:
    '''
    Multivariate Kendall's tau.
    ...
    def grade(q):
        ...
        for j in range(X.shape[0]):
            if np.all(X[j] < X[q]) or np.all(X[j] > X[q]):
                score += 1
            else:
                score -= 1
        return score
    ...
    for i in tqdm.tqdm(range(X.shape[0])):
        for j in range(X.shape[0]):
            if np.all(X[i] < X[j]) or np.all(X[i] > X[j]):
                score += 1
            elif np.all(X[i] == X[j]):
                continue
            else:
