...
class Client:
    def __init__(self) -> None:
        ...
        self.stub: Optional[runtime_pb2_grpc.RemoteExecutorStub] = None

    def dial(self) -> runtime_pb2_grpc.RemoteExecutorStub:
        if self.stub is None:
            if k8s.is_in_cluster():
                ...
                self.stub = runtime_pb2_grpc.RemoteExecutorStub(channel)
            else:
    ...
    async def set_cron_jobs(
        self,
        cron_jobs: List[runtime_pb2.CronJob],
        ...
        stub = self.dial()
        response: runtime_pb2.SetCronJobsResponse = await stub.SetCronJobs(request)
        return response
    ...
    async def create_task(
        ...
        response: runtime_pb2.CreateTaskResponse = await self.dial().CreateTask(request)
    ...
    async def cancel_task(self, task_id: str) -> bool:
        req = runtime_pb2.CancelTaskRequest(task_id=task_id)
        resp: runtime_pb2.CancelTaskResponse = await self.dial().CancelTask(req)
        return resp.success
    ...
    async def post_result(
        ...
        result = runtime_pb2.Result()
        if error:
            result.error.code = runtime_pb2.APPLICATION
        ...
        return cast(runtime_pb2.PostResultResponse, response)

    async def wait_for_result(self, task_id: uuid.UUID) -> Any:
        request = runtime_pb2.WaitForResultRequest(task_id=str(task_id))
    ...
    def _build_create_task_request(
        ...
        task_proto_obj = runtime_pb2.Task(
        ...
        return runtime_pb2.CreateTaskRequest(
            task=task_proto_obj, sdk_version=__version__
        )

    @staticmethod
    def _transform_response_exception(
        response: runtime_pb2.WaitForResultResponse,
    ) -> Any:
