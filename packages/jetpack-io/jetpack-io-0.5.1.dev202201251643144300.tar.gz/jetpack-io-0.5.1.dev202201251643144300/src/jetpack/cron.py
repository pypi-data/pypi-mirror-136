...
from jetpack.proto.runtime.v1alpha1 import runtime_pb2
...
def get_jobs() -> List[runtime_pb2.CronJob]:
    cron_jobs = []
    for job in schedule.get_jobs():
        ...
        target_day_of_week = runtime_pb2.DayOfWeek.UNKNOWN_DAY
        if job.start_day is not None:
            target_day_of_week = runtime_pb2.DayOfWeek.Value(job.start_day.upper())
        ...
        cron_jobs.append(
            runtime_pb2.CronJob(
                ...
                unit=runtime_pb2.Unit.Value(job.unit.upper()),
...
def pretty_print(self: runtime_pb2.CronJob) -> str:
    s = f"Function: {self.qualified_symbol}"
    s += f"\nInterval: every {self.interval} {runtime_pb2.Unit.Name(self.unit).lower()}"
    if self.target_time:
        s += f"\n at {self.target_time}"
    if self.target_day_of_week:
        s += f"\n on {runtime_pb2.DayOfWeek.Name(self.target_day_of_week).capitalize()}"
    return s
