...
from ._pushSignals import PushSignals,PushSignalsOsci,PushSignalsCap,PushSignalsCapOsci,PushSignalsCapNoOsci,PushSignalsNoOsci
...
class ResultTable(v.DataTable):
    ...
    def FirstItemNoOsci(self):

        self.items = [
        {
            'name': ' - '+' / '+ ' - ',
            'stiction_cases': 'No Oscillation found.',
            'magnitude_stiction': 'None'
        }
        ]
    def DeleteItem(self):
        self.items = []
...
def seeq_add_on(sdl_notebook_url):
    def pull_only_signals(url, grid=None):
        ...
        try:
            if  text_field_date_start_signal_1.v_model == '' or text_field_date_end_signal_1.v_model == '':
                start = worksheet.display_range['Start']
                end = worksheet.display_range['End']
        except (NameError, ValueError):
            start = worksheet.display_range['Start']
            end = worksheet.display_range['End']
        search_df = spy.search(url, estimate_sample_period=worksheet.display_range, quiet=True,status=spy.Status(quiet=True))
        if len(search_df)<=0:
        ...
        df = spy.pull(search_signals_df, start=start, end=end, grid=grid, header='ID',shape='samples',capsule_properties=list(), quiet=True,status=spy.Status(quiet=True))
        df_1=df.index[0]
        df_2=df.index[1]
        differnece=df_2-df_1
        sampling_rate = differnece.total_seconds()
        sampling_rate=sampling_rate/60  
        df_capsule = spy.pull(search_capsules_df, start=start, end=end, grid=str(sampling_rate)+'min',shape='samples',header='Name',capsule_properties=list(), quiet=True,status=spy.Status(quiet=True))
        if df.empty:
            df= pd.DataFrame()
        else:
    ...
    def ExecutePullSignal(df):
        ...
        df = df[df[column_2].notna()]
        df = df[df[column_1].notna()]
    ...
    #------ Pull the user selection -----------------#
        ...
        if text_field_date_start_signal_1.v_model!=df_start_displayed or text_field_date_end_signal_1.v_model!=df_end_displayed:
            ...
            df=spy.pull(worksheet_url, start=start_date, end=end_date,grid=None,header='Name', capsule_properties=list(),quiet=True,status=spy.Status(quiet=True))
            df_OP=df[column_2]
            df_PV=df[column_1]
            if select_capsule.v_model=='':
                indexes_stored=[]
    ...
    #-------- Results-Section ------------------------#
        ...
        on_click_close_snackbar_events_completed_start()
        ...
        try:
            ...
            if len(overall_results)>0 and checkbox_selected_stiction.v_model==True:
                ...
                if select_capsule.v_model=="":
                    ...
                    status=PushSignalsCap(df,overall_results,worksheet_url,name,df[select_capsule.v_model])
            else:
                if len(overall_results)<=0 and checkbox_selected_stiction.v_model==True:
                    name=text_field_signal_name.v_model
                    if select_capsule.v_model=="":
                        status=PushSignalsNoOsci(df,worksheet_url,name)
                    else:
                        print('here')
                        status=PushSignalsCapNoOsci(df,worksheet_url,name,df[select_capsule.v_model])
                        print(status)
            ...
            if len(overall_results_oscillation)>0 and checkbox_selected_oscillation.v_model==True:
                ...
                if select_capsule.v_model=="":
                    ...
                    status=PushSignalsCapOsci(df,overall_results_oscillation,worksheet_url,name,df[select_capsule.v_model])
            else:
                if len(overall_results_oscillation)<=0 and checkbox_selected_oscillation.v_model==True:
                    name=text_field_signal_name_osci.v_model
                    if select_capsule.v_model=="":
    ...
    def on_click_scedule(widget, event, data):
        scedule_data={'Job Name':text_field_job_name.v_model,'Frequency':text_field_job_frequency.v_model,
        'Start Date':text_field_job_start.v_model,'Investigated Time':text_field_job_invest_time.v_model,
        'Error Signal':select_error.v_model,'OP Signal':select_op.v_model,'Capsule':select_capsule.v_model,
        'Push Stiction Signal':checkbox_selected_stiction.v_model,'Stiction Signal Name':text_field_signal_name.v_model,
        'Push Oscillation Signal':checkbox_selected_oscillation.v_model,'Oscillation Signal Name':text_field_signal_name_osci.v_model,
        'Worksheet URL':sdl_notebook_url}
    ...
    def on_click_close_snackbar_events_completed_start():
        analysis_finished.v_model = True

    def on_click_close_snackbar_events_completed(widget, event, data):
        analysis_finished.v_model = False
    ...
    workbook_id, worksheet_id, workstep_id=get_workbook_worksheet_workstep_ids(worksheet_url)

    df,df_capsule,search_signals_df=pull_only_signals(worksheet_url)

    #Check if the length of the dataframes match
    if len(df)-len(df_capsule)!=0:
        intersec_idx=df.index.intersection(df_capsule.index)
        df=df.loc[intersec_idx]
        df_capsule=df_capsule.loc[intersec_idx]
    ...
    explanation_expl_stic=v.ExpansionPanelContent(no_gutters=True,color='#E0E0E0',children=['Stiction is an industrially-common, mechanical valve movement problem that occurs when there is too much friction inside the valve. The  valve stem position does not smoothly follow the valve output signal from the control system, resulting in cycling, excessive valve wear, and increased process variability.'])
    ...
    explanation_expl_PV_OP=v.ExpansionPanelContent(color='#E0E0E0',children=['This is the data needed to diagnose stiction in the control valve. Typically, the “Error” signal is the setpoint minus the measurement value (often referred to as PV) for a control loop which is sending an output (“OP signal”) to a control valve. In the case of a constant setpoint value across the analysis time range, the PV signal can be used directly for the “Error” signal. Stiction detection is based on characteristic patterns in the Error versus OP trends.'])
    ...
    explanation_head_scedule=v.ExpansionPanelHeader(class_="white--text",color='#00695C',dark=True,children=['Deploy Stiction Analysis (coming soon)'])
    ...
    checkbox_selected_local=v.Checkbox(v_model=True,disabled=False,label='Send Singals only to current Workbook')
    checkbox_selected_local.on_event('checkbox', on_click_checkbox_local)
    # App layout
    ...
    }], children=['Optional: Select a condition to limit the analysis.'])
    ...
    close_snackbar_compleded = v.Btn(color='white', icon=True, children=[v.Icon(children=['mdi-window-close'])])
    close_snackbar_compleded.on_event('click',on_click_close_snackbar_events_completed)

    analysis_finished = v.Snackbar(absolute=True,timeout=2500,v_model=False, color='#388E3C',
                                        children=['Anlaysis Completed',
                                                close_snackbar_compleded]) 
    ...
    #Card
    container_results=v.Card(class_='',color='grey lighten-3',height='100%',children=[analysis_finished,no_oscillation,no_signals,
        ...
        v.Row(class_='ma-6',no_gutters=True,children=[checkbox_selected_local]),
