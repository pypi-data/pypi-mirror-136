...
class NetBoxInventory:
    ...
    def _get_devices_from_circuits(self, url: str, params: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        while url:
            ...
            resp = r.json()
            print("A resposta em JSON e: ", resp)
            circuits.extend(resp.get("results"))

            url = resp.get("next")

        print("Os circuitos sao: ", circuits)
        for circuit in circuits:
            print("O circuito e: ", circuit)
            print("O termination_a da API Circuitose: ", circuit["termination_a"])
            print("O termination_z da API Circuitos e: ", circuit["termination_z"])
            ...
            print("O termination_a da API Circuito Terminations e: ", termination_a)
            print("O termination_z da API Circuito Terminations e: ", termination_z)
            print("O ID do dispositivo de terminacao a e: ", termination_a[0]["cable_peer"]["device"]["id"])
            print("O ID do dispositivo de terminacao z e: ", termination_z[0]["cable_peer"]["device"]["id"])

            if termination_a:
                device_a = self._get_resources(
                    url=f"{self.nb_url}/api/dcim/devices", params={"id": termination_a[0]["cable_peer"]["device"]["id"]})
                if device_a:
            ...
            if termination_z:
                device_z = self._get_resources(
                    url=f"{self.nb_url}/api/dcim/devices", params={"id": termination_z[0]["cable_peer"]["device"]["id"]})
                if device_z:
