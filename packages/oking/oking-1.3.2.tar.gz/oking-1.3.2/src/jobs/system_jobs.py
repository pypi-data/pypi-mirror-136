...
def send_execution_notification(job_config: dict) -> None:
    ...
    api_okvendas.post_log(Log(f'Oking em execucao desde {job_config.get("execution_start_time")} com {job_config.get("job_qty")} jobs', datetime.now().isoformat(), '', 'NOTIFICACAO'))
