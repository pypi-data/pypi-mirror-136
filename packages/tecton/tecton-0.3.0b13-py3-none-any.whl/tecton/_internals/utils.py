...
def validate_entity_deletion_keys_dataframe(keys: Union[pysparkDF, pd.DataFrame], join_keys: List[str]):
    ...
    if row_count == 0:
        raise errors.EMPTY_ARGUMENT("join_keys")
    if set(keys.columns) != set(join_keys):
        raise errors.INCORRECT_KEYS(", ".join(list(keys.columns)), ", ".join(join_keys))
