...
class Binomial(Distribution):
    ...
    def plot_bar(self):
        ...
        data = [1 if x < self.n * self.p else 0 for x in range(self.n)]
        plt.hist(data)
    ...
    def pdf(self, k):
        ...
        alpha = math.factorial(self.n) / \
            (math.factorial(k) * (math.factorial(self.n - k)))
        
        return alpha * self.p**k * (1-self.p)**(self.n-k)
    ...
    def plot_bar_pdf(self):
        ...
        y = [self.pdf(x) for x in range(self.n)]
