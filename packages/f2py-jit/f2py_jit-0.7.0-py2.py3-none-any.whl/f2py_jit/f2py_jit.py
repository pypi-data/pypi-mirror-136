...
def build_module(source, metadata=None, extra_args='', db_file='.atooms_jit.json', verbose=False):
    ...
    # Acquire lock
    create_modules_path()
    lock_file = os.path.join(modules_dir, db_file + '.lock')
    delay = 0.1
    while os.path.exists(lock_file):
        import time
        # print(os.getpid(), 'sleeping', delay)
        time.sleep(delay)
        delay *= 2
    # print(os.getpid(), 'acquire lock')
    ...
    # Release lock
    # print(os.getpid(), 'release lock')
    if os.path.exists(lock_file):
        os.remove(lock_file)
    else:
        raise OSError('lock does not exist anymore, while it should')
    return uid
...
def jit(source, extra_args='', verbose=False):
    """
