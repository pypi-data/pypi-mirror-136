...
class data_manager:
    ...
    openingChars = '@@'
    closingChars = '@@'
    NOW = str(time.asctime()).replace(' ', '_').replace(':', '_')
    try:
        NOW = libcommons.get_variable('${NOW}') if libcommons.get_variable('${NOW}') else str(time.asctime()).replace(
            ' ', '_').replace(':', '_')
    except:
        a=1
        #do nothing
    ...
    def processResponseChannelization(data, excludeMethod=None):
        try:
            if data_manager.openingChars in data and data_manager.closingChars in data:
            ...
            if rcType == 'body':
                ...
                if expectedData == 'singleNodeValue':
                    ...
                    if len(kwargs) > 2:
                        ...
                        if libcommons.path_exists(jsonToBeMerged.replace('\t', '\\t')):
                            jsonToBeMerged = open(jsonToBeMerged.replace('\t', '\\t')).read()
