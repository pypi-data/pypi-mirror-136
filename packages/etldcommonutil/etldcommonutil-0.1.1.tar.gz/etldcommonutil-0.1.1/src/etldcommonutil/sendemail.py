"""
This package provides a simplified way to send email. There is no functionality here that could not be achieved
with Python's builtin packages.
"""
...
class MailSender:
    """
    Contains email contents, connection settings and recipient settings. Has functions to compose and send mail.
    MailSenders are tied to an SMTP server, which must be specified when the instance is created.
    ...
    :param use_SSL: Select whether to connect over SSL (True) or TLS (False) (default is TLS). Keep in mind that SMTP
    servers use different ports for SSL and TLS.
    ...
    def __init__(self, in_username, in_password, in_server=("smtp.gmail.com", 587), use_SSL=False):
        ...
        if self.use_SSL:
            ...
            if in_username and in_password:
                ...
                #Uses localhost as SMTP server
                self.smtpserver = smtplib.SMTP(self.server_name)
    ...
    def set_message(self, in_plaintext, in_subject="", in_from=None, in_htmltext=None, attachment=None, filename=None):
        """
        Creates the MIME message to be sent by e-mail. Optionally allows adding subject and 'from' field. Sets up empty
        recipient fields. To use html messages specify an htmltext input
        ...
        :param in_htmltext: HTML version of the email body (optional) (If you want to use an HTML message but
        set it later, pass an empty string here)
        """
