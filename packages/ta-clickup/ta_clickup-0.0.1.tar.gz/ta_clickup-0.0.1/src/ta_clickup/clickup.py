...
class ClickUp:
    def __init__(self, credentials):
        ...
        self.email = credentials.get('login')
        self.password = credentials.get('password')
        self.api = credentials.get('API key')
        self.bearer = self.login(self.email, self.password)
        ...
        for team in self.teams:
            ...
            for space in spaces_response["spaces"]:
                self.spaces[space["id"]] = {"name": space["name"], "team": team}
    ...
    def login(self, email, password):
        ...
        response = requests.request(
            method="GET", url=self.server + uri, data=data
        ).json()
    ...
    def send_request(self, method="GET", uri=None, version="v1", **kwargs):
        ...
        response = requests.request(
            method=method, url=self.server + uri, headers=headers, **kwargs
        )
    ...
    def get_folders(self, space_id, archived=False):
        """get folders associated with a space_id

        Keyword Arguments:
            space_id {str} -- Space ID (default: {None})
            archived {bool} -- Archived (default: {False})

        returns
            response -- JSON object of folders
        """
        archived = str(archived).lower()
        uri = f"api/v2/space/{space_id}/folder?archived={archived}"
        response = self.send_request("GET", uri=uri)
        return response

    def get_tasks(self, list_id, **params):
        """Get tasks from a list id, filter with optional parameters
        ...
        KeywordArguments:
            archived {bool}
            page {int,str} -- Page to fetch (starts at 0)
            order_by {str} -- Order by field, defaults to created Options: id, created, updated, due_date
            reverse {bool} -- Reverse order
            subtasks {bool} -- Include subtasks, default false
            statuses[] {[str,]} -- Statuses to query
            include_closed {bool} -- By default, the api does not include closed tasks. Set this to true and dont send a
             status filter to include closed tasks.
            assignees[] {list} -- Assignees to query
            due_date_gt -- Filter due date greater than Unix time in milliseconds
            due_date_lt -- Filter due date less than Unix time in milliseconds
            date_created_gt -- Filter date created greater than Unix time in milliseconds
            date_created_lt -- Filter date created less than Unix time in milliseconds
            date_updated_gt -- Filter date updated greater than Unix time in milliseconds
            date_updated_lt -- Filter date updated less than Unix time in milliseconds
            custom_fields[] {list} -- Filter by Custom Fields.
                Example: [{"field_id":"de761538-8ae0-42e8-91d9-f1a0cdfbd8b5","operator":">","value":2},{...}]
        """
        uri = f"api/v2/list/{list_id}/task"
        if params:
            response = self.send_request("GET", uri=uri, params=params)
        else:
            response = self.send_request("GET", uri=uri)
        return response

    def get_tasks_by_team(
        self, team_id, space_id=None, include_closed="true", version="v1"
    ):
    ...
    def enrich_task_ids(self, team_id, space_id, task_ids):
        ...
        uri = (
            "v2/task?team_id={}&project_ids%5B%5D={}?fields%5B%5D=assignees&fields%5B%5D=assigned_comments_count&"
            "fields%5B%5D=assigned_checklist_items&fields%5B%5D=attachments_thumbnail_count&"
            "fields%5B%5D=dependency_state&fields%5B%5D=parent_task&fields%5B%5D=attachments_count&"
            "fields%5B%5D=followers&fields%5B%5D=totalTimeSpent&fields%5B%5D=subtasks_count&"
            "fields%5B%5D=subtasks_by_status&fields%5B%5D=tags"
            "&fields%5B%5D=simple_statuses&fields%5B%5D=fallback_coverimage&fields%5B%5D=customFields".format(
                team_id, space_id
            )
        )
    ...
    def create_task(self, subcategory, name, timestamp, estimate=None):
        ...
        uri = "v1/subcategory/{}/task".format(subcategory)
        data = {
            "name": name,
            "assignees": [],
