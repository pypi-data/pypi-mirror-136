...
def get_dtype_size(typingctx, dtype=None):
    ...
    def codegen(context, builder, sig, args):
        ncug__fic = context.get_abi_sizeof(context.get_data_type(dtype.dtype))
        return context.get_constant(types.intp, ncug__fic)
...
def tofile_overload(arr, fname):
    if fname == string_type or isinstance(fname, types.StringLiteral):
        ...
        def tofile_impl(arr, fname):
            ...
            wqn__skm = np.ascontiguousarray(arr)
            dtype_size = get_dtype_size(wqn__skm.dtype)
            file_write(unicode_to_utf8(fname), wqn__skm.ctypes, dtype_size *
                wqn__skm.size)
...
def file_write_parallel_overload(fname, arr, start, count):
    if fname == string_type:
        ...
        def _impl(fname, arr, start, count):
            wqn__skm = np.ascontiguousarray(arr)
            dtype_size = get_dtype_size(wqn__skm.dtype)
            beqvh__znt = dtype_size * bodo.libs.distributed_api.get_tuple_prod(
                wqn__skm.shape[1:])
            _file_write_parallel(unicode_to_utf8(fname), wqn__skm.ctypes,
                start, count, beqvh__znt)
            bodo.utils.utils.check_and_propagate_cpp_exception()
        return _impl
...
def get_file_size_overload(fname, count, offset, dtype_size):
    if fname == string_type:
        ...
        def impl(fname, count, offset, dtype_size):
            if offset < 0:
                return -1
            nqrz__llxq = _get_file_size(unicode_to_utf8(fname)) - offset
            bodo.utils.utils.check_and_propagate_cpp_exception()
            if count != -1:
                nqrz__llxq = min(nqrz__llxq, count * dtype_size)
            if nqrz__llxq < 0:
                return -1
            return nqrz__llxq
        return impl
