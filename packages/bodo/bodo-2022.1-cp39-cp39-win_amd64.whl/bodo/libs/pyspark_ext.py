...
def box_spark_session(typ, val, c):
    josy__xvb = c.context.insert_const_string(c.builder.module, 'pyspark')
    tnb__opps = c.pyapi.import_module_noblock(josy__xvb)
    iyqo__avhbc = c.pyapi.object_getattr_string(tnb__opps, 'sql')
    nwdp__astv = c.pyapi.object_getattr_string(iyqo__avhbc, 'SparkSession')
    wkkik__irvn = c.pyapi.object_getattr_string(nwdp__astv, 'builder')
    yon__qrgc = c.pyapi.call_method(wkkik__irvn, 'getOrCreate', ())
    c.pyapi.decref(tnb__opps)
    c.pyapi.decref(iyqo__avhbc)
    c.pyapi.decref(nwdp__astv)
    c.pyapi.decref(wkkik__irvn)
    return yon__qrgc
...
class RowType(types.BaseNamedTuple):
    ...
    def __init__(self, types, fields):
        ...
        skghh__ibcyn = 'Row({})'.format(', '.join(
            f'{lww__zbpxf}:{hzpz__kvl}' for lww__zbpxf, hzpz__kvl in zip(
            self.fields, self.types)))
        super(RowType, self).__init__(skghh__ibcyn)
...
class RowModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        qmy__zlqff = [(lww__zbpxf, hzpz__kvl) for lww__zbpxf, hzpz__kvl in
            zip(fe_type.fields, fe_type.types)]
        super(RowModel, self).__init__(dmm, fe_type, qmy__zlqff)
...
def typeof_row(val, c):
    ...
    return RowType(tuple(numba.typeof(tpy__bzzr) for tpy__bzzr in val), fields)


@box(RowType)
def box_row(typ, val, c):
    ujrbs__cndsx = c.pyapi.unserialize(c.pyapi.serialize_object(pyspark.sql
        .types.Row))
    if all(lww__zbpxf.startswith(ANON_SENTINEL) for lww__zbpxf in typ.fields):
        crms__sysz = [c.box(hzpz__kvl, c.builder.extract_value(val, i)) for
            i, hzpz__kvl in enumerate(typ.types)]
        gyk__wwek = c.pyapi.call_function_objargs(ujrbs__cndsx, crms__sysz)
        for obj in crms__sysz:
            c.pyapi.decref(obj)
        c.pyapi.decref(ujrbs__cndsx)
        return gyk__wwek
    args = c.pyapi.tuple_pack([])
    crms__sysz = []
    ihz__mgagc = []
    for i, hzpz__kvl in enumerate(typ.types):
        ncx__ujhw = c.builder.extract_value(val, i)
        obj = c.box(hzpz__kvl, ncx__ujhw)
        ihz__mgagc.append((typ.fields[i], obj))
        crms__sysz.append(obj)
    kws = c.pyapi.dict_pack(ihz__mgagc)
    gyk__wwek = c.pyapi.call(ujrbs__cndsx, args, kws)
    for obj in crms__sysz:
        c.pyapi.decref(obj)
    c.pyapi.decref(ujrbs__cndsx)
    c.pyapi.decref(args)
...
class RowConstructor(AbstractTemplate):
    ...
    def generic(self, args, kws):
        ...
        kxnzl__ymkb = ', '.join(f'arg{i}' for i in range(len(args)))
        olvw__zlp = ', '.join(f"{uwbae__juep} = ''" for uwbae__juep in kws)
        func_text = f'def row_stub({kxnzl__ymkb}{olvw__zlp}):\n'
        func_text += '    pass\n'
        obq__syij = {}
        exec(func_text, {}, obq__syij)
        yznp__zgwtv = obq__syij['row_stub']
        fmh__egn = numba.core.utils.pysignature(yznp__zgwtv)
        if args:
            rtpfp__mvemy = RowType(args, tuple(f'{ANON_SENTINEL}{i}' for i in
                range(len(args))))
            return signature(rtpfp__mvemy, *args).replace(pysig=fmh__egn)
        kws = dict(kws)
        rtpfp__mvemy = RowType(tuple(kws.values()), tuple(kws.keys()))
        return signature(rtpfp__mvemy, *kws.values()).replace(pysig=fmh__egn)
...
class SparkDataFrameModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        qmy__zlqff = [('df', fe_type.df)]
        super(SparkDataFrameModel, self).__init__(dmm, fe_type, qmy__zlqff)
...
def overload_create_df(sp_session, data, schema=None, samplingRatio=None,
    ...
    ejbc__xgs = data.dtype.fields
    lgaz__brkxo = len(data.dtype.types)
    func_text = (
        'def impl(sp_session, data, schema=None, samplingRatio=None, verifySchema=True):\n'
        )
    func_text += f'  n = len(data)\n'
    djny__qagys = []
    for i, hzpz__kvl in enumerate(data.dtype.types):
        acxh__aqmei = dtype_to_array_type(hzpz__kvl)
        func_text += (
            f'  A{i} = bodo.utils.utils.alloc_type(n, arr_typ{i}, (-1,))\n')
        djny__qagys.append(acxh__aqmei)
    func_text += f'  for i in range(n):\n'
    func_text += f'    r = data[i]\n'
    for i in range(lgaz__brkxo):
        func_text += (
            f'    A{i}[i] = bodo.utils.conversion.unbox_if_timestamp(r[{i}])\n'
            )
    payj__byqax = '({}{})'.format(', '.join(f'A{i}' for i in range(
        lgaz__brkxo)), ',' if len(ejbc__xgs) == 1 else '')
    func_text += (
        '  index = bodo.hiframes.pd_index_ext.init_range_index(0, n, 1, None)\n'
        )
    func_text += f"""  pdf = bodo.hiframes.pd_dataframe_ext.init_dataframe({payj__byqax}, index, {ejbc__xgs})
"""
    ...
    znoj__coq = {'bodo': bodo}
    for i in range(lgaz__brkxo):
        znoj__coq[f'arr_typ{i}'] = djny__qagys[i]
    exec(func_text, znoj__coq, obq__syij)
    impl = obq__syij['impl']
    return impl
...
def overload_df_to_rows(df):
    ...
    byajy__wfps = ', '.join(f'{c}=A{i}[i]' for i, c in enumerate(df.columns))
    func_text += f'    out.append(Row({byajy__wfps}))\n'
    func_text += '  return out\n'
    obq__syij = {}
    znoj__coq = {'bodo': bodo, 'Row': pyspark.sql.types.Row}
    exec(func_text, znoj__coq, obq__syij)
    impl = obq__syij['impl']
    return impl
...
def _gen_df_select(spark_df, cols, avoid_stararg=False):
    ...
    for col in cols:
        ...
        data, off__yys = _gen_col_code(col, df_type)
        func_text += off__yys
...
def _gen_init_spark_df(func_text, out_data, out_col_names):
    payj__byqax = '({}{})'.format(', '.join(out_data), ',' if len(out_data) ==
        1 else '')
    beh__bglnf = '0' if not out_data else f'len({out_data[0]})'
    func_text += f'  n = {beh__bglnf}\n'
    func_text += (
        '  index = bodo.hiframes.pd_index_ext.init_range_index(0, n, 1, None)\n'
        )
    func_text += f"""  pdf = bodo.hiframes.pd_dataframe_ext.init_dataframe({payj__byqax}, index, {tuple(out_col_names)})
"""
    func_text += f'  return bodo.libs.pyspark_ext.init_spark_df(pdf)\n'
    obq__syij = {}
    znoj__coq = {'bodo': bodo, 'np': np}
    exec(func_text, znoj__coq, obq__syij)
    impl = obq__syij['impl']
    return impl
...
def overload_show(spark_df, n=20, truncate=True, vertical=False):
    zlz__hthn = dict(truncate=truncate, vertical=vertical)
    xrhf__wya = dict(truncate=True, vertical=False)
    check_unsupported_args('SparkDataFrameType.show', zlz__hthn, xrhf__wya)
...
def overload_with_column(spark_df, colName, col):
    ...
    lbwy__oyl = spark_df.df.columns
    cgpoa__azgbt = lbwy__oyl if col_name in lbwy__oyl else lbwy__oyl + (
        col_name,)
    uioa__rewnn, mfhpd__wdn = _gen_col_code(col, spark_df.df)
    out_data = [(uioa__rewnn if c == col_name else
        f'bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df, {lbwy__oyl.index(c)})'
        ) for c in cgpoa__azgbt]
    func_text = 'def impl(spark_df, colName, col):\n'
    func_text += '  df = spark_df._df\n'
    func_text += mfhpd__wdn
    return _gen_init_spark_df(func_text, out_data, cgpoa__azgbt)
...
def overload_with_column_renamed(spark_df, existing, new):
    ...
    wix__gtju = get_overload_const_str(existing)
    gwmt__spiaf = get_overload_const_str(new)
    lbwy__oyl = spark_df.df.columns
    cgpoa__azgbt = tuple(gwmt__spiaf if c == wix__gtju else c for c in
        lbwy__oyl)
    out_data = [f'bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df, {i})'
         for i in range(len(lbwy__oyl))]
...
def overload_dataframe_columns(spark_df):
    eik__ggwze = list(str(uwbae__juep) for uwbae__juep in spark_df.df.columns)
    func_text = 'def impl(spark_df):\n'
    func_text += f'  return {eik__ggwze}\n'
    obq__syij = {}
    exec(func_text, {}, obq__syij)
    impl = obq__syij['impl']
    return impl
...
def init_col_from_name(typingctx, col=None):
    ...
    ijnv__xxwy = get_overload_const_str(col)
    uaw__jnar = ColumnType(ExprType('col', (ijnv__xxwy,)))

    def codegen(context, builder, signature, args):
        return context.get_constant_null(uaw__jnar)
    return uaw__jnar(col), codegen
...
def init_f_sum(typingctx, col=None):
    uaw__jnar = ColumnType(ExprType('sum', (col.expr,)))
...
def _gen_col_code_expr(expr, df_type):
    ...
    if expr.op == 'sum':
        dtl__cnmx, bsj__obmg = _gen_col_code_expr(expr.children[0], df_type)
        i = ir_utils.next_label()
        func_text = f"""  A{i} = np.asarray([bodo.libs.array_ops.array_op_sum({dtl__cnmx}, True, 0)])
"""
        return f'A{i}', bsj__obmg + func_text


def _gen_col_code_colname(col_name, df_type):
    dfaug__ajt = df_type.columns.index(col_name)
    i = ir_utils.next_label()
    func_text = (
        f'  A{i} = bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df, {dfaug__ajt})\n'
        )
