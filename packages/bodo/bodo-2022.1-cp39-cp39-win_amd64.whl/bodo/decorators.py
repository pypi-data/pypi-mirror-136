...
def distributed_diagnostics(self, signature=None, level=1):
    ...
    def dump(sig):
        hzo__ygkw = self.overloads[sig]
        hmqj__ifkmr = hzo__ygkw.metadata.get('distributed_diagnostics', None)
        if hmqj__ifkmr is None:
            hfcx__ooh = 'No distributed diagnostic available'
            raise bodo.utils.typing.BodoError(hfcx__ooh)
        hmqj__ifkmr.dump(level, self.get_metadata(sig))
...
def master_mode_wrapper(numba_jit_wrapper):
    ...
    def _wrapper(pyfunc):
        eeoe__usi = numba_jit_wrapper(pyfunc)
        return master_mode.MasterModeDispatcher(eeoe__usi)
    return _wrapper
...
def jit(signature_or_function=None, pipeline_class=None, **options):
    ...
    if 'distributed' in options and isinstance(options['distributed'], bool):
        fpd__zmwiz = options.pop('distributed')
        pipeline_class = (pipeline_class if fpd__zmwiz else bodo.compiler.
            BodoCompilerSeq)
    jsa__ildzq = numba.jit(signature_or_function, pipeline_class=
        pipeline_class, **options)
    if master_mode.master_mode_on and bodo.get_rank(
        ) == master_mode.MASTER_RANK:
        if isinstance(jsa__ildzq, numba.dispatcher._DispatcherBase):
            return master_mode.MasterModeDispatcher(jsa__ildzq)
        else:
            return master_mode_wrapper(jsa__ildzq)
    else:
        return jsa__ildzq
