...
def overload_h5_file_keys(obj_id):
    ...
    def h5f_keys_impl(obj_id):
        hpqi__fblpk = []
        yghf__ceddd = h5g_get_num_objs(obj_id)
        for ddntf__vvj in range(yghf__ceddd):
            hufa__bxnhh = h5g_get_objname_by_idx(obj_id, ddntf__vvj)
            hpqi__fblpk.append(hufa__bxnhh)
        return hpqi__fblpk
...
def overload_h5_file_create_dataset(obj_id, name, shape=None, dtype=None,
    ...
    pwk__ljj = parse_dtype(dtype)
    zdzt__jzd = np.int32(numba_to_c_type(pwk__ljj))
...
def overload_getitem_file(in_f, in_idx):
    if in_f in (h5file_type, h5dataset_or_group_type
        ...
        def impl(in_f, in_idx):
            lrhhi__nsl = h5_open_dset_or_group_obj(unify_h5_id(in_f),
...
def h5read(dset_id, ndim, starts, counts, is_parallel, out_arr):
    ovkad__vqm = tuple_to_ptr(starts)
    eji__tiawx = tuple_to_ptr(counts)
    emwnk__zsf = bodo.libs.distributed_api.get_type_enum(out_arr)
    return _h5_read(cast_to_h5_dset(dset_id), ndim, ovkad__vqm, eji__tiawx,
        is_parallel, out_arr.ctypes, emwnk__zsf)
...
def h5write(dset_id, ndim, starts, counts, is_parallel, out_arr):
    ...
    eji__tiawx = tuple_to_ptr(counts)
    emwnk__zsf = bodo.libs.distributed_api.get_type_enum(out_arr)
    return _h5_write(cast_to_h5_dset(dset_id), ndim, ovkad__vqm, eji__tiawx,
        is_parallel, out_arr.ctypes, emwnk__zsf)
...
class H5ReadType(AbstractTemplate):
    ...
    def generic(self, args, kws):
        ...
        nila__pnnaa = types.Array(dtype, ndim, 'C')
        qpsdn__gup = args[3]
        nila__pnnaa = self.context.resolve_function_type(operator.getitem,
            [nila__pnnaa, qpsdn__gup], {}).return_type.copy(layout='C')
        return signature(nila__pnnaa, *args)
...
def get_filter_read_indices(bool_arr):
    lfax__hsd = bool_arr.nonzero()[0]
    wris__zrixt = bodo.libs.distributed_api.get_rank()
    skhor__xgoba = bodo.libs.distributed_api.get_size()
    rmnlc__kkygt = np.empty(skhor__xgoba, np.int64)
    ydzpp__wbefk = len(bool_arr)
    bodo.libs.distributed_api.allgather(rmnlc__kkygt, ydzpp__wbefk)
    xipj__ijhhs = rmnlc__kkygt.cumsum()[wris__zrixt] - ydzpp__wbefk
    lfax__hsd += xipj__ijhhs
    ynvjw__bby = bodo.libs.distributed_api.dist_reduce(len(lfax__hsd), np.
        int32(sum_op))
    cdp__fwe = bodo.libs.distributed_api.gatherv(lfax__hsd)
    if wris__zrixt == 0:
        oqgau__cetm = cdp__fwe
    else:
        oqgau__cetm = np.empty(ynvjw__bby, lfax__hsd.dtype)
    bodo.libs.distributed_api.bcast(oqgau__cetm)
    xyo__wfg = bodo.libs.distributed_api.get_start(ynvjw__bby, skhor__xgoba,
        wris__zrixt)
    pjh__qljsx = bodo.libs.distributed_api.get_end(ynvjw__bby, skhor__xgoba,
        wris__zrixt)
    return oqgau__cetm[xyo__wfg:pjh__qljsx]
...
def tuple_to_ptr(typingctx, tuple_tp=None):
    ...
    def codegen(context, builder, sig, args):
        czzy__hgiyv = cgutils.alloca_once(builder, args[0].type)
        builder.store(args[0], czzy__hgiyv)
        return builder.bitcast(czzy__hgiyv, lir.IntType(8).as_pointer())
...
def h5read_filter(dset_id, ndim, starts, counts, is_parallel, out_arr,
    ...
    eji__tiawx = tuple_to_ptr(counts)
    emwnk__zsf = bodo.libs.distributed_api.get_type_enum(out_arr)
    return _h5read_filter(cast_to_h5_dset(dset_id), ndim, ovkad__vqm,
        eji__tiawx, is_parallel, out_arr.ctypes, emwnk__zsf, read_indices.
