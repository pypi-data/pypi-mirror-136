...
def check_sklearn_version():
    if not bodo.compiler._is_sklearn_supported_version:
        ebbxd__gkbd = f""" Bodo requires at most version {bodo.compiler._max_sklearn_ver_str} of scikit-learn.
             Installed version is {sklearn.__version__}.
"""
        raise BodoError(ebbxd__gkbd)


def random_forest_model_fit(m, X, y):
    khbh__vofxj = m.n_estimators
    evedv__klpru = MPI.Get_processor_name()
    ozv__eyn = get_host_ranks()
    lhwaz__cfk = len(ozv__eyn)
    hxnbg__odkv = bodo.get_rank()
    m.n_estimators = bodo.libs.distributed_api.get_node_portion(khbh__vofxj,
        lhwaz__cfk, hxnbg__odkv)
    if hxnbg__odkv == ozv__eyn[evedv__klpru][0]:
        m.n_jobs = len(ozv__eyn[evedv__klpru])
    ...
    rscda__svj = create_subcomm_mpi4py(first_rank_node)
    if rscda__svj != MPI.COMM_NULL:
        qejz__bla = 10
        igho__pulwx = bodo.libs.distributed_api.get_node_portion(khbh__vofxj,
            lhwaz__cfk, 0)
        yvgq__wvtv = igho__pulwx // qejz__bla
        if igho__pulwx % qejz__bla != 0:
            yvgq__wvtv += 1
        tiz__ffcqb = []
        for rqooo__jrv in range(yvgq__wvtv):
            got__srwsu = rscda__svj.gather(m.estimators_[rqooo__jrv *
                qejz__bla:rqooo__jrv * qejz__bla + qejz__bla])
            if hxnbg__odkv == 0:
                tiz__ffcqb += list(itertools.chain.from_iterable(got__srwsu))
        if hxnbg__odkv == 0:
            m.estimators_ = tiz__ffcqb
    kmiao__lakc = MPI.COMM_WORLD
    if hxnbg__odkv == 0:
        for rqooo__jrv in range(0, khbh__vofxj, 10):
            kmiao__lakc.bcast(m.estimators_[rqooo__jrv:rqooo__jrv + 10])
        if isinstance(m, sklearn.ensemble.RandomForestClassifier):
            kmiao__lakc.bcast(m.n_classes_)
            kmiao__lakc.bcast(m.classes_)
        kmiao__lakc.bcast(m.n_outputs_)
    else:
        fzm__wwsqz = []
        for rqooo__jrv in range(0, khbh__vofxj, 10):
            fzm__wwsqz += kmiao__lakc.bcast(None)
        if isinstance(m, sklearn.ensemble.RandomForestClassifier):
            m.n_classes_ = kmiao__lakc.bcast(None)
            m.classes_ = kmiao__lakc.bcast(None)
        m.n_outputs_ = kmiao__lakc.bcast(None)
        m.estimators_ = fzm__wwsqz
    assert len(m.estimators_) == khbh__vofxj
    m.n_estimators = khbh__vofxj
...
def precision_recall_fscore_support_helper(MCM, average):
    ...
    def multilabel_confusion_matrix(y_true, y_pred, *, sample_weight=None,
        ...
        return MCM
    lzxoo__ovw = sklearn.metrics._classification.multilabel_confusion_matrix
    result = -1.0
    try:
...
def precision_recall_fscore_parallel(y_true, y_pred, operation, average=
    ...
    smb__kyuw = len(labels)
    wst__jpwd = np.zeros(smb__kyuw, np.int64)
    eedu__csuk = np.zeros(smb__kyuw, np.int64)
    ihae__svr = np.zeros(smb__kyuw, np.int64)
    ssbl__julp = (bodo.hiframes.pd_categorical_ext.
        get_label_dict_from_categories(labels))
    for rqooo__jrv in range(len(y_true)):
        eedu__csuk[ssbl__julp[y_true[rqooo__jrv]]] += 1
        if y_pred[rqooo__jrv] not in ssbl__julp:
            continue
        svq__wfoe = ssbl__julp[y_pred[rqooo__jrv]]
        ihae__svr[svq__wfoe] += 1
        if y_true[rqooo__jrv] == y_pred[rqooo__jrv]:
            wst__jpwd[svq__wfoe] += 1
    wst__jpwd = bodo.libs.distributed_api.dist_reduce(wst__jpwd, np.int32(
        Reduce_Type.Sum.value))
    eedu__csuk = bodo.libs.distributed_api.dist_reduce(eedu__csuk, np.int32
        (Reduce_Type.Sum.value))
    ihae__svr = bodo.libs.distributed_api.dist_reduce(ihae__svr, np.int32(
        Reduce_Type.Sum.value))
    cgvy__muow = ihae__svr - wst__jpwd
    vztpy__ixg = eedu__csuk - wst__jpwd
    wayq__urel = wst__jpwd
    jtq__xekt = y_true.shape[0] - wayq__urel - cgvy__muow - vztpy__ixg
    with numba.objmode(result='float64[:]'):
        MCM = np.array([jtq__xekt, cgvy__muow, vztpy__ixg, wayq__urel]
            ).T.reshape(-1, 2, 2)
...
def mse_mae_dist_helper(y_true, y_pred, sample_weight, multioutput, squared,
    metric):
    if metric == 'mse':
        ijf__xwbaj = sklearn.metrics.mean_squared_error(y_true, y_pred,
            sample_weight=sample_weight, multioutput='raw_values', squared=True
            )
    elif metric == 'mae':
        ijf__xwbaj = sklearn.metrics.mean_absolute_error(y_true, y_pred,
            sample_weight=sample_weight, multioutput='raw_values')
    else:
        raise RuntimeError(
            f"Unrecognized metric {metric}. Must be one of 'mae' and 'mse'")
    kmiao__lakc = MPI.COMM_WORLD
    zoge__uxan = kmiao__lakc.Get_size()
    if sample_weight is not None:
        duvk__alz = np.sum(sample_weight)
    else:
        duvk__alz = np.float64(y_true.shape[0])
    pdcpg__imkao = np.zeros(zoge__uxan, dtype=type(duvk__alz))
    kmiao__lakc.Allgather(duvk__alz, pdcpg__imkao)
    rmy__fpe = np.zeros((zoge__uxan, *ijf__xwbaj.shape), dtype=ijf__xwbaj.dtype
        )
    kmiao__lakc.Allgather(ijf__xwbaj, rmy__fpe)
    rycxm__ofn = np.average(rmy__fpe, weights=pdcpg__imkao, axis=0)
    if metric == 'mse' and not squared:
        rycxm__ofn = np.sqrt(rycxm__ofn)
    if isinstance(multioutput, str) and multioutput == 'raw_values':
        return rycxm__ofn
    elif isinstance(multioutput, str) and multioutput == 'uniform_average':
        return np.average(rycxm__ofn)
    else:
        return np.average(rycxm__ofn, weights=multioutput)
...
def accuracy_score_dist_helper(y_true, y_pred, normalize, sample_weight):
    ...
    kmiao__lakc = MPI.COMM_WORLD
    score = kmiao__lakc.allreduce(score, op=MPI.SUM)
    if normalize:
        pqqm__faoro = np.sum(sample_weight
            ) if sample_weight is not None else len(y_true)
        pqqm__faoro = kmiao__lakc.allreduce(pqqm__faoro, op=MPI.SUM)
        score = score / pqqm__faoro
    return score
...
def check_consistent_length_parallel(*arrays):
    kmiao__lakc = MPI.COMM_WORLD
    pvx__myv = True
    qvxxo__umzwr = [len(irsg__boirb) for irsg__boirb in arrays if 
        irsg__boirb is not None]
    if len(np.unique(qvxxo__umzwr)) > 1:
        pvx__myv = False
    pvx__myv = kmiao__lakc.allreduce(pvx__myv, op=MPI.LAND)
    return pvx__myv
...
def r2_score_dist_helper(y_true, y_pred, sample_weight, multioutput):
    ...
    hkbmi__docri = y_true.shape[0]
    dfcb__nqggk = kmiao__lakc.allreduce(hkbmi__docri, op=MPI.SUM)
    if dfcb__nqggk < 2:
        warnings.warn(
    ...
    if sample_weight is not None:
        ...
        rec__qatap = sample_weight[:, np.newaxis]
    else:
        sample_weight = np.float64(y_true.shape[0])
        rec__qatap = 1.0
    nrnm__dnvx = (rec__qatap * (y_true - y_pred) ** 2).sum(axis=0, dtype=np
        .float64)
    wtc__cfiry = np.zeros(nrnm__dnvx.shape, dtype=nrnm__dnvx.dtype)
    kmiao__lakc.Allreduce(nrnm__dnvx, wtc__cfiry, op=MPI.SUM)
    bzic__ugz = np.nansum(y_true * rec__qatap, axis=0, dtype=np.float64)
    gtsfy__vstr = np.zeros_like(bzic__ugz)
    kmiao__lakc.Allreduce(bzic__ugz, gtsfy__vstr, op=MPI.SUM)
    gti__txn = np.nansum(sample_weight, dtype=np.float64)
    add__ssmu = kmiao__lakc.allreduce(gti__txn, op=MPI.SUM)
    cxd__wfh = gtsfy__vstr / add__ssmu
    uxcqf__tjm = (rec__qatap * (y_true - cxd__wfh) ** 2).sum(axis=0, dtype=
        np.float64)
    figjr__kzq = np.zeros(uxcqf__tjm.shape, dtype=uxcqf__tjm.dtype)
    kmiao__lakc.Allreduce(uxcqf__tjm, figjr__kzq, op=MPI.SUM)
    iwyh__lpxsq = figjr__kzq != 0
    pjyse__rjirw = wtc__cfiry != 0
    kgc__vvugj = iwyh__lpxsq & pjyse__rjirw
    xlpff__zgzrm = np.ones([y_true.shape[1] if len(y_true.shape) > 1 else 1])
    xlpff__zgzrm[kgc__vvugj] = 1 - wtc__cfiry[kgc__vvugj] / figjr__kzq[
        kgc__vvugj]
    xlpff__zgzrm[pjyse__rjirw & ~iwyh__lpxsq] = 0.0
    if isinstance(multioutput, str):
        if multioutput == 'raw_values':
            ...
            yqk__pbv = figjr__kzq
            if not np.any(iwyh__lpxsq):
                if not np.any(pjyse__rjirw):
                    return np.array([1.0])
                else:
                    return np.array([0.0])
    else:
        yqk__pbv = multioutput
    return np.array([np.average(xlpff__zgzrm, weights=yqk__pbv)])
...
def confusion_matrix_dist_helper(y_true, y_pred, labels=None, sample_weight
    ...
    kmiao__lakc = MPI.COMM_WORLD
    try:
        csgz__lwzaz = sklearn.metrics.confusion_matrix(y_true, y_pred,
            labels=labels, sample_weight=sample_weight, normalize=None)
    except ValueError as yqa__uyiu:
        csgz__lwzaz = yqa__uyiu
    mqwnk__hbjio = isinstance(csgz__lwzaz, ValueError
        ) and 'At least one label specified must be in y_true' in csgz__lwzaz.args[
        0]
    loml__bwikq = kmiao__lakc.allreduce(mqwnk__hbjio, op=MPI.LAND)
    if loml__bwikq:
        raise csgz__lwzaz
    elif mqwnk__hbjio:
        dtype = np.int64
        if sample_weight is not None and sample_weight.dtype.kind not in {'i',
            'u', 'b'}:
            dtype = np.float64
        nyyv__jgji = np.zeros((labels.size, labels.size), dtype=dtype)
    else:
        nyyv__jgji = csgz__lwzaz
    quzp__kdiyi = np.zeros_like(nyyv__jgji)
    kmiao__lakc.Allreduce(nyyv__jgji, quzp__kdiyi)
    with np.errstate(all='ignore'):
        if normalize == 'true':
            quzp__kdiyi = quzp__kdiyi / quzp__kdiyi.sum(axis=1, keepdims=True)
        elif normalize == 'pred':
            quzp__kdiyi = quzp__kdiyi / quzp__kdiyi.sum(axis=0, keepdims=True)
        elif normalize == 'all':
            quzp__kdiyi = quzp__kdiyi / quzp__kdiyi.sum()
        quzp__kdiyi = np.nan_to_num(quzp__kdiyi)
    return quzp__kdiyi
...
def overload_confusion_matrix(y_true, y_pred, labels=None, sample_weight=
    ...
    check_sklearn_version()
    qghpd__ypmu = 'def _confusion_matrix_impl(\n'
    qghpd__ypmu += '    y_true, y_pred, labels=None,\n'
    qghpd__ypmu += '    sample_weight=None, normalize=None,\n'
    qghpd__ypmu += '    _is_data_distributed=False,\n'
    qghpd__ypmu += '):\n'
    qghpd__ypmu += (
        '    y_true = bodo.utils.conversion.coerce_to_array(y_true)\n')
    qghpd__ypmu += (
        '    y_pred = bodo.utils.conversion.coerce_to_array(y_pred)\n')
    wzis__vbxv = 'int64[:,:]', 'np.int64'
    if not is_overload_none(normalize):
        wzis__vbxv = 'float64[:,:]', 'np.float64'
    ...
    if not is_overload_none(labels):
        ...
        qghpd__ypmu += '    labels = bodo.allgatherv(labels, False)\n'
        qghpd__ypmu += """    labels = bodo.libs.array_kernels.sort(labels, ascending=True, inplace=False)
"""
    qghpd__ypmu += f"    with numba.objmode(cm='{wzis__vbxv[0]}'):\n"
    if is_overload_false(_is_data_distributed):
        qghpd__ypmu += '      cm = sklearn.metrics.confusion_matrix(\n'
    else:
        qghpd__ypmu += '      cm = confusion_matrix_dist_helper(\n'
    qghpd__ypmu += '        y_true, y_pred, labels=labels,\n'
    qghpd__ypmu += (
        '        sample_weight=sample_weight, normalize=normalize,\n')
    qghpd__ypmu += f'      ).astype({wzis__vbxv[1]})\n'
    qghpd__ypmu += '    return cm\n'
    bttod__rgvlw = {}
    exec(qghpd__ypmu, globals(), bttod__rgvlw)
    mhed__ogsf = bttod__rgvlw['_confusion_matrix_impl']
    return mhed__ogsf
...
def fit_sgd(m, X, y, y_classes=None, _is_data_distributed=False):
    kmiao__lakc = MPI.COMM_WORLD
    ofbx__wgp = kmiao__lakc.allreduce(len(X), op=MPI.SUM)
    yutzw__kgkc = len(X) / ofbx__wgp
    eopa__cyay = kmiao__lakc.Get_size()
    m.n_jobs = 1
    ...
    if m.loss == 'hinge':
        ...
        xix__wot = mean_squared_error
    else:
        raise ValueError('loss {} not supported'.format(m.loss))
    zxuk__djvjk = False
    if isinstance(m, sklearn.linear_model.SGDRegressor):
        zxuk__djvjk = True
    for tct__cgji in range(m.max_iter):
        if zxuk__djvjk:
            m.partial_fit(X, y)
        else:
            m.partial_fit(X, y, classes=y_classes)
        m.coef_ = m.coef_ * yutzw__kgkc
        m.coef_ = kmiao__lakc.allreduce(m.coef_, op=MPI.SUM)
        m.intercept_ = m.intercept_ * yutzw__kgkc
        m.intercept_ = kmiao__lakc.allreduce(m.intercept_, op=MPI.SUM)
        if zxuk__djvjk:
            y_pred = m.predict(X)
            iwklc__tcyoh = xix__wot(y, y_pred)
        else:
            y_pred = m.decision_function(X)
            iwklc__tcyoh = xix__wot(y, y_pred, labels=y_classes)
        sahvb__jrgqw = kmiao__lakc.allreduce(iwklc__tcyoh, op=MPI.SUM)
        iwklc__tcyoh = sahvb__jrgqw / eopa__cyay
        if m.tol > np.NINF and iwklc__tcyoh > evemx__xhmc - m.tol * ofbx__wgp:
            zyzns__blbs += 1
        else:
            zyzns__blbs = 0
        if iwklc__tcyoh < evemx__xhmc:
            evemx__xhmc = iwklc__tcyoh
        if zyzns__blbs >= m.n_iter_no_change:
            break
    return m
...
def kmeans_fit_helper(m, len_X, all_X, all_sample_weight, _is_data_distributed
    ...
    kmiao__lakc = MPI.COMM_WORLD
    hxnbg__odkv = kmiao__lakc.Get_rank()
    evedv__klpru = MPI.Get_processor_name()
    ozv__eyn = get_host_ranks()
    futv__cro = m.n_jobs if hasattr(m, 'n_jobs') else None
    eua__cnhqw = m._n_threads if hasattr(m, '_n_threads') else None
    m.n_jobs = len(ozv__eyn[evedv__klpru])
    m._n_threads = len(ozv__eyn[evedv__klpru])
    if hxnbg__odkv == 0:
        m.fit(X=all_X, y=None, sample_weight=all_sample_weight)
    if hxnbg__odkv == 0:
        kmiao__lakc.bcast(m.cluster_centers_)
        kmiao__lakc.bcast(m.inertia_)
        kmiao__lakc.bcast(m.n_iter_)
    else:
        m.cluster_centers_ = kmiao__lakc.bcast(None)
        m.inertia_ = kmiao__lakc.bcast(None)
        m.n_iter_ = kmiao__lakc.bcast(None)
    if _is_data_distributed:
        lztm__mjszp = kmiao__lakc.allgather(len_X)
        if hxnbg__odkv == 0:
            hmrrv__qlze = np.empty(len(lztm__mjszp) + 1, dtype=int)
            np.cumsum(lztm__mjszp, out=hmrrv__qlze[1:])
            hmrrv__qlze[0] = 0
            kyudg__rvxuh = [m.labels_[hmrrv__qlze[hjoih__qevmk]:hmrrv__qlze
                [hjoih__qevmk + 1]] for hjoih__qevmk in range(len(lztm__mjszp))
                ]
            olth__vbk = kmiao__lakc.scatter(kyudg__rvxuh)
        else:
            olth__vbk = kmiao__lakc.scatter(None)
        m.labels_ = olth__vbk
    elif hxnbg__odkv == 0:
        kmiao__lakc.bcast(m.labels_)
    else:
        m.labels_ = kmiao__lakc.bcast(None)
    m.n_jobs = futv__cro
    m._n_threads = eua__cnhqw
    return m
...
def overload_kmeans_clustering_score(m, X, y=None, sample_weight=None,
    ...
    def _cluster_kmeans_score(m, X, y=None, sample_weight=None,
        ...
        with numba.objmode(result='float64'):
            ...
            if _is_data_distributed:
                kmiao__lakc = MPI.COMM_WORLD
                result = kmiao__lakc.allreduce(result, op=MPI.SUM)
            m._n_threads = eua__cnhqw
            m.n_jobs = futv__cro
        return result
...
def overload_multinomial_nb_model_fit(m, X, y, sample_weight=None,
    ...
    if is_overload_false(_is_data_distributed):
        ...
        qghpd__ypmu = 'def _model_multinomial_nb_fit_impl(\n'
        qghpd__ypmu += (
            '    m, X, y, sample_weight=None, _is_data_distributed=False\n')
        qghpd__ypmu += '):  # pragma: no cover\n'
        qghpd__ypmu += '    y = bodo.utils.conversion.coerce_to_ndarray(y)\n'
        if isinstance(X, DataFrameType):
            qghpd__ypmu += '    X = X.to_numpy()\n'
        else:
            qghpd__ypmu += (
                '    X = bodo.utils.conversion.coerce_to_ndarray(X)\n')
        qghpd__ypmu += '    my_rank = bodo.get_rank()\n'
        qghpd__ypmu += '    nranks = bodo.get_size()\n'
        qghpd__ypmu += '    total_cols = X.shape[1]\n'
        qghpd__ypmu += '    for i in range(nranks):\n'
        qghpd__ypmu += """        start = bodo.libs.distributed_api.get_start(total_cols, nranks, i)
"""
        qghpd__ypmu += (
            '        end = bodo.libs.distributed_api.get_end(total_cols, nranks, i)\n'
            )
        qghpd__ypmu += '        if i == my_rank:\n'
        qghpd__ypmu += (
            '            X_train = bodo.gatherv(X[:, start:end:1], root=i)\n')
        qghpd__ypmu += '        else:\n'
        qghpd__ypmu += '            bodo.gatherv(X[:, start:end:1], root=i)\n'
        qghpd__ypmu += '    y_train = bodo.allgatherv(y, False)\n'
        qghpd__ypmu += '    with numba.objmode(m="multinomial_nb_type"):\n'
        qghpd__ypmu += '        m = fit_multinomial_nb(\n'
        qghpd__ypmu += """            m, X_train, y_train, sample_weight, total_cols, _is_data_distributed
"""
        qghpd__ypmu += '        )\n'
        qghpd__ypmu += '    bodo.barrier()\n'
        qghpd__ypmu += '    return m\n'
        bttod__rgvlw = {}
        exec(qghpd__ypmu, globals(), bttod__rgvlw)
        lqs__syhnl = bttod__rgvlw['_model_multinomial_nb_fit_impl']
        return lqs__syhnl
...
def fit_multinomial_nb(m, X_train, y_train, sample_weight=None, total_cols=
    ...
    tct__cgji, n_features = X_train.shape
    m.n_features_ = n_features
    fjmoq__ssu = LabelBinarizer()
    oupb__qneiz = fjmoq__ssu.fit_transform(y_train)
    m.classes_ = fjmoq__ssu.classes_
    if oupb__qneiz.shape[1] == 1:
        oupb__qneiz = np.concatenate((1 - oupb__qneiz, oupb__qneiz), axis=1)
    if sample_weight is not None:
        oupb__qneiz = oupb__qneiz.astype(np.float64, copy=False)
        sample_weight = _check_sample_weight(sample_weight, X_train)
        sample_weight = np.atleast_2d(sample_weight)
        oupb__qneiz *= sample_weight.T
    class_prior = m.class_prior
    ydf__awsw = oupb__qneiz.shape[1]
    m._init_counters(ydf__awsw, n_features)
    m._count(X_train.astype('float64'), oupb__qneiz)
    alpha = m._check_alpha()
    m._update_class_log_prior(class_prior=class_prior)
    slq__tid = m.feature_count_ + alpha
    uas__hsjec = slq__tid.sum(axis=1)
    kmiao__lakc = MPI.COMM_WORLD
    eopa__cyay = kmiao__lakc.Get_size()
    tgju__qnyp = np.zeros(ydf__awsw)
    kmiao__lakc.Allreduce(uas__hsjec, tgju__qnyp, op=MPI.SUM)
    boa__klkw = np.log(slq__tid) - np.log(tgju__qnyp.reshape(-1, 1))
    dnlgb__wzn = boa__klkw.T.reshape(n_features * ydf__awsw)
    zhhoo__mbxn = np.ones(eopa__cyay) * (total_cols // eopa__cyay)
    fcn__bfj = total_cols % eopa__cyay
    for git__vcgbs in range(fcn__bfj):
        zhhoo__mbxn[git__vcgbs] += 1
    zhhoo__mbxn *= ydf__awsw
    bcxft__ihke = np.zeros(eopa__cyay, dtype=np.int32)
    bcxft__ihke[1:] = np.cumsum(zhhoo__mbxn)[:-1]
    xyg__znc = np.zeros((total_cols, ydf__awsw), dtype=np.float64)
    kmiao__lakc.Allgatherv(dnlgb__wzn, [xyg__znc, zhhoo__mbxn, bcxft__ihke,
        MPI.DOUBLE_PRECISION])
    m.feature_log_prob_ = xyg__znc.T
...
def sklearn_preprocessing_standard_scaler_fit_dist_helper(m, X):
    kmiao__lakc = MPI.COMM_WORLD
    zoge__uxan = kmiao__lakc.Get_size()
    rkubk__pvcj = m.with_std
    hgrd__pwe = m.with_mean
    m.with_std = False
    if rkubk__pvcj:
        m.with_mean = True
    m = m.fit(X)
    m.with_std = rkubk__pvcj
    m.with_mean = hgrd__pwe
    if not isinstance(m.n_samples_seen_, numbers.Integral):
        ...
        m.n_samples_seen_ = np.repeat(m.n_samples_seen_, X.shape[1]).astype(np
            .int64, copy=False)
    mlzm__jsgr = np.zeros((zoge__uxan, *m.n_samples_seen_.shape), dtype=m.
        n_samples_seen_.dtype)
    kmiao__lakc.Allgather(m.n_samples_seen_, mlzm__jsgr)
    tjcll__mguya = np.sum(mlzm__jsgr, axis=0)
    m.n_samples_seen_ = tjcll__mguya
    if m.with_mean or m.with_std:
        wigr__fhukl = np.zeros((zoge__uxan, *m.mean_.shape), dtype=m.mean_.
            dtype)
        kmiao__lakc.Allgather(m.mean_, wigr__fhukl)
        wigr__fhukl[np.isnan(wigr__fhukl)] = 0
        rykjj__qjmgl = np.average(wigr__fhukl, axis=0, weights=mlzm__jsgr)
        m.mean_ = rykjj__qjmgl
    if m.with_std:
        hnpv__lhfx = sklearn_safe_accumulator_op(np.nansum, (X -
            rykjj__qjmgl) ** 2, axis=0) / tjcll__mguya
        nrew__jimjk = np.zeros_like(hnpv__lhfx)
        kmiao__lakc.Allreduce(hnpv__lhfx, nrew__jimjk, op=MPI.SUM)
        m.var_ = nrew__jimjk
        m.scale_ = sklearn_handle_zeros_in_scale(np.sqrt(m.var_))
    dxh__ntp = kmiao__lakc.allreduce(dxh__ntp, op=MPI.LAND)
    if dxh__ntp:
...
def get_data_slice_parallel(data, labels, len_train):
    ckvr__flcl = data[:len_train]
    rysw__zffpu = data[len_train:]
    ckvr__flcl = bodo.rebalance(ckvr__flcl)
    rysw__zffpu = bodo.rebalance(rysw__zffpu)
    edr__msfy = labels[:len_train]
    dmu__hrnjd = labels[len_train:]
    edr__msfy = bodo.rebalance(edr__msfy)
    dmu__hrnjd = bodo.rebalance(dmu__hrnjd)
    return ckvr__flcl, rysw__zffpu, edr__msfy, dmu__hrnjd
...
def overload_train_test_split(data, labels=None, train_size=None, test_size
    ...
    _is_data_distributed=False):
    check_sklearn_version()
    xdk__tfc = {'stratify': stratify}
    snpv__eae = {'stratify': None}
    check_unsupported_args('train_test_split', xdk__tfc, snpv__eae, 'ml')
    if is_overload_false(_is_data_distributed):
        vskdi__ahjf = f'data_split_type_{numba.core.ir_utils.next_label()}'
        szo__gsaq = f'labels_split_type_{numba.core.ir_utils.next_label()}'
        for pyrkz__con, cdvf__lmyc in ((data, vskdi__ahjf), (labels, szo__gsaq)
            ):
            if isinstance(pyrkz__con, (DataFrameType, SeriesType)):
                nuilx__vwbcl = pyrkz__con.copy(index=NumericIndexType(types
                    .int64))
                setattr(types, cdvf__lmyc, nuilx__vwbcl)
            else:
                setattr(types, cdvf__lmyc, pyrkz__con)
        qghpd__ypmu = 'def _train_test_split_impl(\n'
        qghpd__ypmu += '    data,\n'
        qghpd__ypmu += '    labels=None,\n'
        qghpd__ypmu += '    train_size=None,\n'
        qghpd__ypmu += '    test_size=None,\n'
        qghpd__ypmu += '    random_state=None,\n'
        qghpd__ypmu += '    shuffle=True,\n'
        qghpd__ypmu += '    stratify=None,\n'
        qghpd__ypmu += '    _is_data_distributed=False,\n'
        qghpd__ypmu += '):  # pragma: no cover\n'
        qghpd__ypmu += (
            """    with numba.objmode(data_train='{}', data_test='{}', labels_train='{}', labels_test='{}'):
"""
            .format(vskdi__ahjf, vskdi__ahjf, szo__gsaq, szo__gsaq))
        qghpd__ypmu += """        data_train, data_test, labels_train, labels_test = sklearn.model_selection.train_test_split(
"""
        qghpd__ypmu += '            data,\n'
        qghpd__ypmu += '            labels,\n'
        qghpd__ypmu += '            train_size=train_size,\n'
        qghpd__ypmu += '            test_size=test_size,\n'
        qghpd__ypmu += '            random_state=random_state,\n'
        qghpd__ypmu += '            shuffle=shuffle,\n'
        qghpd__ypmu += '            stratify=stratify,\n'
        qghpd__ypmu += '        )\n'
        qghpd__ypmu += (
            '    return data_train, data_test, labels_train, labels_test\n')
        bttod__rgvlw = {}
        exec(qghpd__ypmu, globals(), bttod__rgvlw)
        _train_test_split_impl = bttod__rgvlw['_train_test_split_impl']
        ...
        def _train_test_split_impl(data, labels=None, train_size=None,
            ...
            pbfo__xwkg = bodo.libs.distributed_api.dist_reduce(len(data),
                np.int32(Reduce_Type.Sum.value))
            len_train = int(pbfo__xwkg * train_size)
            ayoj__qmeb = pbfo__xwkg - len_train
            if shuffle:
                labels = set_labels_type(labels, label_type)
                hxnbg__odkv = bodo.get_rank()
                eopa__cyay = bodo.get_size()
                aohor__yko = np.empty(eopa__cyay, np.int64)
                bodo.libs.distributed_api.allgather(aohor__yko, len(data))
                coh__owsjb = np.cumsum(aohor__yko[0:hxnbg__odkv + 1])
                asuv__rbj = np.full(pbfo__xwkg, True)
                asuv__rbj[:ayoj__qmeb] = False
                np.random.seed(42)
                np.random.permutation(asuv__rbj)
                if hxnbg__odkv:
                    hbk__wlvvc = coh__owsjb[hxnbg__odkv - 1]
                else:
                    hbk__wlvvc = 0
                tzwlk__zqgr = coh__owsjb[hxnbg__odkv]
                fblrz__dixm = asuv__rbj[hbk__wlvvc:tzwlk__zqgr]
                ckvr__flcl = data[fblrz__dixm]
                rysw__zffpu = data[~fblrz__dixm]
                edr__msfy = labels[fblrz__dixm]
                dmu__hrnjd = labels[~fblrz__dixm]
                ckvr__flcl = bodo.random_shuffle(ckvr__flcl, seed=
                    random_state, parallel=True)
                rysw__zffpu = bodo.random_shuffle(rysw__zffpu, seed=
                    random_state, parallel=True)
                edr__msfy = bodo.random_shuffle(edr__msfy, seed=
                    random_state, parallel=True)
                dmu__hrnjd = bodo.random_shuffle(dmu__hrnjd, seed=
                    random_state, parallel=True)
                edr__msfy = reset_labels_type(edr__msfy, label_type)
                dmu__hrnjd = reset_labels_type(dmu__hrnjd, label_type)
            else:
                ckvr__flcl, rysw__zffpu, edr__msfy, dmu__hrnjd = (
...
def sklearn_preprocessing_minmax_scaler_fit_dist_helper(m, X):
    ...
    zoge__uxan = kmiao__lakc.Get_size()
    m = m.fit(X)
    tjcll__mguya = kmiao__lakc.allreduce(m.n_samples_seen_, op=MPI.SUM)
    m.n_samples_seen_ = tjcll__mguya
    lwoku__xqgb = np.zeros((zoge__uxan, *m.data_min_.shape), dtype=m.
        data_min_.dtype)
    kmiao__lakc.Allgather(m.data_min_, lwoku__xqgb)
    drje__cbuoy = np.nanmin(lwoku__xqgb, axis=0)
    cdu__swv = np.zeros((zoge__uxan, *m.data_max_.shape), dtype=m.data_max_
        .dtype)
    kmiao__lakc.Allgather(m.data_max_, cdu__swv)
    jgkm__nybo = np.nanmax(cdu__swv, axis=0)
    elyh__tbvk = jgkm__nybo - drje__cbuoy
    m.scale_ = (m.feature_range[1] - m.feature_range[0]
        ) / sklearn_handle_zeros_in_scale(elyh__tbvk)
    m.min_ = m.feature_range[0] - drje__cbuoy * m.scale_
    m.data_min_ = drje__cbuoy
    m.data_max_ = jgkm__nybo
    m.data_range_ = elyh__tbvk
    return m
...
def overload_rf_classifier_model_fit(m, X, y, _is_data_distributed=False):
    ...
    def _model_fit_impl(m, X, y, _is_data_distributed=False):
        with numba.objmode(first_rank_node='int32[:]'):
            first_rank_node = get_nodes_first_ranks()
        if _is_data_distributed:
            lhwaz__cfk = len(first_rank_node)
            X = bodo.gatherv(X)
            y = bodo.gatherv(y)
            if lhwaz__cfk > 1:
                X = bodo.libs.distributed_api.bcast_comm(X, comm_ranks=
                    first_rank_node, nranks=lhwaz__cfk)
...
def _cv_fit_transform_helper(m, X):
    ...
    if m.vocabulary is None:
        ...
        hutr__nmkli = True
    return hutr__nmkli, local_vocabulary
    ...
    if is_overload_true(_is_data_distributed):
        ...
        def _count_vectorizer_fit_transform_impl(m, X, y=None,
            ...
            if changeVoc:
                ...
                tyf__pxcj = bodo.libs.array_kernels.unique(local_vocabulary,
                    parallel=True)
                tyf__pxcj = bodo.allgatherv(tyf__pxcj, False)
                tyf__pxcj = bodo.libs.array_kernels.sort(tyf__pxcj,
                    ascending=True, inplace=True)
                mibhc__lrpec = {}
                for rqooo__jrv in range(len(tyf__pxcj)):
                    mibhc__lrpec[tyf__pxcj[rqooo__jrv]] = rqooo__jrv
            else:
                mibhc__lrpec = local_vocabulary
            with numba.objmode(transformed_X='csr_matrix_int64_int64'):
                if changeVoc:
                    m.vocabulary = mibhc__lrpec
                transformed_X = m.fit_transform(X, y)
                transformed_X.indices = transformed_X.indices.astype(np.int64)
                transformed_X.indptr = transformed_X.indptr.astype(np.int64)
            return transformed_X
