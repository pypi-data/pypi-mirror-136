...
def box_regex_flag(typ, val, c):
    lgur__qgog = c.box(types.int64, val)
    tyz__kge = c.pyapi.unserialize(c.pyapi.serialize_object(re.RegexFlag))
    return c.pyapi.call_function_objargs(tyz__kge, (lgur__qgog,))


@unbox(RegexFlagsType)
def unbox_regex_flag(typ, obj, c):
    lgur__qgog = c.pyapi.object_getattr_string(obj, 'value')
    return c.unbox(types.int64, lgur__qgog)
...
def cast_match_obj_bool(context, builder, fromty, toty, val):
    out = cgutils.alloca_once_value(builder, context.get_constant(types.
        bool_, True))
    kguz__lnkz = context.get_python_api(builder)
    wok__qtqps = builder.icmp_signed('==', val, kguz__lnkz.borrow_none())
    with builder.if_then(wok__qtqps):
...
def init_const_pattern(typingctx, pat, pat_const=None):
    ikzut__pph = get_overload_const_str(pat_const)

    def codegen(context, builder, sig, args):
        return impl_ret_borrowed(context, builder, sig.return_type, args[0])
    return RePatternType(ikzut__pph)(pat, pat_const), codegen
...
def overload_pat_findall(p, string, pos=0, endpos=9223372036854775807):
    if p.pat_const:
        huuu__qden = re.compile(p.pat_const).groups
        typ = types.List(string_type)
        if huuu__qden > 1:
            typ = types.List(types.Tuple([string_type] * huuu__qden))
...
def overload_match_group(m, *args):
    ...
    ftm__mcbsn = types.optional(string_type)
    if len(args) == 1:

        def _match_group_impl_one(m, *args):
            lzy__cdmi = args[0]
            with numba.objmode(out=ftm__mcbsn):
                out = m.group(lzy__cdmi)
            return out
        return _match_group_impl_one
    yyv__rtzp = 'tuple_str_{}'.format(len(args))
    setattr(types, yyv__rtzp, types.Tuple([ftm__mcbsn] * len(args)))
    qvjfy__jtl = ', '.join('group{}'.format(dsr__bgga + 1) for dsr__bgga in
        range(len(args)))
    nsyky__ezwh = 'def _match_group_impl(m, *args):\n'
    nsyky__ezwh += '  ({}) = args\n'.format(qvjfy__jtl)
    nsyky__ezwh += "  with numba.objmode(out='{}'):\n".format(yyv__rtzp)
    nsyky__ezwh += '    out = m.group({})\n'.format(qvjfy__jtl)
    nsyky__ezwh += '  return out\n'
    muft__atsf = {}
    exec(nsyky__ezwh, globals(), muft__atsf)
    mfapi__szxr = muft__atsf['_match_group_impl']
    return mfapi__szxr
...
def overload_match_groups(m, default=None):
    ujk__mggn = types.List(types.optional(string_type))

    def _match_groups_impl(m, default=None):
        with numba.objmode(out=ujk__mggn):
...
def overload_match_groupdict(m, default=None):
    ...
    def _check_dict_none(out):
        if any(iso__obfe is None for iso__obfe in out.values()):
            raise BodoError(
...
def overload_match_lastgroup(p):
    ...
    def _match_lastgroup_impl(p):
        with numba.objmode(lastgroup=ftm__mcbsn):
