...
def install_mpl_class(types_name, python_type):
    act__yubx = ''.join(map(str.title, types_name.split('_')))
    mxy__lzvq = f'class {act__yubx}(types.Opaque):\n'
    mxy__lzvq += '    def __init__(self):\n'
    mxy__lzvq += f"       types.Opaque.__init__(self, name='{act__yubx}')\n"
    mxy__lzvq += '    def __reduce__(self):\n'
    mxy__lzvq += (
        f"        return (types.Opaque, ('{act__yubx}',), self.__dict__)\n")
    qfs__wdlx = {}
    exec(mxy__lzvq, {'types': types, 'bodo': bodo}, qfs__wdlx)
    xag__kgun = qfs__wdlx[act__yubx]
    awr__iymwv = sys.modules[__name__]
    setattr(awr__iymwv, act__yubx, xag__kgun)
    class_instance = xag__kgun()
    setattr(types, types_name, class_instance)
    register_model(xag__kgun)(models.OpaqueModel)
    typeof_impl.register(python_type)(lambda val, c: class_instance)
    unbox(xag__kgun)(unbox_mpl_obj)
    box(xag__kgun)(box_mpl_obj)
...
def _install_mpl_types():
    adrhy__fmyv = [('mpl_figure_type', matplotlib.figure.Figure), (
    ...
    for tni__xhkl, rnvg__lokr in adrhy__fmyv:
        install_mpl_class(tni__xhkl, rnvg__lokr)
...
def generate_matplotlib_signature(return_typ, args, kws, obj_typ=None):
    kws = dict(kws)
    fgvs__xyq = ', '.join(f'e{ddfx__qdbqw}' for ddfx__qdbqw in range(len(args))
        )
    if fgvs__xyq:
        fgvs__xyq += ', '
    cnsss__zrm = ', '.join(f"{avrv__qlear} = ''" for avrv__qlear in kws.keys())
    tescd__qhkbx = 'matplotlib_obj, ' if obj_typ is not None else ''
    zlei__bllp = f'def mpl_stub({tescd__qhkbx} {fgvs__xyq} {cnsss__zrm}):\n'
    zlei__bllp += '    pass\n'
    kdbq__iqqdi = {}
    exec(zlei__bllp, {}, kdbq__iqqdi)
    ufe__arlll = kdbq__iqqdi['mpl_stub']
    owoz__akbkp = numba.core.utils.pysignature(ufe__arlll)
    gbgb__zio = ((obj_typ,) if obj_typ is not None else ()) + args + tuple(kws
        .values())
    return signature(return_typ, *unliteral_all(gbgb__zio)).replace(pysig=
        owoz__akbkp)


def generate_axes_typing(mod_name, nrows, ncols):
    gjua__pzoy = '{}.subplots(): {} must be a constant integer >= 1'
    if not is_overload_constant_int(nrows):
        raise_bodo_error(gjua__pzoy.format(mod_name, 'nrows'))
    if not is_overload_constant_int(ncols):
        raise_bodo_error(gjua__pzoy.format(mod_name, 'ncols'))
    spxxq__hoe = get_overload_const_int(nrows)
    ojo__wxij = get_overload_const_int(ncols)
    if spxxq__hoe < 1:
        raise BodoError(gjua__pzoy.format(mod_name, 'nrows'))
    if ojo__wxij < 1:
        raise BodoError(gjua__pzoy.format(mod_name, 'ncols'))
    if spxxq__hoe == 1 and ojo__wxij == 1:
        znrh__sca = types.mpl_axes_type
    else:
        if ojo__wxij == 1:
            yhk__lbfze = types.mpl_axes_type
        else:
            yhk__lbfze = types.Tuple([types.mpl_axes_type] * ojo__wxij)
        znrh__sca = types.Tuple([yhk__lbfze] * spxxq__hoe)
    return znrh__sca


def generate_pie_return_type(args, kws):
    rloyx__jieg = args[4] if len(args) > 5 else kws.get('autopct', types.none)
    if rloyx__jieg == types.none:
...
def generate_xcorr_return_type(func_mod, args, kws):
    lap__bjpo = args[4] if len(args) > 5 else kws.get('usevlines', True)
    if not is_overload_constant_bool(lap__bjpo):
        raise_bodo_error(
            f'{func_mod}.xcorr(): usevlines must be a constant boolean')
    if is_overload_true(lap__bjpo):
...
class SubplotsTyper(AbstractTemplate):
    ...
    def generic(self, args, kws):
        ...
        lxs__fjab = generate_axes_typing('matplotlib.pyplot', nrows, ncols)
        return generate_matplotlib_signature(types.Tuple([types.
            mpl_figure_type, lxs__fjab]), args, kws)
...
def overload_subplots(fig, nrows=1, ncols=1, sharex=False, sharey=False,
    ...
    lxs__fjab = generate_axes_typing('matplotlib.figure.Figure', nrows, ncols)
    tni__xhkl = str(lxs__fjab)
    if not hasattr(types, tni__xhkl):
        tni__xhkl = f'objmode_type{ir_utils.next_label()}'
        setattr(types, tni__xhkl, lxs__fjab)
    zlei__bllp = f"""def impl(
        fig,
        nrows=1,
        ncols=1,
        sharex=False,
        ...
        with numba.objmode(axes="{tni__xhkl}"):
            axes = fig.subplots(
                nrows=nrows,
                ncols=ncols,
                sharex=sharex,
    ...
    """
    kdbq__iqqdi = {}
    exec(zlei__bllp, {'numba': numba, 'np': np}, kdbq__iqqdi)
    impl = kdbq__iqqdi['impl']
    return impl
