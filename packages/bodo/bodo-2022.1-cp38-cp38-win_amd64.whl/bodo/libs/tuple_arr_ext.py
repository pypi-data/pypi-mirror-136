...
class TupleArrayType(types.ArrayCompatible):
    ...
    @property
    def dtype(self):
        return types.BaseTuple.from_types(tuple(emwir__brgsh.dtype for
            emwir__brgsh in self.data))
...
class TupleArrayModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        flgv__szg = [('data', StructArrayType(fe_type.data))]
        models.StructModel.__init__(self, dmm, fe_type, flgv__szg)
...
def init_tuple_arr(typingctx, data_typ=None):
    ...
    tnpr__ewl = TupleArrayType(data_typ.data)

    def codegen(context, builder, sig, args):
        qajic__hmrxz, = args
        lcnt__vgc = context.make_helper(builder, tnpr__ewl)
        lcnt__vgc.data = qajic__hmrxz
        context.nrt.incref(builder, data_typ, qajic__hmrxz)
        return lcnt__vgc._getvalue()
    return tnpr__ewl(data_typ), codegen


@unbox(TupleArrayType)
def unbox_tuple_array(typ, val, c):
    data_typ = StructArrayType(typ.data)
    kjvs__rjhm = unbox_struct_array(data_typ, val, c, is_tuple_array=True)
    qajic__hmrxz = kjvs__rjhm.value
    lcnt__vgc = c.context.make_helper(c.builder, typ)
    lcnt__vgc.data = qajic__hmrxz
    saxt__rwp = kjvs__rjhm.is_error
    return NativeValue(lcnt__vgc._getvalue(), is_error=saxt__rwp)


@box(TupleArrayType)
def box_tuple_arr(typ, val, c):
    data_typ = StructArrayType(typ.data)
    lcnt__vgc = c.context.make_helper(c.builder, typ, val)
    arr = box_struct_arr(data_typ, lcnt__vgc.data, c, is_tuple_array=True)
    return arr
...
def tuple_arr_getitem(arr, ind):
    ...
    if isinstance(ind, types.Integer):
        kvnn__mgkw = 'def impl(arr, ind):\n'
        xteks__mkgny = ','.join(f'get_data(arr._data)[{ywnsv__imoo}][ind]' for
            ywnsv__imoo in range(len(arr.data)))
        kvnn__mgkw += f'  return ({xteks__mkgny})\n'
        uov__ukenx = {}
        exec(kvnn__mgkw, {'get_data': bodo.libs.struct_arr_ext.get_data},
            uov__ukenx)
        nxjno__wbk = uov__ukenx['impl']
        return nxjno__wbk
...
def tuple_arr_setitem(arr, ind, val):
    ...
    if val == types.none or isinstance(val, types.optional):
        return
    if isinstance(ind, types.Integer):
        afyb__lmfnz = len(arr.data)
        kvnn__mgkw = 'def impl(arr, ind, val):\n'
        kvnn__mgkw += '  data = get_data(arr._data)\n'
        kvnn__mgkw += '  null_bitmap = get_null_bitmap(arr._data)\n'
        kvnn__mgkw += '  set_bit_to_arr(null_bitmap, ind, 1)\n'
        for ywnsv__imoo in range(afyb__lmfnz):
            kvnn__mgkw += f'  data[{ywnsv__imoo}][ind] = val[{ywnsv__imoo}]\n'
        uov__ukenx = {}
        exec(kvnn__mgkw, {'get_data': bodo.libs.struct_arr_ext.get_data,
            'get_null_bitmap': bodo.libs.struct_arr_ext.get_null_bitmap,
            'set_bit_to_arr': bodo.libs.int_arr_ext.set_bit_to_arr}, uov__ukenx
            )
        nxjno__wbk = uov__ukenx['impl']
        return nxjno__wbk
