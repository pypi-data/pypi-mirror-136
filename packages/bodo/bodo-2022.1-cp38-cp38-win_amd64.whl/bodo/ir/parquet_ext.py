...
def remove_dead_pq(pq_node, lives_no_aliases, lives, arg_aliases, alias_map,
    func_ir, typemap):
    bez__odm = pq_node.out_vars[0].name
    igx__ring = pq_node.out_vars[1].name
    if bez__odm not in lives and igx__ring not in lives:
        return None
    elif bez__odm not in lives:
        pq_node.col_indices = []
        pq_node.df_colnames = []
        pq_node.type_usecol_offset = []
    elif igx__ring not in lives:
...
def pq_remove_dead_column(pq_node, column_live_map, equiv_vars, typemap):
    ...
    oer__rrxh = pq_node.out_vars[0].name
    assert isinstance(typemap[oer__rrxh], TableType
        ), 'Parquet Node Table must be a TableType'
    if pq_node.col_indices:
        vsd__wsdd, rvb__czjrv = get_live_column_nums_block(column_live_map,
            equiv_vars, oer__rrxh)
        vsd__wsdd = bodo.ir.connector.trim_extra_used_columns(vsd__wsdd,
            len(pq_node.col_indices))
        if not rvb__czjrv and not vsd__wsdd:
            vsd__wsdd = [0]
        if not rvb__czjrv and len(vsd__wsdd) != len(pq_node.type_usecol_offset
            ):
            pq_node.type_usecol_offset = vsd__wsdd
            return True
    return False
