...
class SeriesIatType(types.Type):
    ...
    def __init__(self, stype):
        self.stype = stype
        qptfh__kornx = 'SeriesIatType({})'.format(stype)
        super(SeriesIatType, self).__init__(qptfh__kornx)
...
class SeriesIatModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        jfx__uoz = [('obj', fe_type.stype)]
        super(SeriesIatModel, self).__init__(dmm, fe_type, jfx__uoz)
...
def init_series_iat(typingctx, obj=None):
    ...
    def codegen(context, builder, signature, args):
        pun__ftg, = args
        itkm__zlup = signature.return_type
        tixco__rqntm = cgutils.create_struct_proxy(itkm__zlup)(context, builder
            )
        tixco__rqntm.obj = pun__ftg
        context.nrt.incref(builder, signature.args[0], pun__ftg)
        return tixco__rqntm._getvalue()
...
def overload_series_iat_setitem(I, idx, val):
    if isinstance(I, SeriesIatType):
        ...
        if I.stype.dtype == types.NPTimedelta('ns'
            ...
            def impl_dt(I, idx, val):
                pjlsj__pbbpc = (bodo.hiframes.datetime_timedelta_ext.
                    _to_nanoseconds(val))
...
class SeriesIlocType(types.Type):
    ...
    def __init__(self, stype):
        self.stype = stype
        qptfh__kornx = 'SeriesIlocType({})'.format(stype)
        super(SeriesIlocType, self).__init__(qptfh__kornx)
...
class SeriesIlocModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        jfx__uoz = [('obj', fe_type.stype)]
        super(SeriesIlocModel, self).__init__(dmm, fe_type, jfx__uoz)
...
def init_series_iloc(typingctx, obj=None):
    ...
    def codegen(context, builder, signature, args):
        pun__ftg, = args
        xkpx__bwvxe = signature.return_type
        wksge__ywlo = cgutils.create_struct_proxy(xkpx__bwvxe)(context, builder
            )
        wksge__ywlo.obj = pun__ftg
        context.nrt.incref(builder, signature.args[0], pun__ftg)
        return wksge__ywlo._getvalue()
...
def overload_series_iloc_getitem(I, idx):
    if isinstance(I, SeriesIlocType):
        ...
        if is_list_like_index_type(idx) and isinstance(idx.dtype, (types.
            ...
            def impl(I, idx):
                S = I._obj
                mbz__iwzc = bodo.utils.conversion.coerce_to_ndarray(idx)
                dho__ioxyi = bodo.hiframes.pd_series_ext.get_series_data(S)[
                    mbz__iwzc]
                xcv__fyt = bodo.hiframes.pd_series_ext.get_series_index(S)[
                    mbz__iwzc]
                qptfh__kornx = bodo.hiframes.pd_series_ext.get_series_name(S)
                return bodo.hiframes.pd_series_ext.init_series(dho__ioxyi,
                    xcv__fyt, qptfh__kornx)
            return impl
        if isinstance(idx, types.SliceType):

            def impl_slice(I, idx):
                S = I._obj
                dho__ioxyi = bodo.hiframes.pd_series_ext.get_series_data(S)[idx
                    ]
                xcv__fyt = bodo.hiframes.pd_series_ext.get_series_index(S)[idx]
                qptfh__kornx = bodo.hiframes.pd_series_ext.get_series_name(S)
                return bodo.hiframes.pd_series_ext.init_series(dho__ioxyi,
                    xcv__fyt, qptfh__kornx)
            return impl_slice
        ...
        if is_list_like_index_type(idx) and isinstance(idx.dtype, (types.
            ...
            if is_scalar_type(val):
                ...
                if I.stype.dtype == types.NPTimedelta('ns'
                    ...
                    def impl_dt(I, idx, val):
                        pjlsj__pbbpc = (bodo.hiframes.
                            datetime_timedelta_ext._to_nanoseconds(val))
                        s = (bodo.hiframes.pd_timestamp_ext.
                            integer_to_timedelta64(pjlsj__pbbpc))
                        mbz__iwzc = bodo.utils.conversion.coerce_to_ndarray(idx
                            )
                        bodo.hiframes.pd_series_ext.get_series_data(I._obj)[
                            mbz__iwzc] = s
            ...
            def impl_arr(I, idx, val):
                mbz__iwzc = bodo.utils.conversion.coerce_to_ndarray(idx)
                bodo.hiframes.pd_series_ext.get_series_data(I._obj)[mbz__iwzc
                    ] = bodo.utils.conversion.coerce_to_array(val, False)
            return impl_arr
...
class SeriesLocType(types.Type):
    ...
    def __init__(self, stype):
        self.stype = stype
        qptfh__kornx = 'SeriesLocType({})'.format(stype)
        super(SeriesLocType, self).__init__(qptfh__kornx)
...
class SeriesLocModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        jfx__uoz = [('obj', fe_type.stype)]
        super(SeriesLocModel, self).__init__(dmm, fe_type, jfx__uoz)
...
def init_series_loc(typingctx, obj=None):
    ...
    def codegen(context, builder, signature, args):
        pun__ftg, = args
        ajqo__fdcr = signature.return_type
        oakk__fnf = cgutils.create_struct_proxy(ajqo__fdcr)(context, builder)
        oakk__fnf.obj = pun__ftg
        context.nrt.incref(builder, signature.args[0], pun__ftg)
        return oakk__fnf._getvalue()
...
def overload_series_loc_getitem(I, idx):
    ...
    if isinstance(idx, types.Integer) and isinstance(I.stype.index,
        ...
        def impl_range_index_int(I, idx):
            ...
            dho__ioxyi = bodo.hiframes.pd_series_ext.get_series_data(S)
            xcv__fyt = bodo.hiframes.pd_series_ext.get_series_index(S)
            rrycf__yptj = (idx - xcv__fyt._start) // xcv__fyt._step
            return dho__ioxyi[rrycf__yptj]
        ...
        if isinstance(idx, types.Integer):
            ...
            if isinstance(S.index, RangeIndexType):
                ...
                def impl(S, idx):
                    ...
                    mbz__iwzc = idx * I._step + I._start
                    return bodo.utils.conversion.box_if_dt64(dho__ioxyi[
                        mbz__iwzc])
                return impl
        ...
        if idx == bodo.string_type or is_overload_constant_str(idx):
            if isinstance(S.index, bodo.hiframes.pd_index_ext.StringIndexType):
                ...
                def impl_str_getitem(S, idx):
                    ehrlf__xtp = bodo.hiframes.pd_series_ext.get_series_index(S
                        )
                    wmevu__lgrsu = ehrlf__xtp.get_loc(idx)
                    if wmevu__lgrsu is None:
                        raise IndexError
                    else:
                        val = bodo.hiframes.pd_series_ext.get_series_data(S)[
                            bodo.utils.indexing.unoptional(wmevu__lgrsu)]
                    return val
    ...
    if isinstance(S, SeriesType):
        ...
        if is_list_like_index_type(idx) and isinstance(idx.dtype, (types.
            ...
            def impl_arr(S, idx, val):
                mbz__iwzc = bodo.utils.conversion.coerce_to_ndarray(idx)
                bodo.hiframes.pd_series_ext.get_series_data(S)[mbz__iwzc
                    ] = bodo.utils.conversion.coerce_to_array(val, False)
            return impl_arr
...
def overload_const_index_series_getitem(S, idx):
    if isinstance(S, (SeriesType, HeterogeneousSeriesType)) and isinstance(S
        ...
        index.data):
        mgitn__eyuuc = get_overload_const_tuple(S.index.data)
        if isinstance(idx, types.Integer) and not any(isinstance(qzc__sypx,
            int) for qzc__sypx in mgitn__eyuuc):
            return lambda S, idx: bodo.hiframes.pd_series_ext.get_series_data(S
                )[idx]
        if is_literal_type(idx):
            fry__vupnw = get_literal_value(idx)
            if fry__vupnw not in mgitn__eyuuc:
                raise_bodo_error(
                    f"Series label-based getitem: '{fry__vupnw}' not in {mgitn__eyuuc}"
                    )
            arr_ind = mgitn__eyuuc.index(fry__vupnw)
            return lambda S, idx: bodo.hiframes.pd_series_ext.get_series_data(S
                )[arr_ind]
...
def cast_series_iat(context, builder, fromty, toty, val):
    tixco__rqntm = cgutils.create_struct_proxy(fromty)(context, builder, val)
    ttrdm__tgf = context.cast(builder, tixco__rqntm.obj, fromty.stype, toty
        .stype)
    jmorh__ljza = cgutils.create_struct_proxy(toty)(context, builder)
    jmorh__ljza.obj = ttrdm__tgf
    return jmorh__ljza._getvalue()
