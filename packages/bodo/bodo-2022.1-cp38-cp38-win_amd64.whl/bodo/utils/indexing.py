...
def get_new_null_mask_bool_index(old_mask, ind, n):
    dmnq__vtz = n + 7 >> 3
    puupg__mrk = np.empty(dmnq__vtz, np.uint8)
    ukww__wsjjk = 0
    for relc__ebpec in range(len(ind)):
        if ind[relc__ebpec]:
            icre__wwy = bodo.libs.int_arr_ext.get_bit_bitmap_arr(old_mask,
                relc__ebpec)
            bodo.libs.int_arr_ext.set_bit_to_arr(puupg__mrk, ukww__wsjjk,
                icre__wwy)
            ukww__wsjjk += 1
    return puupg__mrk
...
def array_getitem_bool_index(A, ind):
    ...
    vfz__ggvmy = A._data[ind]
    n = len(vfz__ggvmy)
    puupg__mrk = get_new_null_mask_bool_index(old_mask, ind, n)
    return vfz__ggvmy, puupg__mrk
...
def get_new_null_mask_int_index(old_mask, ind, n):
    ...
    ukww__wsjjk = 0
    for relc__ebpec in range(len(ind)):
        icre__wwy = bodo.libs.int_arr_ext.get_bit_bitmap_arr(old_mask, ind[
            relc__ebpec])
        bodo.libs.int_arr_ext.set_bit_to_arr(puupg__mrk, ukww__wsjjk, icre__wwy
            )
        ukww__wsjjk += 1
    return puupg__mrk
...
def array_getitem_int_index(A, ind):
    igp__mdyz = bodo.utils.conversion.coerce_to_ndarray(ind)
    old_mask = A._null_bitmap
    vfz__ggvmy = A._data[igp__mdyz]
    n = len(vfz__ggvmy)
    puupg__mrk = get_new_null_mask_int_index(old_mask, igp__mdyz, n)
    return vfz__ggvmy, puupg__mrk


@register_jitable
def get_new_null_mask_slice_index(old_mask, ind, n):
    kjdfw__dderk = numba.cpython.unicode._normalize_slice(ind, n)
    xsh__xxw = numba.cpython.unicode._slice_span(kjdfw__dderk)
    dmnq__vtz = xsh__xxw + 7 >> 3
    puupg__mrk = np.empty(dmnq__vtz, np.uint8)
    ukww__wsjjk = 0
    for relc__ebpec in range(kjdfw__dderk.start, kjdfw__dderk.stop,
        kjdfw__dderk.step):
        icre__wwy = bodo.libs.int_arr_ext.get_bit_bitmap_arr(old_mask,
            relc__ebpec)
        bodo.libs.int_arr_ext.set_bit_to_arr(puupg__mrk, ukww__wsjjk, icre__wwy
...
def array_getitem_slice_index(A, ind):
    n = len(A._data)
    old_mask = A._null_bitmap
    vfz__ggvmy = np.ascontiguousarray(A._data[ind])
    puupg__mrk = get_new_null_mask_slice_index(old_mask, ind, n)
    return vfz__ggvmy, puupg__mrk
...
def array_setitem_int_index_overload(A, idx, val):
    if bodo.utils.utils.is_array_typ(val
        ...
        def impl_arr(A, idx, val):
            ...
            n = len(val._data)
            for relc__ebpec in range(n):
                A._data[idx[relc__ebpec]] = val._data[relc__ebpec]
                icre__wwy = bodo.libs.int_arr_ext.get_bit_bitmap_arr(val.
                    _null_bitmap, relc__ebpec)
                bodo.libs.int_arr_ext.set_bit_to_arr(A._null_bitmap, idx[
                    relc__ebpec], icre__wwy)
        return impl_arr
    if bodo.utils.typing.is_scalar_type(val):

        def impl_scalar(A, idx, val):
            for relc__ebpec in idx:
                A._data[relc__ebpec] = val
...
def array_setitem_bool_index_overload(A, idx, val):
    if bodo.utils.utils.is_array_typ(val
        ...
        def impl_arr(A, idx, val):
            ...
            wsbu__hpd = 0
            for relc__ebpec in range(n):
                if not bodo.libs.array_kernels.isna(idx, relc__ebpec) and idx[
                    relc__ebpec]:
                    A._data[relc__ebpec] = val._data[wsbu__hpd]
                    icre__wwy = bodo.libs.int_arr_ext.get_bit_bitmap_arr(val
                        ._null_bitmap, wsbu__hpd)
                    bodo.libs.int_arr_ext.set_bit_to_arr(A._null_bitmap,
                        relc__ebpec, icre__wwy)
                    wsbu__hpd += 1
        return impl_arr
...
def setitem_slice_index_null_bits(dst_bitmap, src_bitmap, idx, n):
    kjdfw__dderk = numba.cpython.unicode._normalize_slice(idx, n)
    wsbu__hpd = 0
    for relc__ebpec in range(kjdfw__dderk.start, kjdfw__dderk.stop,
        kjdfw__dderk.step):
        icre__wwy = bodo.libs.int_arr_ext.get_bit_bitmap_arr(src_bitmap,
            wsbu__hpd)
        bodo.libs.int_arr_ext.set_bit_to_arr(dst_bitmap, relc__ebpec, icre__wwy
            )
        wsbu__hpd += 1
...
def array_setitem_slice_index_overload(A, idx, val):
    ...
    if bodo.utils.typing.is_scalar_type(val):
        ...
        def impl_scalar(A, idx, val):
            kjdfw__dderk = numba.cpython.unicode._normalize_slice(idx, len(A))
            for relc__ebpec in range(kjdfw__dderk.start, kjdfw__dderk.stop,
                kjdfw__dderk.step):
                A._data[relc__ebpec] = val
                bodo.libs.int_arr_ext.set_bit_to_arr(A._null_bitmap,
    ...
    if not bodo.utils.utils.is_array_typ(A, False):
        ...
        if isinstance(idx, types.Integer):
            ...
            def setitem_none_int_arr(A, idx, val):
                ...
                for relc__ebpec in idx:
                    bodo.libs.array_kernels.setna(A, relc__ebpec)
            ...
            if A == bodo.string_array_type:
                ...
                def string_arr_impl(A, idx, val):
                    n = len(A)
                    idx = bodo.utils.conversion.coerce_to_ndarray(idx)
                    dwhbb__iae = bodo.libs.str_arr_ext.pre_alloc_string_array(n
                        , -1)
                    for relc__ebpec in numba.parfors.parfor.internal_prange(n):
                        if idx[relc__ebpec] or bodo.libs.array_kernels.isna(A,
                            relc__ebpec):
                            dwhbb__iae[relc__ebpec] = ''
                            bodo.libs.str_arr_ext.str_arr_set_na(dwhbb__iae,
                                relc__ebpec)
                        else:
                            dwhbb__iae[relc__ebpec] = A[relc__ebpec]
            ...
            def setitem_none_bool_arr(A, idx, val):
                ...
                n = len(idx)
                for relc__ebpec in range(n):
                    if not bodo.libs.array_kernels.isna(idx, relc__ebpec
                        ) and idx[relc__ebpec]:
                        bodo.libs.array_kernels.setna(A, relc__ebpec)
            ...
            def setitem_none_slice(A, idx, val):
                n = len(A)
                kjdfw__dderk = numba.cpython.unicode._normalize_slice(idx, n)
                for relc__ebpec in range(kjdfw__dderk.start, kjdfw__dderk.
                    stop, kjdfw__dderk.step):
                    bodo.libs.array_kernels.setna(A, relc__ebpec)
            ...
            def setitem_optional_int_arr(A, idx, val):
                ...
                for relc__ebpec in idx:
                    if val is None:
                        bodo.libs.array_kernels.setna(A, relc__ebpec)
                        continue
                    A[relc__ebpec] = bodo.utils.indexing.unoptional(val)
...
def unoptional(typingctx, val_t=None):
    ...
    def codegen(context, builder, signature, args):
        oxab__nikq = context.make_helper(builder, val_t, args[0])
        ngc__klrtg = oxab__nikq.data
        context.nrt.incref(builder, val_t.type, ngc__klrtg)
        return ngc__klrtg
