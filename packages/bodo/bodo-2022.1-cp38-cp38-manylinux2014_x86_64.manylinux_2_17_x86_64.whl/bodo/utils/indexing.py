...
def get_new_null_mask_bool_index(old_mask, ind, n):
    iftc__mzm = n + 7 >> 3
    ulzh__msoh = np.empty(iftc__mzm, np.uint8)
    npgic__fmu = 0
    for iuq__aoevj in range(len(ind)):
        if ind[iuq__aoevj]:
            cbuh__onpgs = bodo.libs.int_arr_ext.get_bit_bitmap_arr(old_mask,
                iuq__aoevj)
            bodo.libs.int_arr_ext.set_bit_to_arr(ulzh__msoh, npgic__fmu,
                cbuh__onpgs)
            npgic__fmu += 1
    return ulzh__msoh
...
def array_getitem_bool_index(A, ind):
    ...
    idsbv__fswz = A._data[ind]
    n = len(idsbv__fswz)
    ulzh__msoh = get_new_null_mask_bool_index(old_mask, ind, n)
    return idsbv__fswz, ulzh__msoh
...
def array_getitem_int_index(A, ind):
    jlozh__msxk = bodo.utils.conversion.coerce_to_ndarray(ind)
    old_mask = A._null_bitmap
    idsbv__fswz = A._data[jlozh__msxk]
    n = len(idsbv__fswz)
    ulzh__msoh = get_new_null_mask_int_index(old_mask, jlozh__msxk, n)
    return idsbv__fswz, ulzh__msoh


@register_jitable
def get_new_null_mask_slice_index(old_mask, ind, n):
    qbkdc__rcpr = numba.cpython.unicode._normalize_slice(ind, n)
    sne__sxmi = numba.cpython.unicode._slice_span(qbkdc__rcpr)
    iftc__mzm = sne__sxmi + 7 >> 3
    ulzh__msoh = np.empty(iftc__mzm, np.uint8)
    npgic__fmu = 0
    for iuq__aoevj in range(qbkdc__rcpr.start, qbkdc__rcpr.stop,
        qbkdc__rcpr.step):
        cbuh__onpgs = bodo.libs.int_arr_ext.get_bit_bitmap_arr(old_mask,
            iuq__aoevj)
        bodo.libs.int_arr_ext.set_bit_to_arr(ulzh__msoh, npgic__fmu,
...
def array_getitem_slice_index(A, ind):
    n = len(A._data)
    old_mask = A._null_bitmap
    idsbv__fswz = np.ascontiguousarray(A._data[ind])
    ulzh__msoh = get_new_null_mask_slice_index(old_mask, ind, n)
    return idsbv__fswz, ulzh__msoh
...
def array_setitem_int_index_overload(A, idx, val):
    if bodo.utils.utils.is_array_typ(val
        ...
        def impl_arr(A, idx, val):
            ...
            n = len(val._data)
            for iuq__aoevj in range(n):
                A._data[idx[iuq__aoevj]] = val._data[iuq__aoevj]
                cbuh__onpgs = bodo.libs.int_arr_ext.get_bit_bitmap_arr(val.
                    _null_bitmap, iuq__aoevj)
                bodo.libs.int_arr_ext.set_bit_to_arr(A._null_bitmap, idx[
                    iuq__aoevj], cbuh__onpgs)
        return impl_arr
    if bodo.utils.typing.is_scalar_type(val):

        def impl_scalar(A, idx, val):
            for iuq__aoevj in idx:
                A._data[iuq__aoevj] = val
...
def array_setitem_bool_index_overload(A, idx, val):
    if bodo.utils.utils.is_array_typ(val
        ...
        def impl_arr(A, idx, val):
            ...
            xajn__sxd = 0
            for iuq__aoevj in range(n):
                if not bodo.libs.array_kernels.isna(idx, iuq__aoevj) and idx[
                    iuq__aoevj]:
                    A._data[iuq__aoevj] = val._data[xajn__sxd]
                    cbuh__onpgs = bodo.libs.int_arr_ext.get_bit_bitmap_arr(val
                        ._null_bitmap, xajn__sxd)
                    bodo.libs.int_arr_ext.set_bit_to_arr(A._null_bitmap,
                        iuq__aoevj, cbuh__onpgs)
                    xajn__sxd += 1
        return impl_arr
...
def setitem_slice_index_null_bits(dst_bitmap, src_bitmap, idx, n):
    qbkdc__rcpr = numba.cpython.unicode._normalize_slice(idx, n)
    xajn__sxd = 0
    for iuq__aoevj in range(qbkdc__rcpr.start, qbkdc__rcpr.stop,
        qbkdc__rcpr.step):
        cbuh__onpgs = bodo.libs.int_arr_ext.get_bit_bitmap_arr(src_bitmap,
            xajn__sxd)
        bodo.libs.int_arr_ext.set_bit_to_arr(dst_bitmap, iuq__aoevj,
            cbuh__onpgs)
        xajn__sxd += 1
...
def array_setitem_slice_index_overload(A, idx, val):
    ...
    if bodo.utils.typing.is_scalar_type(val):
        ...
        def impl_scalar(A, idx, val):
            qbkdc__rcpr = numba.cpython.unicode._normalize_slice(idx, len(A))
            for iuq__aoevj in range(qbkdc__rcpr.start, qbkdc__rcpr.stop,
                qbkdc__rcpr.step):
                A._data[iuq__aoevj] = val
                bodo.libs.int_arr_ext.set_bit_to_arr(A._null_bitmap,
    ...
    if not bodo.utils.utils.is_array_typ(A, False):
        ...
        if isinstance(idx, types.Integer):
            ...
            def setitem_none_int_arr(A, idx, val):
                ...
                for iuq__aoevj in idx:
                    bodo.libs.array_kernels.setna(A, iuq__aoevj)
            ...
            if A == bodo.string_array_type:
                ...
                def string_arr_impl(A, idx, val):
                    n = len(A)
                    idx = bodo.utils.conversion.coerce_to_ndarray(idx)
                    ufvvq__azozr = (bodo.libs.str_arr_ext.
                        pre_alloc_string_array(n, -1))
                    for iuq__aoevj in numba.parfors.parfor.internal_prange(n):
                        if idx[iuq__aoevj] or bodo.libs.array_kernels.isna(A,
                            iuq__aoevj):
                            ufvvq__azozr[iuq__aoevj] = ''
                            bodo.libs.str_arr_ext.str_arr_set_na(ufvvq__azozr,
                                iuq__aoevj)
                        else:
                            ufvvq__azozr[iuq__aoevj] = A[iuq__aoevj]
            ...
            def setitem_none_bool_arr(A, idx, val):
                ...
                n = len(idx)
                for iuq__aoevj in range(n):
                    if not bodo.libs.array_kernels.isna(idx, iuq__aoevj
                        ) and idx[iuq__aoevj]:
                        bodo.libs.array_kernels.setna(A, iuq__aoevj)
            ...
            def setitem_optional_int_arr(A, idx, val):
                ...
                for iuq__aoevj in idx:
                    if val is None:
                        bodo.libs.array_kernels.setna(A, iuq__aoevj)
                        continue
                    A[iuq__aoevj] = bodo.utils.indexing.unoptional(val)
...
def unoptional(typingctx, val_t=None):
    ...
    def codegen(context, builder, signature, args):
        vqwu__eztkj = context.make_helper(builder, val_t, args[0])
        npaw__jcm = vqwu__eztkj.data
        context.nrt.incref(builder, val_t.type, npaw__jcm)
        return npaw__jcm
