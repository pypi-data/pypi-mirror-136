...
def get_dtype_size(typingctx, dtype=None):
    ...
    def codegen(context, builder, sig, args):
        swkss__phs = context.get_abi_sizeof(context.get_data_type(dtype.dtype))
        return context.get_constant(types.intp, swkss__phs)
...
def tofile_overload(arr, fname):
    if fname == string_type or isinstance(fname, types.StringLiteral):
        ...
        def tofile_impl(arr, fname):
            ...
            iudbo__ddo = np.ascontiguousarray(arr)
            dtype_size = get_dtype_size(iudbo__ddo.dtype)
            file_write(unicode_to_utf8(fname), iudbo__ddo.ctypes, 
                dtype_size * iudbo__ddo.size)
...
def file_write_parallel_overload(fname, arr, start, count):
    if fname == string_type:
        ...
        def _impl(fname, arr, start, count):
            iudbo__ddo = np.ascontiguousarray(arr)
            dtype_size = get_dtype_size(iudbo__ddo.dtype)
            kwz__fodcj = dtype_size * bodo.libs.distributed_api.get_tuple_prod(
                iudbo__ddo.shape[1:])
            _file_write_parallel(unicode_to_utf8(fname), iudbo__ddo.ctypes,
                start, count, kwz__fodcj)
            bodo.utils.utils.check_and_propagate_cpp_exception()
        return _impl
...
def get_file_size_overload(fname, count, offset, dtype_size):
    if fname == string_type:
        ...
        def impl(fname, count, offset, dtype_size):
            if offset < 0:
                return -1
            vchvp__lbin = _get_file_size(unicode_to_utf8(fname)) - offset
            bodo.utils.utils.check_and_propagate_cpp_exception()
            if count != -1:
                vchvp__lbin = min(vchvp__lbin, count * dtype_size)
            if vchvp__lbin < 0:
                return -1
            return vchvp__lbin
        return impl
