...
class SeriesIatType(types.Type):
    ...
    def __init__(self, stype):
        self.stype = stype
        cfe__ytm = 'SeriesIatType({})'.format(stype)
        super(SeriesIatType, self).__init__(cfe__ytm)
...
class SeriesIatModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        pwc__sylb = [('obj', fe_type.stype)]
        super(SeriesIatModel, self).__init__(dmm, fe_type, pwc__sylb)
...
def init_series_iat(typingctx, obj=None):
    ...
    def codegen(context, builder, signature, args):
        nsaw__kxcjs, = args
        vib__pzck = signature.return_type
        yohy__veq = cgutils.create_struct_proxy(vib__pzck)(context, builder)
        yohy__veq.obj = nsaw__kxcjs
        context.nrt.incref(builder, signature.args[0], nsaw__kxcjs)
        return yohy__veq._getvalue()
...
def overload_series_iat_setitem(I, idx, val):
    if isinstance(I, SeriesIatType):
        ...
        if I.stype.dtype == types.NPTimedelta('ns'
            ...
            def impl_dt(I, idx, val):
                ggnoa__ihdmy = (bodo.hiframes.datetime_timedelta_ext.
                    _to_nanoseconds(val))
...
class SeriesIlocType(types.Type):
    ...
    def __init__(self, stype):
        self.stype = stype
        cfe__ytm = 'SeriesIlocType({})'.format(stype)
        super(SeriesIlocType, self).__init__(cfe__ytm)
...
class SeriesIlocModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        pwc__sylb = [('obj', fe_type.stype)]
        super(SeriesIlocModel, self).__init__(dmm, fe_type, pwc__sylb)
...
def init_series_iloc(typingctx, obj=None):
    ...
    def codegen(context, builder, signature, args):
        nsaw__kxcjs, = args
        lhyq__ekb = signature.return_type
        vkak__ypu = cgutils.create_struct_proxy(lhyq__ekb)(context, builder)
        vkak__ypu.obj = nsaw__kxcjs
        context.nrt.incref(builder, signature.args[0], nsaw__kxcjs)
        return vkak__ypu._getvalue()
...
def overload_series_iloc_getitem(I, idx):
    if isinstance(I, SeriesIlocType):
        ...
        if is_list_like_index_type(idx) and isinstance(idx.dtype, (types.
            ...
            def impl(I, idx):
                S = I._obj
                rcai__aoa = bodo.utils.conversion.coerce_to_ndarray(idx)
                lfnae__zekuk = bodo.hiframes.pd_series_ext.get_series_data(S)[
                    rcai__aoa]
                rjczq__nnxfb = bodo.hiframes.pd_series_ext.get_series_index(S)[
                    rcai__aoa]
                cfe__ytm = bodo.hiframes.pd_series_ext.get_series_name(S)
                return bodo.hiframes.pd_series_ext.init_series(lfnae__zekuk,
                    rjczq__nnxfb, cfe__ytm)
            return impl
        ...
        if is_list_like_index_type(idx) and isinstance(idx.dtype, (types.
            ...
            if is_scalar_type(val):
                ...
                if I.stype.dtype == types.NPTimedelta('ns'
                    ...
                    def impl_dt(I, idx, val):
                        ggnoa__ihdmy = (bodo.hiframes.
                            datetime_timedelta_ext._to_nanoseconds(val))
                        s = (bodo.hiframes.pd_timestamp_ext.
                            integer_to_timedelta64(ggnoa__ihdmy))
                        rcai__aoa = bodo.utils.conversion.coerce_to_ndarray(idx
                            )
                        bodo.hiframes.pd_series_ext.get_series_data(I._obj)[
                            rcai__aoa] = s
            ...
            def impl_arr(I, idx, val):
                rcai__aoa = bodo.utils.conversion.coerce_to_ndarray(idx)
                bodo.hiframes.pd_series_ext.get_series_data(I._obj)[rcai__aoa
                    ] = bodo.utils.conversion.coerce_to_array(val, False)
            return impl_arr
...
class SeriesLocType(types.Type):
    ...
    def __init__(self, stype):
        self.stype = stype
        cfe__ytm = 'SeriesLocType({})'.format(stype)
        super(SeriesLocType, self).__init__(cfe__ytm)
...
class SeriesLocModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        pwc__sylb = [('obj', fe_type.stype)]
        super(SeriesLocModel, self).__init__(dmm, fe_type, pwc__sylb)
...
def init_series_loc(typingctx, obj=None):
    ...
    def codegen(context, builder, signature, args):
        nsaw__kxcjs, = args
        negkd__sqo = signature.return_type
        afmb__pmb = cgutils.create_struct_proxy(negkd__sqo)(context, builder)
        afmb__pmb.obj = nsaw__kxcjs
        context.nrt.incref(builder, signature.args[0], nsaw__kxcjs)
        return afmb__pmb._getvalue()
...
def overload_series_loc_getitem(I, idx):
    ...
    if isinstance(idx, types.Integer) and isinstance(I.stype.index,
        ...
        def impl_range_index_int(I, idx):
            ...
            lfnae__zekuk = bodo.hiframes.pd_series_ext.get_series_data(S)
            rjczq__nnxfb = bodo.hiframes.pd_series_ext.get_series_index(S)
            senv__ozv = (idx - rjczq__nnxfb._start) // rjczq__nnxfb._step
            return lfnae__zekuk[senv__ozv]
    ...
    if isinstance(S, SeriesType) and not (isinstance(S.index,
        ...
        if isinstance(idx, types.Integer):
            ...
            if isinstance(S.index, RangeIndexType):
                ...
                def impl(S, idx):
                    lfnae__zekuk = bodo.hiframes.pd_series_ext.get_series_data(
                        S)
                    I = bodo.hiframes.pd_series_ext.get_series_index(S)
                    rcai__aoa = idx * I._step + I._start
                    return bodo.utils.conversion.box_if_dt64(lfnae__zekuk[
                        rcai__aoa])
                return impl
        ...
        if idx == bodo.string_type or is_overload_constant_str(idx):
            if isinstance(S.index, bodo.hiframes.pd_index_ext.StringIndexType):
                ...
                def impl_str_getitem(S, idx):
                    rjob__lkxx = bodo.hiframes.pd_series_ext.get_series_index(S
                        )
                    gsk__ots = rjob__lkxx.get_loc(idx)
                    if gsk__ots is None:
                        raise IndexError
                    else:
                        val = bodo.hiframes.pd_series_ext.get_series_data(S)[
                            bodo.utils.indexing.unoptional(gsk__ots)]
                    return val
    ...
    if isinstance(S, SeriesType):
        ...
        if is_list_like_index_type(idx) and isinstance(idx.dtype, (types.
            ...
            def impl_arr(S, idx, val):
                rcai__aoa = bodo.utils.conversion.coerce_to_ndarray(idx)
                bodo.hiframes.pd_series_ext.get_series_data(S)[rcai__aoa
                    ] = bodo.utils.conversion.coerce_to_array(val, False)
            return impl_arr
...
def overload_const_index_series_getitem(S, idx):
    if isinstance(S, (SeriesType, HeterogeneousSeriesType)) and isinstance(S
        ...
        index.data):
        zly__srg = get_overload_const_tuple(S.index.data)
        if isinstance(idx, types.Integer) and not any(isinstance(hqui__yrk,
            int) for hqui__yrk in zly__srg):
            return lambda S, idx: bodo.hiframes.pd_series_ext.get_series_data(S
                )[idx]
        if is_literal_type(idx):
            xpv__rddrq = get_literal_value(idx)
            if xpv__rddrq not in zly__srg:
                raise_bodo_error(
                    f"Series label-based getitem: '{xpv__rddrq}' not in {zly__srg}"
                    )
            arr_ind = zly__srg.index(xpv__rddrq)
            return lambda S, idx: bodo.hiframes.pd_series_ext.get_series_data(S
                )[arr_ind]
...
def cast_series_iat(context, builder, fromty, toty, val):
    yohy__veq = cgutils.create_struct_proxy(fromty)(context, builder, val)
    cno__lqq = context.cast(builder, yohy__veq.obj, fromty.stype, toty.stype)
    sapyg__fvb = cgutils.create_struct_proxy(toty)(context, builder)
    sapyg__fvb.obj = cno__lqq
    return sapyg__fvb._getvalue()
