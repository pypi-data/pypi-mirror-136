...
def overload_h5_file_keys(obj_id):
    ...
    def h5f_keys_impl(obj_id):
        vds__wnsuv = []
        mqsy__fitpp = h5g_get_num_objs(obj_id)
        for ztq__iwms in range(mqsy__fitpp):
            jxtf__jdfrm = h5g_get_objname_by_idx(obj_id, ztq__iwms)
            vds__wnsuv.append(jxtf__jdfrm)
        return vds__wnsuv
...
def overload_h5_file_create_dataset(obj_id, name, shape=None, dtype=None,
    ...
    wsvk__fqbu = parse_dtype(dtype)
    lez__jukr = np.int32(numba_to_c_type(wsvk__fqbu))
...
def overload_getitem_file(in_f, in_idx):
    if in_f in (h5file_type, h5dataset_or_group_type
        ...
        def impl(in_f, in_idx):
            iniis__frygz = h5_open_dset_or_group_obj(unify_h5_id(in_f),
...
def h5read(dset_id, ndim, starts, counts, is_parallel, out_arr):
    eeth__jvwq = tuple_to_ptr(starts)
    vkqn__ypvdn = tuple_to_ptr(counts)
    ixw__oqs = bodo.libs.distributed_api.get_type_enum(out_arr)
    return _h5_read(cast_to_h5_dset(dset_id), ndim, eeth__jvwq, vkqn__ypvdn,
        is_parallel, out_arr.ctypes, ixw__oqs)
...
def h5write(dset_id, ndim, starts, counts, is_parallel, out_arr):
    ...
    vkqn__ypvdn = tuple_to_ptr(counts)
    ixw__oqs = bodo.libs.distributed_api.get_type_enum(out_arr)
    return _h5_write(cast_to_h5_dset(dset_id), ndim, eeth__jvwq,
        vkqn__ypvdn, is_parallel, out_arr.ctypes, ixw__oqs)
...
class H5ReadType(AbstractTemplate):
    ...
    def generic(self, args, kws):
        ...
        hmuah__qbg = types.Array(dtype, ndim, 'C')
        rtpng__krb = args[3]
        hmuah__qbg = self.context.resolve_function_type(operator.getitem, [
            hmuah__qbg, rtpng__krb], {}).return_type.copy(layout='C')
        return signature(hmuah__qbg, *args)
...
def get_filter_read_indices(bool_arr):
    ncih__jsl = bool_arr.nonzero()[0]
    wjnns__pjqgx = bodo.libs.distributed_api.get_rank()
    utac__fuhag = bodo.libs.distributed_api.get_size()
    lposz__ebfjf = np.empty(utac__fuhag, np.int64)
    mtw__ltp = len(bool_arr)
    bodo.libs.distributed_api.allgather(lposz__ebfjf, mtw__ltp)
    upvrh__wqbrw = lposz__ebfjf.cumsum()[wjnns__pjqgx] - mtw__ltp
    ncih__jsl += upvrh__wqbrw
    gudci__hksu = bodo.libs.distributed_api.dist_reduce(len(ncih__jsl), np.
        int32(sum_op))
    qzs__kfx = bodo.libs.distributed_api.gatherv(ncih__jsl)
    if wjnns__pjqgx == 0:
        bigzp__gxvk = qzs__kfx
    else:
        bigzp__gxvk = np.empty(gudci__hksu, ncih__jsl.dtype)
    bodo.libs.distributed_api.bcast(bigzp__gxvk)
    ubt__jwmm = bodo.libs.distributed_api.get_start(gudci__hksu,
        utac__fuhag, wjnns__pjqgx)
    uscrf__cvf = bodo.libs.distributed_api.get_end(gudci__hksu, utac__fuhag,
        wjnns__pjqgx)
    return bigzp__gxvk[ubt__jwmm:uscrf__cvf]
...
def tuple_to_ptr(typingctx, tuple_tp=None):
    ...
    def codegen(context, builder, sig, args):
        wyyf__lcxu = cgutils.alloca_once(builder, args[0].type)
        builder.store(args[0], wyyf__lcxu)
        return builder.bitcast(wyyf__lcxu, lir.IntType(8).as_pointer())
...
def h5read_filter(dset_id, ndim, starts, counts, is_parallel, out_arr,
    ...
    vkqn__ypvdn = tuple_to_ptr(counts)
    ixw__oqs = bodo.libs.distributed_api.get_type_enum(out_arr)
    return _h5read_filter(cast_to_h5_dset(dset_id), ndim, eeth__jvwq,
        vkqn__ypvdn, is_parallel, out_arr.ctypes, ixw__oqs, read_indices.
