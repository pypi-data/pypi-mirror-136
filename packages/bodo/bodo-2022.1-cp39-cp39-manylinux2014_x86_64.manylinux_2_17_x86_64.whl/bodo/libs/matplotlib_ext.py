...
def install_mpl_class(types_name, python_type):
    hdger__ifn = ''.join(map(str.title, types_name.split('_')))
    phhl__yxtwq = f'class {hdger__ifn}(types.Opaque):\n'
    phhl__yxtwq += '    def __init__(self):\n'
    phhl__yxtwq += f"       types.Opaque.__init__(self, name='{hdger__ifn}')\n"
    phhl__yxtwq += '    def __reduce__(self):\n'
    phhl__yxtwq += (
        f"        return (types.Opaque, ('{hdger__ifn}',), self.__dict__)\n")
    jzonn__midj = {}
    exec(phhl__yxtwq, {'types': types, 'bodo': bodo}, jzonn__midj)
    zwb__xcoju = jzonn__midj[hdger__ifn]
    rkzwv__ucd = sys.modules[__name__]
    setattr(rkzwv__ucd, hdger__ifn, zwb__xcoju)
    class_instance = zwb__xcoju()
    setattr(types, types_name, class_instance)
    register_model(zwb__xcoju)(models.OpaqueModel)
    typeof_impl.register(python_type)(lambda val, c: class_instance)
    unbox(zwb__xcoju)(unbox_mpl_obj)
    box(zwb__xcoju)(box_mpl_obj)
...
def _install_mpl_types():
    uyf__znbsj = [('mpl_figure_type', matplotlib.figure.Figure), (
    ...
    for dsvnp__ernq, hrdfd__cepnn in uyf__znbsj:
        install_mpl_class(dsvnp__ernq, hrdfd__cepnn)
...
def generate_matplotlib_signature(return_typ, args, kws, obj_typ=None):
    kws = dict(kws)
    mzd__kwl = ', '.join(f'e{twg__vzo}' for twg__vzo in range(len(args)))
    if mzd__kwl:
        mzd__kwl += ', '
    gjy__mpz = ', '.join(f"{mox__jxydq} = ''" for mox__jxydq in kws.keys())
    zruue__oigdv = 'matplotlib_obj, ' if obj_typ is not None else ''
    wehu__mad = f'def mpl_stub({zruue__oigdv} {mzd__kwl} {gjy__mpz}):\n'
    wehu__mad += '    pass\n'
    yauie__mtcx = {}
    exec(wehu__mad, {}, yauie__mtcx)
    cfini__cvz = yauie__mtcx['mpl_stub']
    gvap__thc = numba.core.utils.pysignature(cfini__cvz)
    xxwxv__tmalk = ((obj_typ,) if obj_typ is not None else ()) + args + tuple(
        kws.values())
    return signature(return_typ, *unliteral_all(xxwxv__tmalk)).replace(pysig
        =gvap__thc)


def generate_axes_typing(mod_name, nrows, ncols):
    fip__ksa = '{}.subplots(): {} must be a constant integer >= 1'
    if not is_overload_constant_int(nrows):
        raise_bodo_error(fip__ksa.format(mod_name, 'nrows'))
    if not is_overload_constant_int(ncols):
        raise_bodo_error(fip__ksa.format(mod_name, 'ncols'))
    vlpmd__axgf = get_overload_const_int(nrows)
    bzja__efgbp = get_overload_const_int(ncols)
    if vlpmd__axgf < 1:
        raise BodoError(fip__ksa.format(mod_name, 'nrows'))
    if bzja__efgbp < 1:
        raise BodoError(fip__ksa.format(mod_name, 'ncols'))
    if vlpmd__axgf == 1 and bzja__efgbp == 1:
        vlaj__iek = types.mpl_axes_type
    else:
        if bzja__efgbp == 1:
            bbgcg__qiki = types.mpl_axes_type
        else:
            bbgcg__qiki = types.Tuple([types.mpl_axes_type] * bzja__efgbp)
        vlaj__iek = types.Tuple([bbgcg__qiki] * vlpmd__axgf)
    return vlaj__iek


def generate_pie_return_type(args, kws):
    stxh__gfsd = args[4] if len(args) > 5 else kws.get('autopct', types.none)
    if stxh__gfsd == types.none:
...
def generate_xcorr_return_type(func_mod, args, kws):
    tyocb__ikuj = args[4] if len(args) > 5 else kws.get('usevlines', True)
    if not is_overload_constant_bool(tyocb__ikuj):
        raise_bodo_error(
            f'{func_mod}.xcorr(): usevlines must be a constant boolean')
    if is_overload_true(tyocb__ikuj):
...
class SubplotsTyper(AbstractTemplate):
    ...
    def generic(self, args, kws):
        ...
        zlf__lkm = generate_axes_typing('matplotlib.pyplot', nrows, ncols)
        return generate_matplotlib_signature(types.Tuple([types.
            mpl_figure_type, zlf__lkm]), args, kws)
...
def overload_subplots(fig, nrows=1, ncols=1, sharex=False, sharey=False,
    ...
    zlf__lkm = generate_axes_typing('matplotlib.figure.Figure', nrows, ncols)
    dsvnp__ernq = str(zlf__lkm)
    if not hasattr(types, dsvnp__ernq):
        dsvnp__ernq = f'objmode_type{ir_utils.next_label()}'
        setattr(types, dsvnp__ernq, zlf__lkm)
    wehu__mad = f"""def impl(
        fig,
        nrows=1,
        ncols=1,
        sharex=False,
        ...
        with numba.objmode(axes="{dsvnp__ernq}"):
            axes = fig.subplots(
                nrows=nrows,
                ncols=ncols,
                sharex=sharex,
    ...
    """
    yauie__mtcx = {}
    exec(wehu__mad, {'numba': numba, 'np': np}, yauie__mtcx)
    impl = yauie__mtcx['impl']
    return impl
