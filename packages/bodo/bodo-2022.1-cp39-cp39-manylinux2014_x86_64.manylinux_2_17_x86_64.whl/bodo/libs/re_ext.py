...
def box_regex_flag(typ, val, c):
    jicj__bft = c.box(types.int64, val)
    rbij__oem = c.pyapi.unserialize(c.pyapi.serialize_object(re.RegexFlag))
    return c.pyapi.call_function_objargs(rbij__oem, (jicj__bft,))


@unbox(RegexFlagsType)
def unbox_regex_flag(typ, obj, c):
    jicj__bft = c.pyapi.object_getattr_string(obj, 'value')
    return c.unbox(types.int64, jicj__bft)
...
def cast_match_obj_bool(context, builder, fromty, toty, val):
    out = cgutils.alloca_once_value(builder, context.get_constant(types.
        bool_, True))
    acxm__ymyx = context.get_python_api(builder)
    gwpde__mwnf = builder.icmp_signed('==', val, acxm__ymyx.borrow_none())
    with builder.if_then(gwpde__mwnf):
...
def init_const_pattern(typingctx, pat, pat_const=None):
    ldicr__puzf = get_overload_const_str(pat_const)

    def codegen(context, builder, sig, args):
        return impl_ret_borrowed(context, builder, sig.return_type, args[0])
    return RePatternType(ldicr__puzf)(pat, pat_const), codegen
...
def overload_pat_findall(p, string, pos=0, endpos=9223372036854775807):
    if p.pat_const:
        rdpxi__rwynl = re.compile(p.pat_const).groups
        typ = types.List(string_type)
        if rdpxi__rwynl > 1:
            typ = types.List(types.Tuple([string_type] * rdpxi__rwynl))
...
def overload_match_group(m, *args):
    ...
    kure__oob = types.optional(string_type)
    if len(args) == 1:

        def _match_group_impl_one(m, *args):
            xsk__omxf = args[0]
            with numba.objmode(out=kure__oob):
                out = m.group(xsk__omxf)
            return out
        return _match_group_impl_one
    cbtp__pwcbp = 'tuple_str_{}'.format(len(args))
    setattr(types, cbtp__pwcbp, types.Tuple([kure__oob] * len(args)))
    uepai__apgjh = ', '.join('group{}'.format(irth__ajtxq + 1) for
        irth__ajtxq in range(len(args)))
    aba__hdvgm = 'def _match_group_impl(m, *args):\n'
    aba__hdvgm += '  ({}) = args\n'.format(uepai__apgjh)
    aba__hdvgm += "  with numba.objmode(out='{}'):\n".format(cbtp__pwcbp)
    aba__hdvgm += '    out = m.group({})\n'.format(uepai__apgjh)
    aba__hdvgm += '  return out\n'
    ghspo__jcsbp = {}
    exec(aba__hdvgm, globals(), ghspo__jcsbp)
    wtuyc__msph = ghspo__jcsbp['_match_group_impl']
    return wtuyc__msph
...
def overload_match_groups(m, default=None):
    mjhh__okun = types.List(types.optional(string_type))

    def _match_groups_impl(m, default=None):
        with numba.objmode(out=mjhh__okun):
...
def overload_match_groupdict(m, default=None):
    ...
    def _check_dict_none(out):
        if any(kpkia__eky is None for kpkia__eky in out.values()):
            raise BodoError(
...
def overload_match_lastgroup(p):
    ...
    def _match_lastgroup_impl(p):
        with numba.objmode(lastgroup=kure__oob):
