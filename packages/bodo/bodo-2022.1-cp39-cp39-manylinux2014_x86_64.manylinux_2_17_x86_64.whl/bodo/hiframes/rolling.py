...
class RollingCovType(AbstractTemplate):
    ...
    def generic(self, args, kws):
        arr = args[0]
        yme__kxt = arr.copy(dtype=types.float64)
        return signature(yme__kxt, *unliteral_all(args))
...
def overload_rolling_fixed(arr, index_arr, win, minp, center, fname, raw=
    ...
    hrxd__vrc = get_overload_const_str(fname)
    if hrxd__vrc not in ('sum', 'mean', 'var', 'std', 'count', 'median',
        'min', 'max'):
        raise BodoError('invalid rolling (fixed window) function {}'.format
            (hrxd__vrc))
    if hrxd__vrc in ('median', 'min', 'max'):
        snzxz__vbnw = 'def kernel_func(A):\n'
        snzxz__vbnw += '  if np.isnan(A).sum() != 0: return np.nan\n'
        snzxz__vbnw += '  return np.{}(A)\n'.format(hrxd__vrc)
        slo__wps = {}
        exec(snzxz__vbnw, {'np': np}, slo__wps)
        kernel_func = register_jitable(slo__wps['kernel_func'])
        return (lambda arr, index_arr, win, minp, center, fname, raw=True,
            parallel=False: roll_fixed_apply(arr, index_arr, win, minp,
...
def overload_rolling_variable(arr, on_arr, index_arr, win, minp, center,
    ...
    if hrxd__vrc in ('median', 'min', 'max'):
        snzxz__vbnw = 'def kernel_func(A):\n'
        snzxz__vbnw += '  arr  = dropna(A)\n'
        snzxz__vbnw += '  if len(arr) == 0: return np.nan\n'
        snzxz__vbnw += '  return np.{}(arr)\n'.format(hrxd__vrc)
        slo__wps = {}
        exec(snzxz__vbnw, {'np': np, 'dropna': _dropna}, slo__wps)
        kernel_func = register_jitable(slo__wps['kernel_func'])
        return (lambda arr, on_arr, index_arr, win, minp, center, fname,
            raw=True, parallel=False: roll_variable_apply(arr, on_arr,
...
def roll_fixed_linear_generic(in_arr, win, minp, center, parallel,
    ...
    if parallel:
        ...
        tnoh__juqxz = _border_icomm(in_arr, rank, n_pes, halo_size, True,
            center)
        (l_recv_buff, r_recv_buff, l_send_req, r_send_req, l_recv_req,
            ovvy__nyrsz) = tnoh__juqxz
    ...
    if parallel:
        ...
        if center and rank != n_pes - 1:
            bodo.libs.distributed_api.wait(ovvy__nyrsz, True)
            for iyupa__sjjn in range(0, halo_size):
                data = add_obs(r_recv_buff[iyupa__sjjn], *data)
                lhw__nskh = in_arr[N + iyupa__sjjn - win]
                data = remove_obs(lhw__nskh, *data)
                output[N + iyupa__sjjn - offset] = calc_out(minp, *data)
        if rank != 0:
            bodo.libs.distributed_api.wait(l_recv_req, True)
            data = init_data()
            for iyupa__sjjn in range(0, halo_size):
                data = add_obs(l_recv_buff[iyupa__sjjn], *data)
            for iyupa__sjjn in range(0, win - 1):
                data = add_obs(in_arr[iyupa__sjjn], *data)
                if iyupa__sjjn > offset:
                    lhw__nskh = l_recv_buff[iyupa__sjjn - offset - 1]
                    data = remove_obs(lhw__nskh, *data)
                if iyupa__sjjn >= offset:
                    output[iyupa__sjjn - offset] = calc_out(minp, *data)
    return output
...
def roll_fixed_linear_generic_seq(in_arr, win, minp, center, init_data,
    ...
    qorru__qgo = max(minp, 1) - 1
    qorru__qgo = min(qorru__qgo, N)
    for iyupa__sjjn in range(0, qorru__qgo):
        data = add_obs(in_arr[iyupa__sjjn], *data)
        if iyupa__sjjn >= offset:
            output[iyupa__sjjn - offset] = calc_out(minp, *data)
    for iyupa__sjjn in range(qorru__qgo, N):
        val = in_arr[iyupa__sjjn]
        data = add_obs(val, *data)
        if iyupa__sjjn > win - 1:
            lhw__nskh = in_arr[iyupa__sjjn - win]
            data = remove_obs(lhw__nskh, *data)
        output[iyupa__sjjn - offset] = calc_out(minp, *data)
    lli__kyra = data
    for iyupa__sjjn in range(N, N + offset):
        if iyupa__sjjn > win - 1:
            lhw__nskh = in_arr[iyupa__sjjn - win]
            data = remove_obs(lhw__nskh, *data)
        output[iyupa__sjjn - offset] = calc_out(minp, *data)
    return output, lli__kyra
...
def roll_fixed_apply_impl(in_arr, index_arr, win, minp, center, parallel,
    ...
    if parallel:
        ...
        (l_recv_buff, r_recv_buff, l_send_req, r_send_req, l_recv_req,
            ovvy__nyrsz) = tnoh__juqxz
        if raw == False:
            bncf__qepze = _border_icomm(index_arr, rank, n_pes, halo_size, 
                True, center)
            (l_recv_buff_idx, r_recv_buff_idx, vush__hkyfh, pje__rin,
                lhk__mefzy, tgqnq__lncfd) = bncf__qepze
    output = roll_fixed_apply_seq(in_arr, index_arr, win, minp, center,
        kernel_func, raw)
    if parallel:
        _border_send_wait(r_send_req, l_send_req, rank, n_pes, True, center)
        if raw == False:
            _border_send_wait(pje__rin, vush__hkyfh, rank, n_pes, True, center)
        if center and rank != n_pes - 1:
            bodo.libs.distributed_api.wait(ovvy__nyrsz, True)
            if raw == False:
                bodo.libs.distributed_api.wait(tgqnq__lncfd, True)
            recv_right_compute(output, in_arr, index_arr, N, win, minp,
                offset, r_recv_buff, r_recv_buff_idx, kernel_func, raw)
        if rank != 0:
            bodo.libs.distributed_api.wait(l_recv_req, True)
            if raw == False:
                bodo.libs.distributed_api.wait(lhk__mefzy, True)
...
def overload_recv_right_compute(output, in_arr, index_arr, N, win, minp,
    ...
    if is_overload_true(raw):
        ...
        def impl(output, in_arr, index_arr, N, win, minp, offset,
            r_recv_buff, r_recv_buff_idx, kernel_func, raw):
            lli__kyra = np.concatenate((in_arr[N - win + 1:], r_recv_buff))
            sqr__sunp = 0
            for iyupa__sjjn in range(max(N - offset, 0), N):
                data = lli__kyra[sqr__sunp:sqr__sunp + win]
                if win - np.isnan(data).sum() < minp:
                    output[iyupa__sjjn] = np.nan
                else:
                    output[iyupa__sjjn] = kernel_func(data)
                sqr__sunp += 1
        return impl
    ...
    def impl_series(output, in_arr, index_arr, N, win, minp, offset,
        r_recv_buff, r_recv_buff_idx, kernel_func, raw):
        lli__kyra = np.concatenate((in_arr[N - win + 1:], r_recv_buff))
        huq__vgnj = np.concatenate((index_arr[N - win + 1:], r_recv_buff_idx))
        sqr__sunp = 0
        for iyupa__sjjn in range(max(N - offset, 0), N):
            data = lli__kyra[sqr__sunp:sqr__sunp + win]
            if win - np.isnan(data).sum() < minp:
                ...
                output[iyupa__sjjn] = kernel_func(pd.Series(data, huq__vgnj
                    [sqr__sunp:sqr__sunp + win]))
            sqr__sunp += 1
    return impl_series
...
def overload_recv_left_compute(output, in_arr, index_arr, win, minp, offset,
    ...
    if is_overload_true(raw):
        ...
        def impl(output, in_arr, index_arr, win, minp, offset, l_recv_buff,
            ...
            lli__kyra = np.concatenate((l_recv_buff, in_arr[:win - 1]))
            for iyupa__sjjn in range(0, win - offset - 1):
                data = lli__kyra[iyupa__sjjn:iyupa__sjjn + win]
                if win - np.isnan(data).sum() < minp:
                    output[iyupa__sjjn] = np.nan
                else:
                    output[iyupa__sjjn] = kernel_func(data)
    ...
    def impl_series(output, in_arr, index_arr, win, minp, offset,
        l_recv_buff, l_recv_buff_idx, kernel_func, raw):
        lli__kyra = np.concatenate((l_recv_buff, in_arr[:win - 1]))
        huq__vgnj = np.concatenate((l_recv_buff_idx, index_arr[:win - 1]))
        for iyupa__sjjn in range(0, win - offset - 1):
            data = lli__kyra[iyupa__sjjn:iyupa__sjjn + win]
            if win - np.isnan(data).sum() < minp:
                output[iyupa__sjjn] = np.nan
...
def overload_roll_fixed_apply_seq(in_arr, index_arr, win, minp, center,
    ...
    def roll_fixed_apply_seq_impl(in_arr, index_arr, win, minp, center,
        ...
        output = np.empty(N, dtype=np.float64)
        offset = (win - 1) // 2 if center else 0
        for iyupa__sjjn in range(0, N):
            start = max(iyupa__sjjn - win + 1 + offset, 0)
            end = min(iyupa__sjjn + 1 + offset, N)
            data = in_arr[start:end]
            if end - start - np.isnan(data).sum() < minp:
                output[iyupa__sjjn] = np.nan
            else:
                output[iyupa__sjjn] = apply_func(kernel_func, data,
...
def roll_var_linear_generic(in_arr, on_arr_dt, win, minp, center, parallel,
    ...
    if parallel:
        ...
        tnoh__juqxz = _border_icomm_var(in_arr, on_arr, rank, n_pes, win)
        (l_recv_buff, l_recv_t_buff, r_send_req, jmuvv__jez, l_recv_req,
            lfv__boutl) = tnoh__juqxz
    ...
    if parallel:
        ...
        _border_send_wait(jmuvv__jez, jmuvv__jez, rank, n_pes, True, False)
        if rank != 0:
            bodo.libs.distributed_api.wait(l_recv_req, True)
            bodo.libs.distributed_api.wait(lfv__boutl, True)
            num_zero_starts = 0
            for iyupa__sjjn in range(0, N):
                if start[iyupa__sjjn] != 0:
                    break
                num_zero_starts += 1
            ...
            recv_starts = _get_var_recv_starts(on_arr, l_recv_t_buff,
                num_zero_starts, win)
            data = init_data()
            for kchjd__glmac in range(recv_starts[0], len(l_recv_t_buff)):
                data = add_obs(l_recv_buff[kchjd__glmac], *data)
            if right_closed:
                data = add_obs(in_arr[0], *data)
            output[0] = calc_out(minp, *data)
            for iyupa__sjjn in range(1, num_zero_starts):
                s = recv_starts[iyupa__sjjn]
                lompw__ydgli = end[iyupa__sjjn]
                for kchjd__glmac in range(recv_starts[iyupa__sjjn - 1], s):
                    data = remove_obs(l_recv_buff[kchjd__glmac], *data)
                for kchjd__glmac in range(end[iyupa__sjjn - 1], lompw__ydgli):
                    data = add_obs(in_arr[kchjd__glmac], *data)
                output[iyupa__sjjn] = calc_out(minp, *data)
    return output
...
def _get_var_recv_starts(on_arr, l_recv_t_buff, num_zero_starts, win):
    ...
    rpfk__sup = cast_dt64_arr_to_int(on_arr)
    left_closed = False
    yboxl__orys = rpfk__sup[0] - win
    if left_closed:
        yboxl__orys -= 1
    recv_starts[0] = halo_size
    for kchjd__glmac in range(0, halo_size):
        if l_recv_t_buff[kchjd__glmac] > yboxl__orys:
            recv_starts[0] = kchjd__glmac
            break
    for iyupa__sjjn in range(1, num_zero_starts):
        yboxl__orys = rpfk__sup[iyupa__sjjn] - win
        if left_closed:
            yboxl__orys -= 1
        recv_starts[iyupa__sjjn] = halo_size
        for kchjd__glmac in range(recv_starts[iyupa__sjjn - 1], halo_size):
            if l_recv_t_buff[kchjd__glmac] > yboxl__orys:
                recv_starts[iyupa__sjjn] = kchjd__glmac
                break
    return recv_starts
...
def roll_var_linear_generic_seq(in_arr, on_arr, win, minp, start, end,
    ...
    data = init_data()
    for kchjd__glmac in range(start[0], end[0]):
        data = add_obs(in_arr[kchjd__glmac], *data)
    output[0] = calc_out(minp, *data)
    for iyupa__sjjn in range(1, N):
        s = start[iyupa__sjjn]
        lompw__ydgli = end[iyupa__sjjn]
        for kchjd__glmac in range(start[iyupa__sjjn - 1], s):
            data = remove_obs(in_arr[kchjd__glmac], *data)
        for kchjd__glmac in range(end[iyupa__sjjn - 1], lompw__ydgli):
            data = add_obs(in_arr[kchjd__glmac], *data)
        output[iyupa__sjjn] = calc_out(minp, *data)
    return output
...
def roll_variable_apply_impl(in_arr, on_arr_dt, index_arr, win, minp,
    ...
    if parallel:
        ...
        (l_recv_buff, l_recv_t_buff, r_send_req, jmuvv__jez, l_recv_req,
            lfv__boutl) = tnoh__juqxz
        if raw == False:
            bncf__qepze = _border_icomm_var(index_arr, on_arr, rank, n_pes, win
                )
            (l_recv_buff_idx, ioqn__epixb, pje__rin, slrv__rppi, lhk__mefzy,
                vvg__xtbxf) = bncf__qepze
    start, end = _build_indexer(on_arr, N, win, left_closed, right_closed)
    ...
    if parallel:
        ...
        _border_send_wait(jmuvv__jez, jmuvv__jez, rank, n_pes, True, False)
        if raw == False:
            _border_send_wait(pje__rin, pje__rin, rank, n_pes, True, False)
            _border_send_wait(slrv__rppi, slrv__rppi, rank, n_pes, True, False)
        if rank != 0:
            bodo.libs.distributed_api.wait(l_recv_req, True)
            bodo.libs.distributed_api.wait(lfv__boutl, True)
            if raw == False:
                bodo.libs.distributed_api.wait(lhk__mefzy, True)
                bodo.libs.distributed_api.wait(vvg__xtbxf, True)
            num_zero_starts = 0
            for iyupa__sjjn in range(0, N):
                if start[iyupa__sjjn] != 0:
                    break
...
def overload_recv_left_var_compute(output, in_arr, index_arr,
    ...
    if is_overload_true(raw):
        ...
        def impl(output, in_arr, index_arr, num_zero_starts, recv_starts,
            ...
            for iyupa__sjjn in range(0, num_zero_starts):
                obcg__bsrly = recv_starts[iyupa__sjjn]
                tyznz__adoiz = np.concatenate((l_recv_buff[obcg__bsrly:],
                    in_arr[:iyupa__sjjn + 1]))
                if len(tyznz__adoiz) - np.isnan(tyznz__adoiz).sum() >= minp:
                    output[iyupa__sjjn] = kernel_func(tyznz__adoiz)
                else:
                    output[iyupa__sjjn] = np.nan
        return impl
    ...
    def impl_series(output, in_arr, index_arr, num_zero_starts, recv_starts,
        ...
        for iyupa__sjjn in range(0, num_zero_starts):
            ...
            tyznz__adoiz = np.concatenate((l_recv_buff[obcg__bsrly:],
                in_arr[:iyupa__sjjn + 1]))
            zxjd__xjj = np.concatenate((l_recv_buff_idx[obcg__bsrly:],
                index_arr[:iyupa__sjjn + 1]))
            if len(tyznz__adoiz) - np.isnan(tyznz__adoiz).sum() >= minp:
                output[iyupa__sjjn] = kernel_func(pd.Series(tyznz__adoiz,
                    zxjd__xjj))
            else:
                output[iyupa__sjjn] = np.nan
    return impl_series
...
def roll_variable_apply_seq_impl(in_arr, on_arr, index_arr, win, minp,
    ...
    for iyupa__sjjn in range(0, N):
        s = start[iyupa__sjjn]
        lompw__ydgli = end[iyupa__sjjn]
        data = in_arr[s:lompw__ydgli]
        if lompw__ydgli - s - np.isnan(data).sum() >= minp:
            output[iyupa__sjjn] = kernel_func(data)
        else:
            output[iyupa__sjjn] = np.nan
    return output
...
def roll_variable_apply_seq_impl_series(in_arr, on_arr, index_arr, win,
    ...
    for iyupa__sjjn in range(0, N):
        ...
        data = in_arr[s:lompw__ydgli]
        if lompw__ydgli - s - np.isnan(data).sum() >= minp:
            output[iyupa__sjjn] = kernel_func(pd.Series(data, index_arr[s:
                lompw__ydgli]))
        else:
            output[iyupa__sjjn] = np.nan
    return output
...
def _build_indexer(on_arr, N, win, left_closed, right_closed):
    ...
    for iyupa__sjjn in range(1, N):
        qlr__cbh = rpfk__sup[iyupa__sjjn]
        yboxl__orys = rpfk__sup[iyupa__sjjn] - win
        if left_closed:
            yboxl__orys -= 1
        start[iyupa__sjjn] = iyupa__sjjn
        for kchjd__glmac in range(start[iyupa__sjjn - 1], iyupa__sjjn):
            if rpfk__sup[kchjd__glmac] > yboxl__orys:
                start[iyupa__sjjn] = kchjd__glmac
                break
        if rpfk__sup[end[iyupa__sjjn - 1]] <= qlr__cbh:
            end[iyupa__sjjn] = iyupa__sjjn + 1
        else:
            end[iyupa__sjjn] = end[iyupa__sjjn - 1]
        if not right_closed:
            end[iyupa__sjjn] -= 1
    return start, end
...
def calc_mean(minp, nobs, sum_x, neg_ct):
    if nobs >= minp:
        arsgv__zdkio = sum_x / nobs
        if neg_ct == 0 and arsgv__zdkio < 0.0:
            arsgv__zdkio = 0
        elif neg_ct == nobs and arsgv__zdkio > 0.0:
            arsgv__zdkio = 0
    else:
        arsgv__zdkio = np.nan
    return arsgv__zdkio
...
def add_var(val, nobs, mean_x, ssqdm_x):
    if not np.isnan(val):
        nobs += 1
        avmy__qby = val - mean_x
        mean_x += avmy__qby / nobs
        ssqdm_x += (nobs - 1) * avmy__qby ** 2 / nobs
...
def calc_var(minp, nobs, mean_x, ssqdm_x):
    msqbn__dmzsz = 1.0
    arsgv__zdkio = np.nan
    if nobs >= minp and nobs > msqbn__dmzsz:
        if nobs == 1:
            arsgv__zdkio = 0.0
        else:
            arsgv__zdkio = ssqdm_x / (nobs - msqbn__dmzsz)
            if arsgv__zdkio < 0.0:
                arsgv__zdkio = 0.0
    return arsgv__zdkio
...
def calc_std(minp, nobs, mean_x, ssqdm_x):
    imh__qwieg = calc_var(minp, nobs, mean_x, ssqdm_x)
    return np.sqrt(imh__qwieg)
...
def shift_impl(in_arr, shift, parallel):
    ...
    if parallel:
        ...
        tnoh__juqxz = _border_icomm(in_arr, rank, n_pes, halo_size,
    ...
    if parallel:
        if send_right:
            ...
            if rank != n_pes - 1:
                bodo.libs.distributed_api.wait(ovvy__nyrsz, True)
                for iyupa__sjjn in range(0, halo_size):
                    if bodo.libs.array_kernels.isna(r_recv_buff, iyupa__sjjn):
                        bodo.libs.array_kernels.setna(output, N - halo_size +
                            iyupa__sjjn)
                        continue
                    output[N - halo_size + iyupa__sjjn] = r_recv_buff[
                        iyupa__sjjn]
    return output
...
def shift_seq(in_arr, shift, output, is_parallel_str=False):
    N = len(in_arr)
    lpgs__wlwm = 1 if shift > 0 else -1
    shift = lpgs__wlwm * min(abs(shift), N)
    if shift > 0 and (not is_parallel_str or bodo.get_rank() == 0):
        bodo.libs.array_kernels.setna_slice(output, slice(None, shift))
    start = max(shift, 0)
    end = min(N, N + shift)
    for iyupa__sjjn in range(start, end):
        if bodo.libs.array_kernels.isna(in_arr, iyupa__sjjn - shift):
            bodo.libs.array_kernels.setna(output, iyupa__sjjn)
            continue
        output[iyupa__sjjn] = in_arr[iyupa__sjjn - shift]
    if shift < 0:
...
def shift_left_recv(r_send_req, l_send_req, rank, n_pes, halo_size,
    ...
    if rank != 0:
        bodo.libs.distributed_api.wait(l_recv_req, True)
        for iyupa__sjjn in range(0, halo_size):
            if bodo.libs.array_kernels.isna(l_recv_buff, iyupa__sjjn):
                bodo.libs.array_kernels.setna(output, iyupa__sjjn)
                continue
            output[iyupa__sjjn] = l_recv_buff[iyupa__sjjn]
    ...
    if parallel:
        if send_right:
            ...
            if rank != 0:
                bodo.libs.distributed_api.wait(l_recv_req, True)
                for iyupa__sjjn in range(0, halo_size):
                    yhsqc__hhom = l_recv_buff[iyupa__sjjn]
                    output[iyupa__sjjn] = (in_arr[iyupa__sjjn] - yhsqc__hhom
                        ) / yhsqc__hhom
        else:
            _border_send_wait(r_send_req, l_send_req, rank, n_pes, False, True)
            if rank != n_pes - 1:
                bodo.libs.distributed_api.wait(ovvy__nyrsz, True)
                for iyupa__sjjn in range(0, halo_size):
                    yhsqc__hhom = r_recv_buff[iyupa__sjjn]
                    output[N - halo_size + iyupa__sjjn] = (in_arr[N -
                        halo_size + iyupa__sjjn] - yhsqc__hhom) / yhsqc__hhom
    return output
...
def get_first_non_na(arr):
    ...
    if arr.dtype == types.float32:
        etxcl__jly = np.float32('nan')

    def impl(arr):
        for iyupa__sjjn in range(len(arr)):
            if not bodo.libs.array_kernels.isna(arr, iyupa__sjjn):
                return arr[iyupa__sjjn]
        return etxcl__jly
    return impl
...
def get_last_non_na(arr):
    ...
    def impl(arr):
        evyng__wkk = len(arr)
        for iyupa__sjjn in range(len(arr)):
            sqr__sunp = evyng__wkk - iyupa__sjjn - 1
            if not bodo.libs.array_kernels.isna(arr, sqr__sunp):
                return arr[sqr__sunp]
        return etxcl__jly
    return impl
...
def pct_change_seq(in_arr, shift):
    ...
    if shift > 0:
        ...
        bodo.libs.array_kernels.setna_slice(output, slice(shift, None))
    if shift > 0:
        plvb__xcja = get_first_non_na(in_arr[:shift])
        qfdpj__vtl = get_last_non_na(in_arr[:shift])
    else:
        plvb__xcja = get_last_non_na(in_arr[:-shift])
        qfdpj__vtl = get_first_non_na(in_arr[:-shift])
    one = get_one_from_arr_dtype(output)
    start = max(shift, 0)
    end = min(N, N + shift)
    for iyupa__sjjn in range(start, end):
        yhsqc__hhom = in_arr[iyupa__sjjn - shift]
        if np.isnan(yhsqc__hhom):
            yhsqc__hhom = plvb__xcja
        else:
            plvb__xcja = yhsqc__hhom
        val = in_arr[iyupa__sjjn]
        if np.isnan(val):
            val = qfdpj__vtl
        else:
            qfdpj__vtl = val
        output[iyupa__sjjn] = val / yhsqc__hhom - one
    return output
...
def _border_icomm(in_arr, rank, n_pes, halo_size, send_right=True,
    send_left=False):
    whyn__hkq = np.int32(comm_border_tag)
...
def _border_icomm_var(in_arr, on_arr, rank, n_pes, win_size):
    ...
    for kchjd__glmac in range(-2, -N, -1):
        ouwvu__mvwl = on_arr[kchjd__glmac]
        if end - ouwvu__mvwl >= win_size:
            halo_size = -kchjd__glmac
            break
    if rank != n_pes - 1:
        bodo.libs.distributed_api.send(halo_size, np.int32(rank + 1), whyn__hkq
            )
        r_send_req = bodo.libs.distributed_api.isend(in_arr[-halo_size:],
            np.int32(halo_size), np.int32(rank + 1), whyn__hkq, True)
        jmuvv__jez = bodo.libs.distributed_api.isend(on_arr[-halo_size:],
            np.int32(halo_size), np.int32(rank + 1), whyn__hkq, True)
    if rank != 0:
        ...
        lfv__boutl = bodo.libs.distributed_api.irecv(l_recv_t_buff, np.
            int32(halo_size), np.int32(rank - 1), whyn__hkq, True)
    return (l_recv_buff, l_recv_t_buff, r_send_req, jmuvv__jez, l_recv_req,
        lfv__boutl)
...
def _is_small_for_parallel(N, halo_size):
    srnf__nkhza = bodo.libs.distributed_api.dist_reduce(int(N <= 2 *
        halo_size + 1), np.int32(Reduce_Type.Sum.value))
    return srnf__nkhza != 0
...
def _handle_small_data(in_arr, win, minp, center, rank, n_pes, init_data,
    add_obs, remove_obs, calc_out):
    N = len(in_arr)
    ztoe__fhy = bodo.libs.distributed_api.dist_reduce(len(in_arr), np.int32
        (Reduce_Type.Sum.value))
    imjy__bzb = bodo.libs.distributed_api.gatherv(in_arr)
    if rank == 0:
        kgb__eorw, ief__zgjgg = roll_fixed_linear_generic_seq(imjy__bzb,
            win, minp, center, init_data, add_obs, remove_obs, calc_out)
    else:
        kgb__eorw = np.empty(ztoe__fhy, np.float64)
    bodo.libs.distributed_api.bcast(kgb__eorw)
    start = bodo.libs.distributed_api.dist_exscan(N, np.int32(Reduce_Type.
        Sum.value))
    end = start + N
    return kgb__eorw[start:end]
...
def _handle_small_data_apply(in_arr, index_arr, win, minp, center, rank,
    ...
    ztoe__fhy = bodo.libs.distributed_api.dist_reduce(len(in_arr), np.int32
        (Reduce_Type.Sum.value))
    imjy__bzb = bodo.libs.distributed_api.gatherv(in_arr)
    zrj__zvae = bodo.libs.distributed_api.gatherv(index_arr)
    if rank == 0:
        kgb__eorw = roll_fixed_apply_seq(imjy__bzb, zrj__zvae, win, minp,
...
def _handle_small_data_shift(in_arr, shift, rank, n_pes):
    ...
    imjy__bzb = bodo.libs.distributed_api.gatherv(in_arr)
    if rank == 0:
        kgb__eorw = alloc_shift(len(imjy__bzb), imjy__bzb, (-1,))
        shift_seq(imjy__bzb, shift, kgb__eorw)
        wlq__ntrfc = bcast_n_chars_if_str_binary_arr(kgb__eorw)
    else:
        wlq__ntrfc = bcast_n_chars_if_str_binary_arr(in_arr)
        kgb__eorw = alloc_shift(ztoe__fhy, in_arr, (wlq__ntrfc,))
    bodo.libs.distributed_api.bcast(kgb__eorw)
    start = bodo.libs.distributed_api.dist_exscan(N, np.int32(Reduce_Type.
        Sum.value))
    end = start + N
...
def _handle_small_data_pct_change(in_arr, shift, rank, n_pes):
    N = len(in_arr)
    ztoe__fhy = bodo.libs.distributed_api.dist_reduce(N, np.int32(
        Reduce_Type.Sum.value))
    imjy__bzb = bodo.libs.distributed_api.gatherv(in_arr)
    if rank == 0:
        kgb__eorw = pct_change_seq(imjy__bzb, shift)
    else:
        kgb__eorw = alloc_pct_change(ztoe__fhy, in_arr)
    bodo.libs.distributed_api.bcast(kgb__eorw)
    start = bodo.libs.distributed_api.dist_exscan(N, np.int32(Reduce_Type.
        Sum.value))
    end = start + N
...
def _is_small_for_parallel_variable(on_arr, win_size):
    if len(on_arr) < 2:
        ...
        foaim__xke = end - start
        qmxn__kmqkk = int(foaim__xke <= win_size)
    srnf__nkhza = bodo.libs.distributed_api.dist_reduce(qmxn__kmqkk, np.
...
def _handle_small_data_variable(in_arr, on_arr, win, minp, rank, n_pes,
    ...
    ztoe__fhy = bodo.libs.distributed_api.dist_reduce(N, np.int32(
        Reduce_Type.Sum.value))
    imjy__bzb = bodo.libs.distributed_api.gatherv(in_arr)
    toa__tol = bodo.libs.distributed_api.gatherv(on_arr)
    if rank == 0:
        start, end = _build_indexer(toa__tol, ztoe__fhy, win, False, True)
        kgb__eorw = roll_var_linear_generic_seq(imjy__bzb, toa__tol, win,
...
def _handle_small_data_variable_apply(in_arr, on_arr, index_arr, win, minp,
    ...
    if rank == 0:
        start, end = _build_indexer(toa__tol, ztoe__fhy, win, False, True)
        kgb__eorw = roll_variable_apply_seq(imjy__bzb, toa__tol, zrj__zvae,
...
def _dropna(arr):
    rhxhy__hrxw = len(arr)
    gffs__wdf = rhxhy__hrxw - np.isnan(arr).sum()
    A = np.empty(gffs__wdf, arr.dtype)
    mswf__sgxk = 0
    for iyupa__sjjn in range(rhxhy__hrxw):
        val = arr[iyupa__sjjn]
        if not np.isnan(val):
            A[mswf__sgxk] = val
            mswf__sgxk += 1
