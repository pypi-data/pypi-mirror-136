...
def generate_col_to_index_func_text(col_names: Tuple):
    if all(isinstance(a, str) for a in col_names) or all(isinstance(a,
        ...
        cfiwj__gtbsm = f'bodo.utils.conversion.coerce_to_array({col_names})'
        return (
            f'bodo.hiframes.pd_index_ext.init_binary_str_index({cfiwj__gtbsm})\n'
            )
...
def overload_dataframe_columns(df):
    ...
    usmu__uuoab = 'def impl(df):\n'
    pttxq__tckb = bodo.hiframes.dataframe_impl.generate_col_to_index_func_text(
        df.columns)
    usmu__uuoab += f'  return {pttxq__tckb}'
    dckt__uglrm = {}
    exec(usmu__uuoab, {'bodo': bodo}, dckt__uglrm)
    impl = dckt__uglrm['impl']
    return impl
...
def overload_dataframe_values(df):
    ...
    fsca__mhuhj = len(df.columns)
    ggfq__lppf = set(i for i in range(fsca__mhuhj) if isinstance(df.data[i],
        IntegerArrayType))
    data_args = ', '.join(
        'bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df, {}){}'.
        format(i, '.astype(float)' if i in ggfq__lppf else '') for i in
        range(fsca__mhuhj))
    usmu__uuoab = 'def f(df):\n'.format()
    usmu__uuoab += '    return np.stack(({},), 1)\n'.format(data_args)
    dckt__uglrm = {}
    exec(usmu__uuoab, {'bodo': bodo, 'np': np}, dckt__uglrm)
    now__aypr = dckt__uglrm['f']
    return now__aypr
...
def overload_dataframe_to_numpy(df, dtype=None, copy=False, na_value=_no_input
    ...
    sbvh__jplnm = {'dtype': dtype, 'na_value': na_value}
    uspn__wiig = {'dtype': None, 'na_value': _no_input}
    check_unsupported_args('DataFrame.to_numpy', sbvh__jplnm, uspn__wiig,
...
def overload_dataframe_size(df):
    if df.has_runtime_cols:
        ...
        def impl(df):
            ...
            lrn__ybqrs = bodo.hiframes.table.compute_num_runtime_columns(t)
            return lrn__ybqrs * len(t)
        return impl
...
def overload_dataframe_dtypes(df):
    ...
    pjx__qlzzo = ',' if len(df.columns) == 1 else ''
    index = f'bodo.hiframes.pd_index_ext.init_heter_index({df.columns})'
    usmu__uuoab += f"""  return bodo.hiframes.pd_series_ext.init_series(({data}{pjx__qlzzo}), {index}, None)
"""
    dckt__uglrm = {}
    exec(usmu__uuoab, {'bodo': bodo}, dckt__uglrm)
    impl = dckt__uglrm['impl']
...
def overload_dataframe_astype(df, dtype, copy=True, errors='raise',
    ...
    sbvh__jplnm = {'copy': copy, 'errors': errors}
    uspn__wiig = {'copy': True, 'errors': 'raise'}
    check_unsupported_args('df.astype', sbvh__jplnm, uspn__wiig,
        package_name='pandas', module_name='DataFrame')
    ...
    if is_overload_constant_dict(dtype) or is_overload_constant_series(dtype):
        rethw__oasdh = get_overload_constant_dict(dtype
            ) if is_overload_constant_dict(dtype) else dict(
            get_overload_constant_series(dtype))
        data_args = ', '.join(
            f'bodo.utils.conversion.fix_arr_dtype(bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df, {i}), {_get_dtype_str(rethw__oasdh[ypgl__tetq])}, copy, nan_to_str=_bodo_nan_to_str, from_series=True)'
             if ypgl__tetq in rethw__oasdh else
            f'bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df, {i})' for
            i, ypgl__tetq in enumerate(df.columns))
    else:
        data_args = ', '.join(
...
def overload_dataframe_copy(df, deep=True):
    ...
    for i in range(len(df.columns)):
        ...
        if is_overload_true(deep):
            iph__uigj.append(arr + '.copy()')
        elif is_overload_false(deep):
            iph__uigj.append(arr)
        else:
            iph__uigj.append(f'{arr}.copy() if deep else {arr}')
    header = 'def impl(df, deep=True):\n'
    return _gen_init_df(header, df.columns, ', '.join(iph__uigj))
...
def overload_dataframe_rename(df, mapper=None, index=None, columns=None,
    ...
    sbvh__jplnm = {'index': index, 'level': level, 'errors': errors}
    uspn__wiig = {'index': None, 'level': None, 'errors': 'ignore'}
    check_unsupported_args('DataFrame.rename', sbvh__jplnm, uspn__wiig,
        package_name='pandas', module_name='DataFrame')
    ...
    if not is_overload_none(mapper):
        ...
        wynb__lpe = get_overload_constant_dict(mapper)
        ...
        wynb__lpe = get_overload_constant_dict(columns)
    else:
        raise_bodo_error(
            "DataFrame.rename(): must pass columns either via 'mapper' and 'axis'=1 or 'columns'"
            )
    cmd__lflpe = [wynb__lpe.get(df.columns[i], df.columns[i]) for i in
    ...
    for i in range(len(df.columns)):
        ...
        if is_overload_true(copy):
            ...
            iph__uigj.append(f'{arr}.copy() if copy else {arr}')
...
"""
    return _gen_init_df(header, cmd__lflpe, ', '.join(iph__uigj))
...
def overload_dataframe_filter(df, items=None, like=None, regex=None, axis=None
    ...
    irjig__zuvfp = not is_overload_none(items)
    ielr__emjy = not is_overload_none(like)
    lnuu__dqs = not is_overload_none(regex)
    jsb__banv = irjig__zuvfp ^ ielr__emjy ^ lnuu__dqs
    hzciz__itam = not (irjig__zuvfp or ielr__emjy or lnuu__dqs)
    if hzciz__itam:
        raise BodoError(
    ...
    if is_overload_constant_str(axis):
        ...
        ekjz__zthm = 0 if axis == 'index' else 1
    ...
    if ekjz__zthm == 1:
        ...
        if irjig__zuvfp:
            if is_overload_constant_list(items):
                cwkaq__lmr = get_overload_const_list(items)
            else:
                raise BodoError(
        ...
        if ielr__emjy:
            if is_overload_constant_str(like):
                wcx__wqsja = get_overload_const_str(like)
            else:
                raise BodoError(
        ...
        if lnuu__dqs:
            if is_overload_constant_str(regex):
                dzrm__xseb = get_overload_const_str(regex)
                sgez__olxhf = re.compile(dzrm__xseb)
            else:
                raise BodoError(
                    "Dataframe.filter(): argument 'regex' must be a constant string."
                    )
        for i, ypgl__tetq in enumerate(df.columns):
            if not is_overload_none(items
                ) and ypgl__tetq in cwkaq__lmr or not is_overload_none(like
                ) and wcx__wqsja in str(ypgl__tetq) or not is_overload_none(
                regex) and sgez__olxhf.search(str(ypgl__tetq)):
                argnr__plj.append(ypgl__tetq)
                swh__ytho.append(i)
        for i in swh__ytho:
            dqce__ljr = f'data_{i}'
            shmwb__wojr.append(dqce__ljr)
            usmu__uuoab += f"""  {dqce__ljr} = bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df, {i})
"""
        data_args = ', '.join(shmwb__wojr)
        return _gen_init_df(usmu__uuoab, argnr__plj, data_args)
...
def overload_dataframe_select_dtypes(df, include=None, exclude=None):
    ...
    amgtt__zutqa = is_overload_none(include)
    kuv__pfl = is_overload_none(exclude)
    ydjmg__dcpsj = 'DataFrame.select_dtypes'
    if amgtt__zutqa and kuv__pfl:
        raise_bodo_error(
    ...
    if not amgtt__zutqa:
        if is_overload_constant_list(include):
            ...
            unr__gqm = [dtype_to_array_type(parse_dtype(elem, ydjmg__dcpsj)
                ) for elem in include]
        elif is_legal_input(include):
            unr__gqm = [dtype_to_array_type(parse_dtype(include, ydjmg__dcpsj))
                ]
        else:
            raise_bodo_error(
                'DataFrame.select_dtypes() only supports constant strings or types as arguments'
                )
        unr__gqm = get_nullable_and_non_nullable_types(unr__gqm)
        egck__nwph = tuple(ypgl__tetq for i, ypgl__tetq in enumerate(df.
            columns) if df.data[i] in unr__gqm)
    else:
        egck__nwph = df.columns
    if not kuv__pfl:
        if is_overload_constant_list(exclude):
            exclude = get_overload_const_list(exclude)
            qvb__dkpiu = [dtype_to_array_type(parse_dtype(elem,
                ydjmg__dcpsj)) for elem in exclude]
        elif is_legal_input(exclude):
            qvb__dkpiu = [dtype_to_array_type(parse_dtype(exclude,
                ydjmg__dcpsj))]
        else:
            raise_bodo_error(
                'DataFrame.select_dtypes() only supports constant strings or types as arguments'
                )
        qvb__dkpiu = get_nullable_and_non_nullable_types(qvb__dkpiu)
        egck__nwph = tuple(ypgl__tetq for ypgl__tetq in egck__nwph if df.
            data[df.columns.index(ypgl__tetq)] not in qvb__dkpiu)
    data_args = ', '.join(
        f'bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df, {df.columns.index(ypgl__tetq)})'
         for ypgl__tetq in egck__nwph)
    header = 'def impl(df, include=None, exclude=None):\n'
    return _gen_init_df(header, egck__nwph, data_args)
...
def overload_dataframe_first(df, offset):
    ...
    mjdal__lkul = (types.unicode_type, bodo.month_begin_type, bodo.
        month_end_type, bodo.week_type, bodo.date_offset_type)
    if types.unliteral(offset) not in mjdal__lkul:
        raise BodoError(
...
def overload_dataframe_isin(df, values):
    ...
    usmu__uuoab = 'def impl(df, values):\n'
    xyrsf__zrwg = {}
    ymqoc__pasm = False
    if isinstance(values, DataFrameType):
        ymqoc__pasm = True
        for i, ypgl__tetq in enumerate(df.columns):
            if ypgl__tetq in values.columns:
                mvl__vejc = 'val{}'.format(i)
                usmu__uuoab += (
                    """  {} = bodo.hiframes.pd_dataframe_ext.get_dataframe_data(values, {})
"""
                    .format(mvl__vejc, values.columns.index(ypgl__tetq)))
                xyrsf__zrwg[ypgl__tetq] = mvl__vejc
    elif is_iterable_type(values) and not isinstance(values, SeriesType):
        xyrsf__zrwg = {ypgl__tetq: 'values' for ypgl__tetq in df.columns}
    else:
        raise_bodo_error(f'pd.isin(): not supported for type {values}')
    data = []
    for i in range(len(df.columns)):
        mvl__vejc = 'data{}'.format(i)
        usmu__uuoab += (
            '  {} = bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df, {})\n'
            .format(mvl__vejc, i))
        data.append(mvl__vejc)
    bdbe__jaca = ['out{}'.format(i) for i in range(len(df.columns))]
    rsttj__xhu = """
...
"""
  ...
  for i in numba.parfors.parfor.internal_prange(n):
    {2}[i] = {0}[i] in {1}
"""
    wzooq__ixuod = '  {} = np.zeros(len(df), np.bool_)\n'
    for i, (cname, yfer__ooe) in enumerate(zip(df.columns, data)):
        if cname in xyrsf__zrwg:
            scj__hdsoh = xyrsf__zrwg[cname]
            if ymqoc__pasm:
                usmu__uuoab += rsttj__xhu.format(yfer__ooe, scj__hdsoh,
                    bdbe__jaca[i])
            else:
                usmu__uuoab += zvuw__nbsau.format(yfer__ooe, scj__hdsoh,
                    bdbe__jaca[i])
        else:
            usmu__uuoab += wzooq__ixuod.format(bdbe__jaca[i])
    return _gen_init_df(usmu__uuoab, df.columns, ','.join(bdbe__jaca))
...
def overload_dataframe_abs(df):
    ...
    data_args = ', '.join(
        ...
        .format(i) for i in range(fsca__mhuhj))
    header = 'def impl(df):\n'
    return _gen_init_df(header, df.columns, data_args)


def overload_dataframe_corr(df, method='pearson', min_periods=1):
    gmbk__gnpk = [ypgl__tetq for ypgl__tetq, qdpr__jvsok in zip(df.columns,
        df.data) if bodo.utils.typing._is_pandas_numeric_dtype(qdpr__jvsok.
        dtype)]
    assert len(gmbk__gnpk) != 0
    shph__ngcq = ''
    if not any(qdpr__jvsok == types.float64 for qdpr__jvsok in df.data):
        shph__ngcq = '.astype(np.float64)'
    nlvn__iuxa = ', '.join(
        'bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df, {}){}'.
        format(df.columns.index(ypgl__tetq), '.astype(np.float64)' if 
        isinstance(df.data[df.columns.index(ypgl__tetq)], IntegerArrayType) or
        df.data[df.columns.index(ypgl__tetq)] == boolean_array else '') for
        ypgl__tetq in gmbk__gnpk)
    knqlq__maxf = 'np.stack(({},), 1){}'.format(nlvn__iuxa, shph__ngcq)
    data_args = ', '.join('res[:,{}]'.format(i) for i in range(len(gmbk__gnpk))
        )
    index = f'{generate_col_to_index_func_text(gmbk__gnpk)}\n'
    header = "def impl(df, method='pearson', min_periods=1):\n"
    header += '  mat = {}\n'.format(knqlq__maxf)
    header += '  res = bodo.libs.array_kernels.nancorr(mat, 0, min_periods)\n'
    return _gen_init_df(header, gmbk__gnpk, data_args, index)
...
def overload_dataframe_cov(df, min_periods=None, ddof=1):
    ...
    zsyg__rnm = dict(ddof=ddof)
    rmiy__ebd = dict(ddof=1)
    check_unsupported_args('DataFrame.cov', zsyg__rnm, rmiy__ebd,
        package_name='pandas', module_name='DataFrame')
    ggd__bhxz = '1' if is_overload_none(min_periods) else 'min_periods'
    gmbk__gnpk = [ypgl__tetq for ypgl__tetq, qdpr__jvsok in zip(df.columns,
        df.data) if bodo.utils.typing._is_pandas_numeric_dtype(qdpr__jvsok.
        dtype)]
    if len(gmbk__gnpk) == 0:
...
def overload_dataframe_count(df, axis=0, level=None, numeric_only=False):
    ...
    zsyg__rnm = dict(axis=axis, level=level, numeric_only=numeric_only)
    rmiy__ebd = dict(axis=0, level=None, numeric_only=False)
    check_unsupported_args('DataFrame.count', zsyg__rnm, rmiy__ebd,
        package_name='pandas', module_name='DataFrame')
    data_args = ', '.join(
        f'bodo.libs.array_ops.array_op_count(bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df, {i}))'
         for i in range(len(df.columns)))
    usmu__uuoab = 'def impl(df, axis=0, level=None, numeric_only=False):\n'
    usmu__uuoab += '  data = np.array([{}])\n'.format(data_args)
    pttxq__tckb = bodo.hiframes.dataframe_impl.generate_col_to_index_func_text(
        df.columns)
    usmu__uuoab += (
        f'  return bodo.hiframes.pd_series_ext.init_series(data, {pttxq__tckb})\n'
        )
    dckt__uglrm = {}
    exec(usmu__uuoab, {'bodo': bodo, 'np': np}, dckt__uglrm)
    impl = dckt__uglrm['impl']
...
def overload_dataframe_nunique(df, axis=0, dropna=True):
    ...
    zsyg__rnm = dict(axis=axis)
    rmiy__ebd = dict(axis=0)
    if not is_overload_bool(dropna):
        raise BodoError('DataFrame.nunique: dropna must be a boolean value')
    check_unsupported_args('DataFrame.nunique', zsyg__rnm, rmiy__ebd,
        package_name='pandas', module_name='DataFrame')
    data_args = ', '.join(
        f'bodo.libs.array_kernels.nunique(bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df, {i}), dropna)'
         for i in range(len(df.columns)))
    usmu__uuoab = 'def impl(df, axis=0, dropna=True):\n'
    usmu__uuoab += '  data = np.asarray(({},))\n'.format(data_args)
    pttxq__tckb = bodo.hiframes.dataframe_impl.generate_col_to_index_func_text(
        df.columns)
    usmu__uuoab += (
        f'  return bodo.hiframes.pd_series_ext.init_series(data, {pttxq__tckb})\n'
...
def overload_dataframe_prod(df, axis=None, skipna=None, level=None,
    ...
    zsyg__rnm = dict(skipna=skipna, level=level, numeric_only=numeric_only,
        min_count=min_count)
    rmiy__ebd = dict(skipna=None, level=None, numeric_only=None, min_count=0)
    check_unsupported_args('DataFrame.prod', zsyg__rnm, rmiy__ebd,
        package_name='pandas', module_name='DataFrame')
...
def overload_dataframe_sum(df, axis=None, skipna=None, level=None,
    ...
    zsyg__rnm = dict(skipna=skipna, level=level, numeric_only=numeric_only,
        min_count=min_count)
    rmiy__ebd = dict(skipna=None, level=None, numeric_only=None, min_count=0)
    check_unsupported_args('DataFrame.sum', zsyg__rnm, rmiy__ebd,
        package_name='pandas', module_name='DataFrame')
...
def overload_dataframe_max(df, axis=None, skipna=None, level=None,
    ...
    zsyg__rnm = dict(skipna=skipna, level=level, numeric_only=numeric_only)
    rmiy__ebd = dict(skipna=None, level=None, numeric_only=None)
    check_unsupported_args('DataFrame.max', zsyg__rnm, rmiy__ebd,
        package_name='pandas', module_name='DataFrame')
...
def overload_dataframe_min(df, axis=None, skipna=None, level=None,
    ...
    zsyg__rnm = dict(skipna=skipna, level=level, numeric_only=numeric_only)
    rmiy__ebd = dict(skipna=None, level=None, numeric_only=None)
    check_unsupported_args('DataFrame.min', zsyg__rnm, rmiy__ebd,
        package_name='pandas', module_name='DataFrame')
...
def overload_dataframe_mean(df, axis=None, skipna=None, level=None,
    ...
    zsyg__rnm = dict(skipna=skipna, level=level, numeric_only=numeric_only)
    rmiy__ebd = dict(skipna=None, level=None, numeric_only=None)
    check_unsupported_args('DataFrame.mean', zsyg__rnm, rmiy__ebd,
        package_name='pandas', module_name='DataFrame')
...
def overload_dataframe_var(df, axis=None, skipna=None, level=None, ddof=1,
    ...
    zsyg__rnm = dict(skipna=skipna, level=level, ddof=ddof, numeric_only=
        numeric_only)
    rmiy__ebd = dict(skipna=None, level=None, ddof=1, numeric_only=None)
    check_unsupported_args('DataFrame.var', zsyg__rnm, rmiy__ebd,
        package_name='pandas', module_name='DataFrame')
...
def overload_dataframe_std(df, axis=None, skipna=None, level=None, ddof=1,
    ...
    zsyg__rnm = dict(skipna=skipna, level=level, ddof=ddof, numeric_only=
        numeric_only)
    rmiy__ebd = dict(skipna=None, level=None, ddof=1, numeric_only=None)
    check_unsupported_args('DataFrame.std', zsyg__rnm, rmiy__ebd,
        package_name='pandas', module_name='DataFrame')
...
def overload_dataframe_median(df, axis=None, skipna=None, level=None,
    ...
    zsyg__rnm = dict(skipna=skipna, level=level, numeric_only=numeric_only)
    rmiy__ebd = dict(skipna=None, level=None, numeric_only=None)
    check_unsupported_args('DataFrame.median', zsyg__rnm, rmiy__ebd,
        package_name='pandas', module_name='DataFrame')
...
def overload_dataframe_quantile(df, q=0.5, axis=0, numeric_only=True,
    ...
    zsyg__rnm = dict(numeric_only=numeric_only, interpolation=interpolation)
    rmiy__ebd = dict(numeric_only=True, interpolation='linear')
    check_unsupported_args('DataFrame.quantile', zsyg__rnm, rmiy__ebd,
        package_name='pandas', module_name='DataFrame')
...
def overload_dataframe_idxmax(df, axis=0, skipna=True):
    ...
    zsyg__rnm = dict(axis=axis, skipna=skipna)
    rmiy__ebd = dict(axis=0, skipna=True)
    check_unsupported_args('DataFrame.idxmax', zsyg__rnm, rmiy__ebd,
        package_name='pandas', module_name='DataFrame')
    for mhwm__thma in df.data:
        if not (bodo.utils.utils.is_np_array_typ(mhwm__thma) and (
            mhwm__thma.dtype in [bodo.datetime64ns, bodo.timedelta64ns] or
            isinstance(mhwm__thma.dtype, (types.Number, types.Boolean))) or
            isinstance(mhwm__thma, (bodo.IntegerArrayType, bodo.
            CategoricalArrayType)) or mhwm__thma in [bodo.boolean_array,
            bodo.datetime_date_array_type]):
            raise BodoError(
                f'DataFrame.idxmax() only supported for numeric column types. Column type: {mhwm__thma} not supported.'
                )
        if isinstance(mhwm__thma, bodo.CategoricalArrayType
            ) and not mhwm__thma.dtype.ordered:
            raise BodoError(
...
def overload_dataframe_idxmin(df, axis=0, skipna=True):
    ...
    zsyg__rnm = dict(axis=axis, skipna=skipna)
    rmiy__ebd = dict(axis=0, skipna=True)
    check_unsupported_args('DataFrame.idxmin', zsyg__rnm, rmiy__ebd,
        package_name='pandas', module_name='DataFrame')
    for mhwm__thma in df.data:
        if not (bodo.utils.utils.is_np_array_typ(mhwm__thma) and (
            mhwm__thma.dtype in [bodo.datetime64ns, bodo.timedelta64ns] or
            ...
            bodo.datetime_date_array_type]):
            raise BodoError(
                f'DataFrame.idxmin() only supported for numeric column types. Column type: {mhwm__thma} not supported.'
                )
        if isinstance(mhwm__thma, bodo.CategoricalArrayType
            ) and not mhwm__thma.dtype.ordered:
            raise BodoError(
...
def _gen_reduce_impl(df, func_name, args=None, axis=None):
    ...
    if func_name in ('idxmax', 'idxmin'):
        ...
        gmbk__gnpk = tuple(ypgl__tetq for ypgl__tetq, qdpr__jvsok in zip(df
            .columns, df.data) if bodo.utils.typing.
            _is_pandas_numeric_dtype(qdpr__jvsok.dtype))
        out_colnames = gmbk__gnpk
    assert len(out_colnames) != 0
    try:
        if func_name in ('idxmax', 'idxmin') and axis == 0:
            comm_dtype = None
        else:
            ealbt__hcjde = [numba.np.numpy_support.as_dtype(df.data[df.
                columns.index(ypgl__tetq)].dtype) for ypgl__tetq in
                out_colnames]
            comm_dtype = numba.np.numpy_support.from_dtype(np.
                find_common_type(ealbt__hcjde, []))
    except NotImplementedError as uvtiq__gjosd:
        raise BodoError(
            f'Dataframe.{func_name}() with column types: {df.data} could not be merged to a common type.'
            )
    wusnj__pwfvu = ''
    if func_name in ('sum', 'prod'):
        wusnj__pwfvu = ', min_count=0'
    ddof = ''
    if func_name in ('var', 'std'):
        ddof = 'ddof=1, '
    usmu__uuoab = (
        'def impl(df, axis=None, skipna=None, level=None,{} numeric_only=None{}):\n'
        .format(ddof, wusnj__pwfvu))
    if func_name == 'quantile':
        usmu__uuoab = (
            "def impl(df, q=0.5, axis=0, numeric_only=True, interpolation='linear'):\n"
            )
    if func_name in ('idxmax', 'idxmin'):
        usmu__uuoab = 'def impl(df, axis=0, skipna=True):\n'
    if axis == 0:
        usmu__uuoab += _gen_reduce_impl_axis0(df, func_name, out_colnames,
            comm_dtype, args)
    else:
        usmu__uuoab += _gen_reduce_impl_axis1(func_name, out_colnames,
            comm_dtype, df)
    dckt__uglrm = {}
    exec(usmu__uuoab, {'bodo': bodo, 'np': np, 'pd': pd, 'numba': numba},
        dckt__uglrm)
    impl = dckt__uglrm['impl']
    return impl
...
def _gen_reduce_impl_axis0(df, func_name, out_colnames, comm_dtype, args):
    ...
    if func_name in ('min', 'max'):
        qcde__zrq = ', dtype=np.{}'.format(comm_dtype)
    if comm_dtype == types.float32 and func_name in ('sum', 'prod', 'mean',
        'var', 'std', 'median'):
        qcde__zrq = ', dtype=np.float32'
    vdb__uvy = f'bodo.libs.array_ops.array_op_{func_name}'
    khep__ngwk = ''
    if func_name in ['sum', 'prod']:
        khep__ngwk = 'True, min_count'
    elif func_name in ['idxmax', 'idxmin']:
        khep__ngwk = 'index'
    elif func_name == 'quantile':
        khep__ngwk = 'q'
    elif func_name in ['std', 'var']:
        khep__ngwk = 'True, ddof'
    elif func_name == 'median':
        khep__ngwk = 'True'
    data_args = ', '.join(
        f'{vdb__uvy}(bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df, {df.columns.index(ypgl__tetq)}), {khep__ngwk})'
         for ypgl__tetq in out_colnames)
    usmu__uuoab = ''
    if func_name in ('idxmax', 'idxmin'):
        usmu__uuoab += (
        ...
        usmu__uuoab += '  data = np.asarray(({},){})\n'.format(data_args,
            qcde__zrq)
    usmu__uuoab += f"""  return bodo.hiframes.pd_series_ext.init_series(data, pd.Index({out_colnames}))
"""
    return usmu__uuoab


def _gen_reduce_impl_axis1(func_name, out_colnames, comm_dtype, df_type):
    wfaik__ppuof = [df_type.columns.index(ypgl__tetq) for ypgl__tetq in
        out_colnames]
    index = 'bodo.hiframes.pd_dataframe_ext.get_dataframe_index(df)'
    data_args = '\n    '.join(
        'arr_{0} = bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df, {0})'
        .format(i) for i in wfaik__ppuof)
    rji__eto = '\n        '.join(f'row[{i}] = arr_{wfaik__ppuof[i]}[i]' for
        i in range(len(out_colnames)))
    assert len(data_args) > 0, f'empty dataframe in DataFrame.{func_name}()'
    mreeo__ossz = f'len(arr_{wfaik__ppuof[0]})'
    pmakc__uej = {'max': 'np.nanmax', 'min': 'np.nanmin', 'sum':
    ...
    if func_name in pmakc__uej:
        kixnp__qajo = pmakc__uej[func_name]
        kssf__hyqby = 'float64' if func_name in ['mean', 'median', 'std', 'var'
            ] else comm_dtype
        usmu__uuoab = f"""
    {data_args}
    numba.parfors.parfor.init_prange()
...
def overload_dataframe_pct_change(df, periods=1, fill_method='pad', limit=
    ...
    zsyg__rnm = dict(fill_method=fill_method, limit=limit, freq=freq)
    rmiy__ebd = dict(fill_method='pad', limit=None, freq=None)
    check_unsupported_args('DataFrame.pct_change', zsyg__rnm, rmiy__ebd,
        package_name='pandas', module_name='DataFrame')
    data_args = ', '.join(
...
def overload_dataframe_cumprod(df, axis=None, skipna=True):
    ...
    zsyg__rnm = dict(axis=axis, skipna=skipna)
    rmiy__ebd = dict(axis=None, skipna=True)
    check_unsupported_args('DataFrame.cumprod', zsyg__rnm, rmiy__ebd,
        package_name='pandas', module_name='DataFrame')
    data_args = ', '.join(
...
def overload_dataframe_cumsum(df, axis=None, skipna=True):
    ...
    zsyg__rnm = dict(skipna=skipna)
    rmiy__ebd = dict(skipna=True)
    check_unsupported_args('DataFrame.cumsum', zsyg__rnm, rmiy__ebd,
        package_name='pandas', module_name='DataFrame')
    data_args = ', '.join(
...
def overload_dataframe_describe(df, percentiles=None, include=None, exclude
    ...
    zsyg__rnm = dict(percentiles=percentiles, include=include, exclude=
        exclude, datetime_is_numeric=datetime_is_numeric)
    rmiy__ebd = dict(percentiles=None, include=None, exclude=None,
        datetime_is_numeric=True)
    check_unsupported_args('DataFrame.describe', zsyg__rnm, rmiy__ebd,
        package_name='pandas', module_name='DataFrame')
    gmbk__gnpk = [ypgl__tetq for ypgl__tetq, qdpr__jvsok in zip(df.columns,
        df.data) if _is_describe_type(qdpr__jvsok)]
    if len(gmbk__gnpk) == 0:
        raise BodoError('df.describe() only supports numeric columns')
    gjade__oydg = sum(df.data[df.columns.index(ypgl__tetq)].dtype == bodo.
        datetime64ns for ypgl__tetq in gmbk__gnpk)

    def _get_describe(col_ind):
        wnf__ttxoa = df.data[col_ind].dtype == bodo.datetime64ns
        if gjade__oydg and gjade__oydg != len(gmbk__gnpk):
            if wnf__ttxoa:
...
"""
    for ypgl__tetq in gmbk__gnpk:
        col_ind = df.columns.index(ypgl__tetq)
        header += f"""  des_{col_ind} = bodo.libs.array_ops.array_op_describe(bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df, {col_ind}))
"""
    data_args = ', '.join(_get_describe(df.columns.index(ypgl__tetq)) for
        ypgl__tetq in gmbk__gnpk)
    iiu__fremk = "['count', 'mean', 'std', 'min', '25%', '50%', '75%', 'max']"
    if gjade__oydg == len(gmbk__gnpk):
        iiu__fremk = "['count', 'mean', 'min', '25%', '50%', '75%', 'max']"
    elif gjade__oydg:
        iiu__fremk = (
            "['count', 'mean', 'min', '25%', '50%', '75%', 'max', 'std']")
    index = f'bodo.utils.conversion.convert_to_index({iiu__fremk})'
    return _gen_init_df(header, gmbk__gnpk, data_args, index)
...
def overload_dataframe_take(df, indices, axis=0, convert=None, is_copy=True):
    ...
    zsyg__rnm = dict(axis=axis, convert=convert, is_copy=is_copy)
    rmiy__ebd = dict(axis=0, convert=None, is_copy=True)
    check_unsupported_args('DataFrame.take', zsyg__rnm, rmiy__ebd,
        package_name='pandas', module_name='DataFrame')
    data_args = ', '.join(
...
def overload_dataframe_shift(df, periods=1, freq=None, axis=0, fill_value=None
    ...
    zsyg__rnm = dict(freq=freq, axis=axis, fill_value=fill_value)
    rmiy__ebd = dict(freq=None, axis=0, fill_value=None)
    check_unsupported_args('DataFrame.shift', zsyg__rnm, rmiy__ebd,
        package_name='pandas', module_name='DataFrame')
    for fslbg__goeqc in df.data:
        if not is_supported_shift_array_type(fslbg__goeqc):
            raise BodoError(
                f'Dataframe.shift() column input type {fslbg__goeqc.dtype} not supported yet.'
                )
...
def overload_dataframe_diff(df, periods=1, axis=0):
    ...
    zsyg__rnm = dict(axis=axis)
    rmiy__ebd = dict(axis=0)
    check_unsupported_args('DataFrame.diff', zsyg__rnm, rmiy__ebd,
        package_name='pandas', module_name='DataFrame')
    for fslbg__goeqc in df.data:
        if not (isinstance(fslbg__goeqc, types.Array) and (isinstance(
            fslbg__goeqc.dtype, types.Number) or fslbg__goeqc.dtype == bodo
            .datetime64ns)):
            raise BodoError(
                f'DataFrame.diff() column input type {fslbg__goeqc.dtype} not supported.'
                )
    if not is_overload_int(periods):
...
def overload_dataframe_set_index(df, keys, drop=True, append=False, inplace
    ...
    sbvh__jplnm = {'inplace': inplace, 'append': append, 'verify_integrity':
        verify_integrity}
    uspn__wiig = {'inplace': False, 'append': False, 'verify_integrity': False}
    check_unsupported_args('DataFrame.set_index', sbvh__jplnm, uspn__wiig,
        package_name='pandas', module_name='DataFrame')
...
"""
    columns = tuple(ypgl__tetq for ypgl__tetq in df.columns if ypgl__tetq !=
        col_name)
    index = (
...
def overload_dataframe_query(df, expr, inplace=False):
    ...
    sbvh__jplnm = {'inplace': inplace}
    uspn__wiig = {'inplace': False}
    check_unsupported_args('query', sbvh__jplnm, uspn__wiig, package_name=
    ...
    def impl(df, expr, inplace=False):
        kwv__lncp = bodo.hiframes.pd_dataframe_ext.query_dummy(df, expr)
        return df[kwv__lncp]
    return impl
...
def overload_dataframe_duplicated(df, subset=None, keep='first'):
    ...
    sbvh__jplnm = {'subset': subset, 'keep': keep}
    uspn__wiig = {'subset': None, 'keep': 'first'}
    check_unsupported_args('DataFrame.duplicated', sbvh__jplnm, uspn__wiig,
        package_name='pandas', module_name='DataFrame')
    fsca__mhuhj = len(df.columns)
    usmu__uuoab = "def impl(df, subset=None, keep='first'):\n"
    for i in range(fsca__mhuhj):
        usmu__uuoab += (
    ...
    usmu__uuoab += (
        ...
        .format(', '.join('data_{}'.format(i) for i in range(fsca__mhuhj)),
        index))
    usmu__uuoab += (
...
def overload_dataframe_drop_duplicates(df, subset=None, keep='first',
    ...
    sbvh__jplnm = {'keep': keep, 'inplace': inplace, 'ignore_index':
        ignore_index}
    uspn__wiig = {'keep': 'first', 'inplace': False, 'ignore_index': False}
    xlr__tse = []
    if is_overload_constant_list(subset):
        xlr__tse = get_overload_const_list(subset)
    elif is_overload_constant_str(subset):
        xlr__tse = [get_overload_const_str(subset)]
    elif is_overload_constant_int(subset):
        xlr__tse = [get_overload_const_int(subset)]
    ...
    for col_name in xlr__tse:
        ...
        dzz__jny.append(df.columns.index(col_name))
    check_unsupported_args('DataFrame.drop_duplicates', sbvh__jplnm,
        uspn__wiig, package_name='pandas', module_name='DataFrame')
    mzzyj__qnf = []
    if dzz__jny:
        for xmfg__crizq in dzz__jny:
            if isinstance(df.data[xmfg__crizq], bodo.MapArrayType):
                mzzyj__qnf.append(df.columns[xmfg__crizq])
    else:
        for i, col_name in enumerate(df.columns):
            if isinstance(df.data[i], bodo.MapArrayType):
                mzzyj__qnf.append(col_name)
    if mzzyj__qnf:
        raise BodoError(
            f'DataFrame.drop_duplicates(): Columns {mzzyj__qnf} ' +
    ...
    fsca__mhuhj = len(df.columns)
    hwx__csi = ['data_{}'.format(i) for i in dzz__jny]
    ybwrr__xecbv = ['data_{}'.format(i) for i in range(fsca__mhuhj) if i not in
        dzz__jny]
    if hwx__csi:
        auy__pzkve = len(hwx__csi)
    else:
        auy__pzkve = fsca__mhuhj
    fan__zxc = ', '.join(hwx__csi + ybwrr__xecbv)
    data_args = ', '.join('data_{}'.format(i) for i in range(fsca__mhuhj))
    usmu__uuoab = (
...
"""
        .format(fan__zxc, index, auy__pzkve))
    usmu__uuoab += (
        '  index = bodo.utils.conversion.index_from_array(index_arr)\n')
    return _gen_init_df(usmu__uuoab, df.columns, data_args, 'index')
...
def _gen_init_df(header, columns, data_args, index=None, extra_globals=None,
    ...
    if out_df_type is not None:
        ...
        tkqs__sqz = 'out_df_type'
    else:
        tkqs__sqz = gen_const_tup(columns)
    data_args = '({}{})'.format(data_args, ',' if data_args else '')
    usmu__uuoab = f"""{header}  return bodo.hiframes.pd_dataframe_ext.init_dataframe({data_args}, {index}, {tkqs__sqz})
"""
    dckt__uglrm = {}
    erui__lxvy = {'bodo': bodo, 'np': np, 'pd': pd, 'numba': numba}
    erui__lxvy.update(extra_globals)
    exec(usmu__uuoab, erui__lxvy, dckt__uglrm)
    impl = dckt__uglrm['impl']
    return impl
...
def _get_binop_columns(lhs, rhs, is_inplace=False):
    if lhs.columns != rhs.columns:
        mdwjv__ldbpn = pd.Index(lhs.columns)
        rnh__iith = pd.Index(rhs.columns)
        inc__ilpoo, hvu__dno, anb__eod = mdwjv__ldbpn.join(rnh__iith, how=
            'left' if is_inplace else 'outer', level=None, return_indexers=True
            )
        return tuple(inc__ilpoo), hvu__dno, anb__eod
...
def create_binary_op_overload(op):
    ...
    def overload_dataframe_binary_op(lhs, rhs):
        sjfi__diglk = numba.core.utils.OPERATORS_TO_BUILTINS[op]
        nnvrl__hcn = operator.eq, operator.ne
        check_runtime_cols_unsupported(lhs, sjfi__diglk)
        check_runtime_cols_unsupported(rhs, sjfi__diglk)
        if isinstance(lhs, DataFrameType):
            if isinstance(rhs, DataFrameType):
                inc__ilpoo, hvu__dno, anb__eod = _get_binop_columns(lhs, rhs)
                data_args = ', '.join(
                    f'bodo.hiframes.pd_dataframe_ext.get_dataframe_data(lhs, {gje__ryv}) {sjfi__diglk}bodo.hiframes.pd_dataframe_ext.get_dataframe_data(rhs, {zuhm__pedag})'
                     if gje__ryv != -1 and zuhm__pedag != -1 else
                    f'bodo.libs.array_kernels.gen_na_array(len(lhs), float64_arr_type)'
                     for gje__ryv, zuhm__pedag in zip(hvu__dno, anb__eod))
                header = 'def impl(lhs, rhs):\n'
                index = (
                    'bodo.hiframes.pd_dataframe_ext.get_dataframe_index(lhs)')
                return _gen_init_df(header, inc__ilpoo, data_args, index,
                    extra_globals={'float64_arr_type': types.Array(types.
                    float64, 1, 'C')})
            hlrw__zuq = []
            steu__bca = []
            if op in nnvrl__hcn:
                for i, fbn__dpo in enumerate(lhs.data):
                    if is_common_scalar_dtype([fbn__dpo.dtype, rhs]):
                        hlrw__zuq.append(
                            f'bodo.hiframes.pd_dataframe_ext.get_dataframe_data(lhs, {i}) {sjfi__diglk} rhs'
                            )
                    else:
                        lamed__zyqyp = f'arr{i}'
                        steu__bca.append(lamed__zyqyp)
                        hlrw__zuq.append(lamed__zyqyp)
                data_args = ', '.join(hlrw__zuq)
            else:
                data_args = ', '.join(
                    f'bodo.hiframes.pd_dataframe_ext.get_dataframe_data(lhs, {i}) {sjfi__diglk} rhs'
            ...
            if len(steu__bca) > 0:
                ...
                header += '  n = len(lhs)\n'
                header += ''.join(
                    f'  {lamed__zyqyp} = np.empty(n, dtype=np.bool_)\n' for
                    lamed__zyqyp in steu__bca)
                header += (
                    '  for i in numba.parfors.parfor.internal_prange(n):\n')
                header += ''.join('    {0}[i] = {1}\n'.format(lamed__zyqyp,
                    op == operator.ne) for lamed__zyqyp in steu__bca)
        ...
        if isinstance(rhs, DataFrameType):
            ...
            steu__bca = []
            if op in nnvrl__hcn:
                for i, fbn__dpo in enumerate(rhs.data):
                    if is_common_scalar_dtype([lhs, fbn__dpo.dtype]):
                        hlrw__zuq.append(
                            f'lhs {sjfi__diglk} bodo.hiframes.pd_dataframe_ext.get_dataframe_data(rhs, {i})'
                            )
                    else:
                        lamed__zyqyp = f'arr{i}'
                        steu__bca.append(lamed__zyqyp)
                ...
                data_args = ', '.join(
                    ...
                    .format(i, sjfi__diglk) for i in range(len(rhs.columns)))
            header = 'def impl(lhs, rhs):\n'
            if len(steu__bca) > 0:
                header += '  numba.parfors.parfor.init_prange()\n'
                header += '  n = len(rhs)\n'
                header += ''.join('  {0} = np.empty(n, dtype=np.bool_)\n'.
                    format(lamed__zyqyp) for lamed__zyqyp in steu__bca)
                header += (
                    '  for i in numba.parfors.parfor.internal_prange(n):\n')
                header += ''.join('    {0}[i] = {1}\n'.format(lamed__zyqyp,
                    op == operator.ne) for lamed__zyqyp in steu__bca)
...
def _install_binary_ops():
    for op in bodo.hiframes.pd_series_ext.series_binary_ops:
        if op in skips:
            continue
        rstk__pxc = create_binary_op_overload(op)
        overload(op)(rstk__pxc)
...
def create_inplace_binary_op_overload(op):
    ...
    def overload_dataframe_inplace_binary_op(left, right):
        sjfi__diglk = numba.core.utils.OPERATORS_TO_BUILTINS[op]
        check_runtime_cols_unsupported(left, sjfi__diglk)
        check_runtime_cols_unsupported(right, sjfi__diglk)
        if isinstance(left, DataFrameType):
            if isinstance(right, DataFrameType):
                inc__ilpoo, fehrn__fqaw, anb__eod = _get_binop_columns(left,
                    right, True)
                usmu__uuoab = 'def impl(left, right):\n'
                for i, zuhm__pedag in enumerate(anb__eod):
                    if zuhm__pedag == -1:
                        usmu__uuoab += f"""  df_arr{i} = bodo.libs.array_kernels.gen_na_array(len(left), float64_arr_type)
"""
                        continue
                    usmu__uuoab += f"""  df_arr{i} = bodo.hiframes.pd_dataframe_ext.get_dataframe_data(left, {i})
"""
                    usmu__uuoab += f"""  df_arr{i} {sjfi__diglk} bodo.hiframes.pd_dataframe_ext.get_dataframe_data(right, {zuhm__pedag})
"""
                data_args = ', '.join(f'df_arr{i}' for i in range(len(
                    inc__ilpoo)))
                index = (
                    'bodo.hiframes.pd_dataframe_ext.get_dataframe_index(left)')
                return _gen_init_df(usmu__uuoab, inc__ilpoo, data_args,
...
"""
                    .format(i))
                usmu__uuoab += '  df_arr{0} {1} right\n'.format(i, sjfi__diglk)
            data_args = ', '.join('df_arr{}'.format(i) for i in range(len(
                left.columns)))
            index = 'bodo.hiframes.pd_dataframe_ext.get_dataframe_index(left)'
            return _gen_init_df(usmu__uuoab, left.columns, data_args, index)
    return overload_dataframe_inplace_binary_op


def _install_inplace_binary_ops():
    for op in bodo.hiframes.pd_series_ext.series_inplace_binary_ops:
        rstk__pxc = create_inplace_binary_op_overload(op)
        overload(op, no_unliteral=True)(rstk__pxc)
...
def create_unary_op_overload(op):
    ...
    def overload_dataframe_unary_op(df):
        if isinstance(df, DataFrameType):
            sjfi__diglk = numba.core.utils.OPERATORS_TO_BUILTINS[op]
            check_runtime_cols_unsupported(df, sjfi__diglk)
            data_args = ', '.join(
                '{1} bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df, {0})'
                .format(i, sjfi__diglk) for i in range(len(df.columns)))
            header = 'def impl(df):\n'
            return _gen_init_df(header, df.columns, data_args)
...
def _install_unary_ops():
    for op in bodo.hiframes.pd_series_ext.series_unary_ops:
        rstk__pxc = create_unary_op_overload(op)
        overload(op, no_unliteral=True)(rstk__pxc)
...
def overload_isna(obj):
    ...
    if is_array_typ(obj):
        ...
        def impl(obj):
            numba.parfors.parfor.init_prange()
            n = len(obj)
            glfmc__vnkz = np.empty(n, np.bool_)
            for i in numba.parfors.parfor.internal_prange(n):
                glfmc__vnkz[i] = bodo.libs.array_kernels.isna(obj, i)
            return glfmc__vnkz
        return impl
...
def overload_dataframe_replace(df, to_replace=None, value=None, inplace=
    ...
    sbvh__jplnm = {'inplace': inplace, 'limit': limit, 'regex': regex,
        'method': method}
    uspn__wiig = {'inplace': False, 'limit': None, 'regex': False, 'method':
        'pad'}
    check_unsupported_args('replace', sbvh__jplnm, uspn__wiig, package_name
        ='pandas', module_name='DataFrame')
    data_args = ', '.join(
...
def _is_col_access(expr_node):
    nnvsk__nclnc = str(expr_node)
    return nnvsk__nclnc.startswith('left.') or nnvsk__nclnc.startswith('right.'
        )


def _insert_NA_cond(expr_node, left_columns, left_data, right_columns,
    right_data):
    jde__psj = {'left': 0, 'right': 0, 'NOT_NA': 0}
    env = pd.core.computation.scope.ensure_scope(2, {}, {}, (jde__psj,))
    tox__yfk = pd.core.computation.parsing.clean_column_name

    def append_null_checks(expr_node, null_set):
        if not null_set:
            return expr_node
        nisi__eyg = ' & '.join([('NOT_NA.`' + x + '`') for x in null_set])
        xmfz__rpdkc = {('NOT_NA', tox__yfk(fbn__dpo)): fbn__dpo for
            fbn__dpo in null_set}
        wkhlh__tdm, fehrn__fqaw, fehrn__fqaw = _parse_query_expr(nisi__eyg,
            env, [], [], None, join_cleaned_cols=xmfz__rpdkc)
        unaeu__aeaf = (pd.core.computation.ops.BinOp.
            _disallow_scalar_only_bool_ops)
        pd.core.computation.ops.BinOp._disallow_scalar_only_bool_ops = (lambda
            self: None)
        try:
            podj__weq = pd.core.computation.ops.BinOp('&', wkhlh__tdm,
                expr_node)
        finally:
    ...
    def _insert_NA_cond_body(expr_node, null_set):
        if isinstance(expr_node, pd.core.computation.ops.BinOp):
            if expr_node.op == '|':
                ona__lyu = set()
                dsbc__yzyob = set()
                fgj__kxxop = _insert_NA_cond_body(expr_node.lhs, ona__lyu)
                exydx__ugxw = _insert_NA_cond_body(expr_node.rhs, dsbc__yzyob)
                jrgea__pvnp = ona__lyu.intersection(dsbc__yzyob)
                ona__lyu.difference_update(jrgea__pvnp)
                dsbc__yzyob.difference_update(jrgea__pvnp)
                null_set.update(jrgea__pvnp)
                expr_node.lhs = append_null_checks(fgj__kxxop, ona__lyu)
                expr_node.rhs = append_null_checks(exydx__ugxw, dsbc__yzyob)
            ...
            sftn__jkowo = expr_node.name
            ihg__sivd, col_name = sftn__jkowo.split('.')
            if ihg__sivd == 'left':
                rsl__zroh = left_columns
                data = left_data
            else:
                rsl__zroh = right_columns
                data = right_data
            fsg__sabzf = data[rsl__zroh.index(col_name)]
            if bodo.utils.typing.is_nullable(fsg__sabzf):
                null_set.add(expr_node.name)
        return expr_node
    null_set = set()
    ffooq__ufwpq = _insert_NA_cond_body(expr_node, null_set)
...
def _extract_equal_conds(expr_node):
    ...
    if expr_node.op == '==' and _is_col_access(expr_node.lhs
        ...
        vaft__zyoi = str(expr_node.lhs)
        dqzx__ofje = str(expr_node.rhs)
        if vaft__zyoi.startswith('left.') and dqzx__ofje.startswith('left.'
            ) or vaft__zyoi.startswith('right.') and dqzx__ofje.startswith(
            'right.'):
            return [], [], expr_node
        left_on = [vaft__zyoi.split('.')[1]]
        right_on = [dqzx__ofje.split('.')[1]]
        if vaft__zyoi.startswith('right.'):
            return right_on, left_on, None
        return left_on, right_on, None
    if expr_node.op == '&':
        ugh__cyqxl, eaju__pmssi, pup__ugm = _extract_equal_conds(expr_node.lhs)
        fhyns__czyrl, uyrvt__jskqz, bufli__xtdjm = _extract_equal_conds(
            expr_node.rhs)
        left_on = ugh__cyqxl + fhyns__czyrl
        right_on = eaju__pmssi + uyrvt__jskqz
        if pup__ugm is None:
            return left_on, right_on, bufli__xtdjm
        if bufli__xtdjm is None:
            return left_on, right_on, pup__ugm
        expr_node.lhs = pup__ugm
        expr_node.rhs = bufli__xtdjm
        expr_node.operands = expr_node.lhs, expr_node.rhs
...
def _parse_merge_cond(on_str, left_columns, left_data, right_columns,
    right_data):
    jde__psj = {'left': 0, 'right': 0}
    env = pd.core.computation.scope.ensure_scope(2, {}, {}, (jde__psj,))
    wynb__lpe = dict()
    tox__yfk = pd.core.computation.parsing.clean_column_name
    for name, lzzb__wkeor in (('left', left_columns), ('right', right_columns)
        ):
        for fbn__dpo in lzzb__wkeor:
            dosd__koxiv = tox__yfk(fbn__dpo)
            nexab__jzx = name, dosd__koxiv
            if nexab__jzx in wynb__lpe:
                raise BodoException(
                    f"pd.merge(): {name} table contains two columns that are escaped to the same Python identifier '{fbn__dpo}' and '{wynb__lpe[dosd__koxiv]}' Please rename one of these columns. To avoid this issue, please use names that are valid Python identifiers."
                    )
            wynb__lpe[nexab__jzx] = fbn__dpo
    xtjd__dnd, fehrn__fqaw, fehrn__fqaw = _parse_query_expr(on_str, env, [],
        [], None, join_cleaned_cols=wynb__lpe)
    left_on, right_on, jdgze__hzml = _extract_equal_conds(xtjd__dnd.terms)
    return left_on, right_on, _insert_NA_cond(jdgze__hzml, left_columns,
...
def overload_dataframe_merge(left, right, how='inner', on=None, left_on=
    ...
    zsyg__rnm = dict(sort=sort, copy=copy, validate=validate)
    rmiy__ebd = dict(sort=False, copy=True, validate=None)
    check_unsupported_args('DataFrame.merge', zsyg__rnm, rmiy__ebd,
        package_name='pandas', module_name='DataFrame')
    validate_merge_spec(left, right, how, on, left_on, right_on, left_index,
        right_index, sort, suffixes, copy, indicator, validate)
    how = get_overload_const_str(how)
    xru__xbmcr = tuple(sorted(set(left.columns) & set(right.columns), key=
        lambda k: str(k)))
    kku__ensp = ''
    if not is_overload_none(on):
        ...
        if is_overload_constant_str(on):
            ...
            if on_str not in xru__xbmcr and ('left.' in on_str or 'right.' in
                on_str):
                left_on, right_on, ionys__puxkb = _parse_merge_cond(on_str,
                    left.columns, left.data, right.columns, right.data)
                if ionys__puxkb is None:
                    kku__ensp = ''
                else:
                    kku__ensp = str(ionys__puxkb)
    if is_overload_none(on) and is_overload_none(left_on) and is_overload_none(
        ...
        right_index):
        left_keys = xru__xbmcr
        right_keys = xru__xbmcr
    else:
    ...
    dqz__feud = get_overload_const_bool(_bodo_na_equal)
    ...
    if is_overload_constant_tuple(suffixes):
        fbjws__fwcaz = get_overload_const_tuple(suffixes)
    if is_overload_constant_list(suffixes):
        fbjws__fwcaz = list(get_overload_const_list(suffixes))
    suffix_x = fbjws__fwcaz[0]
    suffix_y = fbjws__fwcaz[1]
    ...
    usmu__uuoab += """    suffixes=('_x', '_y'), copy=True, indicator=False, validate=None, _bodo_na_equal=True):
"""
    usmu__uuoab += (
        """  return bodo.hiframes.pd_dataframe_ext.join_dummy(left, right, {}, {}, '{}', '{}', '{}', False, {}, {}, '{}')
"""
        .format(left_keys, right_keys, how, suffix_x, suffix_y,
        indicator_val, dqz__feud, kku__ensp))
    dckt__uglrm = {}
    exec(usmu__uuoab, {'bodo': bodo}, dckt__uglrm)
    _impl = dckt__uglrm['_impl']
    return _impl
...
def common_validate_merge_merge_asof_spec(name_func, left, right, on,
    ...
    tobd__fwzqy = {string_array_type, binary_array_type,
        datetime_date_array_type, datetime_timedelta_array_type, boolean_array}
    onhkk__xqt = {get_overload_const_str(ynpcg__orza) for ynpcg__orza in (
        left_on, right_on, on) if is_overload_constant_str(ynpcg__orza)}
    for df in (left, right):
        for i, fbn__dpo in enumerate(df.data):
            if not isinstance(fbn__dpo, valid_dataframe_column_types
                ) and fbn__dpo not in tobd__fwzqy:
                raise BodoError(
                    f'{name_func}(): use of column with {type(fbn__dpo)} in merge unsupported'
                    )
            if df.columns[i] in onhkk__xqt and isinstance(fbn__dpo,
                MapArrayType):
                raise BodoError(
    ...
    if is_overload_constant_list(suffixes):
        fbjws__fwcaz = list(get_overload_const_list(suffixes))
    if len(fbjws__fwcaz) != 2:
        raise BodoError(name_func +
            '(): The number of suffixes should be exactly 2')
    xru__xbmcr = tuple(set(left.columns) & set(right.columns))
    if not is_overload_none(on):
        cxnha__ywlu = False
        if is_overload_constant_str(on):
            on_str = get_overload_const_str(on)
            cxnha__ywlu = on_str not in xru__xbmcr and ('left.' in on_str or
                'right.' in on_str)
        if len(xru__xbmcr) == 0 and not cxnha__ywlu:
...
def validate_keys_dtypes(left, right, left_index, right_index, left_keys,
    right_keys):
    meerf__fiimf = numba.core.registry.cpu_target.typing_context
    if is_overload_true(left_index) or is_overload_true(right_index):
        if is_overload_true(left_index) and is_overload_true(right_index):
            ydudi__lazi = left.index
            lwcol__iqjd = isinstance(ydudi__lazi, StringIndexType)
            wjvm__sje = right.index
            slxk__wzipu = isinstance(wjvm__sje, StringIndexType)
        elif is_overload_true(left_index):
            ydudi__lazi = left.index
            lwcol__iqjd = isinstance(ydudi__lazi, StringIndexType)
            wjvm__sje = right.data[right.columns.index(right_keys[0])]
            slxk__wzipu = wjvm__sje.dtype == string_type
        elif is_overload_true(right_index):
            ydudi__lazi = left.data[left.columns.index(left_keys[0])]
            lwcol__iqjd = ydudi__lazi.dtype == string_type
            wjvm__sje = right.index
            slxk__wzipu = isinstance(wjvm__sje, StringIndexType)
        if lwcol__iqjd and slxk__wzipu:
            return
        ydudi__lazi = ydudi__lazi.dtype
        wjvm__sje = wjvm__sje.dtype
        try:
            chvip__tnjqa = meerf__fiimf.resolve_function_type(operator.eq,
                (ydudi__lazi, wjvm__sje), {})
        except:
            raise_bodo_error(
                'merge: You are trying to merge on {lk_dtype} and {rk_dtype} columns. If you wish to proceed you should use pd.concat'
                .format(lk_dtype=ydudi__lazi, rk_dtype=wjvm__sje))
    else:
        for zdey__kzdv, ade__fnosc in zip(left_keys, right_keys):
            ydudi__lazi = left.data[left.columns.index(zdey__kzdv)].dtype
            tqx__anxps = left.data[left.columns.index(zdey__kzdv)]
            wjvm__sje = right.data[right.columns.index(ade__fnosc)].dtype
            xkw__pdt = right.data[right.columns.index(ade__fnosc)]
            if tqx__anxps == xkw__pdt:
                continue
            fdepi__qpse = (
                ...
                .format(lk=zdey__kzdv, lk_dtype=ydudi__lazi, rk=ade__fnosc,
                rk_dtype=wjvm__sje))
            cre__cspzy = ydudi__lazi == string_type
            uazll__madk = wjvm__sje == string_type
            if cre__cspzy ^ uazll__madk:
                raise_bodo_error(fdepi__qpse)
            try:
                chvip__tnjqa = meerf__fiimf.resolve_function_type(operator.
                    eq, (ydudi__lazi, wjvm__sje), {})
            except:
                raise_bodo_error(fdepi__qpse)


def validate_keys(keys, df):
    wfihm__ueamr = set(keys).difference(set(df.columns))
    if len(wfihm__ueamr) > 0:
        if is_overload_constant_str(df.index.name_typ
            ) and get_overload_const_str(df.index.name_typ) in wfihm__ueamr:
            raise_bodo_error(
                f'merge(): use of index {df.index.name_typ} as key for on/left_on/right_on is unsupported'
                )
        raise_bodo_error(
            f"""merge(): invalid key {wfihm__ueamr} for on/left_on/right_on
...
def overload_dataframe_join(left, other, on=None, how='left', lsuffix='',
    ...
    zsyg__rnm = dict(lsuffix=lsuffix, rsuffix=rsuffix)
    rmiy__ebd = dict(lsuffix='', rsuffix='')
    check_unsupported_args('DataFrame.join', zsyg__rnm, rmiy__ebd,
        package_name='pandas', module_name='DataFrame')
    ...
    left_keys = gen_const_tup(left_keys)
    right_keys = gen_const_tup(right_keys)
    usmu__uuoab = "def _impl(left, other, on=None, how='left',\n"
    usmu__uuoab += "    lsuffix='', rsuffix='', sort=False):\n"
    usmu__uuoab += (
...
def validate_join_spec(left, other, on, how, lsuffix, rsuffix, sort):
    ...
    if not is_overload_none(on):
        xyknb__utep = get_overload_const_list(on)
        validate_keys(xyknb__utep, left)
    if not is_overload_false(sort):
        raise BodoError(
            'join(): sort parameter only supports default value False')
    xru__xbmcr = tuple(set(left.columns) & set(other.columns))
    if len(xru__xbmcr) > 0:
        raise_bodo_error(
            'join(): not supporting joining on overlapping columns:{cols} Use DataFrame.merge() instead.'
            .format(cols=xru__xbmcr))


def validate_unicity_output_column_names(suffix_x, suffix_y, left_keys,
    right_keys, left_columns, right_columns, indicator_val):
    hszfh__lyr = set(left_keys) & set(right_keys)
    chn__oym = set(left_columns) & set(right_columns)
    tqvkq__tdzb = chn__oym - hszfh__lyr
    yapf__wzj = set(left_columns) - chn__oym
    max__hoisi = set(right_columns) - chn__oym
    iny__pnnc = {}

    def insertOutColumn(col_name):
        if col_name in iny__pnnc:
            raise_bodo_error(
                'join(): two columns happen to have the same name : {}'.
                format(col_name))
        iny__pnnc[col_name] = 0
    for irk__gnzr in hszfh__lyr:
        insertOutColumn(irk__gnzr)
    for irk__gnzr in tqvkq__tdzb:
        bmn__hez = str(irk__gnzr) + suffix_x
        zoe__lfx = str(irk__gnzr) + suffix_y
        insertOutColumn(bmn__hez)
        insertOutColumn(zoe__lfx)
    for irk__gnzr in yapf__wzj:
        insertOutColumn(irk__gnzr)
    for irk__gnzr in max__hoisi:
        insertOutColumn(irk__gnzr)
    if indicator_val:
...
def overload_dataframe_merge_asof(left, right, on=None, left_on=None,
    ...
    if isinstance(suffixes, tuple):
        fbjws__fwcaz = suffixes
    if is_overload_constant_list(suffixes):
        fbjws__fwcaz = list(get_overload_const_list(suffixes))
    if isinstance(suffixes, types.Omitted):
        fbjws__fwcaz = suffixes.value
    suffix_x = fbjws__fwcaz[0]
    suffix_y = fbjws__fwcaz[1]
    usmu__uuoab = (
    ...
    usmu__uuoab += "    allow_exact_matches=True, direction='backward'):\n"
    usmu__uuoab += '  suffix_x = suffixes[0]\n'
    usmu__uuoab += '  suffix_y = suffixes[1]\n'
    usmu__uuoab += (
...
def validate_groupby_spec(df, by, axis, level, as_index, sort, group_keys,
    ...
    if not is_overload_constant_bool(dropna):
        raise_const_error(
            ...
            .format(dropna))
    zsyg__rnm = dict(sort=sort, group_keys=group_keys, squeeze=squeeze,
        observed=observed)
    zsd__crkc = dict(sort=False, group_keys=True, squeeze=False, observed=True)
    check_unsupported_args('Dataframe.groupby', zsyg__rnm, zsd__crkc,
...
def pivot_error_checking(df, index, columns, values, func_name):
    ...
    sfd__amnp = get_literal_value(index)
    if isinstance(sfd__amnp, (list, tuple)):
        if len(sfd__amnp) > 1:
            raise BodoError(
                f"{func_name}(): 'index' argument must be a constant column label not a {sfd__amnp}"
                )
        sfd__amnp = sfd__amnp[0]
    zkvxg__wfpu = get_literal_value(columns)
    if isinstance(zkvxg__wfpu, (list, tuple)):
        if len(zkvxg__wfpu) > 1:
            raise BodoError(
                f"{func_name}(): 'columns' argument must be a constant column label not a {zkvxg__wfpu}"
                )
        zkvxg__wfpu = zkvxg__wfpu[0]
    if sfd__amnp not in df.columns:
        raise BodoError(
            f"{func_name}(): 'index' column {sfd__amnp} not found in DataFrame {df}."
            )
    if zkvxg__wfpu not in df.columns:
        raise BodoError(
            f"{func_name}(): 'columns' column {zkvxg__wfpu} not found in DataFrame {df}."
            )
    hfbu__uxlvg = {ypgl__tetq: i for i, ypgl__tetq in enumerate(df.columns)}
    zsrtk__ydr = hfbu__uxlvg[sfd__amnp]
    hmw__udd = hfbu__uxlvg[zkvxg__wfpu]
    if is_overload_none(values):
        qmwvo__wsr = []
        pante__zldqa = []
        for i, ypgl__tetq in enumerate(df.columns):
            if i not in (zsrtk__ydr, hmw__udd):
                qmwvo__wsr.append(i)
                pante__zldqa.append(ypgl__tetq)
    else:
        pante__zldqa = get_literal_value(values)
        if not isinstance(pante__zldqa, (list, tuple)):
            pante__zldqa = [pante__zldqa]
        qmwvo__wsr = []
        for val in pante__zldqa:
            if val not in hfbu__uxlvg:
                raise BodoError(
                    f"{func_name}(): 'values' column {val} not found in DataFrame {df}."
                    )
            qmwvo__wsr.append(hfbu__uxlvg[val])
    if all(isinstance(ypgl__tetq, int) for ypgl__tetq in pante__zldqa):
        pante__zldqa = np.array(pante__zldqa, 'int64')
    elif all(isinstance(ypgl__tetq, str) for ypgl__tetq in pante__zldqa):
        pante__zldqa = pd.array(pante__zldqa, 'string')
    else:
        raise BodoError(
    ...
    iumw__khkwz = set(qmwvo__wsr) | {zsrtk__ydr, hmw__udd}
    if len(iumw__khkwz) != len(qmwvo__wsr) + 2:
        raise BodoError(
            f"{func_name}(): 'index', 'columns', and 'values' must all refer to different columns"
            )
    vqxn__kag = df.data[zsrtk__ydr]
    if isinstance(vqxn__kag, (bodo.ArrayItemArrayType, bodo.MapArrayType,
        bodo.StructArrayType, bodo.TupleArrayType, bodo.IntervalArrayType)):
        raise BodoError(
            f"{func_name}(): 'index' DataFrame column must have scalar rows")
    if isinstance(vqxn__kag, bodo.CategoricalArrayType):
        raise BodoError(
            f"{func_name}(): 'index' DataFrame column does not support categorical data"
            )
    iya__epxqm = df.data[hmw__udd]
    if isinstance(iya__epxqm, (bodo.ArrayItemArrayType, bodo.MapArrayType,
        bodo.StructArrayType, bodo.TupleArrayType, bodo.IntervalArrayType)):
        raise BodoError(
            f"{func_name}(): 'columns' DataFrame column must have scalar rows")
    if isinstance(iya__epxqm, bodo.CategoricalArrayType):
        raise BodoError(
            f"{func_name}(): 'columns' DataFrame column does not support categorical data"
            )
    for lwtvw__dnky in qmwvo__wsr:
        cqn__fst = df.data[lwtvw__dnky]
        if isinstance(cqn__fst, (bodo.ArrayItemArrayType, bodo.MapArrayType,
            bodo.StructArrayType, bodo.TupleArrayType)
            ) or cqn__fst == bodo.binary_array_type:
            raise BodoError(
                f"{func_name}(): 'values' DataFrame column must have scalar rows"
                )
    return (sfd__amnp, zkvxg__wfpu, pante__zldqa, zsrtk__ydr, hmw__udd,
        qmwvo__wsr)
...
def overload_dataframe_pivot(df, index=None, columns=None, values=None):
    ...
    (sfd__amnp, zkvxg__wfpu, pante__zldqa, zsrtk__ydr, hmw__udd, ezjfv__ngh
        ) = (pivot_error_checking(df, index, columns, values,
        'DataFrame.pivot'))
    if len(pante__zldqa) == 1:
        xrqef__dvxf = None
    else:
        xrqef__dvxf = pante__zldqa
    usmu__uuoab = 'def impl(df, index=None, columns=None, values=None):\n'
    usmu__uuoab += f'    pivot_values = df.iloc[:, {hmw__udd}].unique()\n'
    usmu__uuoab += '    return bodo.hiframes.pd_dataframe_ext.pivot_impl(\n'
    usmu__uuoab += f"""        (bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df, {zsrtk__ydr}),),
"""
    usmu__uuoab += f"""        (bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df, {hmw__udd}),),
"""
    usmu__uuoab += '        (\n'
    for lwtvw__dnky in ezjfv__ngh:
        usmu__uuoab += f"""            bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df, {lwtvw__dnky}),
"""
    usmu__uuoab += '        ),\n'
    usmu__uuoab += '        pivot_values,\n'
    usmu__uuoab += '        index_lit,\n'
    usmu__uuoab += '        columns_lit,\n'
    usmu__uuoab += '        values_name_const,\n'
    usmu__uuoab += '    )\n'
    dckt__uglrm = {}
    exec(usmu__uuoab, {'bodo': bodo, 'index_lit': sfd__amnp, 'columns_lit':
        zkvxg__wfpu, 'values_name_const': xrqef__dvxf}, dckt__uglrm)
    impl = dckt__uglrm['impl']
    return impl
...
def overload_dataframe_pivot_table(df, values=None, index=None, columns=
    ...
    zsyg__rnm = dict(fill_value=fill_value, margins=margins, dropna=dropna,
        margins_name=margins_name, observed=observed, sort=sort)
    rmiy__ebd = dict(fill_value=None, margins=False, dropna=True,
        margins_name='All', observed=False, sort=True)
    check_unsupported_args('DataFrame.pivot_table', zsyg__rnm, rmiy__ebd,
        package_name='pandas', module_name='DataFrame')
    if _pivot_values is None:
        ...
        if len(pante__zldqa) == 1:
            ...
            xrqef__dvxf = pante__zldqa
        usmu__uuoab = 'def impl(\n'
        usmu__uuoab += '    df,\n'
        usmu__uuoab += '    values=None,\n'
        usmu__uuoab += '    index=None,\n'
        usmu__uuoab += '    columns=None,\n'
        usmu__uuoab += '    aggfunc="mean",\n'
        usmu__uuoab += '    fill_value=None,\n'
        usmu__uuoab += '    margins=False,\n'
        usmu__uuoab += '    dropna=True,\n'
        usmu__uuoab += '    margins_name="All",\n'
        usmu__uuoab += '    observed=False,\n'
        usmu__uuoab += '    sort=True,\n'
        usmu__uuoab += '    _pivot_values=None,\n'
        usmu__uuoab += '):\n'
        vbtj__lell = [zsrtk__ydr, hmw__udd] + ezjfv__ngh
        usmu__uuoab += f'    df = df.iloc[:, {vbtj__lell}]\n'
        usmu__uuoab += """    df = df.groupby([index_lit, columns_lit], as_index=False).agg(aggfunc)
"""
        usmu__uuoab += '    pivot_values = df.iloc[:, 1].unique()\n'
        usmu__uuoab += (
        ...
        usmu__uuoab += '        (\n'
        for i in range(2, len(ezjfv__ngh) + 2):
            usmu__uuoab += f"""            bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df, {i}),
"""
        usmu__uuoab += '        ),\n'
        usmu__uuoab += '        pivot_values,\n'
        usmu__uuoab += '        index_lit,\n'
        usmu__uuoab += '        columns_lit,\n'
        usmu__uuoab += '        values_name_const,\n'
        usmu__uuoab += '        check_duplicates=False,\n'
        usmu__uuoab += '    )\n'
        dckt__uglrm = {}
        exec(usmu__uuoab, {'bodo': bodo, 'index_lit': sfd__amnp,
            'columns_lit': zkvxg__wfpu, 'values_name_const': xrqef__dvxf},
            dckt__uglrm)
        impl = dckt__uglrm['impl']
        return impl
    if aggfunc == 'mean':
...
def crosstab_overload(index, columns, values=None, rownames=None, colnames=
    ...
    zsyg__rnm = dict(values=values, rownames=rownames, colnames=colnames,
        aggfunc=aggfunc, margins=margins, margins_name=margins_name, dropna
        =dropna, normalize=normalize)
    rmiy__ebd = dict(values=None, rownames=None, colnames=None, aggfunc=
        None, margins=False, margins_name='All', dropna=True, normalize=False)
    check_unsupported_args('pandas.crosstab', zsyg__rnm, rmiy__ebd,
        package_name='pandas', module_name='DataFrame')
...
def overload_dataframe_sort_values(df, by, axis=0, ascending=True, inplace=
    ...
    zsyg__rnm = dict(ignore_index=ignore_index, key=key)
    rmiy__ebd = dict(ignore_index=False, key=None)
    check_unsupported_args('DataFrame.sort_values', zsyg__rnm, rmiy__ebd,
        package_name='pandas', module_name='DataFrame')
...
def validate_sort_values_spec(df, by, axis, ascending, inplace, kind,
    ...
    if is_overload_none(by) or not is_literal_type(by
        ...
        raise_const_error(
            ...
            .format(by))
    vnq__luo = set(df.columns)
    if is_overload_constant_str(df.index.name_typ):
        vnq__luo.add(get_overload_const_str(df.index.name_typ))
    if is_overload_constant_tuple(by):
        lbbcf__dfnt = [get_overload_const_tuple(by)]
    else:
        lbbcf__dfnt = get_overload_const_list(by)
    lbbcf__dfnt = set((k, '') if (k, '') in vnq__luo else k for k in
        lbbcf__dfnt)
    if len(lbbcf__dfnt.difference(vnq__luo)) > 0:
        jgru__mhok = list(set(get_overload_const_list(by)).difference(vnq__luo)
            )
        raise_bodo_error(f'sort_values(): invalid keys {jgru__mhok} for by.')
    if not is_overload_zero(axis):
        raise_bodo_error(
    ...
    if is_overload_constant_str(na_position):
        ...
        vwj__exj = get_overload_const_list(na_position)
        for na_position in vwj__exj:
            if na_position not in ('first', 'last'):
                raise BodoError(
...
def overload_dataframe_sort_index(df, axis=0, level=None, ascending=True,
    ...
    zsyg__rnm = dict(axis=axis, level=level, kind=kind, sort_remaining=
        sort_remaining, ignore_index=ignore_index, key=key)
    rmiy__ebd = dict(axis=0, level=None, kind='quicksort', sort_remaining=
        True, ignore_index=False, key=None)
    check_unsupported_args('DataFrame.sort_index', zsyg__rnm, rmiy__ebd,
        package_name='pandas', module_name='DataFrame')
...
def overload_dataframe_fillna(df, value=None, method=None, axis=None,
    ...
    zsyg__rnm = dict(limit=limit, downcast=downcast)
    rmiy__ebd = dict(limit=None, downcast=None)
    check_unsupported_args('DataFrame.fillna', zsyg__rnm, rmiy__ebd,
        package_name='pandas', module_name='DataFrame')
    if not (is_overload_none(axis) or is_overload_zero(axis)):
        raise BodoError("DataFrame.fillna(): 'axis' argument not supported.")
    ijgr__rsqe = not is_overload_none(value)
    dzr__klfw = not is_overload_none(method)
    if ijgr__rsqe and dzr__klfw:
        raise BodoError(
            "DataFrame.fillna(): Cannot specify both 'value' and 'method'.")
    if not ijgr__rsqe and not dzr__klfw:
        raise BodoError(
            "DataFrame.fillna(): Must specify one of 'value' and 'method'.")
    if ijgr__rsqe:
        umeyd__dgbh = 'value=value'
    else:
        umeyd__dgbh = 'method=method'
    data_args = [(
        f"df['{ypgl__tetq}'].fillna({umeyd__dgbh}, inplace=inplace)" if
        isinstance(ypgl__tetq, str) else
        f'df[{ypgl__tetq}].fillna({umeyd__dgbh}, inplace=inplace)') for
        ypgl__tetq in df.columns]
    usmu__uuoab = """def impl(df, value=None, method=None, axis=None, inplace=False, limit=None, downcast=None):
"""
    if is_overload_true(inplace):
        usmu__uuoab += '  ' + '  \n'.join(data_args) + '\n'
        dckt__uglrm = {}
        exec(usmu__uuoab, {}, dckt__uglrm)
        impl = dckt__uglrm['impl']
        return impl
    else:
        return _gen_init_df(usmu__uuoab, df.columns, ', '.join(qdpr__jvsok +
            '.values' for qdpr__jvsok in data_args))
...
def overload_dataframe_reset_index(df, level=None, drop=False, inplace=
    ...
    zsyg__rnm = dict(col_level=col_level, col_fill=col_fill)
    rmiy__ebd = dict(col_level=0, col_fill='')
    check_unsupported_args('DataFrame.reset_index', zsyg__rnm, rmiy__ebd,
        package_name='pandas', module_name='DataFrame')
    ...
    usmu__uuoab = """def impl(df, level=None, drop=False, inplace=False, col_level=0, col_fill='', _bodo_transformed=False,):
"""
    usmu__uuoab += (
    ...
    if not drop:
        nsd__ugru = 'index' if 'index' not in columns else 'level_0'
        ...
        if isinstance(df.index, MultiIndexType):
            ...
            ioy__bztb = ['m_index._data[{}]'.format(i) for i in range(df.
                index.nlevels)]
            data_args = ioy__bztb + data_args
        else:
            cpy__epre = (
                'bodo.utils.conversion.index_to_array(bodo.hiframes.pd_dataframe_ext.get_dataframe_index(df))'
                )
            data_args = [cpy__epre] + data_args
    return _gen_init_df(usmu__uuoab, columns, ', '.join(data_args), 'index')


def _is_all_levels(df, level):
    gzc__rdwio = len(get_index_data_arr_types(df.index))
    return is_overload_none(level) or is_overload_constant_int(level
        ) and get_overload_const_int(level
        ) == 0 and gzc__rdwio == 1 or is_overload_constant_list(level
        ) and list(get_overload_const_list(level)) == list(range(gzc__rdwio))
...
def overload_dataframe_dropna(df, axis=0, how='any', thresh=None, subset=
    ...
    if is_overload_none(subset):
        boqyh__sluq = list(range(len(df.columns)))
    elif not is_overload_constant_list(subset):
        raise_bodo_error(
            f'df.dropna(): subset argument should a constant list, not {subset}'
            )
    else:
        wvgan__kwzve = get_overload_const_list(subset)
        boqyh__sluq = []
        for zxjx__hfvhh in wvgan__kwzve:
            if zxjx__hfvhh not in df.columns:
                raise_bodo_error(
                    f"df.dropna(): column '{zxjx__hfvhh}' not in data frame columns {df}"
                    )
            boqyh__sluq.append(df.columns.index(zxjx__hfvhh))
    fsca__mhuhj = len(df.columns)
    data_args = ', '.join('data_{}'.format(i) for i in range(fsca__mhuhj))
    usmu__uuoab = (
...
"""
        .format(data_args, index, ', '.join(str(a) for a in boqyh__sluq)))
    usmu__uuoab += (
        '  index = bodo.utils.conversion.index_from_array(index_arr)\n')
    return _gen_init_df(usmu__uuoab, df.columns, data_args, 'index')
...
def overload_dataframe_drop(df, labels=None, axis=0, index=None, columns=
    ...
    zsyg__rnm = dict(index=index, level=level, errors=errors)
    rmiy__ebd = dict(index=None, level=None, errors='raise')
    check_unsupported_args('DataFrame.drop', zsyg__rnm, rmiy__ebd,
        package_name='pandas', module_name='DataFrame')
    ...
    if not is_overload_none(labels):
        ...
        if is_overload_constant_str(labels):
            ugp__cjw = get_overload_const_str(labels),
        elif is_overload_constant_list(labels):
            ugp__cjw = get_overload_const_list(labels)
        else:
            raise_bodo_error(
        ...
        if is_overload_constant_str(columns):
            ugp__cjw = get_overload_const_str(columns),
        elif is_overload_constant_list(columns):
            ugp__cjw = get_overload_const_list(columns)
        else:
            raise_bodo_error(
                'constant list of columns expected for labels in DataFrame.drop()'
                )
    for ypgl__tetq in ugp__cjw:
        if ypgl__tetq not in df.columns:
            raise_bodo_error(
                'DataFrame.drop(): column {} not in DataFrame columns {}'.
                format(ypgl__tetq, df.columns))
    if len(set(ugp__cjw)) == len(df.columns):
        raise BodoError('DataFrame.drop(): Dropping all columns not supported.'
            )
    inplace = is_overload_true(inplace)
    cmd__lflpe = tuple(ypgl__tetq for ypgl__tetq in df.columns if 
        ypgl__tetq not in ugp__cjw)
    data_args = ', '.join(
        'bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df, {}){}'.
        format(df.columns.index(ypgl__tetq), '.copy()' if not inplace else
        '') for ypgl__tetq in cmd__lflpe)
    usmu__uuoab = (
    ...
    index = 'bodo.hiframes.pd_dataframe_ext.get_dataframe_index(df)'
    return _gen_init_df(usmu__uuoab, cmd__lflpe, data_args, index)
...
def overload_dataframe_sample(df, n=None, frac=None, replace=False, weights
    ...
    zsyg__rnm = dict(random_state=random_state, weights=weights, axis=axis,
        ignore_index=ignore_index)
    cyaem__ccg = dict(random_state=None, weights=None, axis=None,
        ignore_index=False)
    check_unsupported_args('DataFrame.sample', zsyg__rnm, cyaem__ccg,
        package_name='pandas', module_name='DataFrame')
    ...
    fsca__mhuhj = len(df.columns)
    data_args = ', '.join('data_{}'.format(i) for i in range(fsca__mhuhj))
    usmu__uuoab = """def impl(df, n=None, frac=None, replace=False, weights=None, random_state=None, axis=None, ignore_index=False):
"""
    for i in range(fsca__mhuhj):
        usmu__uuoab += (
            '  data_{0} = bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df, {0})\n'
            .format(i))
    usmu__uuoab += '  if frac is None:\n'
    usmu__uuoab += '    frac_d = -1.0\n'
    usmu__uuoab += '  else:\n'
    usmu__uuoab += '    frac_d = frac\n'
    usmu__uuoab += '  if n is None:\n'
    usmu__uuoab += '    n_i = 0\n'
    usmu__uuoab += '  else:\n'
    usmu__uuoab += '    n_i = n\n'
    index = (
...
"""
    ...
    usmu__uuoab += (
        '  index = bodo.utils.conversion.index_from_array(index_arr)\n')
    return bodo.hiframes.dataframe_impl._gen_init_df(usmu__uuoab, df.
...
def overload_dataframe_info(df, verbose=None, buf=None, max_cols=None,
    ...
    sbvh__jplnm = {'verbose': verbose, 'buf': buf, 'max_cols': max_cols,
        'memory_usage': memory_usage, 'show_counts': show_counts,
        'null_counts': null_counts}
    uspn__wiig = {'verbose': None, 'buf': None, 'max_cols': None,
        'memory_usage': None, 'show_counts': None, 'null_counts': None}
    check_unsupported_args('DataFrame.info', sbvh__jplnm, uspn__wiig,
        package_name='pandas', module_name='DataFrame')
    fqgna__bfeqz = f"<class '{str(type(df)).split('.')[-1]}"
    if len(df.columns) == 0:

        def _info_impl(df, verbose=None, buf=None, max_cols=None,
            memory_usage=None, show_counts=None, null_counts=None):
            nnwo__iqv = fqgna__bfeqz + '\n'
            nnwo__iqv += 'Index: 0 entries\n'
            nnwo__iqv += 'Empty DataFrame'
            print(nnwo__iqv)
        return _info_impl
    else:
        usmu__uuoab = """def _info_impl(df, verbose=None, buf=None, max_cols=None, memory_usage=None, show_counts=None, null_counts=None): #pragma: no cover
"""
        usmu__uuoab += '    ncols = df.shape[1]\n'
        usmu__uuoab += f'    lines = "{fqgna__bfeqz}\\n"\n'
        usmu__uuoab += f'    lines += "{df.index}: "\n'
        usmu__uuoab += (
            '    index = bodo.hiframes.pd_dataframe_ext.get_dataframe_index(df)\n'
            )
        if isinstance(df.index, bodo.hiframes.pd_index_ext.RangeIndexType):
            usmu__uuoab += """    lines += f"{len(index)} entries, {index.start} to {index.stop-1}\\n\"
"""
        elif isinstance(df.index, bodo.hiframes.pd_index_ext.StringIndexType):
            usmu__uuoab += """    lines += f"{len(index)} entries, {index[0]} to {index[len(index)-1]}\\n\"
"""
        else:
            usmu__uuoab += (
        ...
        usmu__uuoab += '    column_width = max(space, 7)\n'
        usmu__uuoab += '    column= "Column"\n'
        usmu__uuoab += '    underl= "------"\n'
        usmu__uuoab += (
        ...
        usmu__uuoab += '    mem_size = 0\n'
        usmu__uuoab += (
            '    col_name = bodo.libs.str_arr_ext.pre_alloc_string_array(ncols, -1)\n'
            )
        usmu__uuoab += """    non_null_count = bodo.libs.str_arr_ext.pre_alloc_string_array(ncols, -1)
"""
        usmu__uuoab += (
        ...
        cmwzl__vfxb = dict()
        for i in range(len(df.columns)):
            usmu__uuoab += f"""    non_null_count[{i}] = str(bodo.libs.array_ops.array_op_count(bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df, {i})))
"""
            zubw__cug = f'{df.data[i].dtype}'
            if isinstance(df.data[i], bodo.CategoricalArrayType):
                zubw__cug = 'category'
            elif isinstance(df.data[i], bodo.IntegerArrayType):
                dafo__nzkwd = bodo.libs.int_arr_ext.IntDtype(df.data[i].dtype
                    ).name
                zubw__cug = f'{dafo__nzkwd[:-7]}'
            usmu__uuoab += f'    col_dtype[{i}] = "{zubw__cug}"\n'
            if zubw__cug in cmwzl__vfxb:
                cmwzl__vfxb[zubw__cug] += 1
            else:
                cmwzl__vfxb[zubw__cug] = 1
            usmu__uuoab += f'    col_name[{i}] = "{df.columns[i]}"\n'
            usmu__uuoab += f"""    mem_size += bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df, {i}).nbytes
"""
        usmu__uuoab += """    column_info = [f'{i:^3} {name:<{column_width}} {count} non-null      {dtype}' for i, (name, count, dtype) in enumerate(zip(col_name, non_null_count, col_dtype))]
"""
        usmu__uuoab += '    for i in column_info:\n'
        usmu__uuoab += "        lines += f'{i}\\n'\n"
        krpeq__sazgq = ', '.join(f'{k}({cmwzl__vfxb[k]})' for k in sorted(
            cmwzl__vfxb))
        usmu__uuoab += f"    lines += 'dtypes: {krpeq__sazgq}\\n'\n"
        usmu__uuoab += '    mem_size += df.index.nbytes\n'
        usmu__uuoab += '    total_size = _sizeof_fmt(mem_size)\n'
        usmu__uuoab += "    lines += f'memory usage: {total_size}'\n"
        usmu__uuoab += '    print(lines)\n'
        dckt__uglrm = {}
        exec(usmu__uuoab, {'_sizeof_fmt': _sizeof_fmt, 'pd': pd, 'bodo':
            bodo, 'np': np}, dckt__uglrm)
        _info_impl = dckt__uglrm['_info_impl']
        return _info_impl
...
def overload_dataframe_memory_usage(df, index=True, deep=False):
    ...
    usmu__uuoab = 'def impl(df, index=True, deep=False):\n'
    data = ', '.join(
    ...
    if is_overload_true(index):
        ...
        strb__uglgz = ','.join(f"'{ypgl__tetq}'" for ypgl__tetq in df.columns)
        arr = f"bodo.utils.conversion.coerce_to_array(('Index',{strb__uglgz}))"
        index = f'bodo.hiframes.pd_index_ext.init_binary_str_index({arr})'
        usmu__uuoab += f"""  return bodo.hiframes.pd_series_ext.init_series(({kwgim__ubke}{data}), {index}, None)
"""
    else:
        pjx__qlzzo = ',' if len(df.columns) == 1 else ''
        tkqs__sqz = gen_const_tup(df.columns)
        usmu__uuoab += f"""  return bodo.hiframes.pd_series_ext.init_series(({data}{pjx__qlzzo}), pd.Index({tkqs__sqz}), None)
"""
    dckt__uglrm = {}
    exec(usmu__uuoab, {'bodo': bodo, 'pd': pd}, dckt__uglrm)
    impl = dckt__uglrm['impl']
    return impl
...
def overload_read_excel(io, sheet_name=0, header=0, names=None, index_col=
    ...
    oelk__vbi = 'read_excel_df{}'.format(next_label())
    setattr(types, oelk__vbi, df_type)
    hvlse__wzch = False
    if is_overload_constant_list(parse_dates):
        hvlse__wzch = get_overload_const_list(parse_dates)
    nmj__geygy = ', '.join(["'{}':{}".format(cname, _get_pd_dtype_str(t)) for
...
def impl(
    ...
    return df
    """
        .format(oelk__vbi, list(df_type.columns), nmj__geygy, hvlse__wzch))
    dckt__uglrm = {}
    exec(usmu__uuoab, globals(), dckt__uglrm)
    impl = dckt__uglrm['impl']
    return impl
...
def overload_dataframe_plot(df, x=None, y=None, kind='line', figsize=None,
    ...
    usmu__uuoab += '    xticks=None, yticks=None, ax=None):\n'
    if is_overload_none(ax):
        usmu__uuoab += '   fig, ax = plt.subplots()\n'
    else:
        usmu__uuoab += '   fig = ax.get_figure()\n'
    if not is_overload_none(figsize):
        usmu__uuoab += '   fig.set_figwidth(figsize[0])\n'
        usmu__uuoab += '   fig.set_figheight(figsize[1])\n'
    if is_overload_none(xlabel):
        usmu__uuoab += '   xlabel = x\n'
    usmu__uuoab += '   ax.set_xlabel(xlabel)\n'
    if is_overload_none(ylabel):
        usmu__uuoab += '   ylabel = y\n'
    else:
        usmu__uuoab += '   ax.set_ylabel(ylabel)\n'
    if not is_overload_none(title):
        usmu__uuoab += '   ax.set_title(title)\n'
    if not is_overload_none(fontsize):
        usmu__uuoab += '   ax.tick_params(labelsize=fontsize)\n'
    ...
    if kind == 'line':
        if is_overload_none(x) and is_overload_none(y):
            ...
            usmu__uuoab += '   ax.plot(df[y], label=y)\n'
        elif is_overload_none(y):
            fzo__qvbys = get_overload_const_str(x)
            aou__svfg = df.columns.index(fzo__qvbys)
            for i in range(len(df.columns)):
                if isinstance(df.data[i], (types.Array, IntegerArrayType)
                    ) and isinstance(df.data[i].dtype, (types.Integer,
                    types.Float)):
                    if aou__svfg != i:
                        usmu__uuoab += f"""   ax.plot(df[x], df.iloc[:, {i}], label=df.columns[{i}])
"""
        else:
            usmu__uuoab += '   ax.plot(df[x], df[y], label=y)\n'
    elif kind == 'scatter':
        legend = False
        usmu__uuoab += '   ax.scatter(df[x], df[y], s=20)\n'
        usmu__uuoab += '   ax.set_ylabel(ylabel)\n'
    if not is_overload_none(xticks):
        usmu__uuoab += '   ax.set_xticks(xticks)\n'
    if not is_overload_none(yticks):
        usmu__uuoab += '   ax.set_yticks(yticks)\n'
    if is_overload_true(legend):
        usmu__uuoab += '   ax.legend()\n'
    usmu__uuoab += '   return ax\n'
    dckt__uglrm = {}
    exec(usmu__uuoab, {'bodo': bodo, 'plt': plt}, dckt__uglrm)
    impl = dckt__uglrm['impl']
    return impl
...
def is_df_values_numpy_supported_dftyp(df_typ):
    for jfw__xda in df_typ.data:
        if not (isinstance(jfw__xda, IntegerArrayType) or isinstance(
            jfw__xda.dtype, types.Number) or jfw__xda.dtype in (bodo.
...
class SetDfColInfer(AbstractTemplate):
    ...
    def generic(self, args, kws):
        ...
        skwrb__mobz = args[0]
        lhb__uln = args[1].literal_value
        val = args[2]
        assert val != types.unknown
        thfol__xbzg = skwrb__mobz
        check_runtime_cols_unsupported(skwrb__mobz, 'set_df_col()')
        if isinstance(skwrb__mobz, DataFrameType):
            index = skwrb__mobz.index
            if len(skwrb__mobz.columns) == 0:
            ...
            if lhb__uln in skwrb__mobz.columns:
                cmd__lflpe = skwrb__mobz.columns
                xba__eupc = skwrb__mobz.columns.index(lhb__uln)
                mppc__erao = list(skwrb__mobz.data)
                mppc__erao[xba__eupc] = val
                mppc__erao = tuple(mppc__erao)
            else:
                cmd__lflpe = skwrb__mobz.columns + (lhb__uln,)
                mppc__erao = skwrb__mobz.data + (val,)
            thfol__xbzg = DataFrameType(mppc__erao, index, cmd__lflpe,
                skwrb__mobz.dist, skwrb__mobz.is_table_format)
        return thfol__xbzg(*args)
...
def _parse_query_expr(expr, env, columns, cleaned_columns, index_name=None,
    ...
    def _rewrite_membership_op(self, node, left, right):
        fwjk__kmw = node.op
        op = self.visit(fwjk__kmw)
        return op, fwjk__kmw, left, right
    ...
    def visit_Attribute(self, node, **kwargs):
        moyeg__fmd = node.attr
        value = node.value
        ikb__zyqey = pd.core.computation.ops.LOCAL_TAG
        if moyeg__fmd in ('str', 'dt'):
            try:
                rsd__tmpp = str(self.visit(value))
            except pd.core.computation.ops.UndefinedVariableError as pee__wkk:
                col_name = pee__wkk.args[0].split("'")[1]
                raise BodoError(
                    'df.query(): column {} is not found in dataframe columns {}'
                    .format(col_name, columns))
        else:
            rsd__tmpp = str(self.visit(value))
        nexab__jzx = rsd__tmpp, moyeg__fmd
        if nexab__jzx in join_cleaned_cols:
            moyeg__fmd = join_cleaned_cols[nexab__jzx]
        name = rsd__tmpp + '.' + moyeg__fmd
        if name.startswith(ikb__zyqey):
            name = name[len(ikb__zyqey):]
        if moyeg__fmd in ('str', 'dt'):
            pwq__cjwj = columns[cleaned_columns.index(rsd__tmpp)]
            vzq__yfu[pwq__cjwj] = rsd__tmpp
            self.env.scope[name] = 0
            return self.term_type(ikb__zyqey + name, self.env)
        hzsi__pkj.append(name)
    ...
    def math__str__(self):
        ...
        return pd.io.formats.printing.pprint_thing(
            ...
            (op, ','.join(epvh__azkm), lhb__uln))

    def op__str__(self):
        ycjod__ixyd = ('({0})'.format(pd.io.formats.printing.pprint_thing(
            vdomc__ordk)) for vdomc__ordk in self.operands)
        if self.op == 'in':
            return pd.io.formats.printing.pprint_thing(
        ...
        return pd.io.formats.printing.pprint_thing(' {0} '.format(self.op).
            join(ycjod__ixyd))
    dwd__idv = pd.core.computation.expr.BaseExprVisitor._rewrite_membership_op
    onnrd__exfuk = (pd.core.computation.expr.BaseExprVisitor.
        _maybe_evaluate_binop)
    lplq__jkdkg = pd.core.computation.expr.BaseExprVisitor.visit_Attribute
    iuxf__vnpet = (pd.core.computation.expr.BaseExprVisitor.
        _maybe_downcast_constants)
    sqn__sucrh = pd.core.computation.ops.Term.__str__
    esv__ixbd = pd.core.computation.ops.MathCall.__str__
    tyavs__gmy = pd.core.computation.ops.Op.__str__
    unaeu__aeaf = pd.core.computation.ops.BinOp._disallow_scalar_only_bool_ops
    try:
        ...
        pd.core.computation.ops.BinOp._disallow_scalar_only_bool_ops = (lambda
            self: None)
        xtjd__dnd = pd.core.computation.expr.Expr(expr, env=env)
        mqad__clbg = str(xtjd__dnd)
    except pd.core.computation.ops.UndefinedVariableError as pee__wkk:
        if not is_overload_none(index_name) and get_overload_const_str(
            index_name) == pee__wkk.args[0].split("'")[1]:
            raise BodoError(
                "df.query(): Refering to named index ('{}') by name is not supported"
                .format(get_overload_const_str(index_name)))
        else:
            raise BodoError(f'df.query(): undefined variable, {pee__wkk}')
    finally:
        pd.core.computation.expr.BaseExprVisitor._rewrite_membership_op = (
            dwd__idv)
        pd.core.computation.expr.BaseExprVisitor._maybe_evaluate_binop = (
            onnrd__exfuk)
        pd.core.computation.expr.BaseExprVisitor.visit_Attribute = lplq__jkdkg
        (pd.core.computation.expr.BaseExprVisitor._maybe_downcast_constants
            ) = iuxf__vnpet
        pd.core.computation.ops.Term.__str__ = sqn__sucrh
        pd.core.computation.ops.MathCall.__str__ = esv__ixbd
        pd.core.computation.ops.Op.__str__ = tyavs__gmy
        pd.core.computation.ops.BinOp._disallow_scalar_only_bool_ops = (
            unaeu__aeaf)
    tgw__tum = pd.core.computation.parsing.clean_column_name
    vzq__yfu.update({ypgl__tetq: tgw__tum(ypgl__tetq) for ypgl__tetq in
        columns if tgw__tum(ypgl__tetq) in xtjd__dnd.names})
    return xtjd__dnd, mqad__clbg, vzq__yfu
...
class DataFrameTupleIterator(types.SimpleIteratorType):
    ...
    def __init__(self, col_names, arr_typs):
        ...
        ykid__vvbjk = ['{}={}'.format(col_names[i], arr_typs[i]) for i in
            range(len(col_names))]
        name = 'itertuples({})'.format(','.join(ykid__vvbjk))
        kzqws__qsr = namedtuple('Pandas', col_names)
        iis__divw = types.NamedTuple([_get_series_dtype(a) for a in
            arr_typs], kzqws__qsr)
        super(DataFrameTupleIterator, self).__init__(name, iis__divw)
...
class TypeIterTuples(AbstractTemplate):
    ...
    def generic(self, args, kws):
        ...
        klxb__jzy = [if_series_to_array_type(a) for a in args[len(args) // 2:]]
        assert 'Index' not in col_names[0]
        col_names = ['Index'] + col_names
        klxb__jzy = [types.Array(types.int64, 1, 'C')] + klxb__jzy
        fnj__avu = DataFrameTupleIterator(col_names, klxb__jzy)
        return fnj__avu(*args)
...
class DataFrameTupleIteratorModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        buldr__ulqm = [('index', types.EphemeralPointer(types.uintp))] + [(
...
def get_itertuples_impl(context, builder, sig, args):
    yqu__dzaze = args[len(args) // 2:]
    hkoit__tflnn = sig.args[len(sig.args) // 2:]
    myov__uamw = context.make_helper(builder, sig.return_type)
    fcnpy__imam = context.get_constant(types.intp, 0)
    qaqk__nbul = cgutils.alloca_once_value(builder, fcnpy__imam)
    myov__uamw.index = qaqk__nbul
    for i, arr in enumerate(yqu__dzaze):
        setattr(myov__uamw, 'array{}'.format(i), arr)
    for arr, arr_typ in zip(yqu__dzaze, hkoit__tflnn):
        context.nrt.incref(builder, arr_typ, arr)
    res = myov__uamw._getvalue()
...
def iternext_itertuples(context, builder, sig, args, result):
    tpr__dbyg, = sig.args
    ruutw__bnhi, = args
    myov__uamw = context.make_helper(builder, tpr__dbyg, value=ruutw__bnhi)
    oxj__kmgth = signature(types.intp, tpr__dbyg.array_types[1])
    vqugy__qbt = context.compile_internal(builder, lambda a: len(a),
        oxj__kmgth, [myov__uamw.array0])
    index = builder.load(myov__uamw.index)
    ulyd__yic = builder.icmp(lc.ICMP_SLT, index, vqugy__qbt)
    result.set_valid(ulyd__yic)
    with builder.if_then(ulyd__yic):
        values = [index]
        for i, arr_typ in enumerate(tpr__dbyg.array_types[1:]):
            ssl__kjwc = getattr(myov__uamw, 'array{}'.format(i))
            if arr_typ == types.Array(types.NPDatetime('ns'), 1, 'C'):
                pighu__akl = signature(pd_timestamp_type, arr_typ, types.intp)
                val = context.compile_internal(builder, lambda a, i: bodo.
                    ...
                    pighu__akl, [ssl__kjwc, index])
            else:
                pighu__akl = signature(arr_typ.dtype, arr_typ, types.intp)
                val = context.compile_internal(builder, lambda a, i: a[i],
                    pighu__akl, [ssl__kjwc, index])
            values.append(val)
        value = context.make_tuple(builder, tpr__dbyg.yield_type, values)
        result.yield_(value)
        rwyux__ivkai = cgutils.increment_index(builder, index)
        builder.store(rwyux__ivkai, myov__uamw.index)
...
def _analyze_op_pair_first(self, scope, equiv_set, expr, lhs):
    ...
    iui__gca = ir.Assign(rhs, lhs, expr.loc)
    ksnht__svuas = lhs
    srd__ujc = []
    zrm__asw = []
    qml__dikoz = typ.count
    for i in range(qml__dikoz):
        utouc__wumkd = ir.Var(ksnht__svuas.scope, mk_unique_var('{}_size{}'
            .format(ksnht__svuas.name, i)), ksnht__svuas.loc)
        jtbd__hyzd = ir.Expr.static_getitem(lhs, i, None, ksnht__svuas.loc)
        self.calltypes[jtbd__hyzd] = None
        srd__ujc.append(ir.Assign(jtbd__hyzd, utouc__wumkd, ksnht__svuas.loc))
        self._define(equiv_set, utouc__wumkd, types.intp, jtbd__hyzd)
        zrm__asw.append(utouc__wumkd)
    frqi__hzbo = tuple(zrm__asw)
    return numba.parfors.array_analysis.ArrayAnalysis.AnalyzeResult(shape=
        frqi__hzbo, pre=[iui__gca] + srd__ujc)
