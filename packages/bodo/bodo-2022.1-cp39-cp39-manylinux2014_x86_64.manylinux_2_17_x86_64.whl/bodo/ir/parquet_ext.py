...
def remove_dead_pq(pq_node, lives_no_aliases, lives, arg_aliases, alias_map,
    func_ir, typemap):
    iwisk__vmwoj = pq_node.out_vars[0].name
    hrgf__dvcgh = pq_node.out_vars[1].name
    if iwisk__vmwoj not in lives and hrgf__dvcgh not in lives:
        return None
    elif iwisk__vmwoj not in lives:
        pq_node.col_indices = []
        pq_node.df_colnames = []
        pq_node.type_usecol_offset = []
    elif hrgf__dvcgh not in lives:
...
def pq_remove_dead_column(pq_node, column_live_map, equiv_vars, typemap):
    ...
    myt__ofavl = pq_node.out_vars[0].name
    assert isinstance(typemap[myt__ofavl], TableType
        ), 'Parquet Node Table must be a TableType'
    if pq_node.col_indices:
        oiqca__zxth, mfw__fey = get_live_column_nums_block(column_live_map,
            equiv_vars, myt__ofavl)
        oiqca__zxth = bodo.ir.connector.trim_extra_used_columns(oiqca__zxth,
            len(pq_node.col_indices))
        if not mfw__fey and not oiqca__zxth:
            oiqca__zxth = [0]
        if not mfw__fey and len(oiqca__zxth) != len(pq_node.type_usecol_offset
            ):
            pq_node.type_usecol_offset = oiqca__zxth
            return True
    return False
