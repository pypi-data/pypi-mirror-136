...
def distributed_diagnostics(self, signature=None, level=1):
    ...
    def dump(sig):
        pnfdj__htf = self.overloads[sig]
        rufbq__cdyp = pnfdj__htf.metadata.get('distributed_diagnostics', None)
        if rufbq__cdyp is None:
            fpgs__jucy = 'No distributed diagnostic available'
            raise bodo.utils.typing.BodoError(fpgs__jucy)
        rufbq__cdyp.dump(level, self.get_metadata(sig))
...
def master_mode_wrapper(numba_jit_wrapper):
    ...
    def _wrapper(pyfunc):
        lto__rzs = numba_jit_wrapper(pyfunc)
        return master_mode.MasterModeDispatcher(lto__rzs)
    return _wrapper
...
def jit(signature_or_function=None, pipeline_class=None, **options):
    ...
    if 'distributed' in options and isinstance(options['distributed'], bool):
        zegfa__zodfq = options.pop('distributed')
        pipeline_class = (pipeline_class if zegfa__zodfq else bodo.compiler
            .BodoCompilerSeq)
    spzx__etinj = numba.jit(signature_or_function, pipeline_class=
        pipeline_class, **options)
    if master_mode.master_mode_on and bodo.get_rank(
        ) == master_mode.MASTER_RANK:
        if isinstance(spzx__etinj, numba.dispatcher._DispatcherBase):
            return master_mode.MasterModeDispatcher(spzx__etinj)
        else:
            return master_mode_wrapper(spzx__etinj)
    else:
        return spzx__etinj
