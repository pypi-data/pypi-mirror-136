...
mjlib.mj_printFormattedModel.__doc__ = """
Print mjModel to text file, specifying format. float_format_str must be a valid
printf-style format string for a single float value.

Args:
  m: ctypes.POINTER(MJMODEL) <const>
  filename: ctypes.POINTER(ctypes.c_char) <const>
  float_format_str: ctypes.POINTER(ctypes.c_char) <const>
"""
mjlib.mj_printFormattedModel.argtypes = [
    ctypes.POINTER(MJMODEL),
    ctypes.POINTER(ctypes.c_char),
    ctypes.POINTER(ctypes.c_char),
]
mjlib.mj_printFormattedModel.restype = None
