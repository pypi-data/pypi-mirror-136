...
class ExcelToJson(ExcelImporter):
    ...
    def __init__(self, __useCols:list, __startRowNo:int):
        ...
        self.useCols = __useCols
        """ 読込対象列番号配列 """

        self.startRowNo = __startRowNo
        """ 読込開始行番号 """

    def put_excelToJson(self):
    ...
    def get_sheetByDataFrame(self, sheetname:str):
        ...
        # シートをdict型に変換
        dict = pd.read_excel(self.filepath, sheet_name=sheetname, skiprows=self.startRowNo, usecols=self.useCols)
        # dict->DataFrame
