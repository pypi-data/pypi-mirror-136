...
def multiclass_matrix(C_mod, key_list,target_label=None,target_columns_label = None,addtional_model=True):
    ...
    for tl1 in target_label:
        ...
        fig = plt.figure(figsize=(1.0 * len_key_l, 1.0 * len(x_line)))
        ax = fig.add_subplot(1, 1, 1)

        base_df = base_df.dropna(how='any')

        sns.set(rc={'figure.figsize': (1.0 * len_key_l, 1.0 * len(x_line))})
...
def multiclass_f_importance(C_mod, key_list, bar_type = None, show_range="top20",target_label=None,addtional_model=True):
    ...
    for tl1 in target_label:
        ...
        if bar_type == None:
            ...
            def print_fi(iii, par_name, gfii=None):
                ...
                print_fi_sub(gfiiout, par_name)



            def print_fi_sub(gfiiout, par_name):
                ...
                if output_stat == 1:
                    print_fi2(gfii, ml_name,special_label)
                elif output_stat == 2:
                    for gfii_0 in gfii:
                        print_fi_sub(gfii_0, ml_name)
                return

            def print_fi2(gfii, graph_name,special_label=None):
                ...
                if (gfii_min < 0):
                    gfii_min *= 1.0
                    gfii = gfii - gfii_min
                fe_i = pd.DataFrame(gfii, index=C_mod.get_input().columns, columns=[lbiii])
