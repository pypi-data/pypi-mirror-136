...
def get_mapping_variable_jinja2(raw_string: Text, variables_mapping: VariablesMapping) -> Any:
    ...
    """
    # template = Template(raw_string, variable_start_string="${", variable_end_string="}")
...
def parse_string(
    ...
    def get_function_compile_match() -> Optional[Match[AnyStr]]:
    ...
    def get_variable_compile_match() -> Optional[Match[AnyStr]]:
    ...
    try:
        ...
        if get_function_compile_match() or get_variable_compile_match():
...
def parse_string_new(
    ...
    try:
        return get_mapping_variable_jinja2(raw_string, variables_mapping)
    except Exception:
        try:
            match_start_position = raw_string.index("$", 0)
            parsed_string = raw_string[0:match_start_position]
    ...
    while match_start_position < len(raw_string):
        ...
        try:
            ...
            match_start_position = len(raw_string)
        try:
            remain_string = parse_data(remain_string, variables_mapping, functions_mapping)
        except Exception:
            pass
        parsed_string += remain_string
