...
def mix_all(args, mappings, tags):
	type_mappings = {x[0]: x[1] for x in mappings if len(x) == 2}
	...
	if args.cmd == 'scaffold':
		...
		for f, tbls in targets.items():
			...
			if type_tpls:
				...
				for x in tbls:
					out = template.render(table(Table(x, this=x, kind=x.get('kind'), schema=schema_map, args=args)))
					if out.strip():
			...
			if header_tpls:
				...
				headers.append(
					template1.render(tables=schema['data'], kind=schema['kind'], schema=schema_map, args=args, unknown_types=[y for x in tbls for y in unknown_types(x['fields'])]))

			footers = []
			footer_tpls = [x for x in tpls if x.startswith("footer.")]
			if footer_tpls:
				template3 = env.get_template(footer_tpls[0])
				footers.append(template3.render(tables=schema['data'], kind=schema['kind'], schema=schema_map, args=args))
...
def main():
	...
	if os.path.exists(mappings_path):
		lines = [x for x in csv.reader(open(mappings_path).read().strip().splitlines()[1:]) or [] if x[0].strip() and x[0].strip()[0] != "#"]
