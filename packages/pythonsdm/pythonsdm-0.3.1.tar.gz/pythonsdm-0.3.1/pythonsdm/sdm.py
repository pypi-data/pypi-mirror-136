...
def valid_shapes(images: List[ndarray], shapes: ndarray):
    max_a = np.array([
        np.array(img.shape) - np.array(PATCH_SIZE)
        for img in images])

    return np.maximum(0, np.minimum(shapes, max_a.reshape(-1, 1, 2)))


def validate_shapes(images, shapes):
    for image, shape in zip(images, shapes):
        assert np.all(np.array(image.shape) - shape >= 0), (image.shape, shape)
        assert np.all(shapes >= 0), (image.shape, shapes)
...
def get_features(image: ndarray, shape: ndarray):
    ...
    for r, c in shape:
        ...
        assert patch.shape == PATCH_SIZE, (r, c, image.shape)
...
def get_X(images: List[ndarray], shapes: ndarray):
    ...
    return np.stack([
        ...
        for i in tqdm(range(len(images)), desc='Getting features')])
...
def sdm_train(
    ...
    for i in range(num_regressors):
        print('MSE:', mse(shapes, target_shapes))
...
def sdm_test(
    ...
    target_shapes = valid_shapes(images, load_pts(images_path, keys))
    ...
    shapes = valid_shapes(images, shapes)
    ...
    if len(target_shapes):
        mark_shapes(out_images, target_shapes, [0, 255, 0])
