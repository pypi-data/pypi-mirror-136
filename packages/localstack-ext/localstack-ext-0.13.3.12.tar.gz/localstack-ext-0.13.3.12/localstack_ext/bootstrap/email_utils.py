...
SENT_EMAILS=[]
EMAIL_BLACKLIST=oPNkY()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",oPNkD(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=oPNka,from_name=oPNka,smtp_host=oPNka,smtp_user=oPNka,smtp_pass=oPNka,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if oPNkK(recipients,oPNkv)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=oPNka):
 ...
 result["From"]=formataddr((oPNkD(Header(from_name,"utf-8")),from_email))
 return result
