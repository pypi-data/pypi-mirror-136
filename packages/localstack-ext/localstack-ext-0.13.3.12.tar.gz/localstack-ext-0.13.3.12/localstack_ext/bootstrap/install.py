...
def install_package(packages:Union[List[VuUOj],VuUOj],api_name:VuUOj):
 if not(is_debian()and in_docker()):
  ...
  return
 if VuUOF(packages,VuUOj):
  packages=packages.split()
 LOG.info("Downloading dependencies for %s API. This may take a while."%api_name)
 with INSTALL_LOCK:
  run(["apt-get","update"],shell=VuUOy)
  run(["apt-get","install","-y","--no-install-recommends"]+packages,shell=VuUOy)
def check_or_install(command:VuUOj,packages:Union[List[VuUOj],VuUOj],api:VuUOj):
...
def setup_ssl_cert():
 ...
 try:
  ...
  try:
   ...
   raise
def download_github_artifact(url:VuUOj,target_file:VuUOj,timeout:int=VuUON):
 def do_download(url,print_error=VuUOy):
  try:
