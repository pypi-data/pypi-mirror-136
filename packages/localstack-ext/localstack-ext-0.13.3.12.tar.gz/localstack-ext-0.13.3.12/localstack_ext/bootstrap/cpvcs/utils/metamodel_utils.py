...
CAPTURE_METAMODEL_SEPARATELY=mCglv
...
class MetamodelDelta(SubtypesInstanceManager):
 def create_delta_for_state_files(self,sf1:StateFileRef,sf2:StateFileRef):
  ...
  if not sf1:
   backend2=load_persisted_object(config_context.get_obj_file_path(sf2.hash_ref))
   clazz=mCgli(backend2)
   backend1=_infer_backend_init(clazz,sf2)
  else:
   backend1=load_persisted_object(config_context.get_obj_file_path(sf1.hash_ref))
   clazz=mCgli(backend1)
   backend2=_infer_backend_init(clazz,sf1)
  diff=self.create_delta_json(backend1,backend2)
  return diff
 def create_delta_log(self,state_from:Set[StateFileRef],state_to:Set[StateFileRef])->mCglx:
  ...
  def _create_sf_lookup(state_files:Set[StateFileRef])->Dict[mCglx,StateFileRef]:
  ...
  for state_file_from in state_files_from_regular:
   ...
   if state_file_from.any_congruence(state_files_to_regular):
    ...
    diff_json=self.create_delta_for_state_files(state_file_from,mCglS)
  ...
  for state_files_to in state_files_to_lookup.values():
   ...
   diff_json=self.create_delta_for_state_files(mCglS,state_files_to)
...
class CommitMetamodelUtils:
 @mCglR
 def create_metadata_archive(cls,version:Version,delete_reference:bool=mCglv,overwrite:bool=mCglv,metamodels_file:mCglx=mCglS):
 ...
 def get_metamodel_delta(cls,prev_metamodel:Dict,this_metamodel:Dict)->Dict:
  ...
  return result
 @mCglR
 def create_delta_metamodel_file(cls,version:mCglM,revision:mCglM,store_to_zip=mCglv)->Tuple[Dict,Dict]:
 ...
 @mCglR
 def reconstruct_metamodel(cls,version:mCglM,revision:mCglM=mCglS)->Dict:
  result={}
  for rev in mCglD(1,revision+1):
  ...
  return result
 @mCglR
 def get_version_metamodel(cls,version:mCglM,revision:mCglM=mCglS)->Dict:
 ...
 @mCglR
 def get_commit_diff(cls,version_no:mCglM,commit_no:mCglM)->Optional[Dict]:
  ...
  return result
def _infer_backend_init(clazz:Type,sf:StateFileRef)->mCglV:
 if mCglb(clazz,mCglH):
  return{}
 constructor=mCglN(clazz,"__init__",mCglS)
...
def _create_metamodel_helper(obj,width=25,visited:set=mCglS):
 if obj is mCglS:
  return obj
