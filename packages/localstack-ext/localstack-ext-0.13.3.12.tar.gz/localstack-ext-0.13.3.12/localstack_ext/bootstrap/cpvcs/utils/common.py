...
class CPVCSConfigContext:
 ...
 def get_version_state_archive_path(self,version,with_format:giXPW=giXPa)->giXPc:
 ...
 def update_ver_log(self,author:giXPc,ver_no:giXPf,rev_id:giXPc,rev_no:giXPf):
  with giXPG(self.get_ver_log_path(),"a")as fp:
   fp.write(f"{VER_LOG_STRUCTURE.format(author=author, ver_no=ver_no, rev_rid_no=f'{rev_id}_{rev_no}')}\n")
 def create_version_symlink(self,name:giXPc,key:giXPc=giXPq)->giXPc:
  return self._create_symlink(name,key,self.get_ver_refs_path())
 def create_revision_symlink(self,name:giXPc,key:giXPc=giXPq)->giXPc:
  return self._create_symlink(name,key,self.get_rev_refs_path())
 def is_initialized(self)->giXPW:
  return self.pod_root_dir and os.path.isdir(self.pod_root_dir)
 def _create_symlink(self,name:giXPc,key:giXPc,path:giXPc)->giXPc:
...
def zip_directories(zip_dest:giXPc,directories:List[giXPc])->giXPc:
 ...
 return zip_dest
def add_file_to_archive(archive:giXPc,entry_name:giXPc,content:Union[giXPc,giXPn]):
 with zipfile.ZipFile(archive,"a")as zip_file:
  zip_file.writestr(entry_name,content)
def read_file_from_archive(archive_path:giXPc,file_name:giXPc)->giXPc:
 try:
