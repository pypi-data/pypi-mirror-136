...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or RQnIP
 @RQnIk
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return RQnIy(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @RQnIa
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if RQnIx(origins,RQnIO)else origins.get("Items")
  origins_to_create=RQnIS([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if RQnIx(aliases,RQnIO)else aliases.get("Items")
  aliases_to_create=RQnIS(aliases)
  for distr in distrs:
   origins=RQnIS([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=RQnIS(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",RQnIM)
   config["Enabled"]=RQnIN(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if RQnIx(custom_errors,RQnIh)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=RQnIN(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":RQnIM}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=RQnIN(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=RQnIP):
    parent=config if parent is RQnIP else parent
    items=parent.get(key)or[]
    if RQnIx(items,RQnIh)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":RQnId(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if RQnIx(cache_behavs,RQnIh)else cache_behavs)
