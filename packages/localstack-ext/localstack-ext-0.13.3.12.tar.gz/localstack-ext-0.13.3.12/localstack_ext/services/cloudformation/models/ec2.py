...
class EC2ElasticIP(GenericBaseModel):
 ...
 def cloudformation_type():
  return "AWS::EC2::EIP"
 def get_physical_resource_id(self,attribute=kjeSU,**kwargs):
...
class SecurityGroupInOrEgress(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  for perm in perms:
   if kjeSG(perm["IpProtocol"])!=kjeSG(rp["IpProtocol"]):
    continue
 ...
 @kjeSh
 def get_vpc_and_name_for_security_group(cls,group_id:kjeSG)->Tuple[kjeSG,kjeSG]:
  client=aws_stack.connect_to_service("ec2")
  groups=client.describe_security_groups(GroupIds=[group_id])["SecurityGroups"]
  vpc_id=groups and groups[0].get("VpcId")or kjeSU
  group_name=groups and groups[0]["GroupName"]or kjeSU
 ...
 def get_deploy_templates(cls):
  def create_params(params,**kwargs):
   ...
   source_group_id=result.pop("SourceSecurityGroupId",kjeSU)
   vpc_id=kjeSU
   ...
   dst_group_id=result.pop("DestinationSecurityGroupId",kjeSU)
   ...
   if result.get("IpProtocol"):
    result["IpProtocol"]=kjeSG(result.get("IpProtocol"))
   description=result.pop("Description",kjeSU)
   cidr_ipv6=result.pop("CidrIpv6",kjeSU)
