...
class SESTemplate(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(template or[MEOQv])[0]
 def get_physical_resource_id(self,attribute=MEOQv,**kwargs):
 ...
 def update_resource(self,new_resource,stack_name,resources):
  ...
  if MEOQP(template.get(attr,"")==new_template.get(attr,"")for attr in["SubjectPart","TextPart","HtmlPart"]):
   return
...
class SESReceiptRuleSet(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return MEOQe(SESReceiptRuleSet,self).get_cfn_attribute(attribute)
 @MEOQS
 def add_defaults(resource,stack_name:MEOQR):
...
class SESReceiptRule(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return MEOQe(SESReceiptRule,self).get_cfn_attribute(attribute)
 @MEOQr
