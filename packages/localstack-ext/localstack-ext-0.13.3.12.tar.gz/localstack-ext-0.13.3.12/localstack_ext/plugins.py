...
def api_key_configured()->NxewT:
 return(Nxewy if os.environ.get("LOCALSTACK_API_KEY")and os.environ.get("LOCALSTACK_API_KEY").strip()else Nxewp)
...
def configure_pro_container(container:LocalstackContainer):
 try:
  ...
  if NxewD([is_api_enabled(api)for api in EXTERNAL_PORT_APIS]):
