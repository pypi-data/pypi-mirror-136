...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return LWOyf if e else LWOyX
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->LWOyC:
...
@click.option("--revert",is_flag=LWOyf,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:LWOyP):
...
def _cpvcs_initialized(pod_name:LWOyI)->LWOyP:
...
@click.option("-n","--name",help="Name of the cloud pod",required=LWOyf)
def cmd_cpvcs_init(name:LWOyI):
...
@click.option("-n","--name",help="Name of the cloud pod",required=LWOyf)
@click.option("-r","--remote",help="Whether the Pod should also be deleted remotely.",is_flag=LWOyf,default=LWOyX)
def cmd_cpvcs_delete(name:LWOyI,remote:LWOyP):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=LWOyf)
def cmd_cpvcs_register_remote(name:LWOyI):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Current Name of the cloud pod",required=LWOyf)
@click.option("-nn","--new-name",help="New name of the cloud pod",required=LWOyf)
def cmd_cpvcs_rename(name:LWOyI,new_name:LWOyI):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=LWOyf)
def cmd_cpvcs_commit(message:LWOyI,name:LWOyI):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("--squash",is_flag=LWOyf,help="Squashes commits together, so only the latest commit is stored in the revision graph")
@click.option("--three-way",is_flag=LWOyf,default=LWOyX,help="")
@click.option("-m","--message",help="Add a comment describing the version")
@click.option("-n","--name",help="Name of the cloud pod",required=LWOyf)
def cmd_cpvcs_push(squash:LWOyP,message:LWOyI,name:LWOyI,three_way:LWOyP):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=LWOyf)
@click.option("-v","--version",type=LWOyT)
@click.option("-m","--message",required=LWOyX)
def cmd_cpvcs_push_overwrite(version:LWOyT,message:LWOyI,name:LWOyI):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-v","--version",default="-1",type=LWOyT,help="Loads the state of the specified version - Most recent one by default")
@click.option("--reset",is_flag=LWOyf,default=LWOyX,help="Will reset the application state before injecting")
@click.option("-n","--name",help="Name of the cloud pod",required=LWOyf)
def cmd_cpvcs_inject(version:LWOyT,reset:LWOyP,name:LWOyI):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--inject/--no-inject",default=LWOyf,help="Whether the latest version of the pulled pod should be injected")
@click.option("--reset/--no-reset",default=LWOyf,help="Whether the current application state should be reset after the pod has been pulled")
@click.option("-n","--name",help="Name of the cloud pod",required=LWOyf)
@click.option("--lazy/--eager",default=LWOyf,help="Will only fetch references to existing versions, i.e. version state is only downloaded when required")
@cpvcs.command(name="pull",help="Injects the state from a version into the application runtime")
def cmd_cpvcs_pull(name:LWOyI,inject:LWOyP,reset:LWOyP,lazy:LWOyP):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--remote","-r",is_flag=LWOyf,default=LWOyX)
def cmd_cpvcs_list_pods(remote:LWOyP):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=LWOyf)
def cmd_cpvcs_versions(name:LWOyI):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=LWOyf,type=LWOyT)
@click.option("-n","--name",help="Name of the cloud pod",required=LWOyf)
def cmd_cpvcs_version_info(version:LWOyT,name:LWOyI):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",type=LWOyT,default=-1,help="Latest version by default")
@click.option("-n","--name",help="Name of the cloud pod",required=LWOyf)
def cmd_cpvcs_version_metamodel(version:LWOyT,name:LWOyI):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=LWOyf,type=LWOyT,help="The version the state should be set to")
@click.option("--inject/--no-inject",default=LWOyf,help="Whether the state should be directly injected into the application runtime after changing version")
@click.option("--reset/--no-reset",default=LWOyf,help="Whether the current application state should be reset before changing version")
@click.option("--commit-before",is_flag=LWOyX,help="Whether the current application state should be commited to the currently selected version before changing version")
@click.option("-n","--name",help="Name of the cloud pod",required=LWOyf)
def cmd_cpvcs_set_version(version:LWOyT,inject:LWOyP,reset:LWOyP,commit_before:LWOyP,name:LWOyI):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=LWOyf)
def cmd_cpvcs_commits(version:LWOyT,name:LWOyI):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("--version","-v",required=LWOyf)
@click.option("--commit","-c",required=LWOyf)
@click.option("-n","--name",help="Name of the cloud pod",required=LWOyf)
def cmd_cpvcs_commit_diff(version:LWOyT,commit:LWOyT,name:LWOyI):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
def cmd_pod_list():
 ...
 from localstack_ext.bootstrap import pods_client
 try:
  result=pods_client.list_pods(LWOyC)
  status.stop()
...
def print_table(columns:Dict[LWOyI,LWOyI],rows:List[Dict[LWOyI,Any]],formatters:Dict[LWOyI,Callable[[Any],LWOyI]]=LWOyC):
 from rich.table import Table
 if formatters is LWOyC:
  formatters=LWOye()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  LWOyt(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->LWOyC:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->LWOyC:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->LWOyC:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->LWOyC:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
