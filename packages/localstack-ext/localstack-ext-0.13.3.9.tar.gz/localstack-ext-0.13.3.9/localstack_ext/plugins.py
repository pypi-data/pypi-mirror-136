...
def api_key_configured()->oyCqd:
 return(oyCqY if os.environ.get("LOCALSTACK_API_KEY")and os.environ.get("LOCALSTACK_API_KEY").strip()else oyCqU)
...
def configure_pro_container(container:LocalstackContainer):
 try:
  ...
  if oyCqV([is_api_enabled(api)for api in EXTERNAL_PORT_APIS]):
