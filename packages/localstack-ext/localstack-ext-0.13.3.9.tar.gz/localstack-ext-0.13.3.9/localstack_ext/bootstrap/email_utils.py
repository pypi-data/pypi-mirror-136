...
SENT_EMAILS=[]
EMAIL_BLACKLIST=fuhyX()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",fuhyQ(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=fuhyg,from_name=fuhyg,smtp_host=fuhyg,smtp_user=fuhyg,smtp_pass=fuhyg,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if fuhyO(recipients,fuhyw)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=fuhyg):
 ...
 result["From"]=formataddr((fuhyQ(Header(from_name,"utf-8")),from_email))
 return result
