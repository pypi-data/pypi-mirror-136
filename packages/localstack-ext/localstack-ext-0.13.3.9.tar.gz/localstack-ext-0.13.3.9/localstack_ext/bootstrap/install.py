...
def install_package(packages:Union[List[GgqSu],GgqSu],api_name:GgqSu):
 if not(is_debian()and in_docker()):
  ...
  return
 if GgqSt(packages,GgqSu):
  packages=packages.split()
 LOG.info("Downloading dependencies for %s API. This may take a while."%api_name)
 with INSTALL_LOCK:
  run(["apt-get","update"],shell=GgqSl)
  run(["apt-get","install","-y","--no-install-recommends"]+packages,shell=GgqSl)
def check_or_install(command:GgqSu,packages:Union[List[GgqSu],GgqSu],api:GgqSu):
...
def setup_ssl_cert():
 ...
 try:
  ...
  try:
   ...
   raise
def download_github_artifact(url:GgqSu,target_file:GgqSu,timeout:int=GgqSQ):
 def do_download(url,print_error=GgqSl):
  try:
