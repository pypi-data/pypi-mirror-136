...
class CPVCSConfigContext:
 ...
 def get_version_state_archive_path(self,version,with_format:GVTIR=GVTIC)->GVTIW:
 ...
 def update_ver_log(self,author:GVTIW,ver_no:GVTIh,rev_id:GVTIW,rev_no:GVTIh):
  with GVTIm(self.get_ver_log_path(),"a")as fp:
   fp.write(f"{VER_LOG_STRUCTURE.format(author=author, ver_no=ver_no, rev_rid_no=f'{rev_id}_{rev_no}')}\n")
 def create_version_symlink(self,name:GVTIW,key:GVTIW=GVTIA)->GVTIW:
  return self._create_symlink(name,key,self.get_ver_refs_path())
 def create_revision_symlink(self,name:GVTIW,key:GVTIW=GVTIA)->GVTIW:
  return self._create_symlink(name,key,self.get_rev_refs_path())
 def is_initialized(self)->GVTIR:
  return self.pod_root_dir and os.path.isdir(self.pod_root_dir)
 def _create_symlink(self,name:GVTIW,key:GVTIW,path:GVTIW)->GVTIW:
...
def zip_directories(zip_dest:GVTIW,directories:List[GVTIW])->GVTIW:
 ...
 return zip_dest
def add_file_to_archive(archive:GVTIW,entry_name:GVTIW,content:Union[GVTIW,GVTIx]):
 with zipfile.ZipFile(archive,"a")as zip_file:
  zip_file.writestr(entry_name,content)
def read_file_from_archive(archive_path:GVTIW,file_name:GVTIW)->GVTIW:
 try:
