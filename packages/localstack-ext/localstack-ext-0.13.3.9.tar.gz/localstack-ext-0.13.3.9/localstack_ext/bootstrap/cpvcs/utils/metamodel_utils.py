...
CAPTURE_METAMODEL_SEPARATELY=MSRci
...
class MetamodelDelta(SubtypesInstanceManager):
 def create_delta_for_state_files(self,sf1:StateFileRef,sf2:StateFileRef):
  ...
  if not sf1:
   backend2=load_persisted_object(config_context.get_obj_file_path(sf2.hash_ref))
   clazz=MSRcD(backend2)
   backend1=_infer_backend_init(clazz,sf2)
  else:
   backend1=load_persisted_object(config_context.get_obj_file_path(sf1.hash_ref))
   clazz=MSRcD(backend1)
   backend2=_infer_backend_init(clazz,sf1)
  diff=self.create_delta_json(backend1,backend2)
  return diff
 def create_delta_log(self,state_from:Set[StateFileRef],state_to:Set[StateFileRef])->MSRco:
  ...
  def _create_sf_lookup(state_files:Set[StateFileRef])->Dict[MSRco,StateFileRef]:
  ...
  for state_file_from in state_files_from_regular:
   ...
   if state_file_from.any_congruence(state_files_to_regular):
    ...
    diff_json=self.create_delta_for_state_files(state_file_from,MSRcP)
  ...
  for state_files_to in state_files_to_lookup.values():
   ...
   diff_json=self.create_delta_for_state_files(MSRcP,state_files_to)
...
class CommitMetamodelUtils:
 @MSRcO
 def create_metadata_archive(cls,version:Version,delete_reference:bool=MSRci,overwrite:bool=MSRci,metamodels_file:MSRco=MSRcP):
 ...
 def get_metamodel_delta(cls,prev_metamodel:Dict,this_metamodel:Dict)->Dict:
  ...
  return result
 @MSRcO
 def create_delta_metamodel_file(cls,version:MSRcT,revision:MSRcT,store_to_zip=MSRci)->Tuple[Dict,Dict]:
 ...
 @MSRcO
 def reconstruct_metamodel(cls,version:MSRcT,revision:MSRcT=MSRcP)->Dict:
  result={}
  for rev in MSRcK(1,revision+1):
  ...
  return result
 @MSRcO
 def get_version_metamodel(cls,version:MSRcT,revision:MSRcT=MSRcP)->Dict:
 ...
 @MSRcO
 def get_commit_diff(cls,version_no:MSRcT,commit_no:MSRcT)->Optional[Dict]:
  ...
  return result
def _infer_backend_init(clazz:Type,sf:StateFileRef)->MSRca:
 if MSRcp(clazz,MSRcj):
  return{}
 constructor=MSRcl(clazz,"__init__",MSRcP)
...
def _create_metamodel_helper(obj,width=25,visited:set=MSRcP):
 if obj is MSRcP:
  return obj
