...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or SylnI
 @SylnV
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return SylnF(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @SylnE
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if Sylnq(origins,SylnN)else origins.get("Items")
  origins_to_create=SylnT([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if Sylnq(aliases,SylnN)else aliases.get("Items")
  aliases_to_create=SylnT(aliases)
  for distr in distrs:
   origins=SylnT([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=SylnT(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",Sylnv)
   config["Enabled"]=SylnD(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if Sylnq(custom_errors,SylnG)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=SylnD(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":Sylnv}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=SylnD(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=SylnI):
    parent=config if parent is SylnI else parent
    items=parent.get(key)or[]
    if Sylnq(items,SylnG)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":Sylno(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if Sylnq(cache_behavs,SylnG)else cache_behavs)
