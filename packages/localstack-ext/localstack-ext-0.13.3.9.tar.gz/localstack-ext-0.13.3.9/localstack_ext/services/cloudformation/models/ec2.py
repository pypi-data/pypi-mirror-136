...
class EC2ElasticIP(GenericBaseModel):
 ...
 def cloudformation_type():
  return "AWS::EC2::EIP"
 def get_physical_resource_id(self,attribute=PdXzJ,**kwargs):
...
class SecurityGroupInOrEgress(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  for perm in perms:
   if PdXzK(perm["IpProtocol"])!=PdXzK(rp["IpProtocol"]):
    continue
 ...
 @PdXzu
 def get_vpc_and_name_for_security_group(cls,group_id:PdXzK)->Tuple[PdXzK,PdXzK]:
  client=aws_stack.connect_to_service("ec2")
  groups=client.describe_security_groups(GroupIds=[group_id])["SecurityGroups"]
  vpc_id=groups and groups[0].get("VpcId")or PdXzJ
  group_name=groups and groups[0]["GroupName"]or PdXzJ
 ...
 def get_deploy_templates(cls):
  def create_params(params,**kwargs):
   ...
   source_group_id=result.pop("SourceSecurityGroupId",PdXzJ)
   vpc_id=PdXzJ
   ...
   dst_group_id=result.pop("DestinationSecurityGroupId",PdXzJ)
   ...
   if result.get("IpProtocol"):
    result["IpProtocol"]=PdXzK(result.get("IpProtocol"))
   description=result.pop("Description",PdXzJ)
   cidr_ipv6=result.pop("CidrIpv6",PdXzJ)
