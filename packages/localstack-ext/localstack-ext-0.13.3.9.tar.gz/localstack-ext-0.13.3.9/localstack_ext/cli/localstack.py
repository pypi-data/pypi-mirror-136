...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return QCuPg if e else QCuPG
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->QCuPT:
...
@click.option("--revert",is_flag=QCuPg,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:QCuPq):
...
def _cpvcs_initialized(pod_name:QCuPD)->QCuPq:
...
@click.option("-n","--name",help="Name of the cloud pod",required=QCuPg)
def cmd_cpvcs_init(name:QCuPD):
...
@click.option("-n","--name",help="Name of the cloud pod",required=QCuPg)
@click.option("-r","--remote",help="Whether the Pod should also be deleted remotely.",is_flag=QCuPg,default=QCuPG)
def cmd_cpvcs_delete(name:QCuPD,remote:QCuPq):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=QCuPg)
def cmd_cpvcs_register_remote(name:QCuPD):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Current Name of the cloud pod",required=QCuPg)
@click.option("-nn","--new-name",help="New name of the cloud pod",required=QCuPg)
def cmd_cpvcs_rename(name:QCuPD,new_name:QCuPD):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=QCuPg)
def cmd_cpvcs_commit(message:QCuPD,name:QCuPD):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("--squash",is_flag=QCuPg,help="Squashes commits together, so only the latest commit is stored in the revision graph")
@click.option("--three-way",is_flag=QCuPg,default=QCuPG,help="")
@click.option("-m","--message",help="Add a comment describing the version")
@click.option("-n","--name",help="Name of the cloud pod",required=QCuPg)
def cmd_cpvcs_push(squash:QCuPq,message:QCuPD,name:QCuPD,three_way:QCuPq):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=QCuPg)
@click.option("-v","--version",type=QCuPm)
@click.option("-m","--message",required=QCuPG)
def cmd_cpvcs_push_overwrite(version:QCuPm,message:QCuPD,name:QCuPD):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-v","--version",default="-1",type=QCuPm,help="Loads the state of the specified version - Most recent one by default")
@click.option("--reset",is_flag=QCuPg,default=QCuPG,help="Will reset the application state before injecting")
@click.option("-n","--name",help="Name of the cloud pod",required=QCuPg)
def cmd_cpvcs_inject(version:QCuPm,reset:QCuPq,name:QCuPD):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--inject/--no-inject",default=QCuPg,help="Whether the latest version of the pulled pod should be injected")
@click.option("--reset/--no-reset",default=QCuPg,help="Whether the current application state should be reset after the pod has been pulled")
@click.option("-n","--name",help="Name of the cloud pod",required=QCuPg)
@click.option("--lazy/--eager",default=QCuPg,help="Will only fetch references to existing versions, i.e. version state is only downloaded when required")
@cpvcs.command(name="pull",help="Injects the state from a version into the application runtime")
def cmd_cpvcs_pull(name:QCuPD,inject:QCuPq,reset:QCuPq,lazy:QCuPq):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--remote","-r",is_flag=QCuPg,default=QCuPG)
def cmd_cpvcs_list_pods(remote:QCuPq):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=QCuPg)
def cmd_cpvcs_versions(name:QCuPD):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=QCuPg,type=QCuPm)
@click.option("-n","--name",help="Name of the cloud pod",required=QCuPg)
def cmd_cpvcs_version_info(version:QCuPm,name:QCuPD):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",type=QCuPm,default=-1,help="Latest version by default")
@click.option("-n","--name",help="Name of the cloud pod",required=QCuPg)
def cmd_cpvcs_version_metamodel(version:QCuPm,name:QCuPD):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=QCuPg,type=QCuPm,help="The version the state should be set to")
@click.option("--inject/--no-inject",default=QCuPg,help="Whether the state should be directly injected into the application runtime after changing version")
@click.option("--reset/--no-reset",default=QCuPg,help="Whether the current application state should be reset before changing version")
@click.option("--commit-before",is_flag=QCuPG,help="Whether the current application state should be commited to the currently selected version before changing version")
@click.option("-n","--name",help="Name of the cloud pod",required=QCuPg)
def cmd_cpvcs_set_version(version:QCuPm,inject:QCuPq,reset:QCuPq,commit_before:QCuPq,name:QCuPD):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=QCuPg)
def cmd_cpvcs_commits(version:QCuPm,name:QCuPD):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("--version","-v",required=QCuPg)
@click.option("--commit","-c",required=QCuPg)
@click.option("-n","--name",help="Name of the cloud pod",required=QCuPg)
def cmd_cpvcs_commit_diff(version:QCuPm,commit:QCuPm,name:QCuPD):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
def cmd_pod_list():
 ...
 from localstack_ext.bootstrap import pods_client
 try:
  result=pods_client.list_pods(QCuPT)
  status.stop()
...
def print_table(columns:Dict[QCuPD,QCuPD],rows:List[Dict[QCuPD,Any]],formatters:Dict[QCuPD,Callable[[Any],QCuPD]]=QCuPT):
 from rich.table import Table
 if formatters is QCuPT:
  formatters=QCuPt()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  QCuPN(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->QCuPT:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->QCuPT:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->QCuPT:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->QCuPT:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
