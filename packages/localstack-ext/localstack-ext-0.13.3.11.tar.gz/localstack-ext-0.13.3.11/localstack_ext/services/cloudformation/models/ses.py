...
class SESTemplate(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(template or[UwcQm])[0]
 def get_physical_resource_id(self,attribute=UwcQm,**kwargs):
 ...
 def update_resource(self,new_resource,stack_name,resources):
  ...
  if UwcQr(template.get(attr,"")==new_template.get(attr,"")for attr in["SubjectPart","TextPart","HtmlPart"]):
   return
...
class SESReceiptRuleSet(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return UwcQJ(SESReceiptRuleSet,self).get_cfn_attribute(attribute)
 @UwcQF
 def add_defaults(resource,stack_name:UwcQv):
...
class SESReceiptRule(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return UwcQJ(SESReceiptRule,self).get_cfn_attribute(attribute)
 @UwcQh
