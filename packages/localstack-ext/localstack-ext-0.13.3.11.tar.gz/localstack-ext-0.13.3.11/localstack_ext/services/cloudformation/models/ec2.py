...
class EC2ElasticIP(GenericBaseModel):
 ...
 def cloudformation_type():
  return "AWS::EC2::EIP"
 def get_physical_resource_id(self,attribute=vwITG,**kwargs):
...
class SecurityGroupInOrEgress(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  for perm in perms:
   if vwITO(perm["IpProtocol"])!=vwITO(rp["IpProtocol"]):
    continue
 ...
 @vwITU
 def get_vpc_and_name_for_security_group(cls,group_id:vwITO)->Tuple[vwITO,vwITO]:
  client=aws_stack.connect_to_service("ec2")
  groups=client.describe_security_groups(GroupIds=[group_id])["SecurityGroups"]
  vpc_id=groups and groups[0].get("VpcId")or vwITG
  group_name=groups and groups[0]["GroupName"]or vwITG
 ...
 def get_deploy_templates(cls):
  def create_params(params,**kwargs):
   ...
   source_group_id=result.pop("SourceSecurityGroupId",vwITG)
   vpc_id=vwITG
   ...
   dst_group_id=result.pop("DestinationSecurityGroupId",vwITG)
   ...
   if result.get("IpProtocol"):
    result["IpProtocol"]=vwITO(result.get("IpProtocol"))
   description=result.pop("Description",vwITG)
   cidr_ipv6=result.pop("CidrIpv6",vwITG)
