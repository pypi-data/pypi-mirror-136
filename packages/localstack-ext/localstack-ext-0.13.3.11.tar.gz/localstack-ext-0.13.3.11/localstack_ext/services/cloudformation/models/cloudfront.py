...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or beEzk
 @beEzs
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return beEzY(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @beEzR
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if beEzd(origins,beEzi)else origins.get("Items")
  origins_to_create=beEzp([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if beEzd(aliases,beEzi)else aliases.get("Items")
  aliases_to_create=beEzp(aliases)
  for distr in distrs:
   origins=beEzp([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=beEzp(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",beEzS)
   config["Enabled"]=beEzg(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if beEzd(custom_errors,beEzA)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=beEzg(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":beEzS}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=beEzg(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=beEzk):
    parent=config if parent is beEzk else parent
    items=parent.get(key)or[]
    if beEzd(items,beEzA)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":beEzc(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if beEzd(cache_behavs,beEzA)else cache_behavs)
