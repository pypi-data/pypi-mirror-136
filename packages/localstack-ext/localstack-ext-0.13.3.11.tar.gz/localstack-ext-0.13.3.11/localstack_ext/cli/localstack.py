...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return JUeSb if e else JUeSv
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->JUeSo:
...
@click.option("--revert",is_flag=JUeSb,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:JUeSy):
...
def _cpvcs_initialized(pod_name:JUeSK)->JUeSy:
...
@click.option("-n","--name",help="Name of the cloud pod",required=JUeSb)
def cmd_cpvcs_init(name:JUeSK):
...
@click.option("-n","--name",help="Name of the cloud pod",required=JUeSb)
@click.option("-r","--remote",help="Whether the Pod should also be deleted remotely.",is_flag=JUeSb,default=JUeSv)
def cmd_cpvcs_delete(name:JUeSK,remote:JUeSy):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=JUeSb)
def cmd_cpvcs_register_remote(name:JUeSK):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Current Name of the cloud pod",required=JUeSb)
@click.option("-nn","--new-name",help="New name of the cloud pod",required=JUeSb)
def cmd_cpvcs_rename(name:JUeSK,new_name:JUeSK):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=JUeSb)
def cmd_cpvcs_commit(message:JUeSK,name:JUeSK):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("--squash",is_flag=JUeSb,help="Squashes commits together, so only the latest commit is stored in the revision graph")
@click.option("--three-way",is_flag=JUeSb,default=JUeSv,help="")
@click.option("-m","--message",help="Add a comment describing the version")
@click.option("-n","--name",help="Name of the cloud pod",required=JUeSb)
def cmd_cpvcs_push(squash:JUeSy,message:JUeSK,name:JUeSK,three_way:JUeSy):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=JUeSb)
@click.option("-v","--version",type=JUeSA)
@click.option("-m","--message",required=JUeSv)
def cmd_cpvcs_push_overwrite(version:JUeSA,message:JUeSK,name:JUeSK):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-v","--version",default="-1",type=JUeSA,help="Loads the state of the specified version - Most recent one by default")
@click.option("--reset",is_flag=JUeSb,default=JUeSv,help="Will reset the application state before injecting")
@click.option("-n","--name",help="Name of the cloud pod",required=JUeSb)
def cmd_cpvcs_inject(version:JUeSA,reset:JUeSy,name:JUeSK):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--inject/--no-inject",default=JUeSb,help="Whether the latest version of the pulled pod should be injected")
@click.option("--reset/--no-reset",default=JUeSb,help="Whether the current application state should be reset after the pod has been pulled")
@click.option("-n","--name",help="Name of the cloud pod",required=JUeSb)
@click.option("--lazy/--eager",default=JUeSb,help="Will only fetch references to existing versions, i.e. version state is only downloaded when required")
@cpvcs.command(name="pull",help="Injects the state from a version into the application runtime")
def cmd_cpvcs_pull(name:JUeSK,inject:JUeSy,reset:JUeSy,lazy:JUeSy):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--remote","-r",is_flag=JUeSb,default=JUeSv)
def cmd_cpvcs_list_pods(remote:JUeSy):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=JUeSb)
def cmd_cpvcs_versions(name:JUeSK):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=JUeSb,type=JUeSA)
@click.option("-n","--name",help="Name of the cloud pod",required=JUeSb)
def cmd_cpvcs_version_info(version:JUeSA,name:JUeSK):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",type=JUeSA,default=-1,help="Latest version by default")
@click.option("-n","--name",help="Name of the cloud pod",required=JUeSb)
def cmd_cpvcs_version_metamodel(version:JUeSA,name:JUeSK):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=JUeSb,type=JUeSA,help="The version the state should be set to")
@click.option("--inject/--no-inject",default=JUeSb,help="Whether the state should be directly injected into the application runtime after changing version")
@click.option("--reset/--no-reset",default=JUeSb,help="Whether the current application state should be reset before changing version")
@click.option("--commit-before",is_flag=JUeSv,help="Whether the current application state should be commited to the currently selected version before changing version")
@click.option("-n","--name",help="Name of the cloud pod",required=JUeSb)
def cmd_cpvcs_set_version(version:JUeSA,inject:JUeSy,reset:JUeSy,commit_before:JUeSy,name:JUeSK):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=JUeSb)
def cmd_cpvcs_commits(version:JUeSA,name:JUeSK):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("--version","-v",required=JUeSb)
@click.option("--commit","-c",required=JUeSb)
@click.option("-n","--name",help="Name of the cloud pod",required=JUeSb)
def cmd_cpvcs_commit_diff(version:JUeSA,commit:JUeSA,name:JUeSK):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
def cmd_pod_list():
 ...
 from localstack_ext.bootstrap import pods_client
 try:
  result=pods_client.list_pods(JUeSo)
  status.stop()
...
def print_table(columns:Dict[JUeSK,JUeSK],rows:List[Dict[JUeSK,Any]],formatters:Dict[JUeSK,Callable[[Any],JUeSK]]=JUeSo):
 from rich.table import Table
 if formatters is JUeSo:
  formatters=JUeSG()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  JUeSg(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->JUeSo:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->JUeSo:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->JUeSo:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->JUeSo:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
