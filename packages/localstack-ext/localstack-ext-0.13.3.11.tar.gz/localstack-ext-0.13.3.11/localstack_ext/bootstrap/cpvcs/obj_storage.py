...
class StateFileLocator(abc.ABC):
 location="_none_"
 active_instance=wvUyG
 @abc.abstractmethod
 def get_state_file_location_by_key(self,key:wvUyR)->wvUyR:
  pass
 @wvUyg
 def get(cls,requested_file_locator:wvUyR):
...
class ObjectStorageProvider(abc.ABC):
 ...
 active_instance=wvUyG
 @wvUyg
 def get(cls,state_file_locator:wvUyR,requested_storage:wvUyR,serializers:Dict[wvUyR,CPVCSSerializer]):
  ...
  return cls.active_instance
 def __init__(self,state_file_locator:StateFileLocator,serializers:Dict[wvUyR,CPVCSSerializer]):
 ...
 def get_state_file_location_by_key(self,key:wvUyR)->wvUyR:
  pass
 @abc.abstractmethod
 def get_version_by_key(self,key:wvUyR)->Version:
  pass
 @abc.abstractmethod
 def get_revision_by_key(self,key:wvUyR)->Revision:
  pass
 @abc.abstractmethod
 def get_revision_or_version_by_key(self,key:wvUyR)->Union[Revision,Version]:
  pass
 @abc.abstractmethod
 def get_delta_file_by_key(self,key:wvUyR)->wvUyR:
  pass
 @abc.abstractmethod
 def upsert_objects(self,*args:CPVCSObj)->List[wvUyR]:
  pass
 @abc.abstractmethod
 def update_revision_key(self,old_key:wvUyR,new_key:wvUyR,referenced_by_version:wvUyR=wvUyG):
  pass
 @abc.abstractmethod
 def version_exists(self,key:wvUyR)->wvUyx:
  pass
 @abc.abstractmethod
 def merge_remote_into_local_version(self,remote_location:wvUyR,key:wvUyR):
  pass
 @abc.abstractmethod
 def _update_key(self,old_key:wvUyR,new_key:wvUyR,base_path:wvUyR)->wvUyx:
  pass
 @abc.abstractmethod
 def _serialize(self,*args:CPVCSObj)->List[wvUyR]:
  pass
 @abc.abstractmethod
 def _deserialize(self,key_serializer:Dict[wvUyR,wvUyR],remote_location:wvUyR=wvUyG)->List[CPVCSObj]:
  pass
class ObjectStorageLocal(ObjectStorageProvider):
 ...
 def _serialize(self,*args:CPVCSObj)->List[wvUyR]:
  ...
  for cpvcs_obj in args:
   if wvUym(cpvcs_obj,Version):
    hash_refs.append(self.serializers["version"].store_obj(cpvcs_obj))
   elif wvUym(cpvcs_obj,Revision):
    hash_refs.append(self.serializers["revision"].store_obj(cpvcs_obj))
  return hash_refs
 def _deserialize(self,key_serializer:Dict[wvUyR,wvUyR],remote_location:wvUyR=wvUyG)->List[Union[Revision,Version]]:
