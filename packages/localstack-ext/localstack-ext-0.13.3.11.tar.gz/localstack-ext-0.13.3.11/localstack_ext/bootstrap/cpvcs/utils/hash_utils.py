...
def random_hash()->DvVsX:
 return DvVsU(random.getrandbits(160))
def compute_file_hash(file_path:DvVsX,accum=DvVsg)->Optional[DvVsX]:
 try:
  with DvVse(file_path,"rb")as fp:
   if accum:
...
def compute_version_archive_hash(version_no:DvVsI,state_archive:DvVsX)->DvVsX:
 ...
 m.update(DvVsX(version_no).encode("utf-8"))
 rm_rf(tmp_state_dir)
 return m.hexdigest()
def compute_revision_hash(cpvcs_node:Revision)->DvVsX:
 if not cpvcs_node.state_files:
  return random_hash()
 state_file_keys=DvVsE(lambda state_file:state_file.hash_ref,cpvcs_node.state_files)
 m=hashlib.sha1()
 for key in state_file_keys:
  try:
   with DvVse(os.path.join(config_context.get_obj_store_path(),key),"rb")as fp:
    m.update(fp.read())
  except DvVsK as e:
   LOG.warning(f"Failed to open file and compute hash for {key}: {e}")
 if DvVsh(cpvcs_node,Revision):
  m.update(cpvcs_node.rid.encode("utf-8"))
  m.update(DvVsX(cpvcs_node.revision_number).encode("utf-8"))
 elif DvVsh(cpvcs_node,Version):
  m.update(DvVsX(cpvcs_node.version_number).encode("utf-8"))
 return m.hexdigest()
