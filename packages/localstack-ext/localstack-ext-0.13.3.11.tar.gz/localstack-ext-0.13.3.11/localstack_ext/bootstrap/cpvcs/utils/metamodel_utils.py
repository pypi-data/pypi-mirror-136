...
CAPTURE_METAMODEL_SEPARATELY=rYAUV
...
class MetamodelDelta(SubtypesInstanceManager):
 def create_delta_for_state_files(self,sf1:StateFileRef,sf2:StateFileRef):
  ...
  if not sf1:
   backend2=load_persisted_object(config_context.get_obj_file_path(sf2.hash_ref))
   clazz=rYAUc(backend2)
   backend1=_infer_backend_init(clazz,sf2)
  else:
   backend1=load_persisted_object(config_context.get_obj_file_path(sf1.hash_ref))
   clazz=rYAUc(backend1)
   backend2=_infer_backend_init(clazz,sf1)
  diff=self.create_delta_json(backend1,backend2)
  return diff
 def create_delta_log(self,state_from:Set[StateFileRef],state_to:Set[StateFileRef])->rYAUD:
  ...
  def _create_sf_lookup(state_files:Set[StateFileRef])->Dict[rYAUD,StateFileRef]:
  ...
  for state_file_from in state_files_from_regular:
   ...
   if state_file_from.any_congruence(state_files_to_regular):
    ...
    diff_json=self.create_delta_for_state_files(state_file_from,rYAUN)
  ...
  for state_files_to in state_files_to_lookup.values():
   ...
   diff_json=self.create_delta_for_state_files(rYAUN,state_files_to)
...
class CommitMetamodelUtils:
 @rYAUR
 def create_metadata_archive(cls,version:Version,delete_reference:bool=rYAUV,overwrite:bool=rYAUV,metamodels_file:rYAUD=rYAUN):
 ...
 def get_metamodel_delta(cls,prev_metamodel:Dict,this_metamodel:Dict)->Dict:
  ...
  return result
 @rYAUR
 def create_delta_metamodel_file(cls,version:rYAUJ,revision:rYAUJ,store_to_zip=rYAUV)->Tuple[Dict,Dict]:
 ...
 @rYAUR
 def reconstruct_metamodel(cls,version:rYAUJ,revision:rYAUJ=rYAUN)->Dict:
  result={}
  for rev in rYAUx(1,revision+1):
  ...
  return result
 @rYAUR
 def get_version_metamodel(cls,version:rYAUJ,revision:rYAUJ=rYAUN)->Dict:
 ...
 @rYAUR
 def get_commit_diff(cls,version_no:rYAUJ,commit_no:rYAUJ)->Optional[Dict]:
  ...
  return result
def _infer_backend_init(clazz:Type,sf:StateFileRef)->rYAUG:
 if rYAUf(clazz,rYAUz):
  return{}
 constructor=rYAUF(clazz,"__init__",rYAUN)
...
def _create_metamodel_helper(obj,width=25,visited:set=rYAUN):
 if obj is rYAUN:
  return obj
