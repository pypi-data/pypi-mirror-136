...
SENT_EMAILS=[]
EMAIL_BLACKLIST=XNhBs()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",XNhBg(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=XNhBF,from_name=XNhBF,smtp_host=XNhBF,smtp_user=XNhBF,smtp_pass=XNhBF,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if XNhBc(recipients,XNhBK)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=XNhBF):
 ...
 result["From"]=formataddr((XNhBg(Header(from_name,"utf-8")),from_email))
 return result
