...
def install_package(packages:Union[List[wzJUg],wzJUg],api_name:wzJUg):
 if not(is_debian()and in_docker()):
  ...
  return
 if wzJUP(packages,wzJUg):
  packages=packages.split()
 LOG.info("Downloading dependencies for %s API. This may take a while."%api_name)
 with INSTALL_LOCK:
  run(["apt-get","update"],shell=wzJUt)
  run(["apt-get","install","-y","--no-install-recommends"]+packages,shell=wzJUt)
def check_or_install(command:wzJUg,packages:Union[List[wzJUg],wzJUg],api:wzJUg):
...
def setup_ssl_cert():
 ...
 try:
  ...
  try:
   ...
   raise
def download_github_artifact(url:wzJUg,target_file:wzJUg,timeout:int=wzJUj):
 def do_download(url,print_error=wzJUt):
  try:
