...
def api_key_configured()->PEWnL:
 return(PEWnw if os.environ.get("LOCALSTACK_API_KEY")and os.environ.get("LOCALSTACK_API_KEY").strip()else PEWnC)
...
def configure_pro_container(container:LocalstackContainer):
 try:
  ...
  if PEWnJ([is_api_enabled(api)for api in EXTERNAL_PORT_APIS]):
