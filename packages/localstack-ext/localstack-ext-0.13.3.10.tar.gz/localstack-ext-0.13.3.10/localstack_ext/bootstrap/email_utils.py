...
SENT_EMAILS=[]
EMAIL_BLACKLIST=XBWEj()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",XBWEp(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=XBWEo,from_name=XBWEo,smtp_host=XBWEo,smtp_user=XBWEo,smtp_pass=XBWEo,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if XBWEU(recipients,XBWEl)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=XBWEo):
 ...
 result["From"]=formataddr((XBWEp(Header(from_name,"utf-8")),from_email))
 return result
