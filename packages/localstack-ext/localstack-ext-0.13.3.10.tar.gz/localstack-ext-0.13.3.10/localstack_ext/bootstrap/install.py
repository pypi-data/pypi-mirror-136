...
def install_package(packages:Union[List[LIXVj],LIXVj],api_name:LIXVj):
 if not(is_debian()and in_docker()):
  ...
  return
 if LIXVM(packages,LIXVj):
  packages=packages.split()
 LOG.info("Downloading dependencies for %s API. This may take a while."%api_name)
 with INSTALL_LOCK:
  run(["apt-get","update"],shell=LIXVl)
  run(["apt-get","install","-y","--no-install-recommends"]+packages,shell=LIXVl)
def check_or_install(command:LIXVj,packages:Union[List[LIXVj],LIXVj],api:LIXVj):
...
def setup_ssl_cert():
 ...
 try:
  ...
  try:
   ...
   raise
def download_github_artifact(url:LIXVj,target_file:LIXVj,timeout:int=LIXVF):
 def do_download(url,print_error=LIXVl):
  try:
