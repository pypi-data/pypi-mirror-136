...
CAPTURE_METAMODEL_SEPARATELY=mpilf
...
class MetamodelDelta(SubtypesInstanceManager):
 def create_delta_for_state_files(self,sf1:StateFileRef,sf2:StateFileRef):
  ...
  if not sf1:
   backend2=load_persisted_object(config_context.get_obj_file_path(sf2.hash_ref))
   clazz=mpilh(backend2)
   backend1=_infer_backend_init(clazz,sf2)
  else:
   backend1=load_persisted_object(config_context.get_obj_file_path(sf1.hash_ref))
   clazz=mpilh(backend1)
   backend2=_infer_backend_init(clazz,sf1)
  diff=self.create_delta_json(backend1,backend2)
  return diff
 def create_delta_log(self,state_from:Set[StateFileRef],state_to:Set[StateFileRef])->mpilv:
  ...
  def _create_sf_lookup(state_files:Set[StateFileRef])->Dict[mpilv,StateFileRef]:
  ...
  for state_file_from in state_files_from_regular:
   ...
   if state_file_from.any_congruence(state_files_to_regular):
    ...
    diff_json=self.create_delta_for_state_files(state_file_from,mpilt)
  ...
  for state_files_to in state_files_to_lookup.values():
   ...
   diff_json=self.create_delta_for_state_files(mpilt,state_files_to)
...
class CommitMetamodelUtils:
 @mpilT
 def create_metadata_archive(cls,version:Version,delete_reference:bool=mpilf,overwrite:bool=mpilf,metamodels_file:mpilv=mpilt):
 ...
 def get_metamodel_delta(cls,prev_metamodel:Dict,this_metamodel:Dict)->Dict:
  ...
  return result
 @mpilT
 def create_delta_metamodel_file(cls,version:mpilE,revision:mpilE,store_to_zip=mpilf)->Tuple[Dict,Dict]:
 ...
 @mpilT
 def reconstruct_metamodel(cls,version:mpilE,revision:mpilE=mpilt)->Dict:
  result={}
  for rev in mpilN(1,revision+1):
  ...
  return result
 @mpilT
 def get_version_metamodel(cls,version:mpilE,revision:mpilE=mpilt)->Dict:
 ...
 @mpilT
 def get_commit_diff(cls,version_no:mpilE,commit_no:mpilE)->Optional[Dict]:
  ...
  return result
def _infer_backend_init(clazz:Type,sf:StateFileRef)->mpilk:
 if mpilQ(clazz,mpilB):
  return{}
 constructor=mpilJ(clazz,"__init__",mpilt)
...
def _create_metamodel_helper(obj,width=25,visited:set=mpilt):
 if obj is mpilt:
  return obj
