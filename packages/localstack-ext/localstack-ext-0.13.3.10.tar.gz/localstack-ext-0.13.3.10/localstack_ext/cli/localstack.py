...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return mLqsW if e else mLqsS
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->mLqso:
...
@click.option("--revert",is_flag=mLqsW,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:mLqsz):
...
def _cpvcs_initialized(pod_name:mLqsY)->mLqsz:
...
@click.option("-n","--name",help="Name of the cloud pod",required=mLqsW)
def cmd_cpvcs_init(name:mLqsY):
...
@click.option("-n","--name",help="Name of the cloud pod",required=mLqsW)
@click.option("-r","--remote",help="Whether the Pod should also be deleted remotely.",is_flag=mLqsW,default=mLqsS)
def cmd_cpvcs_delete(name:mLqsY,remote:mLqsz):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=mLqsW)
def cmd_cpvcs_register_remote(name:mLqsY):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Current Name of the cloud pod",required=mLqsW)
@click.option("-nn","--new-name",help="New name of the cloud pod",required=mLqsW)
def cmd_cpvcs_rename(name:mLqsY,new_name:mLqsY):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=mLqsW)
def cmd_cpvcs_commit(message:mLqsY,name:mLqsY):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("--squash",is_flag=mLqsW,help="Squashes commits together, so only the latest commit is stored in the revision graph")
@click.option("--three-way",is_flag=mLqsW,default=mLqsS,help="")
@click.option("-m","--message",help="Add a comment describing the version")
@click.option("-n","--name",help="Name of the cloud pod",required=mLqsW)
def cmd_cpvcs_push(squash:mLqsz,message:mLqsY,name:mLqsY,three_way:mLqsz):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=mLqsW)
@click.option("-v","--version",type=mLqsh)
@click.option("-m","--message",required=mLqsS)
def cmd_cpvcs_push_overwrite(version:mLqsh,message:mLqsY,name:mLqsY):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-v","--version",default="-1",type=mLqsh,help="Loads the state of the specified version - Most recent one by default")
@click.option("--reset",is_flag=mLqsW,default=mLqsS,help="Will reset the application state before injecting")
@click.option("-n","--name",help="Name of the cloud pod",required=mLqsW)
def cmd_cpvcs_inject(version:mLqsh,reset:mLqsz,name:mLqsY):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--inject/--no-inject",default=mLqsW,help="Whether the latest version of the pulled pod should be injected")
@click.option("--reset/--no-reset",default=mLqsW,help="Whether the current application state should be reset after the pod has been pulled")
@click.option("-n","--name",help="Name of the cloud pod",required=mLqsW)
@click.option("--lazy/--eager",default=mLqsW,help="Will only fetch references to existing versions, i.e. version state is only downloaded when required")
@cpvcs.command(name="pull",help="Injects the state from a version into the application runtime")
def cmd_cpvcs_pull(name:mLqsY,inject:mLqsz,reset:mLqsz,lazy:mLqsz):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--remote","-r",is_flag=mLqsW,default=mLqsS)
def cmd_cpvcs_list_pods(remote:mLqsz):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=mLqsW)
def cmd_cpvcs_versions(name:mLqsY):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=mLqsW,type=mLqsh)
@click.option("-n","--name",help="Name of the cloud pod",required=mLqsW)
def cmd_cpvcs_version_info(version:mLqsh,name:mLqsY):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",type=mLqsh,default=-1,help="Latest version by default")
@click.option("-n","--name",help="Name of the cloud pod",required=mLqsW)
def cmd_cpvcs_version_metamodel(version:mLqsh,name:mLqsY):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=mLqsW,type=mLqsh,help="The version the state should be set to")
@click.option("--inject/--no-inject",default=mLqsW,help="Whether the state should be directly injected into the application runtime after changing version")
@click.option("--reset/--no-reset",default=mLqsW,help="Whether the current application state should be reset before changing version")
@click.option("--commit-before",is_flag=mLqsS,help="Whether the current application state should be commited to the currently selected version before changing version")
@click.option("-n","--name",help="Name of the cloud pod",required=mLqsW)
def cmd_cpvcs_set_version(version:mLqsh,inject:mLqsz,reset:mLqsz,commit_before:mLqsz,name:mLqsY):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=mLqsW)
def cmd_cpvcs_commits(version:mLqsh,name:mLqsY):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("--version","-v",required=mLqsW)
@click.option("--commit","-c",required=mLqsW)
@click.option("-n","--name",help="Name of the cloud pod",required=mLqsW)
def cmd_cpvcs_commit_diff(version:mLqsh,commit:mLqsh,name:mLqsY):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
def cmd_pod_list():
 ...
 from localstack_ext.bootstrap import pods_client
 try:
  result=pods_client.list_pods(mLqso)
  status.stop()
...
def print_table(columns:Dict[mLqsY,mLqsY],rows:List[Dict[mLqsY,Any]],formatters:Dict[mLqsY,Callable[[Any],mLqsY]]=mLqso):
 from rich.table import Table
 if formatters is mLqso:
  formatters=mLqsU()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  mLqst(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->mLqso:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->mLqso:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->mLqso:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->mLqso:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
