...
class EC2ElasticIP(GenericBaseModel):
 ...
 def cloudformation_type():
  return "AWS::EC2::EIP"
 def get_physical_resource_id(self,attribute=VYbEz,**kwargs):
...
class SecurityGroupInOrEgress(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  for perm in perms:
   if VYbEX(perm["IpProtocol"])!=VYbEX(rp["IpProtocol"]):
    continue
 ...
 @VYbEv
 def get_vpc_and_name_for_security_group(cls,group_id:VYbEX)->Tuple[VYbEX,VYbEX]:
  client=aws_stack.connect_to_service("ec2")
  groups=client.describe_security_groups(GroupIds=[group_id])["SecurityGroups"]
  vpc_id=groups and groups[0].get("VpcId")or VYbEz
  group_name=groups and groups[0]["GroupName"]or VYbEz
 ...
 def get_deploy_templates(cls):
  def create_params(params,**kwargs):
   ...
   source_group_id=result.pop("SourceSecurityGroupId",VYbEz)
   vpc_id=VYbEz
   ...
   dst_group_id=result.pop("DestinationSecurityGroupId",VYbEz)
   ...
   if result.get("IpProtocol"):
    result["IpProtocol"]=VYbEX(result.get("IpProtocol"))
   description=result.pop("Description",VYbEz)
   cidr_ipv6=result.pop("CidrIpv6",VYbEz)
