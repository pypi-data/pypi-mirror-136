...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or xAkRX
 @xAkRd
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return xAkRy(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @xAkRN
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if xAkRa(origins,xAkRg)else origins.get("Items")
  origins_to_create=xAkRf([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if xAkRa(aliases,xAkRg)else aliases.get("Items")
  aliases_to_create=xAkRf(aliases)
  for distr in distrs:
   origins=xAkRf([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=xAkRf(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",xAkRi)
   config["Enabled"]=xAkRK(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if xAkRa(custom_errors,xAkRV)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=xAkRK(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":xAkRi}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=xAkRK(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=xAkRX):
    parent=config if parent is xAkRX else parent
    items=parent.get(key)or[]
    if xAkRa(items,xAkRV)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":xAkRC(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if xAkRa(cache_behavs,xAkRV)else cache_behavs)
