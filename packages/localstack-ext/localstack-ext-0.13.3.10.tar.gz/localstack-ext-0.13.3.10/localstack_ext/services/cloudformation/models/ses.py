...
class SESTemplate(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(template or[uNiVb])[0]
 def get_physical_resource_id(self,attribute=uNiVb,**kwargs):
 ...
 def update_resource(self,new_resource,stack_name,resources):
  ...
  if uNiVE(template.get(attr,"")==new_template.get(attr,"")for attr in["SubjectPart","TextPart","HtmlPart"]):
   return
...
class SESReceiptRuleSet(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return uNiVz(SESReceiptRuleSet,self).get_cfn_attribute(attribute)
 @uNiVd
 def add_defaults(resource,stack_name:uNiVv):
...
class SESReceiptRule(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return uNiVz(SESReceiptRule,self).get_cfn_attribute(attribute)
 @uNiVT
