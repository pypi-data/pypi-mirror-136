...
def api_key_configured()->NaEzw:
 return(NaEzm if os.environ.get("LOCALSTACK_API_KEY")and os.environ.get("LOCALSTACK_API_KEY").strip()else NaEzr)
...
def configure_pro_container(container:LocalstackContainer):
 try:
  ...
  if NaEzP([is_api_enabled(api)for api in EXTERNAL_PORT_APIS]):
   docker_flags+=["-p","{start}-{end}:{start}-{end}".format(start=config.EXTERNAL_SERVICE_PORTS_START,end=config.EXTERNAL_SERVICE_PORTS_END)]
