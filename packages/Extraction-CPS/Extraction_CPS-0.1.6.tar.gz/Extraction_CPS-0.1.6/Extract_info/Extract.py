...
class Extraction:
    ...
    def Extract_all_together(self):
        ...
        Patterns_Abstract = [r"(\\begin{abstract})(?P<Abstract>.+)(\\end{abstract})",
                             r"(\\chapter{\\abstractname})((?:(?!\btableofcontents\b).)*)(\\tableofcontents)",
                             r"(\\chapter{\\abstractname})((?:(?!\bmicrotypesetup\b).)*)(\\microtypesetup)",
                             r"(\\chapter{Abstract})((?:(?!\btableofcontents\b).)*)(\\tableofcontents)",
                             r"(\\section{Abstract})((?:(?!\btableofcontents\b).)*)(\\tableofcontents)",
                             r"(\\RAIstudentthesisAbstract)({(?:(?!\bRAI\b).)*})(\\RAI)",
                             r"(\\section{Goal})((?:(?!\brenewcommand\b).)*)(\\renewcommand)",
                             r"(\\addcontentsline{toc}{chapter}{Abstract})((?:(?!\bcleardoublepage\b).)*)(\\cleardoublepage)",
                             r"(\\addcontentsline{toc}{chapter}{Abstract})((?:(?!\bclearemptydoublepage\b).)*)(\\clearemptydoublepage)",
                             r"(\\RAIstudentthesisAbstract)((?:(?!\bRAI\b).)*)(\\RAI)"]
                ...
                if filepath.endswith(format.lower()) or filepath.endswith(format.upper()):
                    ...
                    for pattern in Patterns_Authors:
                        ...
                        if Authors != []:
                            ...
                            df_all.loc[number_of_students, "Authors"] = Authors[0][2].strip()
                            break
                    for pattern in Patterns_Abstract:
                        ...
                        if Abstract != []:
                            ...
                            elif np.size(Abstract[0][1].split("\\chapter{\\abstractname}")) == 2:
                                df_all.loc[number_of_students, "Abstract_en"] = \
                                Abstract[0][1].split("\\chapter{\\abstractname}")[0].strip()
                                df_all.loc[number_of_students, "Abstract_de"] = \
                                Abstract[0][1].split("\\chapter{\\abstractname}")[1].strip()
                            break
