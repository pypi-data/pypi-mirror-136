...
def un_plug(shortname):
    from .. import asst, Owen_bot

    try:
        all_func = LOADED[shortname]
        for client in [Owen_bot, asst]:
...
async def safeinstall(event):
    ...
    if plug in HELP:
        ...
        for i in HELP[plug]:
            output += i
        output += "\n© @OwenUserbot"
        await eod(ok, f"✓ `Owen - Installed`: `{plug}` ✓\n\n{output}")
...
def gen_chlog(repo, diff):
    ...
    ch_log = tldr_log = ""
    ch = f"<b>Owen {Owen_version} updates for <a href={UPSTREAM_REPO_URL}/tree/{ac_br}>[{ac_br}]</a>:</b>"
    ch_tl = f"Owen {Owen_version} updates for {ac_br}:"
    d_form = "%d/%m/%y || %H:%M"
...
def make_html_telegraph(title, author, text):
    ...
    page = client.post(
        title=title,
        author=author,
        author_url="https://t.me/OwenProjects",
        text=text,
    )
    return page["url"]
...
async def restart(ult):
    if Var.HEROKU_APP_NAME and Var.HEROKU_API:
        ...
        os.execl(sys.executable, sys.executable, "-m", "pyOwen")
...
async def heroku_logs(event):
    ...
    ok = app.get_log()
    with open("Owen-heroku.log", "w") as log:
        log.write(ok)
    await event.client.send_file(
        event.chat_id,
        file="Owen-heroku.log",
        thumb="resources/extras/Owen.jpg",
        caption="**Owen Heroku Logs.**",
    )

    os.remove("Owen-heroku.log")
    await xx.delete()
