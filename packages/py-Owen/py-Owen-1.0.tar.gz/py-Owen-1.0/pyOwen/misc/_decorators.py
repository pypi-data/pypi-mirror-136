...
from ..version import Owen_version as ult_ver
...
def owen_cmd(pattern=None, manager=False, **kwargs):
    ...
    def decor(dec):
        async def wrapp(ult):
            ...
            try:
                ...
                await owen_bot.disconnect()
                await asyncio.sleep(fwerr.seconds + 10)
                await owen_bot.connect()
                await asst.send_message(
                    int(udB.get_key("LOG_CHANNEL")),
                ...
                await asst.send_message(
                    ...
                    buttons=[
                        ...
                        Button.url(
                            "Repl",
                            "https://replit.com/@TeamOwen/OwenStringSession",
                        ),
                    ],
                )
                sys.exit()
                ...
                ftext = (
                    "**Owen Client Error:** `Forward this to` @OwenSupport\n\n"
                )
                ftext += "**Py-Owen Version:** `" + str(pyver)
                ftext += "`\n**Owen Version:** `" + str(ult_ver)
                ftext += "`\n**Telethon Version:** `" + str(telever)
                ftext += f"`\n**Hosted At:** `{HOSTED_ON}`\n\n"
                ftext += "--------START Owen CRASH LOG--------"
                ftext += "\n**Date:** `" + date
                ...
                if len(ftext) > 4096:
                    with BytesIO(ftext.encode()) as file:
                        ...
                        error_log = await asst.send_file(
                            ...
                            file,
                            caption="**Owen Client Error:** `Forward this to` @OwenSupport\n\n",
                        )
                else:
