...
class MongoDB:
    def __init__(self, key):
        ...
        self.db = self.dB.OwenDB
        self.re_cache()

    def __repr__(self):
        return f"<Owen.MonGoDB\n -total_keys: {len(self.keys())}\n>"
...
# for SQL Implementation in Owen.
#
...
class SqlDB:
    def __init__(self, url):
        ...
        try:
            ...
            self._cursor.execute(
                "CREATE TABLE IF NOT EXISTS Owen (OwenCli varchar(70))"
            )
    ...
    def usage(self):
        self._cursor.execute(
            "SELECT pg_size_pretty(pg_relation_size('Owen')) AS size"
        )
    ...
    def keys(self):
        self._cursor.execute(
            "SELECT column_name FROM information_schema.columns WHERE table_schema = 'public' AND table_name  = 'Owen'"
        )  # case sensitive
        data = self._cursor.fetchall()
    ...
    def get(self, variable):
        try:
            self._cursor.execute(f"SELECT {variable} FROM Owen")
    ...
    def set_key(self, key, value):
        try:
            self._cursor.execute(f"ALTER TABLE Owen DROP COLUMN IF EXISTS {key}")
        except (psycopg2.errors.UndefinedColumn, psycopg2.errors.SyntaxError):
            pass
        except BaseException as er:
            LOGS.exception(er)
        self._cursor.execute(f"ALTER TABLE Owen ADD {key} TEXT")
        self._cursor.execute(f"INSERT INTO Owen ({key}) values (%s)", (str(value),))
        return True

    def del_key(self, key):
        try:
            self._cursor.execute(f"ALTER TABLE Owen DROP COLUMN {key}")
        except psycopg2.errors.UndefinedColumn:
            return False
        return True
    ...
    def flushall(self):
        self._cursor.execute("DROP TABLE Owen")
        self._cursor.execute(
            "CREATE TABLE IF NOT EXISTS Owen (OwenCli varchar(70))"
        )
        return True
