...
project_version = ''
project_path = ''
def lab_start_kernel(ctx, connection, registry_name: str, options: DockerRunOptions):
    kernel_image = f"{registry_name}/{project_name}:{project_version}"
    lab_start_kernel_image(ctx, connection, kernel_image, project_name, project_path, options)
