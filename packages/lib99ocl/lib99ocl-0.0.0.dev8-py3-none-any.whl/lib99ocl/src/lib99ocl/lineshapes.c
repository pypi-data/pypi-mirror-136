...
WITHIN_KERNEL ftype
poly1(const double t, const double a, const double b)
{
  return a*t+b;
}
...
WITHIN_KERNEL ftype
poly2(const double t, const double a, const double b, const double c)
{
...
WITHIN_KERNEL ftype
poly3(const double t, const double a, const double b, const double c,
      const double d)
{
...
*/
WITHIN_KERNEL ftype
chebyshev(const ftype _x, const GLOBAL_MEM ftype *t, const int deg,
          const ftype xmin, const ftype xmax)
{
  ...
  ftype sum = 0;
  switch (deg) {
    case  7: sum += t[7] * x * poly3(x2,64,-112,56,-7);
    case  6: sum += t[6] *     poly3(x2,32,-48,18,-1);
    case  5: sum += t[5] * x * poly2(x2,16,-20,5);
    case  4: sum += t[4] *     poly2(x2,8,-8,1);
    case  3: sum += t[3] * x * poly1(x2,4,-3);
    case  2: sum += t[2] *     poly1(x2,2,-1);
    case  1: sum += t[1] * x;
    case  0: sum += t[0] *     1; break;
    default: return Inf; //printf("Higher than 7 poly not implemented\n");
  }
  return sum;
}
...
WITHIN_KERNEL ftype
chebyshevIntegral(const ftype _x, const GLOBAL_MEM ftype *t, const int deg,
                  const ftype xmin, const ftype xmax)
{
  const ftype x = -1. + 2. * (_x-xmin)/(xmax-xmin);
  const ftype x2 = x*x;
  ftype sum = 0;
  switch (deg) {
    case  7: sum += t[7] * x2 * poly3(x2,8.,-112./6.,14.,-7./2.);
    case  6: sum += t[6] * x  * poly3(x2,32./7.,-48./5.,6.,-1.);
    case  5: sum += t[5] * x2 * poly2(x2,16./6.,-5.,2.5);
    case  4: sum += t[4] * x  * poly2(x2,8./5.,-8./3.,1.);
    case  3: sum += t[3] * x2 * poly1(x2,1.,-3./2.);
    case  2: sum += t[2] * x  * poly1(x2,2./3.,-1.);
    case  1: sum += t[1] * x2 * 0.5;
    case  0: sum += t[0] * x; break;
    default: return Inf; //printf("Higher than 7 poly not implemented\n");
  }
