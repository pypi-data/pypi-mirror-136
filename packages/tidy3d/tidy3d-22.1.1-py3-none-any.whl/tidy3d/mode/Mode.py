...
class Mode(object):
    ...
    def fix_efield_phase(self):
        ...
        We only look at the tangential E components because those are the ones
        that ever have a physical importance, and because I got some strange
        results when including the longitudinal E when finding the max.
        """

        ind_max = np.argmax(np.abs(self.E[:2]))
        Emax = self.E[:2].ravel()[ind_max]
        phi = np.angle(Emax)
