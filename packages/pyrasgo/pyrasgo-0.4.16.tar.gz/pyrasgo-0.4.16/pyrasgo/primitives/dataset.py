...
# -----------------------------------------------------
    ...
    def run_stats(self, only_if_data_changed: Optional[bool] = True) -> None:
        ...
        # time_index is the attribute that should hold the target column to generate stats for
        dimension_column_id = self._api_dataset.attributes.get('time_index') if self._api_dataset.attributes else None

        create._dataset_correlation_stats(table_id=table_id, dimension_column_id=dimension_column_id, only_if_data_changed=only_if_data_changed)
