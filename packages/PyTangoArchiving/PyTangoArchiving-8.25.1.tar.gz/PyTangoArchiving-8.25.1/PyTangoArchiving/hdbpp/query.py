...
class HDBppReader(HDBppDB):
    ...
    def get_db_size_per_months(self):
        sizes = fn.defaultdict(int)
        for t in self.get_data_tables():
            parts = self.getTablePartitions(t)
            if parts:
                for p in parts:
                    try:
                        t0 = self.get_partition_time_by_name(p)
                        m = fn.time2str(t0).rsplit('-',1)[0]
                    except:
                        m = 'WRONG_NAME'
                    sizes[m] += self.getPartitionSize(t,p)
            else:
                sizes['NOT_PARTITIONED'] += self.getTableSize(t)
        print('dbsize: %sG vs %sG' % (self.getDbSize()/1e9,sum(sizes.values())/1e9))
        return dict(sizes)
                
                        

    @Cached(expire=300)
    def get_last_partition(self, table, min_size = 1, 
                           add_last = False, tref = None):
        """
        Return the last partition updated (size > min_size)
        Returns None if the table is not partitioned
        
        min_size checks for a min numbers of rows (1 by default)
        
        :params: table, min_size, add_last (not used), 
            tref (last date to be considered)
        """    
        parts = [p[0] for p in self.get_partitions_rows(table,min_size)]
        sparts = self.get_partitions_sorted_by_time(table,parts,tmax=tref)
        ...
        if not sparts:
            ...
            lparts = [p for p in sparts if '_last' not in p or add_last]
            return lparts[-1] if len(lparts) else sparts[-1]
        
    def get_partitions_sorted_by_time(self, table, parts = None, tmax = None):
        tmax = tmax or fn.END_OF_TIME
        parts = parts or self.getTablePartitions(table)
        parts = sorted((self.get_partition_time_by_name(p),p) for p in parts)
        return [p for t,p in parts if t<=tmax]
    
    def get_partitions_by_size(self, table, min_size = MIN_FILE_SIZE,
                max_size = 1e15):
        parts = self.getTablePartitions(table)
        sizes = [(p,self.getPartitionSize(table,p)) for p in parts if p]
        return [(p,s) for p,s in sizes if min_size < s < max_size]
    
    def get_partitions_rows(self, table, min_size = 1):
        rows = [(p,self.getPartitionRows(table,p))
                 for p in self.get_partitions_sorted_by_time(table)]
        return [(p,r) for p,r in rows if r >= min_size]
    
    def get_partitions_to_expire(self):
        """
        returns tables and partitions that need repartitioning
        """
        r = []
        for t in self.get_data_tables():
            if not self.getTableRows():
                continue
            parts = self.getTablePartitions(t)
            if parts:
                used = self.get_last_partition(t,min_size=1) or ''
                last = self.get_last_partition(t,min_size=0,add_last=False)
                if 'last' in used:
                    r.append((t,used,self.getPartitionSize(t,used)))
                elif self.get_partition_time_by_name(last) <= fn.now()+864.e4:
                    r.append((t,last,self.getPartitionSize(t,last)))
            else:
                s = self.getTableSize(t)
                if s > 1e9:
                    r.append((t,t,s))
        return r
    ...
    def get_partition_name_for_date(self, table, date, 
        ...
        day would be one in following: '01', '16', '[0-9][0-9]'
        """
        if not fn.isString(date):
            date = fn.time2str(date)
        p = partition_prefixes.get(table,'')
        date = date.split()[0].split('T')[0]
        p += ''.join(date.split('-')[0:2])
        if day:
            p = p[:-2]+day
        return p
    
    def get_partition_rules(self, table):
        partitions = [l for l in self.getTableCreator(table).split('\n') 
                      if 'PARTITION' in l]
        rexp = "(?:[()])"
        rexp = rexp + '(%s)'%fn.refloat + rexp
        for i,p in enumerate(partitions):
            try:
                m = fn.clsearch(rexp,p)
                m = m and m.groups()
                partitions[i] += ' #%s' % fn.time2str(int(m[0])) if m else ''
            except:
                pass
        return partitions
    ...
    def get_partitions_at_dates(self, table, date1, date2=None):
        ...
        """
        date1 = fn.str2time(date1) if fn.isString(date1) else date1
        if date2:
            date2 = fn.str2time(date2) if fn.isString(date2) else date2
        else:
            date2 = date1
        times = sorted(fn.kmap(self.get_partition_time_by_name,
                               self.getTablePartitions(table)))
        parts = [None]
        #print(times,date1,date2)
        for t in times:
            t,p = t[1],t[0]
            if t <= date1:
                parts[0] = p
    ...
    def get_table_timestamp(self, table, method='max', 
            ...
            use_partitions = True, verbose = False): #, tref = -180*86400):
        """
        ...
        if use_partitions = True, it will search only on min/max partitions
        ...
        if use_partitions:
            parts = self.get_partitions_rows(table)
            if parts:
                if method=='min': 
                    q += 'partition (%s)' % parts[0][0]
                if method =='max':
                    q += 'partition (%s)' % parts[-1][0]               

        for i in ids:
            qi = q+' where att_conf_id=%d' % i
            #if tref and int_time: where += ('int_time <= %d'% (tref))
            if verbose: print('get_table_timestamp(%s):%s' % (table,qi))
