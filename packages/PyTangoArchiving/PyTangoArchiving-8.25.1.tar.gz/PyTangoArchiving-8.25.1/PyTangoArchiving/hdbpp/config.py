...
class HDBppDB(ArchivingDB,SingletonMap):
    ...
    def __init__(self,db_name='',host='',user='',
            ...
            if other:
                ...
                if sch:
                    ...
                    try:
                        ...
                        if self.manager:
                            print('HDBpp(): Getting config from %s' % self.manager)
                            t = HDBppDB.get_db_config(manager=self.manager,db_name=db_name)
                            host,user,passwd = t[1],t[2],t[3]
                    except:
                        print('HDBpp(): Unable to find a valid manager for %s' % db_name)

        self.port = port ##FriendlyDB does not handle MySQL port!?
        user = user or DEFAULT_USER
        passwd = passwd or DEFAULT_PASS
        ...
        try:
            ArchivingDB.__init__(self,db_name,host,user,passwd, #port=port,
    ...
    def get_attribute_freq(self,attribute, from_db = False, n = 10, 
                           epoch = None, errors = False, values = None):
        """
        from_db = False: This method get attribute frequency  (60s period) 
        from its current archiver and divides per 60.
        
        if from_db, it will query n values to calcullate the right frequency,
        using epoch as timestamp of last value and period for the time window
        """
        try:
            vals = []
            if from_db:
                vals = values or self.load_last_values(attribute, n = n,
                        epoch = epoch)[attribute]
                if vals is not None and errors:
                    vals = [v for v in vals if None in v]
                if vals is None or not len(vals):
                    return 0
                return float(n)/max((1e-9,abs(vals[0][0]-vals[-1][0])))
            else:
                attribute = self.is_attribute_archived(attribute)
                archiver = fn.get_device(self.get_attribute_subscriber(attribute),keep=True)
                freqs = archiver.AttributeFailureFreqList if errors else archiver.AttributeRecordFreqList
                freqs = dict(zip(archiver.AttributeList,freqs))
