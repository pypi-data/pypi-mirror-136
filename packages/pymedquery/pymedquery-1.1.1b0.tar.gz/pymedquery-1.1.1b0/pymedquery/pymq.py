...
class PyMedQuery:
    ...
    def __init__(self):
        ...
        self.mh = MinioHandler(
                    access_key=os.environ.get('MQUSER'),
                    secret_key=os.environ.get('MQPWD')
                            )
        self.log = Logger(__name__)

    def _set_extract_params(
                        self, get_all: bool, format_params: dict,
                        sql_file_path: str, project_id: str, limit: Union[int, str],
                        include_mask: bool
                        ):
        ...
        format_params = {'project_id': str(project_id), 'limit': limit, 'sub': sub} if get_all else format_params
