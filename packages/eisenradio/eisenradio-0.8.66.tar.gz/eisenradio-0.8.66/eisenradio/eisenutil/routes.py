...
from eisenradio.eisenutil.eisutil import make_folder, is_in_db_view, remove_blank, get_stream_content_type
from eisenradio.eisenutil.measure import measure_meta, stopped_stations
from eisenradio.eisenutil.tools import delete_all_radios, export_radios, import_radios
from eisenradio.eisenutil.browser_stream import audio_feed, fill_sound_serve
...
@eisenutil_bp.route('/sound/<radio_name>', methods=('GET', 'POST'))
def sound(radio_name):
    ...
    audio_chunks = audio_feed(name)
    rv = Response(audio_chunks, mimetype=content_type)
    return rv
