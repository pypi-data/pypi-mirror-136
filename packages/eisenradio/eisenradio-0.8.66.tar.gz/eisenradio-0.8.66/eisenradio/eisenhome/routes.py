...
@eisenhome_bp.route('/cookie_set_no_visuals', methods=['GET', 'POST'])
def cookie_set_no_visuals():
    resp = make_response("disable visualisation")
    resp.set_cookie('eisen-cookie-visuals', 'no_visuals', max_age=60*60*24*365*2, secure=False, httponly=True)
    ghettoApi.init_ghetto_show_analyser(False)
    return resp


@eisenhome_bp.route('/cookie_get_no_visuals', methods=['GET'])
def cookie_get_no_visuals():
    rv = request.cookies.get('eisen-cookie-visuals', None)
    if rv == 'no_visuals':
        return jsonify({'str_visuals': 'no_visuals'})
    else:
        return jsonify({'str_visuals': '-empty-'})


@eisenhome_bp.route('/cookie_del_no_visuals', methods=['POST'])
def cookie_del_no_visuals():
    resp = make_response("bye\nbye")
...
@eisenhome_bp.route('/is_play_station', methods=['GET'])
def is_play_station():
    current_station, current_table_id = eisen_radio.curr_radio_listen()
    if not current_station:
        current_station = '-empty-'
        current_table_id = '-empty-'

    return jsonify({'is_play_station_id': str(current_table_id), 'is_play_station_name': current_station})
