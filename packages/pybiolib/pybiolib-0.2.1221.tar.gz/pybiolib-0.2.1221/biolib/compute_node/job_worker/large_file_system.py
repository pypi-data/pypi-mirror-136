...
class LargeFileSystem:
    ...
    def _download_and_unzip(self) -> None:
        ...
        if self._is_job_federated:
            try:
                with requests.get(self._lfs_mapping['presigned_download_url'], stream=True) as response:
                    with open(tmp_data_zip_path, 'wb') as file:
                        shutil.copyfileobj(response.raw, file)
            except Exception as error:
                raise LargeFileSystemError('Failed to download Large File System data.zip') from error
