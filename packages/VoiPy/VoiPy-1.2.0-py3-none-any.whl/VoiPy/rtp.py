...
class RTPClient:
    ...
    def start(self) -> None:
        ...
        recv_timer = threading.Timer((2 / 100), self.recv)
    ...
    def write(self, data) -> None:
        ...
        print("self.out_offset 1", len(data))
        self.out_offset += len(data)
        print("self.out_offset 2", self.out_offset)
    ...
    def trans(self, ii=0) -> None:
        ...
        if self.read_sequence > 0 and ii != 3:
        ...
        try:
            if not self.is_hold or ii == 2 or ii == 3:
                self.socket.sendto(packet, (self.out_ip, self.out_port))
            if ii == 1:
                self.socket.sendto(packet, (self.out_ip, self.out_port))
            if self.write_packet is not None:
                print("write packet", self.out_timestamp - self.write_packet)
    ...
    def parse_PCMU(self, packet) -> None:
        ...
        if self.recording:
            self.in_RTPpacket.write(packet.timestamp, data, self.hold_offset)
