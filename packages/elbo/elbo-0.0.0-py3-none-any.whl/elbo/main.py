...
def cli():
    ...
    if is_outdated:
        ...
        logger.warning(f"pip3 install -i https://test.pypi.org/simple/ elbo=={latest_version}")
    pass
...
def ls():
    ...
    print(tabulate.tabulate(records, headers="keys", tablefmt="pretty"))
    print("")
    print("")
...
def ssh(task_id):
    ...
    """
    logger.info(f"SSH into {task_id}...")
    params = {
        'task_id': task_id
    }
    response = __elbo_connector.request(ElboConnector.SHOW_ENDPOINT,
...
def is_elbo_outdated():
    ...
    response = requests.get(f'https://test.pypi.org/pypi/elbo/json')
