...
def _enrich_span(
    ...
    span.set_attribute(SpanAttributes.MESSAGING_DESTINATION, task_destination) if task_destination else None
    ...
    if not hasattr(channel.connection, "params"):
        host = channel.connection._impl.params.host
        span.set_attribute(SpanAttributes.NET_PEER_NAME, host) if host else None
        port = channel.connection._impl.params.port
        span.set_attribute(SpanAttributes.NET_PEER_PORT, port) if port else None
    else:
