...
def extract_inputs(input_csv_dir):
...
def extract_array(input_csv_dir):
    X, Y = extract_inputs(input_csv_dir)
...
def retrain_search(model, input_csv_dir, num_trials, samples, sensitive_param_idx, num_params, input_bounds):
    current_model = model
    ...
    X_original, Y_original = extract_array(input_csv_dir)
    X_retrain, Y_retrain = extract_inputs(input_csv_dir)
    ...
    for i in range(7):
        ...
        retrained_estimate = get_estimate(model, num_trials, samples, sensitive_param_idx, num_params, input_bounds)
...
def retrain_sklearn(input_pkl_dir, improved_pkl_dir, input_csv_dir, num_trials, samples,\
                            sensitive_param_idx, num_params, input_bounds):
