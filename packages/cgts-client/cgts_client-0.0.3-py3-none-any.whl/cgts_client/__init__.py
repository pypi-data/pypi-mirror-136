...
class Engine:
    ...
    def _skip_played_prefix(self, sequence):
        ...
        for played_move in self._played_sequence:
            if not remaining:
                break
            elif played_move.lower() != remaining[0].lower():
                raise ValueError()
            remaining.popleft()
    ...
    def clear_board(self):
        self.communicate('clear_board')
        self._played_sequence = []

    def _ensure_sequence(self, sequence: List[str]):
    ...
    def ensure_sequence(self, sequence: List[str]):
        try:
            self._ensure_sequence(sequence)
        except ValueError:
            self.clear_board()
            self._ensure_sequence(sequence)
...
class Server:
    ...
    def _heartbeat(self) -> Beat:
        ...
        if response.status_code == 204:
            self._logger.debug(f'received status {response.status_code} from {response.url}')
