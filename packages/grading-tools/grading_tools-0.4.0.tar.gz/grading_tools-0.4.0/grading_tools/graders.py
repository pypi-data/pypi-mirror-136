...
class PandasGrader(BaseGrader):
    ...
    def check_series(
        ...
        match_index : bool, default=True
            ...
            answer. If ``False``, both submission and answer are sorted ascending and
            their indices reset.
        ...
        match_name : bool, default=True
            Whether to consider the submission's name when evaluating against answer.

        tolerance: int or float, default=0.0
            For numerical values, what is the maximum allowed
            difference between ``submission`` and ``answer``? If ``tolerance=0.1``,
        ...
        If the Series are put into a ``PandasGrader`` and then ``check_series`` is
        ...
        [submission]:  Int64Index([2, 1, 3], dtype='int64')
        [answer]: Int64Index([1, 2, 3], dtype='int64')

        If we add tolerance and remove requirements for index, index order and name
        ...
        if not isinstance(self.submission, pd.Series):
            raise TypeError(
                f"check_series method can only be used with Series submissions, not {type(self.submission).__name__}."
            )

        if not match_index:
            # Note that there's a sort here that's not in `check_df`
            self.submission = self.submission.sort_values().reset_index(
                drop=True
            )
            self.answer = self.answer.sort_values().reset_index(drop=True)
        ...
        if not match_name:
            self.submission = self.submission.rename("foo")
            self.answer = self.answer.rename("foo")
        ...
        try:
            ...
            if len(self.submission) > 10:
                comment = (
                    "Your Series doesn't match the expected result:\n\n"
                    + str(e)[: str(e).find("\n")]
                )
            else:
                comment = (
...
class SklearnGrader(BaseGrader):
    ...
    def check_model(
        ...
        if check_fitted and not ans_fitted:
            raise NotFittedError(
                "`check_fitted` cannot be set to `True` if answer model is not fitted."
            )

        # Do we need to check if the submission is fitted
        if check_fitted and ans_fitted:
            try:
        ...
        if check_steps and not is_pipeline:
            raise ValueError(
                f"`check_steps` can only be `True` when answer Pipeline, not {type(self.answer).__name__}."
            )

        if check_steps and is_pipeline:
            sub_steps = [s for s in self.submission]
            ans_steps = [s for s in self.answer]
            if len(sub_steps) != len(ans_steps):
                self.update_comment(
                    f"Your model Pipeline should have {len(ans_steps)},",
                    f"not {[s for s in self.submission]}.",
                )
        ...
        if check_hyperparameters:
            ...
            if not g.check_dict(tolerance=tolerance, return_bool=True):
