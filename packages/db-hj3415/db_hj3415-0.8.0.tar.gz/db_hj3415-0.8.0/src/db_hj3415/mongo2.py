...
class MI(Atlas):
    ...
    def get_recent(self) -> Tuple[str, float]:
        ...
        """
        if self.chk_col():
            d = self.client[self.db][self.index].find({'date': {'$exists': True}}).sort('date', pymongo.DESCENDING).next()
            del d['_id']
    ...
    def save(self, mi_dict: dict, index=None) -> bool:
        ...
        try:
            result = self.client[self.db][self.index].insert_one(mi_dict)
        except errors.DuplicateKeyError:
            self.client[self.db][self.index].delete_many({'date': {"$eq": mi_dict['date']}})
            result = self.client[self.db][self.index].insert_one(mi_dict)
...
class Corps(Atlas):
    ...
    def _save_df(self, df: pd.DataFrame) -> bool:
        ...
        if df.empty:
            print('Dataframe is empty..So we will skip saving db..')
            return False
        self.client[self.code][self.page].insert_one({'test':'test','e':'d'})
        self.clear_col()
        result = self.client[self.code][self.page].insert_many(df.to_dict('index'))
        return result.acknowledged

    def _save_list(self, list_data: list) -> bool:
        # 주로 c108, c103, c104에서 사용하는 저장방식
        # delete all documents in a collection
        self.clear_col()
        result = self.client[self.code][self.page].insert_many(list_data)
        return result.acknowledged
...
class C101(Corps):
    ...
    def get_recent(self) -> dict:
        ...
        """
        try:
            d = self.client[self.code][self.page].find({'date': {'$exists': True}}).sort('date', pymongo.DESCENDING).next()
            del d['_id']
...
class C108(Corps):
    ...
    def save(self, c108_data: list) -> bool:
        """

        Args:
            c108_data (list): c108로 저장할 리스트 형식
        Note:
            c108_list 구조\n
            [{'날짜': '21/07/19', '제목': '...',\n
            '작성자': '...', '제공처': '...', \n
            ...
            '내용': '...'},]\n
        """
        return super(C108, self)._save_list(c108_data)
    ...
    def get_recent(self) -> list:
        ...
        try:
            r_date = self.client[self.code][self.page].find({'날짜': {'$exists': True}}).sort('날짜', pymongo.DESCENDING).next()['날짜']
        except StopIteration:
            # 날짜에 해당하는 데이터가 없는 경우
            return []
...
class C104(C1034):
    ...
    def save(self, c104_list: list) -> bool:
        ...
        if self.client[self.code][self.page].find_one({'stamp': {'$lt': time_now - datetime.timedelta(days=.5)}}):
            # delete all documents in a collection
            self.clear_col()
        result = self.client[self.code][self.page].insert_many(c104_list)
        return result.acknowledged
...
if __name__ == "__main__":
    ...
    #Atlas(client)
    data = {'전일종가': {'린드먼아시아': '6500', '삼성스팩2호': '8280', '큐캐피탈': '4085', '엠벤처투자': '305', '나우IB': '14100'},
            '시가총액': {'린드먼아시아': '877.5', '삼성스팩2호': '854.1', '큐캐피탈': '885.5', '엠벤처투자': '835.0', '나우IB': '890.3'},
            '재무상태표': {'린드먼아시아': float('nan'), '삼성스팩2호': float('nan'), '큐캐피탈': float('nan'), '엠벤처투자': float('nan'),
                      '나우IB': float('nan')},
            '자산총계': {'린드먼아시아': '655.6', '삼성스팩2호': '545.1', '큐캐피탈': '1430.3', '엠벤처투자': '1250.3', '나우IB': '681.3'},
            '부채총계': {'린드먼아시아': '167.1', '삼성스팩2호': '225.1', '큐캐피탈': '653.8', '엠벤처투자': '492.4', '나우IB': '198.9'},
            '포괄손익계산서': {'린드먼아시아': float('nan'), '삼성스팩2호': float('nan'), '큐캐피탈': float('nan'), '엠벤처투자': float('nan'),
                        '나우IB': float('nan')},
            '매출액': {'린드먼아시아': '115.5', '삼성스팩2호': '446.1', '큐캐피탈': '643.0', '엠벤처투자': '280.4', '나우IB': '435.5'},
            '영업이익': {'린드먼아시아': '65.5', '삼성스팩2호': '53.2', '큐캐피탈': '-69.5', '엠벤처투자': '-86.3', '나우IB': '58.5'},
            '당기순이익': {'린드먼아시아': '50.8', '삼성스팩2호': '52.0', '큐캐피탈': '-39.3', '엠벤처투자': '-165.2', '나우IB': '44.2'},
            '당기순이익(지배)': {'린드먼아시아': '50.8', '삼성스팩2호': '52.0', '큐캐피탈': '-39.3', '엠벤처투자': '-128.9', '나우IB': '44.2'},
            '수익성지표': {'린드먼아시아': float('nan'), '삼성스팩2호': float('nan'), '큐캐피탈': float('nan'), '엠벤처투자': float('nan'),
                      '나우IB': float('nan')},
            '영업이익률': {'린드먼아시아': '56.68', '삼성스팩2호': '11.92', '큐캐피탈': '-10.81', '엠벤처투자': '-30.78', '나우IB': '13.44'},
            '순이익률': {'린드먼아시아': '43.96', '삼성스팩2호': '11.67', '큐캐피탈': '-6.12', '엠벤처투자': '-58.94', '나우IB': '10.16'},
            'ROE': {'린드먼아시아': '10.79', '삼성스팩2호': '17.73', '큐캐피탈': '-5.62', '엠벤처투자': '-18.00', '나우IB': '9.63'},
            '안정성지표': {'린드먼아시아': float('nan'), '삼성스팩2호': float('nan'), '큐캐피탈': float('nan'), '엠벤처투자': float('nan'),
                      '나우IB': float('nan')},
            '부채비율': {'린드먼아시아': '34.22', '삼성스팩2호': '70.36', '큐캐피탈': '84.21', '엠벤처투자': '64.96', '나우IB': '41.23'},
            '가치지표': {'린드먼아시아': float('nan'), '삼성스팩2호': float('nan'), '큐캐피탈': float('nan'), '엠벤처투자': float('nan'),
                     '나우IB': float('nan')},
            'PER': {'린드먼아시아': '13.96', '삼성스팩2호': '25.67', '큐캐피탈': float('nan'), '엠벤처투자': float('nan'),
                    '나우IB': '21.41'},
            'PBR': {'린드먼아시아': '1.43', '삼성스팩2호': '4.06', '큐캐피탈': '1.19', '엠벤처투자': '1.50', '나우IB': '1.96'},
            '투자의견': {'린드먼아시아': float('nan'), '삼성스팩2호': float('nan'), '큐캐피탈': float('nan'), '엠벤처투자': float('nan'),
                     '나우IB': float('nan')},
            '목표주가': {'린드먼아시아': float('nan'), '삼성스팩2호': float('nan'), '큐캐피탈': float('nan'), '엠벤처투자': float('nan'),
                     '나우IB': float('nan')},
            '재무연월': {'린드먼아시아': '2020/12', '삼성스팩2호': '2020/12', '큐캐피탈': '2020/12', '엠벤처투자': '2020/12',
                     '나우IB': '2020/12'}}
    print(pd.DataFrame(data).to_dict('index'))
    #C106Q(client,'005930').save(pd.DataFrame(data))
