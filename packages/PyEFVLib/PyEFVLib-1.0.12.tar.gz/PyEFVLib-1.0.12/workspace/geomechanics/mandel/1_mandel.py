...
TIME_STEP = 20
stepInTimeSamples = [1, 2]

def geomechanics(problemData, mandelIterativeCycle):
	global LOAD
	...
	def assembleMatrix():
		...
		loadVertex = [vertex for vertex in grid.vertices if vertex.x==1.0 and vertex.y==1.0][0]
		for bCondition in problemData.neumannBoundaries["u_y"]:
			for vertex in bCondition.boundary.vertices:
				if mandelIterativeCycle and bCondition.boundary.name == "North" and not ( vertex.x == 1.0 and vertex.y==1.0 ):
					matrix[vertex.handle+1*numberOfVertices] = np.zeros((1+dimension)*numberOfVertices)
					matrix[vertex.handle+1*numberOfVertices][vertex.handle+1*numberOfVertices] = 1.0
					matrix[vertex.handle+1*numberOfVertices][loadVertex.handle+1*numberOfVertices] = -1.0
	...
	def assembleIndependent():
		...
		for bCondition in problemData.neumannBoundaries["u_y"]:
			for facet in bCondition.boundary.facets:
				for outerFace in facet.outerFaces:
					if mandelIterativeCycle and bCondition.boundary.name == "North" and outerFace.vertex.x == 1.0 and outerFace.vertex.y==1.0:
						independent[outerFace.vertex.handle+(1)*numberOfVertices] += LOAD * np.linalg.norm(outerFace.area.getCoordinates())
					elif mandelIterativeCycle and bCondition.boundary.name == "North" and not ( outerFace.vertex.x == 1.0 and outerFace.vertex.y==1.0 ):
						independent[outerFace.vertex.handle+(1)*numberOfVertices] = 0
					else:
	...
	while not converged:
		if not mandelIterativeCycle:
			...
			for forceIteration in range(10):
				...
				northBoundary = [boundary for boundary in grid.boundaries if boundary.name=="North"][0]
				for facet in northBoundary.facets:
					elementU_yValues = np.array([u_yField[vertex.handle] for vertex in facet.element.vertices])
					elementU_xValues = np.array([u_xField[vertex.handle] for vertex in facet.element.vertices])
					for outerFace in facet.outerFaces:
						localDerivatives = outerFace.facet.element.shape.outerFaceShapeFunctionDerivatives[facet.elementLocalIndex][outerFace.local]
						globalDerivatives = np.matmul(np.linalg.inv(facet.element.getTransposedJacobian(localDerivatives)) , np.transpose(localDerivatives))
						sigma_yy = gradU_y = (2*G+lame) * np.matmul(elementU_yValues, globalDerivatives.T)[1] + (lame) * np.matmul(elementU_xValues, globalDerivatives.T)[0]
						appliedForce -= sigma_yy * np.linalg.norm(outerFace.area.getCoordinates())
				
				LOAD -= (appliedForce - FORCE) / NORTH_AREA
				if abs(appliedForce-FORCE) <= FORCE_CRITERION:
					break
			print(appliedForce)
	...
	return uField[0*numberOfVertices:1*numberOfVertices], uField[1*numberOfVertices:2*numberOfVertices], pField
...
def main():
	boundaryConditionsDict = {
		...
		'u_y': {
			...
			'North': { 'condition' : PyEFVLib.Neumann, 'type' : PyEFVLib.Constant, 'value' : FORCE/NORTH_AREA },
		},
		'p': {
			'InitialValue': 0.0,
			'West': { 'condition' : PyEFVLib.Neumann, 'type' : PyEFVLib.Constant, 'value' : 0.0 },
	...
	kwargs = {
		...
		"outputFilePath": ".",
		"numericalSettings": PyEFVLib.NumericalSettings( timeStep = TIME_STEP, finalTime = 100, maxNumberOfIterations = 600 ),
	...
	uField, vField, pField = geomechanics( PyEFVLib.ProblemData(**kwargs), False )

	boundaryConditionsDict["p"]["East"]["condition"] = PyEFVLib.Dirichlet
	boundaryConditionsDict["u_x"]["InitialValue"] = uField
	boundaryConditionsDict["u_y"]["InitialValue"] = vField
	boundaryConditionsDict["p"]["InitialValue"] = pField
	kwargs["boundaryConditions"] = PyEFVLib.BoundaryConditions(boundaryConditionsDict)

	uField, vField, pField = geomechanics( PyEFVLib.ProblemData(**kwargs), True )

	X,Y = zip(*[(v.x, v.y) for v in PyEFVLib.ProblemData(**kwargs).grid.vertices])	

	import matplotlib.pyplot as plt

	x,v = zip(*sorted([(x,v) for v,x,y in zip(vField, X, Y) if y==1.0]))
...
def main_plot():
	...
	mandel = Solution(LENGTH, HEIGHT, FORCE/NORTH_AREA, solid, fluid)
	...
	times_1 = [N*TIME_STEP for N in stepInTimeSamples]
	...
	times_n = TIME_STEP * np.arange(1,numberOfTimeSteps+1)
