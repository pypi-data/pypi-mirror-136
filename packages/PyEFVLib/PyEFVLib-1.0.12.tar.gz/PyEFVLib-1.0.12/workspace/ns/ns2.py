...
def navierStokes(problemData):
	...
	axs[0,0].plot(*zip(*[(U[vtx.handle], vtx.y) for vtx in grid.vertices if vtx.x==1.5]),color='k')
	axs[0,0].set(title="u [m/s] at x=1.5", ylabel="y [m]")
	axs[0,1].plot(*zip(*[(U[vtx.handle], vtx.y) for vtx in grid.vertices if vtx.x==0.0]),color='k')
	axs[0,1].set(title="u [m/s] at x=0", ylabel="y [m]")
	axs[1,1].plot(*zip(*[(U[vtx.handle], vtx.y) for vtx in grid.vertices if vtx.x==3.0]),color='k')
	axs[1,1].set(title="u [m/s] at x=3", ylabel="y [m]")
	plt.show()

	fig, axs = plt.subplots(2,2)
	axs[0,0].plot(*zip(*[(P[vtx.handle]/1e3, vtx.y) for vtx in grid.vertices if vtx.x==1.5]),color='k')
	axs[0,0].set(title="p [kPa] at x=1.5", ylabel="y [m]")
	axs[0,1].plot(*zip(*[(P[vtx.handle]/1e3, vtx.y) for vtx in grid.vertices if vtx.x==0.0]),color='k')
	axs[0,1].set(title="p [kPa] at x=0", ylabel="y [m]")
	axs[1,1].plot(*zip(*[(P[vtx.handle]/1e3, vtx.y) for vtx in grid.vertices if vtx.x==3.0]),color='k')
	axs[1,1].set(title="p [kPa] at x=3", ylabel="y [m]")
	axs[1,0].plot(*zip(*[(vtx.x, P[vtx.handle]/1e3) for vtx in grid.vertices if vtx.y==0.5]),color='k')
