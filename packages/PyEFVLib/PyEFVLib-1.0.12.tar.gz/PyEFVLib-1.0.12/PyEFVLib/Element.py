...
class Element:
	...
	def __buildInnerFaces(self):
		for local in range(self.shape.numberOfInnerFaces):
			innerFace = InnerFace(self, self.grid.numberOfInnerFaces + local, local)
			innerFace.area = self.shape.getInnerFaceAreaVector(local, self.centroid, self.vertices)

			self.innerFaces.append(innerFace)
			self.faces.append(innerFace)

		self.grid.numberOfInnerFaces += self.shape.numberOfInnerFaces
