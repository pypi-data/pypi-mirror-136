...
def get_files(search_path=False, **kwargs):
    ...
    if isinstance(data_include, (str)):
        data_include = [data_include]
        # if data_include is provided and include_meta_data is True
        if include_meta_data is True and len(data_include) > 0:
            # Confirm that extension is in data_include, if not, add it.
            # The extension is required to gather the data.
