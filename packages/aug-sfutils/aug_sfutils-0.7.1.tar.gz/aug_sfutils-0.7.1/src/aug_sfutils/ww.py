...
class WW:
    ...
    def SetObject(self, signame, data, tim_axis=0):
...
#                stride  = ct.c_uint32(slen)
    ...
    def SetAreabase(self, areaname, data, sizes=None, tim_axis=0):
        ...
        if hasattr(self, '_diaref'):
            ...
            if data.ndim > 2:
                logger.error('AB %s hat %d dims: only a time and a space dependence are allowed, flatten the AB space-data', areaname, data.ndim)
                return
            ...
            if tim_axis is None:
                ...
                nt = data.shape[tim_axis]
                if sizes is None:
                    csizes[:data.ndim-1] = np.delete(data.shape, tim_axis)[:data.ndim-1]
                else:
                    nx = len(sizes)
                    csizes[:nx] = sizes[:nx]
