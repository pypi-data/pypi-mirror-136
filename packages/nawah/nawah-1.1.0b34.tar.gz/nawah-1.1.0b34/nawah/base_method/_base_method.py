        ...
        *,
        ...
        if Event.ARGS not in skip_events:
            try:
                await _validate_args(
                    args=query, args_list_label='query', args_list=self.query_args
                )
            ...
            try:
                await _validate_args(
                    args=doc, args_list_label='doc', args_list=self.doc_args
                )
            except InvalidCallArgsException as e:
                test_doc = e.args[0]
        ...
        try:
            ...
            if self.watch_method:
                ...
                try:
                    method_params = {}
                    if 'skip_events' in inspect.signature(method).parameters: method_params['skip_events'] = skip_events
                    if 'env' in inspect.signature(method).parameters: method_params['env'] = env
                    if 'query' in inspect.signature(method).parameters: method_params['query'] = query
                    if 'doc' in inspect.signature(method).parameters: method_params['doc'] = doc
