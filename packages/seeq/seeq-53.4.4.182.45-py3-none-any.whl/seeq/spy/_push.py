    ...
    if data is not None:
        ...
        if item_type == 'Signal':
            for column in data:
                try:
                    ...
                    status.add_job(status_index,
                                   (_push_signal, session, column, signal_metadata, data, signal_output,
            ...
            try:
                ...
                status.add_job(0,
                               (_push_condition, session, condition_metadata, data, 0, status),
                               _on_success)
        ...
        if len([w for w in workbooks_to_push if isinstance(w, spy.workbooks.Analysis)]) == 0:
            ...
            if worksheet is not None:
                ...
                _auto_populate_worksheet(sample_stats.earliest_sample_in_ms, sample_stats.latest_sample_in_ms,
                                         push_result_df, primary_worksheet)
            else:
...
def _push_signal(session: Session, column, signal_metadata, data, signal_output, type_mismatches, status_index,
                 status: Status):
    signals_api = SignalsApi(session.client)
...
def _push_condition(session: Session, condition_metadata, data, status_index, status: Status):
