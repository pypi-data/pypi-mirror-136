...
def dados_hist_fundos(dataini, datafim, cnpj, campos, output):
    ...
    if output == 'dict':
        ...
        for a in range(0, len(dictbase[campos[0]])):
            line1 = ''
            for b in dictbase:
                line1 += f'{dictbase[b][a]};'
            print(line1[:-1].replace('.',','))

        sys.stdout = orig_stdout
        f.close()
    print("Dados históricos atualizado com sucesso.")
...
def lamina(dataref, cnpj, caminhoexportar, bench):
    ...
    print(f"Lamina do cnpj {cnpj} atualizada com sucesso.")
    return dictlamina
