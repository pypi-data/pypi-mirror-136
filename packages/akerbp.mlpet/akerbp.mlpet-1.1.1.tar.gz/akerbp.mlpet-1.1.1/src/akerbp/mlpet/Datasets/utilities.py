...
def get_well_metadata(
    ...
    for well in well_names:
        try:
            ...
            if len(asset) == 0:
                ...
                # well name. Trust the fuzzy searches top result as a match with the well
                # is this safe?
                ...
                asset = client.assets.list(name=wellName)
            if len(asset) > 1:
                ...
                # In this case find the entry which has the most matches in
                # metadata keys that we want
    ...
    # Helper function/class for apply operation
    class InputValueError(ValueError):
        pass

    def _apply_function(x: pd.Series) -> Any:
        unique = x.dropna().unique()
        msg = (
            "At least two unique values were found for the same metadata field!"
            f" Problematic well {x.name}: \n {x.to_string()}"
        )
        # Check for multiple unique values per well (float & string)
        try:
            unique = unique.astype(float)
            if not np.all(np.isclose(unique, unique[0])):
                raise InputValueError(msg)
        except (ValueError, IndexError) as e:
            if len(unique) > 1:
                raise InputValueError(msg) from e
        # Group
        if len(unique) == 0:
            return np.nan
        else:
    ...
    for mapping_name, mapping in map_similar_keys.items():
        ...
        if len(idx) == 0:  # No metadata matching this mapping
            ...
            values = cdf_meta.loc[idx].apply(_apply_function, axis=0)
