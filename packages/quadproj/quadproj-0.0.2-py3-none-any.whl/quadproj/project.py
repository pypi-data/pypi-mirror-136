...
def get_KKT_point_root(Q, x0_not_std, flag_get_all_KKT=False):
    ...
    if F.e1 != -np.inf:
        ...
        print(F.inv_I_lA(mu_star), mu_star)

        x_star_not_std = F.x_not_std(mu_star)
        print(F.interval, mu_star, 'star')
        print('root', F.f(mu_star))
...
def my_newton(F, mu_0):
    ...
    while (output.iteration < iteration_max and output.rtol > eps_rtol and
        ...
        print('Interval', F.interval, x)
        print('Interval', output.is_in_interval(F.interval))
        output.fx = F.f(x)
        print('f(x) = {0}', F.f(x))
        output.dfx = F.d_f(x)
        ...
        output.iteration += 1
        print('output', output.iteration < iteration_max, output.rtol > eps_rtol, output.xtol> eps_xtol)
        print(output.rtol)
        print(output.is_in_interval(F.interval), output.root)
