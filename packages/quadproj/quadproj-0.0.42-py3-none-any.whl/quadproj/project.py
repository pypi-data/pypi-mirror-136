...
def project(Q, x0_not_std, flag_get_all_KKT=False):
    ...
    mu_star, x_star = get_KKT_point_root(Q, x0_not_std, flag_get_all_KKT=False)
    Xd = []
    F = fun.Fun(Q, x0)
...
def plot_x0_x_project(Q, x0, x_project, flag_circle=False):
    fig, ax = Q.plot()
    ax.scatter(x0[0], x0[1], color='black', label=r'$x_0$')
    ax.scatter(x_project[0], x_project[1], color='red', zorder=2,
               label=r'$\mathrm{P}_{\mathcal{Q}} (x_0)$')
    ax.legend()
