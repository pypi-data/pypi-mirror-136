...
class _Domain:
    ...
    def merge(self, messages: Dict[str, str]):
        """ Merges this dictionary with the another one, taking the ``messages``
    ...
    def merge_with(self, merging_domain: '_Domain'):
        """ Merges this dictionary with the another one of ``_Domain`` type. """
        self.messages.update(merging_domain.messages)
        self.messages_lc.update(merging_domain.messages_lc)

    @property
    def as_dict(self) -> Dict[str, str]:
        """ Returns this dictionary domain as ``dict`` with keys representing
        untranslated texts and values representing corresponding translated
        ones.
        """
