...
def density(df, T = 298.15):
    ...
    """
    if os.path.isfile('./empirical_parameters.xlsx') == True:
        pass
    else:
        print("Empirical parameters cannot be found.")
        files = [f for f in os.listdir('.') if os.path.isfile(f)]
        for f in files:
            print(f)
            
    empirical_para = pd.read_excel("./empirical_parameters.xlsx")
    with open('./density_temp.pickle', 'rb') as f:
        clf = pickle.load(f)
