...
def dielectric_constant(T = 298.15):
    ...
    """
    dielectric_path = pkg_resources.resource_filename('pySolution', 'core/dielectric_model_h2o.pickle')
    
    with open(dielectric_path, 'rb') as f:
        clf = pickle.load(f)
...
def F_calc(test_df, A_):
    ...
    #mixing_para = pd.read_excel(empirical_path)
    b = 1.2
