...
class ThreadWorker():
    ...
    def __init__(self, func, name="", on_abort=None, enable_keyboard_interrupt=True):
        if name:
            if name in ThreadWorkerManager.allWorkers.keys():
                name = name+str(ThreadWorkerManager.counts)
        else:
            name = 'worker'+str(ThreadWorkerManager.counts)
        self.name = name
    ...
    def __call__(self, *args: Any, **kwargs: Any) -> Any:
        ...
        ThreadWorkerManager.allWorkers[self.name] = self
    ...
    def work_time(self):
        if self.thread and not self.finished:
            self.__work_time = round(time.perf_counter()-self.start_time, 2)
        ...
        if margs:
            if type(margs[0]) == FunctionType:
                def register(*args,**kargs):
                    workerName = 'worker'+str(ThreadWorkerManager.counts)
                    ThreadWorkerManager.allWorkers[workerName] = ThreadWorker(
						lambda: margs[0](*args,**kargs),
						workerName, 
						on_abort, interrupt)
                    ThreadWorkerManager.allWorkers[workerName].work()
                    ThreadWorkerManager.counts += 1
                    return ThreadWorkerManager.allWorkers[workerName]
                return register
            elif type(margs[0]) == str:
                def applying(func):
                    def register(*args,**kargs):
                        workerName = margs[0] if margs[0] not in ThreadWorkerManager.allWorkers.keys() else margs[0]+str(ThreadWorkerManager.counts)
                        ThreadWorkerManager.allWorkers[workerName] = ThreadWorker(
...
def worker(
        *args,
        ...
        enable_keyboard_interrupt: Optional[bool] = True,
        **kargs
    ):
    """
    ...
    if args:
        if type(args[0]) == FunctionType:
            return ThreadWorker(args[0], name, on_abort, enable_keyboard_interrupt)
        elif type(args[0]) == str:
            return ThreadWorkerManager.create_worker(args[0], *args[1:], on_abort=on_abort, keyboard_interrupt=enable_keyboard_interrupt, **kargs)
    else:
        return ThreadWorkerManager.create_worker(name, on_abort=on_abort, keyboard_interrupt=enable_keyboard_interrupt, **kargs)
