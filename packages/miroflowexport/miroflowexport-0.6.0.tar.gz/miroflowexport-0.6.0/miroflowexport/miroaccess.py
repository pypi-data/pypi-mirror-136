...
class MiroAccess:
    ...
    def export_board_to_excel(self, path = "tmp/excel.xlsx", source_path = None):
        ...
        if source_path:
            (success, old_tasks_dict) = mac_exporter.tasks_from_excel(self._log, source_path)
            if not success:
                self._log.error("Loading existing tasks from {} failed. I will continue without using existing Excel data.".format(source_path))
            else:
                success, tasks_dict = mac_exporter.update_tasks(self._log, tasks_dict, old_tasks_dict)
                if not success:
