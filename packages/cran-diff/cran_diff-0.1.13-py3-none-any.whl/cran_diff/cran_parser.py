...
#        self.soft_update()
    ...
    def soft_insert(self, data_vals):
        ...
        names = [data['name'] for data in data_vals]
        ...
        updated = [data['name'] for data in data_vals]
        updated = list(set(updated))   # remove duplicates
        for package in updated:
            if not os.path.exists(self.JSON_path + '/' + package):
                os.makedirs(self.JSON_path + '/' + package)
            versions = [data['version'] for data in data_vals if data['name'] == package]
            versions_json = self.JSON_path + '/' + package + '/versions.json'
            if os.path.exists(versions_json):
                with open(versions_json, 'r') as f:
                    current = json.load(f)
                versions.extend(current)
            with open(versions_json, 'w+') as f:
                f.write(json.dumps(sorted(versions, reverse = True)))

        # Store imports, suggests, exports and news of each version
