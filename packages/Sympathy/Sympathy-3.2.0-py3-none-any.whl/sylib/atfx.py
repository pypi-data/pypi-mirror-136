...
def _atfxroot_to_ctx(atfxroot):
    applmodel = atfxroot.find(
        _ns(atfxroot, 'application_model'))
    if applmodel is None:
        raise ATFX_ImportFailed('missing application model')
    applelems = applmodel.findall(
...
class ATFX_Error(Exception):
    pass


class ATFX_ImportFailed(ATFX_Error):
    pass
...
def _filename_to_ctx(filename):
    ...
    with open(filename, 'rb') as atfx:
        ...
        if atfxroot is None:
            raise ATFX_ImportFailed('no document')
