...
class SectionsTree:
    ...
    def get_leaf_sections_names(self, section_name="Техническое задание") -> list:
        """
        Возвращает список имён листовых элементов дерева разделов относительно корневого раздела.
        Если явно указан параметр `section_name`, то возвращается список имён листовых элементов относительно
        одноимённого раздела.
        """
        return [node.name for node in self.get_leaf_sections(section_name)]
