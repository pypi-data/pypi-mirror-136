...
from .request import longpoll_get, default_session
...
class LP:
    ...
    async def check(self):
        if self._session is None and default_session is None:
    ...
    async def start(self) -> None:
        warn(DeprecationWarning(
            'Метод "start" устарел и будет удален в будущих версиях'))
