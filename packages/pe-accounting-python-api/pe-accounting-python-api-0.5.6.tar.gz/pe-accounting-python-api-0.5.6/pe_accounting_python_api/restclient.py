...
class PeRestClient:
    ...
    def users(self, include_inactive: bool = False, as_dict: bool = False) -> List[dict]:
    ...
    def employment_contracts(self, *, as_dict: bool = False):
        all_contracts = self.http_request(verb="GET", path=f"payroll/user")["payroll-user-readables"]
        if as_dict:
