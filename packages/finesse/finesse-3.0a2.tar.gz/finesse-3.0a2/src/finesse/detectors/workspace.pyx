...
cdef class DetectorWorkspace(ElementWorkspace):
    ...
    def __init__(
        ...
        if not (needs_noise or needs_signal or needs_carrier or needs_modal_update):
            raise Exception(f"Detector workspace {self} has needs_noise or needs_signal or needs_carrier or needs_modal_update == False, so no calculations will be done.")
