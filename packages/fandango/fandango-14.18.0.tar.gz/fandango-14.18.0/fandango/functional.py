    ...
    try:
        ...
        try:
            for i in range(1,MAX+1):
                n = seq.next() #will except when reaching the end
            if i>(MAX-1):
...
def inCl(exp,seq,regexp=True):
    ...
    """
    if isString(seq):
        return (searchCl(exp,seq) if (regexp and isRegexp(exp)) 
...
def isNaN(arg):
    return (isinstance(arg,float) and math.isnan(arg) or 
                (isString(arg) and lowstr(arg) == 'nan'))
...
def isDate(seq,cad=''):
    try:
        return seq and str2time(seq,cad=cad)
    except:
        return False
...
def isSequence(seq,iterators= True,**kwargs):
    ...
    unless include_generators=False is set as argument
    
    if include_generators is False and iterators is False only fixed-length objects are accepted
    """
    if kwargs.get('INCLUDE_GENERATORS',False):
        iterators = True
...
def evalF(formula):
    ...
    return (lambda *args: 
        eval(c,{'args':args,'x':args and args[0],
            'y':len(args)>1 and args[1],'z':len(args)>2 and args[2]}))
