...
def rest_url_from_arrow_url(url: str) -> str:
    """get arrow url from rest url"""
    parsed_url = urllib.parse.urlsplit(url)
    host = parsed_url.netloc.split(":")[0]

    scheme_translation = {"grpc+tls": "https", "grpc": "http"}
    port_translation = {"grpc+tls": "443", "grpc": "8080"}

    new_scheme = scheme_translation.get(parsed_url.scheme, "https")
    new_port = port_translation.get(parsed_url.scheme, "443")

    return parsed_url._replace(
        netloc=host + ":" + new_port, scheme=new_scheme
    ).geturl()
