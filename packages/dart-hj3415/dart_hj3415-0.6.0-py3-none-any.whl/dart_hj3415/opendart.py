...
class DartInfo:
    def __init__(self,
        ...
        if self.__code != '' and self.__rdt != '':
            self.유통주식 = self.유통주식계산(code=self.__code, date=self.__rdt)
        else:
            self.유통주식 = None
    ...
    def 유통주식계산(code: str, date: str) -> float:
        ...
        logger.debug(pprint.pformat(f'c101 : {c101_dict}', width=80))
    ...
    def rdt(self, rdt):
        if utils.isYmd(rdt):
            ...
            raise ValueError(f'Invalid date - {rdt}')
...
class Dart:
    ...
    def make_dartinfos(self, sdate: str = '', edate: str = '', code: str = '', title: str = '', filter_needless_title: bool = True) -> list:
        ...
        if status == '000':
            ...
            df = self._make_df(full_url=full_url, filter_needless_title=filter_needless_title, title=title)
            logger.debug(df)
    ...
    @staticmethod
    def _make_df(full_url: str, filter_needless_title: bool, title: str = '') -> pd.DataFrame:
        ...
        if filter_needless_title:
            ...
            print(f'Filtered unnecessary words : total {len(refined_df)} items')
        else:
