...
ROOT_MACROS = \
{
    ...
    "    template<typename T>\n"
    "    std::vector<T> GetBinErrorArray(TH1* h)\n"
    "    {\n"
    "        const size_t n_bin = h->GetNbinsX();\n"
    "        std::vector<T> result;\n"
    "        result.reserve(n_bin);\n"
    "        for (size_t bin_index = 1; bin_index <= n_bin; bin_index++)\n"
    "            result.push_back(h->GetBinError(bin_index));\n"
    "        return result;\n"
    "    }\n"
    "    \n"
    "    template<typename T>\n"
    "    std::vector<T> GetBinCenterArray(TH1* h)\n"
    "    {\n"
    "        const size_t n_bin = h->GetNbinsX();\n"
    "        std::vector<T> result;\n"
    "        result.reserve(n_bin);\n"
    ...
    "\n"
    "    template<typename T>\n"
    "    std::vector<T> GetBinContentArray(TH1* h)\n"
    "    {\n"
    "        const size_t n_bin = h->GetNbinsX();\n"
    "        std::vector<T> result;\n"
    "        result.reserve(n_bin);\n"
    ...
    "    \n"
    "    template<typename T>\n"
    "    std::vector<T> GetBinWidthArray(TH1* h)\n"
    "    {\n"
    "        const size_t n_bin = h->GetNbinsX();\n"
    "        std::vector<T> result;\n"
    "        result.reserve(n_bin);\n"
    ...
    "    \n"
    "    template<typename T>\n"
    "    std::vector<T> GetBinLowEdgeArray(TH1* h)\n"
    "    {\n"
    "        const size_t n_bin = h->GetNbinsX();\n"
    "        std::vector<T> result;\n"
    "        result.reserve(n_bin);\n"
