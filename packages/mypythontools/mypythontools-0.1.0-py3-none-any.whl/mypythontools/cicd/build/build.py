...
"""
    ...
    try:
        subprocess.run(" ".join(command_list), check=True, cwd=PROJECT_PATHS.root.as_posix(), shell=True)

    except (Exception,):
        mylogging.traceback(
            "Build with pyinstaller failed. First, check if 'pyinstaller' is installed. "
            "Check it with pip list in used python interpreter. "
            f" Try (if windows, use cmd) \n\n\t{' '.join(command_list)}\n\n in folder "
            "'{PROJECT_PATHS.root.as_posix()}'.\n\n"
            "Troubleshooting: If there are still errors, try to install newset pyinstaller locally with "
