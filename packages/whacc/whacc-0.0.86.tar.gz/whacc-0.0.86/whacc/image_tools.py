...
class h5_iterative_creator():
    ...
    def check_images_uint8(self, images):
        if not self.ignore_image_range_warning:
            ...
            if min_img < 0 or max_img > 255:
                warnings.warn('image data must be uint8 compatible, 0 to 255, but given range is '+ str(min_img)+ ' to '+ str(max_img))
            if -1 <= min_img <= 1 and -1 <= max_img <= 1:
                warnings.warn('image data must be uint8 compatible, 0 to 255, but given range is '+ str(min_img)+ ' to '+ str(max_img))
                warnings.warn('it seems your values are may be formatted between -1 and 1')
            if 0 <= min_img <= 1 and 0 <= max_img <= 1:
                warnings.warn('image data must be uint8 compatible, 0 to 255, but given range is '+ str(min_img)+ ' to '+ str(max_img))
                warnings.warn('it seems your values are may be formatted between 0 and 1')
    ...
    def _create_h5(self, images, labels):
        ...
        self.hf_file.create_dataset('labels',
                                    np.shape(labels),
                                    h5py.h5t.STD_I32LE,  # ....... commented this out because we may want floats....
                                    maxshape=(None,),
                                    chunks=True,
                                    data=labels)
    ...
    def image_transform(self, raw_X):
        ...
        if self.IMG_SIZE is not None:
            if len(raw_X.shape) <=4:
                ...
                new_img = np.zeros(s).astype('uint8')
                for lstm_i in range(raw_X.shape[1]):
                    new_img[:, lstm_i, ...] = tf.image.resize(rgb_tensor[:, lstm_i, ...], (self.IMG_SIZE, self.IMG_SIZE))
                rgb_tensor = new_img
            else:
