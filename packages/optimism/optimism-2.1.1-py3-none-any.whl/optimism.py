...
case.checkOutputLines('The message is:', '-hello-') # line 22
...
case.checkResult(('Name', 'thirty')) # line 34
case.checkOutputLines(
...
- The `TestManager.case` method establishes a test case by specifying
  ...
  are needed.
- The `TestCase.checkResult` and `TestCase.checkOutputLines`, and/or
  `TestCase.checkCustom` can be used to run checks for the result and/or
  output of a test case.
- Normally, output printed during tests is hidden, but `showOutput` can
  be used to show the output that's being captured instead.
...
is suppressed. Control this using the `showOutput` function.
"""
...
class TestCase:
    ...
    def __init__(self, manager, args=None, kwargs=None):
        ...
        # Whether to echo captured outputs (overrides global)
        self.echo = None

        # Location and tag for test case creation
        self.location = get_my_location()
        self.tag = f"{self.location['file']}:{self.location['line']}"
    ...
    def showOutput(self, show=True):
        """
        Overrides the global `showOutput` setting for this test. Use
        None as the parameter to remove the override.
        """
        self.echo = show
    ...
    def run(self):
        ...
        the first time an expectation method like `TestCase.checkResult`
        is used, but the cached value will be re-used for subsequent
        expectations, unless you manually call this method again.
    ...
    def checkResult(self, expectedValue):
        """
        ...
        if self.manager.category in ("file", "block"):
            raise ValueError(
                "You cannot use checkResult with a file or block test"
                " because those tests do not have result values. Did"
                " you mean to use checkOutputLines instead?"
            )
    ...
    def checkOutputLines(self, *expectedLines):
        """
        ...
        if results["error"] is not None:
            ...
            # We produced output, so check it
...
class CapturingStream(io.StringIO):
    ...
    def install(self):
        ...
        old `sys.stdout` value so that `uninstall` can work. Note that if
        someone else changes `sys.stdout` after this is installed,
        uninstall will set `sys.stdout` back to what it was when
        `install` was called, which could cause issues. For example, if
        we have two capturing streams A and B, and we call:
...
def showOutput(show=True):
    """
    Changes the testing mechanisms so that output produced during tests
    is shown as normal in addition to being captured. Call it with False
    as an argument to disable this.
    """
    global _SHOW_OUTPUT
    _SHOW_OUTPUT = show
...
def expect(expr, value):
    ...
    tag = f"{context['file']}:{context['line']}"
